...
if (!inputPath || !coreModulePath || !outputPath) {
  throw new Error("Usage: bun convert-circuit-json.ts circuit.json /path/to/core/dist/index.js output.srj.json")
}
...
const removedTypes = new Set([
  "pcb_trace", "pcb_via", "pcb_copper_pour", "pcb_trace_hint", "pcb_breakout_point",
])
const circuitJson = source
  .filter((e) => !removedTypes.has(e.type) && !e.type.endsWith("_error") && !e.type.endsWith("_warning"))
  .map((e) => Object.fromEntries(Object.entries(e).filter(([key]) => !key.includes("routing_phase"))))
const { simpleRouteJson }: { simpleRouteJson: SimpleRouteJson } =
  getSimpleRouteJsonFromCircuitJson({ circuitJson, ignoreExistingTopLevelPcbRouteState: true })
...
const portToConnection = new Map(simpleRouteJson.connections.flatMap((connection) =>
  connection.pointsToConnect.map((point) => [point.pcb_port_id, connection.name] as const),
))
const sourceToPcbPort = new Map(circuitJson.filter((e) => e.type === "pcb_port")
  .map((e) => [e.source_port_id, e.pcb_port_id]))
const traces = new Map(circuitJson.filter((e) => e.type === "source_trace")
  .map((e) => [e.source_trace_id, e]))
simpleRouteJson.buses = circuitJson.filter((e) => e.type === "source_bus").map((bus) => {
  ...
  return { busId: bus.name, name: bus.name, connectionNames: [...names], maxLengthSkew: bus.max_length_skew }
})
...
console.log(JSON.stringify({
  ...
  terminals: simpleRouteJson.connections.reduce((sum, c) => sum + c.pointsToConnect.length, 0),
  obstacles: simpleRouteJson.obstacles.length,
  buses: simpleRouteJson.buses.map((b) => [b.name, b.connectionNames.length]),
}, null, 2))
