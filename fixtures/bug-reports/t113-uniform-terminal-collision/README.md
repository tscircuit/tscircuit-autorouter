...
The file contains all 177 routed capacity nodes, 4,055 pathing input nodes,
and 458 board obstacles from that stage. It is deterministically
gzip-compressed with `mtime=0`. The regression reuses the exact board
SimpleRouteJson from `t113-linux-exact-pipeline9-root` for electrical
connectivity.
