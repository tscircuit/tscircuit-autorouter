...
const getTraceLength = (srj: SimpleRouteJson): number => {
  let length = 0
...
test("simplifies traces from the RP2040 motor controller board", async () => {
  ...
  const baselineSolver = new AutoroutingPipelineSolver11_Simplification(input, {
    enableVertexShortcuts: false,
  })
  baselineSolver.solve()
  expect(baselineSolver.solved).toBe(true)
  expect(baselineSolver.failed).toBe(false)
  const baseline = baselineSolver.getOutputSimpleRouteJson()
  ...
  const baselineDrcErrors = evaluateRelaxedDrc({
    inputSrj: drcInputSrj,
    srjWithPointPairs: input,
    routedTraces: baseline.traces!,
  }).errors
  expect(baselineDrcErrors).toEqual([])
  ...
  const baselineSvg = getSvgFromGraphicsObject(
    convertSrjToGraphicsObject(baseline, { traceColorMode: "layer" }),
    renderOptions,
  )
  await expect(
    stackSvgsHorizontally(
      [
        addPanelTitle(
          baselineSvg,
          `BEFORE · ${baselinePointCount} POINTS · ${getTraceLength(baseline).toFixed(2)} mm · ${baselineViaCount} VIAS · 0 DRC`,
        ),
        addPanelTitle(
          outputSvg,
          `AFTER · ${outputPointCount} POINTS · ${getTraceLength(output).toFixed(2)} mm · ${outputViaCount} VIAS · 0 DRC`,
        ),
      ],
      { gap: 12, normalizeSize: false },
    ),
  ).toMatchSvgSnapshot(import.meta.path, {
    svgName: "rp2040-motor-controller-vertex-shortcuts",
  })
})
