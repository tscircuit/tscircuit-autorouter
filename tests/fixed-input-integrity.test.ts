...
import { loadFixedInput, validateFixedInput } from "../scripts/benchmark/fixed-input/loadFixedInput"
...
test("the benchmark refuses changed fixture bytes and unsupported input features", async () => {
  ...
  try {
    for (const file of ["manifest.json", "fixed-input.srj.json"]) {
      await copyFile(join(import.meta.dir, "fixtures/allwinner-t113", file), join(directory, file))
    }
    ...
    expect(() => validateFixedInput(unsupported)).toThrow("Unsupported obstacle")
    await appendFile(join(directory, "fixed-input.srj.json"), "\n")
    await expect(loadFixedInput(join(directory, "manifest.json"))).rejects.toThrow("SHA256 does not match")
  } finally {
