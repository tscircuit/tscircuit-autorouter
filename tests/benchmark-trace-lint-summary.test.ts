...
} from "../scripts/benchmark/trace-lint-metrics.js"
...
test("benchmark style averages include zero-error runs and exclude failed or unmeasured runs", () => {
  ...
  expect(
    averageTraceLintIssues([
      { ...result, traceLintIssueCounts: { odd_angle: 0 } },
    ]),
  ).toEqual({ odd_angle: 0 })
  expect(() =>
    averageTraceLintIssues([
      { ...result, traceLintIssueCounts: { odd_angle: -1 } },
    ]),
  ).toThrow("Invalid trace lint count")
  expect(formatTraceLintTable([summary])).toContain(
    "| Avg Angled Traces | 3.00 |",
  )
  expect(formatTraceLintTable([summary])).toContain(
    "| Avg future_rule | 1.00 |",
  )
})
