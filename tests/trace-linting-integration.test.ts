...
test("trace linting solves the final SRJ and locates wire/via issues without changing routing", () => {
  const srj: SimpleRouteJson = {
    ...
    traces: [{
      ...
      route: [
        ...
        { route_type: "via", x: 2, y: 1, from_layer: "top", to_layer: "bottom" },
  ...
  expect(linter.getOutput().map((issue) => [issue.segmentIndex, issue.layer])).toEqual([[0, "top"], [2, "bottom"]])
  ...
  expect(() => runTraceLinting({ ...srj, bounds: { ...srj.bounds, minX: NaN } })).toThrow()
})
