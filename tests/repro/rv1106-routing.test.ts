...
test("routes the RV1106 remaining phase through Pipeline9", async (): Promise<void> => {
  const phases: Phases = JSON.parse(gunzipSync(new Uint8Array(readFileSync(new URL("./assets/rv1106-routing/phases.json.gz", import.meta.url)))).toString())
  ...
  for (const [name, phase] of Object.entries({ clocks: phases.clocks, bootFlash: phases.bootFlash })) {
    await expect(getSvgFromGraphicsObject(convertSrjToGraphicsObject(phase.input), { backgroundColor: "white" })).toMatchSvgSnapshot(import.meta.path, { svgName: `${name}-input` })
    // Saved phase outputs include all preloaded copper already.
    await expect(getBugReportSnapshotSvg({ inputSrj: { ...phase.input, traces: [] }, srjWithPointPairs: phase.input, routedTraces: phase.output })).toMatchSvgSnapshot(import.meta.path, { svgName: `${name}-output` })
  }
  await expect(getSvgFromGraphicsObject(convertSrjToGraphicsObject(phases.remaining), { backgroundColor: "white" })).toMatchSvgSnapshot(import.meta.path, { svgName: "remaining-input" })
  ...
  if (process.env.RV1106_FULL_REPRO !== "1") {
    const checkpoint: Checkpoint = JSON.parse(gunzipSync(new Uint8Array(readFileSync(new URL("./assets/rv1106-routing/checkpoint.json.gz", import.meta.url)))).toString())
  ...
  while (!pipeline.solved && !pipeline.failed) {
    ...
    if (performance.now() - lastProgress > 30_000) {
      console.log({ phase: pipeline.getCurrentPhase(), iterations: pipeline.iterations, activeIterations: pipeline.activeSubSolver?.iterations })
