...
test("width improvement retains the completed route when the wider trace cannot fit", (): void => {
  const input: SimpleRouteJson = {
    ...
    obstacles: [
      {
        type: "rect", layers: ["top"], center: { x: -11.4, y: 0 },
        width: 0.8, height: 0.8, connectedTo: ["WIDE_POWER"],
      },
      {
        type: "rect", layers: ["top"], center: { x: 11.4, y: 0 },
        width: 0.8, height: 0.8, connectedTo: ["WIDE_POWER"],
      ...
      {
        type: "rect", layers: ["top"], center: { x: 0, y: 1.425 },
        width: 20, height: 2.35, connectedTo: [],
      },
      {
        type: "rect", layers: ["top"], center: { x: 0, y: -1.425 },
        width: 20, height: 2.35, connectedTo: [],
  ...
  expect(solver.hypergraphTraceWidthImprovementSolver?.stats.attemptedRouteCount).toBe(1)
  expect(solver.hypergraphTraceWidthImprovementSolver?.stats.improvedRouteCount).toBe(0)
  const wires = solver.getOutputSimplifiedPcbTraces().flatMap((trace) =>
