...
test("width improvement keeps another connection's hypergraph assignment", (): void => {
  const input: SimpleRouteJson = {
    ...
    connections: [
      {
        name: "WIDE_POWER", nominalTraceWidth: 0.4,
        pointsToConnect: [
  ...
  expect(solver.hypergraphTraceWidthImprovementSolver?.stats.rejectedCrossingCount).toBe(1)
  const before = solver.portPointPathingSolver!.getOutput().nodesWithPortPoints
  const after = solver.hypergraphTraceWidthImprovementSolver!.getOutput().nodesWithPortPoints
