...
test("sparse via placement checks preserve centers, duplicate IDs and pad order with fewer index builds", (): void => {
  ...
  for (const dense of [false, true]) {
    const vias: PcbVia[] = Array.from({ length: 128 }, (_, index): PcbVia => ({
      ...baseVia,
      ...
      x: dense || index === 3 || index === 109 ? 0.01 * (index % 7) : index + 2,
      y: 0,
    ...
    const expected = vias.flatMap(
      (via): ReturnType<typeof getViaInPadErrorsWithCenters> =>
        checks.checkViasInPads([...context, via]).map(
          (error): ReturnType<typeof getViaInPadErrorsWithCenters>[number] => ({
            ...error,
