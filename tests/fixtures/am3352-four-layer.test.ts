...
import board from "../../fixtures/bug-reports/bugreport108-am3352-four-layer/am3352-four-layer.srj.json"
...
test("AM3352 fixture contains the complete unphased board without saved routing", () => {
  ...
  expect(serialized).not.toMatch(
    /routing_phase|routingPhase|externallyConnectedPointIds|pcb_via_id|isCopperPour/,
  )
  expect(points.some((p) => p.port_selector?.startsWith("U1."))).toBe(true)
  expect(points.some((p) => p.port_selector?.startsWith("U3."))).toBe(true)
  expect(srj.buses!.map((b) => b.connectionNames.length)).toEqual([
    11, 11, 2, 2, 2, 2,
  ])
