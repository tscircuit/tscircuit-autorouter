...
test("direct intra-node users retain live geometry unless they select spatial search", () => {
  ...
  const futureChild = child as import("../lib/solvers/HighDensitySolver/SingleHighDensityRouteSolver6_VertHorzLayer_FutureCost").SingleHighDensityRouteSolver6_VertHorzLayer_FutureCost
  futureChild.futureConnectionPoints = [replacement]
  expect(futureChild.getClosestFutureConnectionPoint({ ...replacement, g: 0, h: 0, f: 0, parent: null })).toBe(replacement)
  ...
  const linearPortfolio = new PortfolioSingleIntraNodeSolver({ nodeWithPortPoints, futurePointSearch: "linear" })
