...
import { FullConnectionRerouteSolver, TinyHyperGraphSolver } from "tiny-hypergraph/lib/index"
...
type TinyHypergraphParams = ConstructorParameters<typeof TinyHypergraphPortPointPathingSolver>[0]
...
test("TinyHypergraph port-point pathing consumes the final full-connection reroute stage", () => {
  ...
  const reroute = harness.tinyPipelineSolver.getSolver<FullConnectionRerouteSolver>(
    "rerouteFullConnections",
  )
  expect(reroute).toBeDefined()
  expect(reroute!.solved).toBe(true)
  expect(harness.tinyPipelineSolver.getSolvedTinySolver()).toBe(reroute!.getSolvedSolver())
  expect(harness.getCurrentTinySolver()).toBe(reroute!.getSolvedSolver())
  expect(solver.stats.finalMaxRegionCost).toBe(reroute!.stats.finalMaxRegionCost)
