...
test("shared planar via queries preserve complete searches across exact starts, bounds and layers", () => {
  ...
  for (let sample = 0; sample < 24; sample++) {
    ...
    shared.solve()
    reference.solve()
    if (sampleSharedQueries < sampleIndividualQueries) {
      casesWithQuerySavings++
    }
    expect({
  ...
  expect(casesWithQuerySavings).toBeGreaterThanOrEqual(20)
  expect(sharedQueries).toBeLessThan(individualQueries * 0.6)
})
