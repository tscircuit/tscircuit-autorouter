...
class OriginalFutureViaSolver extends SingleHighDensityRouteSolver6_VertHorzLayer_FutureCost {
  override isViaTooCloseToFutureConnectionTrace(node: Node): boolean {
...
test("future via bounds preserve complete search routes and expansion order", () => {
  let seed = 71329
  const random = (): number => {
  ...
  for (let sample = 0; sample < 24; sample++) {
    ...
    const reference = new OriginalFutureViaSolver(opts)
    solver.solve()
    reference.solve()
    expect({
