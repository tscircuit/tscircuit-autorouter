...
test("debug frames expose selection, temporary blocking, search, and the committed detour", (): void => {
  ...
  while (!solver.solved && !solver.failed) {
    ...
    const fixedLines =
      fullGraphics.lines?.filter((line) =>
        line.label?.startsWith("route: route-1\n"),
      ) ?? []
    expect(fixedLines.length).toBeGreaterThan(0)
    expect(
      fixedLines.every((line) => line.strokeColor?.endsWith(", 0.15)")),
    ).toBe(true)
    if (solver.phase === "accepted") {
      const activeLines =
        fullGraphics.lines?.filter((line) =>
          line.label?.startsWith("route: route-0\n"),
        ) ?? []
      expect(activeLines.length).toBeGreaterThan(0)
      expect(
        activeLines.every((line) => !line.strokeColor?.endsWith(", 0.15)")),
      ).toBe(true)
    }
    const obstacle = graphics.rects?.find((rect) =>
      rect.label?.includes("TEMPORARY OBSTACLE"),
    )
    expect(Boolean(obstacle)).toBe(
      solver.phase === "blocked" || solver.phase === "searching",
    )
    if (solver.phase === "blocked") {
      expect(
        solver.activeSubSolver
          ?.visualize()
          .rects?.some((rect) => rect.label?.startsWith("TEMPORARY OBSTACLE:")),
      ).toBe(true)
      ...
      expect(
        graphics.lines?.some((line) =>
          line.label?.startsWith("Previous path: route 0"),
        ),
      ).toBe(true)
      expect(
        graphics.lines?.some((line) =>
          line.label?.startsWith("Assigned path: route 0"),
        ),
      ).toBe(false)
      expect(
        graphics.lines?.some((line) =>
          line.label?.startsWith("Assigned path: route 1"),
        ),
      ).toBe(true)
    }
    if (solver.phase === "accepted") {
