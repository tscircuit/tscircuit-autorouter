...
test("tiny pathing runs congestion search before exposing the final assignments", (): void => {
  const params = structuredClone(input) as unknown as ConstructorParameters<typeof TinyHypergraphPortPointPathingSolver>[0]
  params.flags.USE_CONGESTION_REROUTING = true
  params.preserveTerminalPcbPortIds = true
  params.connections[0].simpleRouteConnection!.pointsToConnect[0].pcb_port_id = "start"
  params.connections[0].simpleRouteConnection!.pointsToConnect[1].pcb_port_id = "end"
  ...
  expect(solver.getOutput().nodesWithPortPoints.flatMap(n => n.portPoints)
    .filter(p => p.pcb_port_id).map(p => p.pcb_port_id).sort()).toEqual(["end", "start"])
})
