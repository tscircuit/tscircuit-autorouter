...
test("portal spacing and existing ordered placement retain directed decimal bounds", (): void => {
  for (const edge of [
    ...
    { start: 0, end: 0.30000000000000004, expected: [0, 0.1, 0.2, 0.30000000000000004] },
  ]) {
    ...
    expect(result.layers[0]!.sites.map((site): number => site.x)).toEqual(edge.expected)
    for (let index = 1; index < result.layers[0]!.sites.length; index++) {
      expect(
        result.layers[0]!.sites[index]!.x - result.layers[0]!.sites[index - 1]!.x,
  ...
  expect(existingPlacement[1]! - existingPlacement[0]!).toBeGreaterThanOrEqual(0.1)
})
