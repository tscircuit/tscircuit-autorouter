...
test("restores shared plated-hole endpoints without moving adjusted vias", (): void => {
  ...
  for (let i = 0; i < output.length; i++) {
    ...
    expect(route.route[0]).toMatchObject({ x: original[i].route[0].x, y: original[i].route[0].y, z: original[i].route[0].z })
    expect(route.route.at(-1)).toMatchObject({ x: original[i].route.at(-1)!.x, y: original[i].route.at(-1)!.y, z: original[i].route.at(-1)!.z })
