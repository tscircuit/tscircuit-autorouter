...
test("Pipeline9 repairs every node when the board has more than 80 repair samples", () => {
  const nodes: NodeWithPortPoints[] = Array.from({ length: 81 }, (_, index) => ({
  ...
  const pipeline = new AutoroutingPipelineSolver9_PreloadedTraceGraph({
    ...
    connections: routes.map((route) => ({
      ...
      pointsToConnect: route.route.map(({ x, y }) => ({ x, y, layer: "top" })),
    })),
  }, { cacheProvider: null })
  ...
  const stage = pipeline.pipelineDef.find((step) => step.solverName === "highDensityRepairSolver")!
  const params = stage.getConstructorParams(pipeline)[0] as ConstructorParameters<typeof Pipeline4HighDensityRepairSolver>[0]
