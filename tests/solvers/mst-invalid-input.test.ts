...
test("MST rejects invalid coordinates, ambiguous references, and foreign extra-edge data", () => {
  ...
  expect(() => buildMinimumSpanningTree([{ x: NaN, y: 0 }])).toThrow(
    "non-finite coordinates",
  )
  expect(() =>
    buildMinimumSpanningTree([first, { x: 1, y: Infinity }]),
  ).toThrow("non-finite coordinates")
  expect(() => buildMinimumSpanningTree([first, first])).toThrow(
    "repeats an input point reference",
  )
  expect(() =>
    buildMinimumSpanningTree([first, second], {
