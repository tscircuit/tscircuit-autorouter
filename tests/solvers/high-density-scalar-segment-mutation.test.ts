...
test("scalar obstacle clearance preserves public segment edits, custom queries, margins and rebuilt indexes", () => {
  const options = {
    ...
    A: { x: -2, y: -1, z: 0 },
    B: { x: 2, y: -1, z: 0 },
    obstacleRoutes: [{ connectionName: "obstacle", traceThickness: 0.15,
  ...
  for (const endpointY of [0, Number.EPSILON, -Number.EPSILON, 0.3, 0.4, NaN]) {
    segment.A.y = endpointY
    segment.B = { x: 1, y: endpointY, z: 0 }
    for (const margin of [0.15, 0, -0.15, -1, Infinity, NaN]) {
      const expected = pointToSegmentDistance(node, segment.A, segment.B) < solver.traceThickness + margin
      expect(solver.isNodeTooCloseToObstacle(node, margin, false, query)).toBe(expected)
    }
  }
  segment.connectedToCurrentConnection = true
  expect(solver.isNodeTooCloseToObstacle(node, Infinity, false, query)).toBe(false)
  segment.connectedToCurrentConnection = false
  segment.A = { x: -1, y: 0.3, z: 0 }
  segment.B = { x: 1, y: 0.3, z: 0 }
  expect(solver.isNodeTooCloseToObstacle(node, undefined, false, query)).toBe(true)
  query.segmentIds = []
  expect(solver.isNodeTooCloseToObstacle(node, undefined, false, query)).toBe(false)

  options.obstacleRoutes[0]!.route = [{ x: -1, y: 0.3, z: 0 }, { x: 1, y: 0.3, z: 0 }]
