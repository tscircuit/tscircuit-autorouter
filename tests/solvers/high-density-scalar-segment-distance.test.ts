...
test("scalar segment distance matches math-utils numerical boundaries and observable coordinate reads", () => {
  ...
  const values = [0, -0, Number.MIN_VALUE, -Number.MIN_VALUE, 1e-160, -1e-160,
    1e-150, -1e-150, 1e-12, -1e-12, 0.15, 0.3, 1, -1, 1 + Number.EPSILON,
    1 - Number.EPSILON, 1e150, -1e150, Number.MAX_VALUE, -Number.MAX_VALUE,
  ...
  const check = (p: Point, v: Point, w: Point): void => {
    ...
    for (const margin of [expected, expected - Number.EPSILON, expected + Number.EPSILON, 0, -0, -1, Infinity, NaN]) {
  ...
  for (let i = 0; i < 4096; i++) {
    const point = (): Point => ({ x: (random() - 0.5) * 10 ** (random() * 320 - 160),
      y: (random() - 0.5) * 10 ** (random() * 320 - 160) })
    check(point(), point(), point())
  }

  for (const degenerate of [false, true]) for (const mutable of [false, true]) {
    const run = (calculate: typeof pointToSegmentDistance): { result: number; reads: string[] } => {
      const reads: string[] = []
      let access = 0
      const point = (name: string, x: number, y: number): Point => new Proxy({ x, y }, {
        get(target, key: "x" | "y") {
          reads.push(`${name}.${key}`)
      ...
      const result = calculate(point("p", 0.3, -0.7), point("v", 0.1, 0.2),
