...
} from "../../../scripts/benchmark/fixed-input/freeroutingAdapter"
...
test("DSN pads, SES units, paths and layer-spanning vias preserve physical geometry", () => {
  ...
  expect(traces[2].route[0]).toMatchObject({
    route_type: "via",
    x: 0,
    y: -2,
    from_layer: "top",
  ...
  expect(() => parseFreeroutingSession(session.slice(0, -1), srj)).toThrow(
    "Incomplete",
  )
  expect(() =>
    parseFreeroutingSession(session.replace("600000", "700000"), srj),
  ).toThrow("via geometry")
  expect(() =>
    parseFreeroutingSession(
      session.replace("(net signal", "(net unknown"),
      srj,
    ),
  ).toThrow("Unknown")
  expect(() =>
    createFreeroutingDsn({
      ...srj,
      differentialPairs: [{ connectionNames: ["a", "b"], lengthTolerance: 1 }],
    }),
  ).toThrow("advanced routing")
  const unmapped = structuredClone(srj)
  unmapped.obstacles[0].center.x += 0.1
  expect(() => createFreeroutingDsn(unmapped)).toThrow(
    "exactly one physical pad",
  )
})
