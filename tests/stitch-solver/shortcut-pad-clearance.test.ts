...
test("stitch shortcuts respect pad copper, clearance, layers, and net aliases", () => {
  ...
  expect(validator.isSegmentClear({
    ...segment,
  ...
  expect(createValidator({ ...obstacle, layers: ["bottom"] })
    .isSegmentClear(segment)).toBe(true)
  expect(createValidator({ ...obstacle, connectedTo: ["signal"] })
    .isSegmentClear(segment)).toBe(true)
