...
test("native CPU reports separate routing ancestry and deduplicate recursive inclusive samples in compressed profiles", async () => {
  ...
  try {
    ...
    const frame = (functionName: string, url: string): { functionName: string; url: string; lineNumber: number; columnNumber: number } => ({ functionName, url, lineNumber: 0, columnNumber: 0 })
    const profile = {
      nodes: [
        { id: 1, callFrame: frame("root", ""), children: [2, 5] },
        { id: 2, callFrame: frame("step", "lib/solvers/BaseSolver.ts"), children: [3] },
      ...
      samples: [4, 5], timeDeltas: [2000, 3000], startTime: 0, endTime: 5000,
    }
    await writeFile(join(directory, "sample.cpuprofile.gz"), Bun.gzipSync(JSON.stringify(profile)))
    await summarizeCpuProfiles(outDir)
    const summary = await Bun.file(join(outDir, "cpu-summary.json")).json()
    const search = summary.functions.find((row: { function: string }) => row.function === "search")
    ...
    expect(summary.functions.find((row: { function: string }) => row.function === "validation").routingSelfMs).toBe(0)
  } finally {
