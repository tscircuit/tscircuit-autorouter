...
test("STM32 full board routes LED_GREEN without disconnected fragments", async (): Promise<void> => {
  ...
  const polylineSolver = new MultiHeadPolyLineIntraNodeSolver3({
    nodeWithPortPoints: revisitedNode,
  })
  polylineSolver.solve()
  expect(polylineSolver.solved).toBeTrue()
  expect(polylineSolver.failed).toBeFalse()
  expect(polylineSolver.error).toBeNull()
  expect(
    polylineSolver.solvedRoutes.filter(
      (route) => route.connectionName === "source_net_2",
    ),
  ).toHaveLength(3)
  expect(
    areNodePortPointPairsConnectedByRoutes(
      polylineSolver.solvedRoutes,
      revisitedNode,
    ),
  ).toBeTrue()
  expect(
