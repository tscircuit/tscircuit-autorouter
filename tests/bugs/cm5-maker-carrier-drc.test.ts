...
test("Pipeline 7 records CM5 carrier DRC violations at 1x effort", async () => {
  ...
  const capturedViaInPadErrors = getActualViaInPadErrors(
    capturedDrc.circuitJson,
  )
  expect(capturedViaInPadErrors).toHaveLength(3)
  ...
  const finalViaInPadErrors = getActualViaInPadErrors(finalDrc.circuitJson)
  expect(finalViaInPadErrors.length).toBeGreaterThan(0)

  await expect(
    convertCircuitJsonToPcbSvg(capturedDrc.circuitJson, {
  ...
  await expect(
    convertCircuitJsonToPcbSvg(
      [
        ...capturedDrc.circuitJson,
        ...legitimateCapturedErrors,
        ...capturedViaInPadErrors,
      ],
      {
        backgroundColor: "white",
        matchBoardAspectRatio: true,
