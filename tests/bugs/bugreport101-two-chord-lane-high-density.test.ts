...
import { TwoChordLaneIntraNodeSolver } from "lib/solvers/HighDensitySolver/two-chord-lane-intra-node-solver"
...
test("bugreport101 exact two-chord lane solver routes the narrow physical-capacity node", () => {
  ...
  const fabricatedPortalNode = structuredClone(node)
  fabricatedPortalNode.portPointsInPairs![0]![0].duplicatedFromPortId =
    "fabricated"
  expect(
    TwoChordLaneIntraNodeSolver.isApplicable({
      nodeWithPortPoints: fabricatedPortalNode,
      traceWidth: 0.15,
      viaDiameter: 0.3,
      clearance: 0.1,
      obstacles: [],
