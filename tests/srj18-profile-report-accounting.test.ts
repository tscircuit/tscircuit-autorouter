...
import type {
  ...
  StageSummary,
  ClassTiming,
} from "../scripts/srj18Profile/reportTypes"
...
test("reports retain failed-stage time, take medians and attribute reused solver calls to their measured stages", async () => {
  ...
  try {
    ...
    const fixture: SampleResult = {
      ...
      solved: false,
      failed: true,
      error: "expected iteration limit",
      durationMs: 0,
      ...
      iterations: 7,
      outputSha256: null,
      outputTraceCount: null,
      relaxedDrcErrors: null,
      highDensityStats: null,
      highDensityNodes: [],
      nodeTimings: [],
      profile: null,
      stages: [
    ...
    for (const [index, durationMs] of [10, 50, 20].entries()) {
      ...
      await writeFile(
        join(directory, "sample001.json"),
        JSON.stringify({
          ...fixture,
          durationMs,
          stages: [
            fixture.stages[0],
            { ...fixture.stages[1], durationMs: durationMs - 1 },
          ],
        }),
      )
    }
    ...
    await writeFile(
      join(outDir, "detailed", "sample001.json.gz"),
      Bun.gzipSync(JSON.stringify(detailed)),
    )
    await summarizeResults(outDir)
    const summary = (await Bun.file(join(outDir, "summary.json")).json()) as {
    ...
    expect(
      summary.classes.find((row) => row.stage === "failed_stage")!.selfMs,
    ).toBe(28)
    expect(
      summary.classes.reduce(
        (sum: number, row: { selfMs: number }) => sum + row.selfMs,
        0,
      ),
    ).toBe(30)
  } finally {
