...
async function main() {
  ...
  try {
    ...
    const fixtureParam = JSON.stringify({ path: "fixtures/features/tracewidthsolver/tracewidthsolver01.fixture.tsx" });
    await page.goto(`http://localhost:5000/?_fixtureId=${encodeURIComponent(fixtureParam)}`, { waitUntil: "networkidle" });
    ...
    await page.screenshot({ path: path.join(videoDir, "tracewidth_screenshot.png") });
  } finally {
    ...
    const videoFile = files.find(f => f.endsWith(".webm") && f !== "tracewidth_demo.webm");
    if (videoFile) {
