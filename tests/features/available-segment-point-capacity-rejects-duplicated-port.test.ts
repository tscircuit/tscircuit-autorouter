...
test("available segment point capacity rejects a duplicated input port", () => {
  ...
  const segmentPortPoints: SegmentPortPoint[] = [
    {
      segmentPortPointId: "shared-edge-point",
  ...
  graph.ports[0]!.d.duplicatedFromPortId = "source-port"
  ...
  ).toThrow("Available segment point capacity received 1 duplicated port")
})
