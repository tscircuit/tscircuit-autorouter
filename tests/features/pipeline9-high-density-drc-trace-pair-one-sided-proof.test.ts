...
test("native one-sided trace repair preserves bounded copper rejected by two-sided HD force", (): void => {
  ...
  let activeFamily: Pipeline9HighDensityForceFamily = "native"
  for (const candidate of getPipeline9HighDensityForceCandidates({
    ...
    onCandidateRejected: (reason): void => {
      if (activeFamily === "native" && reason === "geometry") {
        geometryRejections++
      }
    },
  })) {
    // Keep this direct comparison specific to the original bilateral family;
    // integrated one-sided generator repair has its own regression.
    if (activeFamily !== "native") continue
    yieldedCandidates++
    expect(
