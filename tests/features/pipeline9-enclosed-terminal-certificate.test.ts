...
test("four foreign pads certify an enclosed terminal even with diagonal trace escapes considered", (): void => {
  ...
  expect(diagonalGap).toBeLessThan(
    srj.minTraceWidth + 2 * srj.minTraceToPadEdgeClearance!,
  )
  const error = getEnclosedTerminalError(
    srj,
  ...
  ).toContain("Unroutable terminal center_port")
})
