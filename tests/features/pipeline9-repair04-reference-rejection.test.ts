...
import { AutoroutingDrcEngine, type DrcEvaluator } from "high-density-repair03/lib"
...
test("repair04 rejects a local DRC improvement when the full-board reference worsens", () => {
  ...
  const engine = new AutoroutingDrcEngine(fixture.srj, { connMap: fixture.connMap })
  ...
  const referenceDrcEvaluator: DrcEvaluator = ({ routes }) => {
    ...
    return [
      { type: "reference_only_constraint", message: "Candidate violates a full-board reference constraint" },
      { type: "reference_only_constraint", message: "Candidate violates another full-board reference constraint" },
    ]
  }
