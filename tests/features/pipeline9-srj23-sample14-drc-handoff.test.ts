...
test("Pipeline9 diagnoses remaining SRJ23 sample 14 copper by ownership", async (): Promise<void> => {
  const { scenario } = await loadScenarioBySampleNumber("srj23", 14)
  ...
  // Current e1 same-machine evidence is eight, not a claimed new repair.
  expect(finalErrorCount).toBeLessThanOrEqual(8)
})
