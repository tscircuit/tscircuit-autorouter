...
import scenario from "./srj/preexisting-connected-traces04.srj.json" with {
  type: "json",
}
...
test(
  ...
  () => {
    ...
    const { outputSrj } = solveAndSnapshot(srj, import.meta.path, {
      problem:
        "A physical preexisting route is present but connectsTo declares only one endpoint.",
      expected:
        "Pipeline7 should ignore that trace for MST suppression and emit two traces.",
    })
