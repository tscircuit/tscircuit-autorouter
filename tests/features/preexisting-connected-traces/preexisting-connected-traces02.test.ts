...
import scenario from "./srj/preexisting-connected-traces02.srj.json" with {
  type: "json",
}
...
test(
  ...
  () => {
    ...
    const { outputSrj } = solveAndSnapshot(srj, import.meta.path, {
      problem:
        "A four-point net has two preexisting routed pairs: U1-R1 and R2-R3.",
      expected:
        "Pipeline7 should emit one bridge between the two already-routed groups.",
    })
