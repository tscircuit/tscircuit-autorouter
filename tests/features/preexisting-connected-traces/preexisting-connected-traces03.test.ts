...
import scenario from "./srj/preexisting-connected-traces03.srj.json" with {
  type: "json",
}
...
test(
  ...
  () => {
    ...
    const { outputSrj } = solveAndSnapshot(srj, import.meta.path, {
      problem:
        "The preexisting trace declares all three points on the net as connected.",
      expected:
        "Pipeline7 should emit no new traces for this fully routed net.",
    })
