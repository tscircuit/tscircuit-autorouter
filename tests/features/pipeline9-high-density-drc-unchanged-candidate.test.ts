...
test("Pipeline9 rejects a cloned incumbent without repeating official candidate checks", (): void => {
  ...
  expect(
    candidateGate.evaluateCandidateRoutes(
      [{ vias, ...clonedRoute }],
      "node-a",
      new Set(["A"]),
    ),
