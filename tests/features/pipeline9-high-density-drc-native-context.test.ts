...
test("Pipeline9 indexes copper and retries across the native port-point domain", (): void => {
  ...
  // Copper decomposition remains useful independently of the removed rerouter.
  // Its domain must still include the actual port outside the nominal node.
  const nativeBounds = getBoundsFromNodeWithPortPoints(nodes[0]!)
  const padding = solver.params.viaDiameter / 2 + solver.params.obstacleMargin
  const searchBounds = {
