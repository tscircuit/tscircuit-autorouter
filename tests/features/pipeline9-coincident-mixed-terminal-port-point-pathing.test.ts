...
test("Pipeline9 preserves absent PCB metadata on a coincident logical terminal", (): void => {
    ...
    connections: [
      {
        ...
        pointsToConnect: [
          ...
          { x: 0, y: 0, layer: "top", pointId: "b", pcb_port_id: "pcb-b" },
        ],
      },
    ],
  }
  ...
  const routedTerminalIds = hdRoutes
    .flatMap((route) => [route.startPcbPortId, route.endPcbPortId])
    .filter((id) => id !== undefined)
  expect(routedTerminalIds).toEqual(["pcb-b"])
  const traces = solver.getOutputSimplifiedPcbTraces()
  expect(traces).toHaveLength(1)
  expect(traces[0].connectsTo).toEqual(["a", "b"])
  expect(traces[0].route).toHaveLength(2)
  const wirePoints = traces[0].route.filter(
    (point) => point.route_type === "wire",
  )
  expect(wirePoints).toHaveLength(2)
  const terminalIds = wirePoints
    .flatMap((point) => [point.start_pcb_port_id, point.end_pcb_port_id])
    .filter((id) => id !== undefined)
  expect(terminalIds).toEqual(["pcb-b"])
})
