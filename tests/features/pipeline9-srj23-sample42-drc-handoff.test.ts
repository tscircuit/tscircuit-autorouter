...
test("Pipeline9 diagnoses remaining SRJ23 sample 42 copper by ownership", async (): Promise<void> => {
  const { scenario } = await loadScenarioBySampleNumber("srj23", 42)
  ...
  // Current e1 same-machine evidence is two, not a claimed new repair.
  expect(finalErrorCount).toBeLessThanOrEqual(2)
})
