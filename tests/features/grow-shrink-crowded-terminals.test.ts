...
import rawNode from "../fixtures/am3352-crowded-node.json"

test("crowded AM3352 terminals retain a search budget after growing to fit trace width", (): void => {
  const node = rawNode as NodeWithPortPoints
  const params = {
  ...
  expect(
    new Set(solver.solvedRoutes.map((route) => route.connectionName)),
  ).toEqual(new Set(node.portPoints.map((point) => point.connectionName)))
