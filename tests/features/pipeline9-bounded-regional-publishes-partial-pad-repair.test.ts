...
const renderPcbCloseup = (
  ...
  const svg = convertCircuitJsonToPcbSvg(
    [...circuitJson, createPcbBoardElement(fixture.srj as SimpleRouteJson)],
    {
...
test("publishes the SRJ18 repairs while the imported C43 pad still covers TP5", async (): Promise<void> => {
  ...
  const regions: SnapshotRegion[] = [
    {
      name: "via-inner2",
      title: "Via / inner2 trace",
      center: { x: -5.05, y: 14.25 },
      width: 1.5,
      height: 1.07,
    },
    {
      name: "u2-pad-clearance",
      title: "U2 pad clearance",
      center: { x: 5.65, y: 1.72 },
      width: 1.5,
      height: 1.07,
    },
    {
      name: "c43-tp5-unresolved",
      title: "C43 / TP5 input overlap remains",
      center: { x: -1.8, y: -5.1 },
      width: 2.8,
      height: 2,
    },
