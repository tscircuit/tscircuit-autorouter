...
test("node simplification is enabled by default and can be disabled for comparisons", () => {
  const params = {
  ...
  const normal = createPipeline9RegularNodeSolver({ ...params, enableNodeSimplification: false })
  const experimental = createPipeline9RegularNodeSolver(params)
