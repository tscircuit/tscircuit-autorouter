...
test("preloaded trace segments occupy existing hypergraph regions", () => {
  ...
  const tinyPipeline = (solver as any).tinyPipelineSolver
  const tinySolver =
    tinyPipeline.getInitialVisualizationSolver() as TinyHyperGraphSolver
  ...
  const northPortId = getPortId("north-center::0")
  const southPortId = getPortId("center-south::0")

  expect(tinySolver.topology.regionCount).toBe(capacityMeshNodes.length + 2)
  expect(centerRegionId).toBeGreaterThanOrEqual(0)
  expect(westPortId).toBeGreaterThanOrEqual(0)
  expect(eastPortId).toBeGreaterThanOrEqual(0)
  expect(
