...
test("Pipeline9 preserves physical terminal fragments through final DRC", async (): Promise<void> => {
  for (const sample of [22, 74]) {
    const { scenario } = await loadScenarioBySampleNumber("srj19", sample)
