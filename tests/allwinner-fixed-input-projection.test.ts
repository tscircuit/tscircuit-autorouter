...
test("the frozen Allwinner source regenerates the same common routing problem", () => {
  ...
  const sourceBytes = gunzipSync(
    new Uint8Array(
      readFileSync(new URL("frozen-source.srj.json.gz", fixtureRoot)),
    ),
  )
  expect(
    createHash("sha256").update(new Uint8Array(sourceBytes)).digest("hex"),
  ).toBe(manifest.source.uncompressedSha256)
  ...
  expect(projected.connections.flatMap((c) => c.pointsToConnect)).toHaveLength(
    434,
  )
  expect(projected.obstacles.filter((o) => o.connectedTo.length)).toHaveLength(
    434,
  )
  expect(projected.obstacles.filter((o) => !o.connectedTo.length)).toHaveLength(
    265,
  ...
  const resultRotated = projected.obstacles.find(
    (o) =>
      o.obstacleId ===
      `allwinner_obstacle_${source.obstacles.indexOf(originalRotated)}`,
  )!
