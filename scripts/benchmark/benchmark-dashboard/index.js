...
async function loadRun(runId) {
  ...
  if (!run) throw new Error('Benchmark run ' + runId + ' is missing from the dashboard index')
  const request = fetch(new URL('./data/' + run.path, location.href))
    .then((response) => {
      if (!response.ok) throw new Error('Could not load benchmark run ' + runId + ': HTTP ' + response.status)
      return response.json()
    })
    .then((loadedRun) => {
      if (!loadedRun || loadedRun.runId !== runId) {
        throw new Error('Benchmark run data does not match requested run ' + runId)
      }
      return loadedRun
    })
...
async function getScopedSeriesPoints() {
  ...
  const points = await Promise.all(getBaseSeriesPoints().map(async (point) => {
    const samples = await getPointSamples(point)
    const sample = samples.find((item) => item.sampleNumber === selectedSampleNumber)
...
async function renderChart() {
  ...
  plottedPoints.forEach(({ point, x, y }, pointIndex) => {
    ...
    hitTarget.addEventListener('mouseenter', () => { void showTooltip(point, metric, visiblePoint) })
    hitTarget.addEventListener('mouseleave', () => { chartTooltip.hidden = true })
    hitTarget.addEventListener('focus', () => { void showTooltip(point, metric, visiblePoint) })
    hitTarget.addEventListener('blur', () => { chartTooltip.hidden = true })
    hitTarget.addEventListener('click', () => runDashboardTask(selectPoint(point)))
...
async function renderRunDetails() {
  ...
  const previous = await getPreviousPoint(point)
  const detailPoint = point.samples ? point : { ...point, samples: await getPointSamples(point) }
  const detailPrevious = previous && (previous.samples ? previous : { ...previous, samples: await getPointSamples(previous) })
...
async function rebuildSampleOptions(preferredValue) {
  ...
  const sampleNumbers = [...new Set(samples.map((sample) => sample.sampleNumber))]
...
async function rebuildSeriesControls(preferredSolver, preferredEffort, preferredSample) {
...
async function getSelectedExportData() {
  ...
  const previous = await getPreviousPoint(point)
  return {
    point: point.samples ? point : { ...point, samples: await getPointSamples(point) },
    previous: previous && (previous.samples ? previous : { ...previous, samples: await getPointSamples(previous) }),
...
function reportDashboardError(error) {
  ...
  getRequiredElement('selected-context').textContent = 'Dashboard error: ' + message
}
...
sampleSelect.addEventListener('change', () => runDashboardTask(renderDashboard(true)))
rangeSelect.addEventListener('change', () => runDashboardTask(renderDashboard(true)))
...
sampleStatusSelect.addEventListener('change', () => runDashboardTask(renderRunDetails()))
sampleSearchInput.addEventListener('input', () => runDashboardTask(renderRunDetails()))
...
getRequiredElement('download-csv').addEventListener('click', () =>
  runDashboardTask(getSelectedExportData().then(({ point, previous }) => {
    const previousByNumber = new Map((previous?.samples ?? []).map((sample) =>
      [sample.sampleNumber, sample],
    ))
...
async function initializeDashboard() {
  ...
  if (!response.ok) throw new Error('Could not load benchmark dashboard index: HTTP ' + response.status)
  dashboardData = await response.json()
  if (!dashboardData || !Array.isArray(dashboardData.runs) || !Array.isArray(dashboardData.points)) {
