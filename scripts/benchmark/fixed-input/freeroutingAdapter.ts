...
const sessionLayers: Record<string, string> = {
  "F.Cu": "top",
  "B.Cu": "bottom",
}
...
export function createFreeroutingDsn(srj: SimpleRouteJson): string {
  ...
  if (
    ...
    throw new Error(
      "DSN benchmark supports only two layers, rectangular bounds, no preloads or advanced routing constraints",
    )
  }
  ...
  if (
    ...
    throw new Error(
      "DSN benchmark requires canonical via dimension fields only",
    )
  if (
    ![width, clearance, viaDiameter, holeDiameter].every(
      (value) =>
        typeof value === "number" && Number.isFinite(value) && value > 0,
    ) ||
    srj.nominalTraceWidth !== width ||
    srj.minTraceToPadEdgeClearance !== clearance ||
    srj.minViaEdgeToPadEdgeClearance !== clearance ||
    srj.minBoardEdgeClearance !== clearance ||
    holeDiameter! >= viaDiameter!
  ) {
    throw new Error(
      "DSN benchmark requires explicit uniform widths/clearances and valid via dimensions",
    )
  }
  const pads: Pad[] = []
  for (const [index, obstacle] of srj.obstacles.entries()) {
    if (
      ...
      new Set(obstacle.layers).size !== obstacle.layers.length ||
      obstacle.layers.some((layer) => !layerNames[layer]) ||
      ![
        obstacle.center.x,
        obstacle.center.y,
        obstacle.width,
    ...
    if (obstacle.connectedTo.length === 1) {
      ...
      if (!names.has(net))
        throw new Error(`Obstacle ${index} refers to unknown net ${net}`)
  ...
  for (const connection of srj.connections) {
    if (
      (connection.nominalTraceWidth !== undefined &&
        connection.nominalTraceWidth !== width) ||
      connection.isOffBoard ||
      connection.externallyConnectedPointIds?.length ||
      connection.mergedConnectionNames?.length ||
      ...
      (connection.__netConnectionName &&
        connection.__netConnectionName !== connection.name) ||
      connection.__rootConnectionNames?.some((name) => name !== connection.name)
    ) {
      throw new Error(
        `Unsupported connection constraints on ${connection.name}`,
      )
    }
    ...
    for (const point of connection.pointsToConnect) {
      ...
      const matches = netPads.filter(
        ({ obstacle }) =>
          Math.abs(point.x - obstacle.center.x) < 1e-9 &&
          Math.abs(point.y - obstacle.center.y) < 1e-9 &&
          layers.length === obstacle.layers.length &&
          layers.every((layer) => obstacle.layers.includes(layer)),
      )
      if (
        matches.length !== 1 ||
        new Set(layers).size !== layers.length ||
        ("terminalVia" in point && point.terminalVia) ||
        ("busId" in point && point.busId)
      )
        throw new Error(
          `Terminal on ${connection.name} must map to exactly one physical pad with identical layers`,
        )
      mapped.add(matches[0].index)
    }
    if (
      mapped.size !== netPads.length ||
      netPads.length !== connection.pointsToConnect.length ||
      netPads.length < 2
    )
      throw new Error(
        `Net ${connection.name} does not have a one-to-one pad/terminal mapping`,
      )
  }
  ...
  if (
    ...
    b.minX >= b.maxX ||
    b.minY >= b.maxY ||
    names.size !== srj.connections.length ||
    names.size === 0 ||
    names.has("")
  )
    throw new Error("Invalid bounds or duplicate/empty connection names")
  const lines = [
    '(pcb "fixed_input"',
    ...
    `  (boundary (rect pcb ${units(b.minX)} ${units(b.minY)} ${units(b.maxX)} ${units(b.maxY)}))`,
    `  (via ${quoted(viaName)})`,
  ...
  for (const [index, obstacle] of srj.obstacles.entries()) {
    ...
    for (const layer of obstacle.layers)
      lines.push(
        `  (keepout "keepout_${index}_${layer}" (rect ${layerNames[layer]} ${units(obstacle.center.x - obstacle.width / 2)} ${units(obstacle.center.y - obstacle.height / 2)} ${units(obstacle.center.x + obstacle.width / 2)} ${units(obstacle.center.y + obstacle.height / 2)}))`,
      )
  }
  lines.push(" )", " (placement")
  for (const { obstacle, index } of pads)
    lines.push(
      `  (component "pad_${index}" (place "pad_${index}" ${units(obstacle.center.x)} ${units(obstacle.center.y)} front 0))`,
    )
  lines.push(" )", " (library")
  for (const { obstacle, index } of pads) {
    lines.push(`  (image "pad_${index}" (pin "padstack_${index}" 1 0 0))`)
    const shapes = obstacle.layers
      .map(
        (layer) =>
          `(shape (rect ${layerNames[layer]} ${units(-obstacle.width / 2)} ${units(-obstacle.height / 2)} ${units(obstacle.width / 2)} ${units(obstacle.height / 2)}))`,
      )
      .join(" ")
    lines.push(`  (padstack "padstack_${index}" ${shapes} (attach off))`)
  }
  lines.push(
    `  (padstack ${quoted(viaName)} (shape (circle F.Cu ${units(viaDiameter!)} 0 0)) (shape (circle B.Cu ${units(viaDiameter!)} 0 0)) (attach off))`,
    " )",
    " (network",
  )
  for (const connection of srj.connections)
    lines.push(
      `  (net ${quoted(connection.name)} (pins ${pads
        .filter((pad) => pad.net === connection.name)
        .map((pad) => `pad_${pad.index}-1`)
        .join(" ")}))`,
    )
  lines.push(
    `  (class "benchmark" ${[...names].map((name) => quoted(name)).join(" ")} (circuit (use_via ${quoted(viaName)}) (use_layer F.Cu B.Cu)) (rule (width ${units(width)}) (clearance ${units(clearance!)})))`,
    " )",
    " (wiring)",
    ")",
  )
...
function parseSExpressions(text: string): SExpression[] {
  ...
  if (!tokens || tokens.length === 0)
    throw new Error("Empty Freerouting session")
  ...
  if (!root || stack.length || root[0] !== "session")
    throw new Error("Incomplete or invalid Freerouting session")
  return root
}
 ...
 */
export function parseFreeroutingSession(
  session: string,
  srj: SimpleRouteJson,
): SimplifiedPcbTrace[] {
  ...
  if (resolution[1] !== "um" || numeric(resolution[2]) <= 0)
    throw new Error("Unsupported session resolution")
  const scale = numeric(resolution[2]) * 1000
  const library = requiredChild(routes, "library_out")
  const padstacks = children(library, "padstack").filter(
    (node) => node[1] === viaName,
  )
  if (padstacks.length !== 1)
    throw new Error("Session requires exactly one benchmark via padstack")
  ...
  for (const shape of shapes) {
    ...
    if (
      typeof circle[1] !== "string" ||
      !sessionLayers[circle[1]] ||
      Math.abs(numeric(circle[2]) / scale - srj.minViaPadDiameter!) > 1e-6 ||
      numeric(circle[3]) !== 0 ||
      numeric(circle[4]) !== 0
    )
      throw new Error("Session changed the benchmark via geometry")
    viaLayers.add(circle[1])
  }
  if (shapes.length !== 2 || viaLayers.size !== 2)
    throw new Error("Session via must span both benchmark layers")
  const network = requiredChild(routes, "network_out")
  if (
    network.slice(1).some((node) => !Array.isArray(node) || node[0] !== "net")
  )
    throw new Error("Unsupported session network item")
  const traces: SimplifiedPcbTrace[] = []
  const names = new Set(srj.connections.map((connection) => connection.name))
  const seen = new Set<string>()
  for (const net of children(network, "net")) {
    const name = net[1]
    if (typeof name !== "string" || !names.has(name) || seen.has(name))
      throw new Error(`Unknown or duplicate session net ${name}`)
    seen.add(name)
    for (const item of net.slice(2)) {
      ...
      if (item[0] === "wire") {
        if (
          item
            ...
            .some(
              (node) =>
                !Array.isArray(node) ||
                (node[0] !== "path" && node[0] !== "type"),
            )
        )
          throw new Error("Unsupported session wire item")
        const path = requiredChild(item, "path")
        if (
          typeof path[1] !== "string" ||
          !sessionLayers[path[1]] ||
          path.length < 7 ||
          (path.length - 3) % 2 !== 0
        )
          throw new Error("Unsupported or malformed session path")
        ...
        for (let i = 3; i < path.length; i += 2)
          route.push({
            route_type: "wire",
            x: numeric(path[i]) / scale,
            y: numeric(path[i + 1]) / scale,
            width,
            layer: sessionLayers[path[1]],
          })
        traces.push({
          type: "pcb_trace",
          pcb_trace_id: `freerouting_${traces.length}`,
        ...
        if (
          ...
          item
            .slice(4)
            .some((node) => !Array.isArray(node) || node[0] !== "type")
        )
          throw new Error("Unknown or malformed session via")
        traces.push({
          type: "pcb_trace",
          pcb_trace_id: `freerouting_${traces.length}`,
          connection_name: name,
          route: [
            {
              route_type: "via",
              x: numeric(item[2]) / scale,
              y: numeric(item[3]) / scale,
              from_layer: "top",
              to_layer: "bottom",
              via_diameter: srj.minViaPadDiameter,
              via_hole_diameter: srj.minViaHoleDiameter,
            },
          ],
        })
