...
import { runProcessWithDeadline, type ProcessDeadlineResult } from "./processDeadline"
import { scoreRouting, type RoutingScore } from "./scoreRouting"

export const FREEROUTING_SHA256 = "251101c3eeac22d7e7dfcf6796603279e5d1000283eb82d8f093780f7afc6aa9"
...
const { values } = parseArgs({
  options: {
    manifest: { type: "string", default: "tests/fixtures/allwinner-t113/manifest.json" },
...
if (!Number.isSafeInteger(budgetMs) || budgetMs < 100 || budgetMs > 3600000 ||
  !Number.isSafeInteger(repetitions) || repetitions < 1 || repetitions > 10) {
  throw new Error("Use an integer budget of 100–3600000 ms and 1–10 repetitions")
}
if (!values.jar) throw new Error("--jar must identify the pinned Freerouting 2.4.1 JAR")
if (!values.manifest || !values.output || !values.java) throw new Error("Missing manifest, output, or Java path")
const jarPath = resolve(values.jar)
const jarSha256 = createHash("sha256").update(new Uint8Array(await readFile(jarPath))).digest("hex")
if (jarSha256 !== FREEROUTING_SHA256) throw new Error("Freerouting JAR SHA256 mismatch")
...
await writeFile(join(outputDirectory, "manifest.json"), JSON.stringify(input.manifest, null, 2))
await copyFile(join(repositoryRoot, "package.json"), join(outputDirectory, "package.json"))

const gitVersion = spawnSync("git", ["rev-parse", "HEAD"], { cwd: repositoryRoot, encoding: "utf8" })
...
for (const command of [gitVersion, gitStatus, javaVersion]) {
  ...
  if (command.status !== 0) throw new Error(`Benchmark metadata command failed: ${command.stderr}`)
}
const dependencies = JSON.stringify(await getInstalledDependencyInventory(repositoryRoot), null, 2)
await writeFile(join(outputDirectory, "resolved-dependencies.json"), dependencies)
const report: ComparisonReport = {
  ...
  metadata: {
    ...
    cpuModel: cpus()[0]?.model, logicalCpuCount: cpus().length, totalMemoryBytes: totalmem(),
    runnerName: process.env.RUNNER_NAME ?? null,
    budgetMs, repetitions,
    timingBoundary: "parent spawn through worker exit; includes runtime startup, input parsing, adaptation, constructors, routing, and output serialization; excludes shared preflight and scoring",
    order: "native/freerouting on odd repetitions; freerouting/native on even repetitions; sequential fresh processes",
    nativeOptions: { solver: "AutoroutingPipelineSolver9_PreloadedTraceGraph", effort: 1, cacheProvider: null },
    freeroutingOptions: { version: "2.4.1", maxPasses: 999999, maxThreads: 1, optimizer: false, fanout: false, automaticNeckdown: false, copperToEdgeClearanceUm: 150 },
    randomness: "Engine defaults; no shared seed API is assumed. Repeated outputs are not promised bit-identical.",
    dependencyPolicy: "Repository disables lockfile generation; resolved dependency inventory and package specifications are archived. Reinstallation may resolve newer ranged dependencies.",
  },
  runs: [],
}

for (let repetition = 1; repetition <= repetitions; repetition++) {
  const engines: FixedInputEngine[] = repetition % 2 === 1 ? ["native", "freerouting"] : ["freerouting", "native"]
  for (const engine of engines) {
    const artifactDirectory = `${repetition}-${engine}`
    const runDirectory = join(outputDirectory, artifactDirectory)
    await mkdir(runDirectory)
    console.log(`Starting ${artifactDirectory}: ${budgetMs} ms, input ${input.inputSha256}`)
    const processResult = await runProcessWithDeadline({
      command: [process.execPath, join(repositoryRoot, "scripts/benchmark/fixed-input/engineWorker.ts"),
        engine, input.manifestPath, runDirectory, values.java, jarPath],
      cwd: repositoryRoot, budgetMs,
      logPath: join(runDirectory, "engine.log"),
    })
    const run: ComparisonRun = {
      engine, repetition, order: report.runs.length + 1,
      process: processResult,
      status: processResult.status === "timed_out" ? "timed_out" : "failed",
      engineReportedSolved: null, score: null, artifactDirectory,
    }
    if (processResult.status === "exited" && processResult.exitCode === 0) {
      try {
        const result = JSON.parse(await readFile(join(runDirectory, "engine-result.json"), "utf8")) as EngineResult
        if (result.engine !== engine) throw new Error("Engine result identity mismatch")
        run.engineReportedSolved = result.engineReportedSolved
        run.status = result.status
        if (result.status === "completed") {
          if (!Array.isArray(result.traces)) throw new Error("Completed engine did not export traces")
          run.score = scoreRouting(input.srj, result.traces)
        } else {
          run.error = result.error
          run.failureStage = result.status === "solver_failed" ? "solver" : "engine"
        }
      } catch (error) {
        run.status = "failed"
    ...
    await writeFile(join(outputDirectory, "comparison.json"), JSON.stringify(report, null, 2))
    const rows = report.runs.map((entry) => `| ${entry.order} | ${entry.engine} | ${entry.status} | ${(entry.process.elapsedMs / 1000).toFixed(3)} | ${entry.score ? `${entry.score.connectedConnections}/${entry.score.totalConnections}` : "unavailable"} | ${entry.score?.drcViolationCount ?? "unavailable"} | ${entry.score?.viaCount ?? "unavailable"} | ${entry.score?.traceLengthMm.toFixed(2) ?? "unavailable"} |`)
    await writeFile(join(outputDirectory, "comparison.md"), [
      `# ${input.manifest.name}`, "",
    ...
    console.log(`Finished ${artifactDirectory}: ${run.status}, ${(processResult.elapsedMs / 1000).toFixed(3)} s`)
  }
}
