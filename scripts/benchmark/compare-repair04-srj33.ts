...
for (const summary of [baseline, candidate]) {
  if (
    ...
    summary.evaluated !== 37 ||
    !summary.complete ||
    summary.results.length !== 37
  ) {
    throw new Error(
      "A comparison requires complete results for all 37 boards at the pinned SRJ33 revision",
    )
  }
  ...
  if (
    JSON.stringify(summary.results.map((result) => result.sample).sort()) !==
    JSON.stringify(expectedSampleNames)
  ) {
    throw new Error(
      "Sample IDs differ from the pinned 37-board SRJ33 membership",
    )
  }
}
...
if (
  candidate.kind === "checkpoint-replay" &&
  candidate.results.some(
    (result) => result.solved && result.baselineReplayMatches !== true,
  )
) {
  throw new Error(
    "A solved checkpoint candidate has no validated baseline geometry/DRC equivalence",
  )
}
const candidateBySample = new Map(
  candidate.results.map((result) => [result.sample, result]),
)
...
if (currentManifestPath || pinnedManifestPath) {
  if (!currentManifestPath || !pinnedManifestPath)
    throw new Error(
      "Both dataset manifests are required for the current-membership report",
    )
  ...
  type Manifest = {
    sampleCount: number
    samples: Array<{ id: string; inputSha256: string }>
  }
  const current: Manifest = JSON.parse(currentText)
  const pinned: Manifest = JSON.parse(pinnedText)
  const publishedNames = [
    2, 3, 5, 6, 44, 45, 46, 48, 49, 50, 51, 53, 54, 55, 56,
  ].map((id) => `sample${String(id).padStart(3, "0")}`)
  if (
    current.sampleCount !== 15 ||
    JSON.stringify(current.samples.map((sample) => sample.id)) !==
      JSON.stringify(publishedNames)
  ) {
    throw new Error(
      "Current manifest differs from the complete published 15-board revision",
    )
  }
  const pinnedHashes = new Map(
    pinned.samples.map((sample) => [sample.id, sample.inputSha256]),
  )
  for (const sample of current.samples) {
    if (sample.inputSha256 !== pinnedHashes.get(sample.id))
      throw new Error(`Published input changed for ${sample.id}`)
  }
  reportedSampleNames = publishedNames
  reportedCommit = "026a78cb005ab33dde24f2db8fefbfd8d8efa614"
  membershipProvenance = {
    selection:
      "Every board in the separately published current SRJ33 manifest; no board selected by repair04 outcome",
    inputFileHashesMatchPinnedManifest: true,
    currentManifestSha256: createHash("sha256")
      .update(currentText)
      .digest("hex"),
...
const beforeResults = baseline.results.filter((result) =>
  reportedSet.has(result.sample),
)
const afterResults = candidate.results.filter((result) =>
  reportedSet.has(result.sample),
)
...
const boards = beforeResults.map((before) => {
  ...
  return {
    ...
    beforeSolved: before.solved,
    afterSolved: after.solved,
    ...
    baselineReplayMaximumNumericDifference:
      after.baselineReplayMaximumNumericDifference,
    beforeElapsedTimeMs: before.elapsedTimeMs,
    afterElapsedTimeMs: after.elapsedTimeMs,
  }
})
const metric = (kind: "relaxedErrors" | "strictErrors"): object => {
  const beforePasses = beforeResults.filter(
    (result) => result.solved && result[kind]?.length === 0,
  )
  const afterPasses = afterResults.filter(
    (result) => result.solved && result[kind]?.length === 0,
  )
  ...
  const relativeGain =
    beforePasses.length === 0 ? null : gain / beforePasses.length
  return {
    beforePassed: beforePasses.length,
    afterPassed: afterPasses.length,
    beforeRate: beforePasses.length / denominator,
    afterRate: afterPasses.length / denominator,
    additionalPassingBoards: gain,
    percentagePointGain: (gain / denominator) * 100,
    relativeGainPercent: relativeGain === null ? null : relativeGain * 100,
    relativeGainUndefinedReason:
      relativeGain === null ? "The baseline has zero passing boards" : null,
    meetsThirtyPercentRelativeGain:
      relativeGain !== null && relativeGain >= 0.3,
...
const comparison = {
  ...
  relaxedDrc: metric("relaxedErrors"),
  strictDrc: metric("strictErrors"),
  boards,
}
