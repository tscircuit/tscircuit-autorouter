...
if (!baselineDirectory || !outputDirectory || !frozenBundle) {
  throw new Error(
    "Usage: bun scripts/benchmark/replay-repair04-srj33.ts baseline-directory output-directory frozen-replay.js [concurrency] [comma-separated-samples]",
  )
}
...
const bundleSha256 = createHash("sha256")
  .update(await readFile(bundle, "utf8"))
  .digest("hex")
await mkdir(outDir, { recursive: true })
const baselineSummaryBytes = await readFile(
...
baselineFingerprint.update("repair04-baseline-summary-and-outputs-v1\0")
const addFingerprintBytes = (name: string, content: Buffer): void => {
  baselineFingerprint.update(`${name}\0${content.byteLength}\0`)
  baselineFingerprint.update(
    new Uint8Array(content.buffer, content.byteOffset, content.byteLength),
  )
  baselineFingerprint.update("\0")
}
addFingerprintBytes("summary.json", baselineSummaryBytes)
for (const before of [...baseline.results].sort((a, b) =>
  a.sample.localeCompare(b.sample),
)) {
  for (const suffix of ["post-repair03.json", "output.json"]) {
    const filename = `${before.sample}.${suffix}`
    try {
      addFingerprintBytes(
        filename,
        await readFile(resolve(baselineDir, filename)),
      )
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error
      if (before.solved)
        throw new Error(
          `Solved baseline is missing ${filename}; cannot validate replay provenance`,
        )
      // Failed upstream boards can lack checkpoints or final outputs. Their
      // absence is part of the fingerprint and they remain in the denominator.
      baselineFingerprint.update(`${filename}\0missing\0`)
    }
  }
}
...
if (await Bun.file(configurationPath).exists()) {
  ...
  if (!configuration.baselineFingerprintSha256)
    throw new Error(
      "Existing replay configuration has no baseline fingerprint; use a new output directory",
    )
  if (configuration.baselineFingerprintSha256 !== baselineFingerprintSha256)
    throw new Error(
      "Baseline summary, checkpoints, or outputs changed; use a new replay output directory",
    )
} else {
  if (
    (await readdir(outDir)).some((filename) =>
      filename.endsWith(".result.json"),
    )
  ) {
    throw new Error(
      "Existing replay results have no baseline configuration; use a new output directory",
    )
  }
  await writeFile(
    configurationPath,
...
if (new Set(selected).size !== selected.length)
  throw new Error("Sample selection contains duplicates")
for (const sample of selected) {
  if (!baseline.results.some((result) => result.sample === sample))
    throw new Error(`Unknown sample ${sample}`)
}
const queue = [...baseline.results]
  .filter((result) => selected.length === 0 || selected.includes(result.sample))
  .sort((a, b) =>
    selected.length > 0
...
const saveSummary = (): Promise<void> => {
  ...
  const content = JSON.stringify(
    {
      ...
      baselineReplayMatches: ordered.filter(
  ...
  saving = saving.then(() =>
    writeFile(resolve(outDir, "summary.json"), content),
  )
  return saving
}
const runReplay = async (
  ...
  const child = Bun.spawn(
    [
      process.execPath,
      bundle,
      resolve(baselineDir, `${sample}.post-repair03.json`),
      mode,
      output,
  ...
  if (code !== 0)
    throw new Error(
      `${timedOut ? "Timeout: " : ""}${mode} replay ${sample} exited ${code}; see ${sample}.${mode}.log`,
    )
}
await Promise.all(
  Array.from({ length: concurrency }, async (): Promise<void> => {
    while (queue.length > 0) {
      ...
      if (await Bun.file(resultPath).exists()) {
        ...
        try {
          if (!before.solved)
            throw new Error(`Full baseline did not solve: ${before.error}`)
          console.log(`START ${before.sample}`)
          await runReplay(
            before.sample,
            "baseline",
            resolve(baselineDir, `${before.sample}.output.json`),
          )
          ...
          if (
            ...
            throw new Error(
              "Disabled replay DRC counts differ from the full baseline",
            )
          }
          result.baselineReplayMatches = true
          result.baselineReplayByteExact = restored.baselineMatches
          result.baselineReplayMaximumNumericDifference =
            restored.maximumNumericDifference
      ...
      console.log(
        JSON.stringify({
          ...
          baselineMatches: result.baselineReplayMatches,
