...
const summarizeDrcFailingCircuits = (
  results: WorkerResult[],
): DrcFailingCircuitSummary[] =>
  results
    .filter((result) => result.didSolve && !result.relaxedDrcPassed)
    .map((result) => ({
      solverName: result.solverName,
      circuitId: result.scenarioName,
      relaxedDrcErrorCount: result.relaxedDrcErrorCount,
      relaxedDrcErrorTypes: result.relaxedDrcErrorTypes,
    }))
    .sort(
      (a, b) =>
...
const formatDrcFailingCircuitSummary = (
  summary: DrcFailingCircuitSummary[],
) => {
  if (summary.length === 0) {
    return "DRC failing circuits: none"
  }

  return [
    "DRC failing circuits:",
    ...summary.map((entry) => {
      const errorTypes = Object.entries(entry.relaxedDrcErrorTypes)
        .sort(
        ...
        .map(([type, count]) => `${type}=${count}`)
        .join(", ")
      const errorTypeSuffix = errorTypes ? ` (${errorTypes})` : ""
      return `- ${entry.circuitId} [${entry.solverName}]: ${entry.relaxedDrcErrorCount} error${entry.relaxedDrcErrorCount === 1 ? "" : "s"}${errorTypeSuffix}`
    }),
  ].join("\n")
}
...
const main = async () => {
  ...
  const drcFailingCircuits = summarizeDrcFailingCircuits(results)
  const drcFailingCircuitOutput =
    formatDrcFailingCircuitSummary(drcFailingCircuits)
  const output = `Benchmark Results (${effortLabel})\n\n${table}\n\n${drcErrorTypeOutput}\n\n${drcFailingCircuitOutput}\n\nDataset: ${datasetName}\nScenarios: ${scenarios.length}\n`
