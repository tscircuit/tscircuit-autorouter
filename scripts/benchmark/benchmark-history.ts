...
export const writeBenchmarkHistoryDashboard = async ({
  ...
  const [html, css, script] = await Promise.all([
    readFile(join(dashboardDirectory, "index.html")),
    readFile(join(dashboardDirectory, "index.css")),
    readFile(join(dashboardDirectory, "index.js")),
  ])
  ...
  await Promise.all([
    ...
    writeJsonFileAtomically(join(outputDataDirectory, "index.json"), dashboardIndex),
    ...runs.map((run) =>
      writeJsonFileAtomically(join(outputRunsDirectory, `${run.runId}.json`), run),
    ),
  ])
}
