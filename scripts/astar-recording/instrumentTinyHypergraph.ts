...
const EXPECTED_STEP_HASH =
  "a76b77a113fd40903a80d26016189ce0ae1e6470cdfac81d238bb89b266aa631"

export function instrumentTinyHypergraph(
  source: string,
  recorderPath: string,
): string {
  ...
  if (
    ...
    createHash("sha256").update(original).digest("hex") !== EXPECTED_STEP_HASH
  ) {
    throw new Error(
      "A* recorder does not recognize tiny-hypergraph's search loop; update its instrumentation before recording",
    )
  }
  ...
  step = step.replace(
    ...
    '      trace.event("frontier-exhausted")\n      this.onOutOfCandidates()',
  )
  step = step.replace(
  ...
  step = step.replace(
    ...
    'if (this.isPortReservedForDifferentNet(neighborPortId)) { outcome = "port-reserved"; continue }',
  )
  ...
  step = step.replaceAll(
    "        this.onPathFound(",
    '        outcome = "goal-contact"\n        this.onPathFound(',
  )
  step = step.replace(
    "if (neighborPortId === currentCandidate.portId) continue",
    'if (neighborPortId === currentCandidate.portId) { outcome = "same-port"; continue }',
  )
  step = step.replace(
    "if (problem.portSectionMask[neighborPortId] === 0) continue",
    'if (problem.portSectionMask[neighborPortId] === 0) { outcome = "outside-section"; continue }',
  )
  step = step.replace(
  ...
  return (
    `import { recorder } from ${JSON.stringify(recorderPath)}\n` +
