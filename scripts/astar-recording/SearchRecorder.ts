...
type CandidateRecord = Omit<Candidate, "prevCandidate"> & { id: number; parentId: number | null }
type SolverSummary = { id: number; name: string; events: number; attempts: number; counts: Record<string, number> }
type SearchTrace = {
...
export class SearchRecorder {
  ...
  attach(solver: TinyHyperGraphSolver): SearchTrace {
    ...
    const summary: SolverSummary = { id: this.summaries.length + 1, name: solver.constructor.name, events: 0, attempts: 0, counts: {} }
    this.summaries.push(summary)
    const solverDir = path.join(this.outputDir, "solvers", String(summary.id))
    mkdirSync(solverDir)
    writeFileSync(path.join(solverDir, "input.json"), json({ solverId: summary.id, name: summary.name, stage: this.stage, topology: solver.topology, problem: solver.problem }))
    const candidate = (value: Candidate): CandidateRecord => {
      ...
      const parentId = value.prevCandidate ? this.candidates.get(value.prevCandidate) : null
      if (parentId === undefined) throw new Error("A* candidate parent was not recorded")
    ...
    const event = (type: string, data: EventData = {}): void => {
      ...
      if (this.writeIndividualEvents && shard !== this.currentShard) {
        mkdirSync(path.join(this.outputDir, "events", shardName), { recursive: true })
        this.currentShard = shard
      }
      const record = { sequence, type, solverId: summary.id, stage: this.stage, iteration: solver.iterations, attempt: summary.attempts, routeId: routeId ?? null, queueLength: solver.state.candidateQueue.length, ...data }
      const serialized = json(record)
      if (this.writeIndividualEvents) {
        writeFileSync(path.join(this.outputDir, "events", shardName, `${String(sequence).padStart(9, "0")}.json`), serialized + "\n")
      }
      if (this.eventsFd === undefined) throw new Error("A* event log is closed")
      writeSync(this.eventsFd, serialized + "\n")
      this.examples[type] ??= this.writeIndividualEvents ? `events/${shardName}/${String(sequence).padStart(9, "0")}.json` : "events.jsonl"
    ...
    const attempt = (): void => {
      ...
      summary.attempts++
      writeFileSync(path.join(solverDir, `attempt-${summary.attempts}.json`), json({ routeId, state: { portAssignment: solver.state.portAssignment, regionSegments: solver.state.regionSegments, regionCongestionCost: solver.state.regionCongestionCost, unroutedRoutes: solver.state.unroutedRoutes } }))
      event("attempt-start", { route: routeId === undefined ? null : solver.problem.routeMetadata?.[routeId] })
    }
    ...
    queue.clear = (): void => {
      event("frontier-clear", { discardedCandidateIds: queue.toArray().map((value): number => candidate(value).id) })
      originalClear()
    }
    const originalPath = solver.onPathFound.bind(solver)
    solver.onPathFound = (value: Candidate): void => {
      event("path-proposed", { candidate: candidate(value), segments: solver.getSolvedPathSegments(value) })
      originalPath(value)
      event("path-result", { accepted: solver.state.currentRouteId === undefined, solved: solver.solved, failed: solver.failed })
    }
    const trace: SearchTrace = { event, candidate, attempt }
    this.traces.set(solver, trace)
    event("solver-start")
    for (const value of queue.toArray()) event("candidate", { candidate: candidate(value), existingAtAttach: true })
    return trace
  }

  finish(status: string, details: EventData = {}): void {
    if (!this.outputDir) throw new Error("A* recorder has not been started")
    writeFileSync(path.join(this.outputDir, "manifest.json"), JSON.stringify({ schemaVersion: 1, status, eventCount: this.sequence, candidateCount: this.nextCandidateId, counts: this.counts, solvers: this.summaries, examples: this.examples, exampleSequences: this.exampleSequences, events: "events.jsonl", individualEvents: this.writeIndividualEvents ? "events/<floor((sequence-1)/1000), padded to 6>/<sequence, padded to 9>.json" : null, ...details }, null, 2))
    const lines: string[] = [
      ...
      "",
      ...Object.entries(this.examples).map(([type, file]): string => this.writeIndividualEvents ? `- [${type}](${file}) — ${this.counts[type]} events` : `- ${type}: ${this.counts[type]} events (see events.jsonl)` ),
      "",
      ...
      ...this.summaries.flatMap((summary): string[] => [
        ...
        ...Array.from({ length: summary.attempts }, (_, index): string => `  - [Attempt ${index + 1}](solvers/${summary.id}/attempt-${index + 1}.json)`),
      ]),
      "",
