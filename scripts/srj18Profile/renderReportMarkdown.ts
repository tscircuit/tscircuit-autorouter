...
import type { ClassTiming, NodeSummary, SampleSummary, StageSummary } from "./reportTypes"
...
type Summary = {
  ...
  nodes: NodeSummary[]; classes: ClassTiming[];
}

function markdownTable(columns: string[], rows: (string | number | null)[][]): string {
  const header = `| ${columns.join(" | ")} |`
  const separator = `| ${columns.map(() => "---").join(" | ")} |`
  const body = rows.map((row) => `| ${row.map((field) => String(field ?? "—").replaceAll("|", "\\|").replaceAll("\n", " ")).join(" | ")} |`)
...
export async function renderReportMarkdown(outDir: string): Promise<void> {
  const summary = await Bun.file(resolve(outDir, "summary.json")).json() as Summary
  const solved = summary.samples.filter((sample) => sample.solved)
  const solverTotals = new Map<SolverName, { solver: string; selfMs: number; constructionSelfMs: number; constructions: number; stepCalls: number }>()
  for (const timing of summary.classes) {
    let total = solverTotals.get(timing.solver)
    if (!total) {
      total = { solver: timing.solver, selfMs: 0, constructionSelfMs: 0, constructions: 0, stepCalls: 0 }
  ...
  const solvers = [...solverTotals.values()].sort((left, right) => right.selfMs - left.selfMs)
  const markdown = [
    ...
    "## Every sample",
    markdownTable(["Sample", "Median s", "Min s", "Max s", "Solved", "DRC errors", "Output parity", "Outcome parity", "Profiler ×"], summary.samples.map((sample) => [sample.sampleId, formatDurationSeconds(sample.medianMs), formatDurationSeconds(sample.minMs), formatDurationSeconds(sample.maxMs), String(sample.solved), sample.drcErrors, sample.outputParity === null ? "not applicable / not measured" : String(sample.outputParity), sample.outcomeParity === null ? "not measured" : String(sample.outcomeParity), sample.overheadRatio?.toFixed(3) ?? null])),
    ...
    markdownTable(["Stage", "Total median s", "Share %", "Slowest sample", "Its stage s"], summary.stages.map((stage) => [stage.stage, formatDurationSeconds(stage.durationMs), stage.sharePercent.toFixed(2), stage.maxSample, formatDurationSeconds(stage.maxSampleMs)])),
    ...
    markdownTable(["Sample", "Node", "Median s", "Winner", "Ports", "Connections", "Width mm", "Height mm", "Growths"], summary.nodes.slice(0, 40).map((node) => [node.sampleId, node.nodeId, formatDurationSeconds(node.medianMs), node.winningSolver, node.ports, node.connections, node.width.toFixed(3), node.height.toFixed(3), node.resizeCount])),
    ...
    markdownTable(["Solver", "Detailed self s", "Constructor self s", "Constructions", "step calls"], solvers.slice(0, 30).map((solver) => [solver.solver, formatDurationSeconds(solver.selfMs), formatDurationSeconds(solver.constructionSelfMs), solver.constructions, solver.stepCalls])),
    ...
    "## Failures",
    ...summary.samples.filter((sample) => !sample.solved).map((sample) => `- **${sample.sampleId}**: ${sample.error}`),
    "## Raw evidence",
