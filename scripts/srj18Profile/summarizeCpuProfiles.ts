...
type CpuFunctionTiming = {
  ...
  selfMs: number
  inclusiveMs: number
  routingSelfMs: number
  routingInclusiveMs: number
  samples: number
...
export async function summarizeCpuProfiles(outDir: string): Promise<void> {
  ...
  const profileOptions = (await optionsFile.exists())
    ? ((await optionsFile.json()) as Record<
        string,
        { intervalUs: number; smol: boolean }
      >)
    : null
  const manifests = (await readdir(outDir))
    .filter(
      (name) => name === "manifest-cpu.json" || name === "manifest-all.json",
    )
    .sort()
    .reverse()
  const manifest = manifests.length
    ? ((await Bun.file(resolve(outDir, manifests[0]!)).json()) as {
  ...
  const sampleRows: {
    ...
    samples: number
    sampledMs: number
    routingSampledMs: number
    profileDurationMs: number
  }[] = []
  for (const sampleId of (await readdir(cpuDir))
    .filter((name) => /^sample\d{3}$/.test(name))
    .sort()) {
    const profiles = (await readdir(resolve(cpuDir, sampleId))).filter((name) =>
      /\.cpuprofile(?:\.gz)?$/.test(name),
    )
    if (profiles.length !== 1)
      throw new Error(
        `Expected exactly one native CPU profile for ${sampleId}, got ${profiles.length}`,
      )
    const profileFile = Bun.file(resolve(cpuDir, sampleId, profiles[0]!))
    const profile = (
      profiles[0]!.endsWith(".gz")
        ? JSON.parse(
            new TextDecoder().decode(
              Bun.gunzipSync(await profileFile.arrayBuffer()),
            ),
          )
        : await profileFile.json()
    ) as CpuProfile
    if (profile.samples.length !== profile.timeDeltas.length)
      throw new Error(`Invalid CPU sample timing lengths: ${sampleId}`)
    ...
    for (let index = 0; index < profile.samples.length; index++) {
      ...
      if (durationMs < 0)
        throw new Error(`Negative CPU sample interval: ${sampleId}`)
      ...
      const routing = stack.some(
        (node) =>
          node.callFrame.functionName === "step" &&
          node.callFrame.url.includes("BaseSolver"),
      )
      ...
      for (let depth = 0; depth < stack.length; depth++) {
        ...
        const file =
          frame.url.replace("file://", "").replace(`${process.cwd()}/`, "") ||
          "[native]"
        ...
        if (depth === 0) {
          ...
          timing.samples++
          if (routing) {
            timing.routingSelfMs += durationMs
            timing.routingSamples++
          }
        }
      }
    }
    ...
    sampleRows.push({
      sampleId,
      intervalUs: options?.intervalUs ?? manifest?.cpuIntervalUs ?? 1000,
  ...
  const functionRows = [...functions.values()].sort(
    (left, right) => right.routingSelfMs - left.routingSelfMs,
  )
  ...
  await writeFile(
    resolve(outDir, "cpu-summary.json"),
    JSON.stringify({ samples: sampleRows, functions: functionRows }),
  )
  console.log(
    `Native CPU summary: ${sampleRows.length} samples, ${functionRows.length} functions`,
  )
}
