...
export type SampleSummary = {
  ...
  outputParity: boolean | null
  outcomeParity: boolean | null
  error: string | null
  detailedMs: number | null
  overheadRatio: number | null
  connections: number
  obstacles: number
  highDensityNodes: number
}
export type StageSummary = {
  ...
  omittedByInternalMs: number
}
export type SampleStageSummary = {
  sampleId: string
  stage: string
  durationMs: number
  sharePercent: number
...
export type NodeSummary = {
  ...
  winningSolver: string | null
  resizeCount: number | null
  totalRecordedGrowthAttempts: number | null
  regionalRecovery: boolean | null
  width: number
  height: number
  x: number
  y: number
