...
export function instrumentSolverSource(options: {
  ...
  while (pending.length) {
    ...
    if (
      ...
      node.body &&
      ["step", "_step", "solve", "setup", "_setup"].includes(
      ...
      if (
        ...
        throw new Error(
          `Async/generator solver lifecycle cannot use synchronous profiling: ${options.path}`,
        )
      }
      const owner =
        ts.isClassLike(node.parent) && node.parent.name
          ? node.parent.name.text
          : options.path
      const method = node.name.getText(syntax)
      const label = JSON.stringify({
        owner: `${options.path}:${owner}`,
        method,
      })
      edits.push({
        offset: node.body.getStart(syntax) + 1,
    ...
    if (
      ...
      /solver/i.test(node.expression.getText(syntax))
    ) {
      const constructor = node.expression.getText(syntax)
      edits.push({
        offset: node.getStart(syntax),
        insertion: "__srj18Profile.construct(() => (",
      })
      edits.push({
        offset: node.end,
        insertion: `), ${JSON.stringify(constructor)})`,
      })
  ...
  for (const edit of edits
    ...
    .sort(
      (left, right) => right.offset - left.offset || right.index - left.index,
    )) {
    contents =
      contents.slice(0, edit.offset) +
      edit.insertion +
      contents.slice(edit.offset)
  }
  if (edits.length) {
