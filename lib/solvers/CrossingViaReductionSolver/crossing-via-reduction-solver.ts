...
export class CrossingViaReductionSolver extends TraceSimplificationSolverAdapter {
  ...
  private collapseDetourSection(input: {
  ...
  private relocateTransitionVia(input: {
    route: HighDensityRoute
    section: RouteSection
    targetZ: number
    side: "start" | "end"
  ...
  private relocateTransitionVias(input: {
    route: HighDensityRoute
    sections: RouteSection[]
    crossingGroups: Array<{
  ...
  getReducedHdRoutes(): HighDensityRoute[] {
    return this.callSolver(this.binding.getReducedHdRoutes, [])
  }
