...
export class CapacityPathingSolver extends BaseSolver {
  ...
  getTotalCapacity(node: CapacityMeshNode, connectionName?: string): number {
    // If we have a specific connection, use its trace thickness for capacity calculation
    if (connectionName) {
      ...
      if (connection) {
        ...
        return getTunedTotalCapacity1(
          node,
          this.hyperParameters.MAX_CAPACITY_FACTOR ?? 1,
          traceThickness,
          viaDiameter,
        )
      }
    ...
    // Fallback to the original depth-based calculation for backward compatibility
  ...
  doesNodeHaveCapacityForTrace(
    ...
    const totalCapacity = this.getTotalCapacity(node, connectionName)
  ...
  _step() {
    ...
    for (const neighborNode of neighborNodes) {
      ...
      if (
        !this.doesNodeHaveCapacityForTrace(
          neighborNode,
