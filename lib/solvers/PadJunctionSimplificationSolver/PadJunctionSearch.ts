...
type SearchGoal = { kind: "anchor"; point: PadJunctionPoint } | { kind: "pad" }
...
export class PadJunctionSearch {
  ...
  constructor(private readonly input: SearchInput) {
    ...
    if (xIndex < 0 || yIndex < 0)
      throw new Error("PadJunctionSearch: start missing from grid")
  ...
  private createCoordinates(axis: "x" | "y", padding: number): number[] {
    ...
    const exact = [
      input.start[axis],
      input.anchors[0][axis],
      input.anchors[1][axis],
    ...
    return [...new Set(exact)]
      ...
      .filter((value, index, values): boolean => {
        ...
        return (
          ...
          value === input.start[axis] ||
          value === input.anchors[0][axis] ||
  ...
  private distanceToGoal(point: PadJunctionPoint): number {
    ...
    if (goal.kind === "anchor")
      return Math.hypot(point.x - goal.point.x, point.y - goal.point.y)
    const halfWidth = this.input.pad.width / 2 - this.input.width / 2
    const halfHeight = this.input.pad.height / 2 - this.input.width / 2
    const dx = Math.max(
      0,
      Math.abs(point.x - this.input.pad.center.x) - halfWidth,
    )
    const dy = Math.max(
      0,
      Math.abs(point.y - this.input.pad.center.y) - halfHeight,
    )
    return Math.hypot(dx, dy)
  }

  private precedes(first: FrontierEntry, second: FrontierEntry): boolean {
    if (first.estimate.bends !== second.estimate.bends)
      return first.estimate.bends < second.estimate.bends
    if (first.estimate.length !== second.estimate.length)
      return first.estimate.length < second.estimate.length
    if (first.cost.length !== second.cost.length)
      return first.cost.length > second.cost.length
  ...
  private getFrontierEntryOrThrow(index: number): FrontierEntry {
    ...
    if (!entry) {
      throw new Error(
        `PadJunctionSearch: missing frontier entry at index ${index}`,
      )
    }
    return entry
  }
  ...
  step(): void {
    ...
    if (this.distanceToGoal(current.point) < EPSILON) {
      ...
      if (path.length === 0)
        throw new Error("PadJunctionSearch: reached goal without a path")
      this.result = { status: "found", path }
      return
    }
    for (const { direction, opposite, xOffset, yOffset } of DIRECTIONS) {
      if (current.cost.length === 0 && direction !== this.input.direction)
        continue
      ...
      if (
        xIndex < 0 ||
        yIndex < 0 ||
        xIndex >= this.xCoordinates.length ||
        yIndex >= this.yCoordinates.length
      )
        continue
      const x = this.xCoordinates[xIndex]
      const y = this.yCoordinates[yIndex]
      if (x === undefined || y === undefined)
        throw new Error("PadJunctionSearch: neighbor missing from grid")
      ...
      const cost = {
        ...
        length:
          current.cost.length +
          Math.hypot(point.x - current.point.x, point.y - current.point.y),
      }
      const previous = this.bestCosts.get(key)
      const alreadyCheaper =
        previous &&
        (previous.bends < cost.bends ||
