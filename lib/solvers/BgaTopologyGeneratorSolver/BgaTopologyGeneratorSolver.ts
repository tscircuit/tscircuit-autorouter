...
class InitialTopology extends BaseSolver {
  ...
  step() {
    ...
    const obstaclesPartOfComponent = srj.obstacles.filter((obstacle) => {
      return (
        doBoundsOverlap(getBoundFromCenteredRect(obstacle), compoentBounds) &&
    ...
    const getCopperPourInsideOurComponentBounds = getAllCopperPours.filter(
      (obstacle) => {
    ...
    const blockedLayers = getCopperPourInsideOurComponentBounds
      .map((obstacle) => obstacle.layers.map(mapLayerNameToZ))
      .flat()
    ...
    const sortedYs = [
      ...new Set(
        obstaclesPartOfComponent
          .map((obstacle) => obstacle.center.y)
          .sort((a, b) => a - b),
      ),
    ]
    const sortedXs = [
