...
export class GrowShrinkHighDensityIntraNodeSolver extends BaseSolver {
  ...
  constructor(params: GrowShrinkHighDensityIntraNodeSolverParams) {
    ...
    const nativeInput = {
      ...input,
      hasCustomValidator:
        typeof params.growShrinkSolutionValidator === "function",
    }
    this.binding = this.scope.run(() =>
      GrowShrinkHighDensityIntraNodeSolver.createBinding(key, nativeInput),
    )
  ...
  private static createBinding(
    key: number,
    input: bindings.HighDensityValue,
  ): bindings.GrowShrinkHighDensityIntraNodeSolver {
    const owner = (): GrowShrinkHighDensityIntraNodeSolver => {
      const scope = PortfolioCallbackScope.current
      if (!scope)
        throw new Error("Grow/shrink callback requires an execution scope")
    ...
    return new bindings.GrowShrinkHighDensityIntraNodeSolver(
      ...
      ): { accepted: boolean; state: Record<string, unknown> } => {
        const solver = owner()
    ...
    for (const field of [
      ...
      Object.defineProperty(this, field, {
        ...
        get: (): unknown => {
          if (
            !this.synchronizing &&
            PortfolioCallbackScope.current !== this.scope
          ) {
            this.observed = true
            this.syncSolverState()
          }
        ...
        set: (incoming: unknown): void => {
          if (!this.synchronizing) {
            if (PortfolioCallbackScope.current !== this.scope)
              this.syncSolverState()
            this.dirty = true
          }
          value = incoming
  ...
  prepareSolverRun(): void {
    if (this.initialized && (this.dirty || this.observed || this.routes))
      this.pushSolverState()
  }

  finishSolverRun(): void {
    this.syncObservedState()
  }
  ...
  override getConstructorParams(): GrowShrinkHighDensityIntraNodeSolverParams {
  ...
  get solvedRoutes(): HighDensityIntraNodeRoute[] {
    ...
    if (!this.routes) {
      this.routes =
        this.winningSolver && this.scaleFactor === 1
  ...
  set solvedRoutes(routes: HighDensityIntraNodeRoute[]) {
    if (PortfolioCallbackScope.current !== this.scope) this.syncSolverState()
    this.routes = routes
    this.dirty = true
  }
  ...
  private statePatch(): Record<string, unknown> {
    ...
    return {
      ...
      constructorParams: {
        ...settings,
        hasCustomValidator:
          typeof this.constructorParams.growShrinkSolutionValidator ===
          "function",
      },
      nodeWithPortPoints: this.nodeWithPortPoints,
      scaleFactor: this.scaleFactor,
      growthAttempts: this.growthAttempts,
      maxGrowthAttempts: this.maxGrowthAttempts,
      stats: this.stats,
  ...
  pushSolverState(): void {
    ...
    try {
      ...
      this.binding.restore({
        ...
        maxGrowthAttempts: this.maxGrowthAttempts,
        stats: this.stats,
        activeId: this.activeSubSolver
          ? this.childId(this.activeSubSolver)
          : null,
        winnerId: this.winningSolver ? this.childId(this.winningSolver) : null,
        failedIds: this.failedSolvers.map((child): number =>
          this.childId(child),
        ),
        ...(this.routes ? { solvedRoutes: this.routes } : {}),
      })
  ...
  syncSolverState(detachedSnapshot?: SolverStateSnapshot): void {
    ...
    try {
      ...
      if (detachedSnapshot && !this.routes && state.solvedRoutes)
        this.routes = state.solvedRoutes
      ...
      this.activeSubSolver =
        state.activeId === null ? null : this.child(state.activeId)
      this.winningSolver =
        state.winnerId === null ? undefined : this.child(state.winnerId)
      this.failedSolvers.splice(
        0,
        this.failedSolvers.length,
        ...state.failedIds.map(
          (id): PortfolioSingleIntraNodeSolver => this.child(id),
        ),
      )
      for (const child of this.failedSolvers)
        child.getPortfolioAdapter().finishSolverRun()
  ...
  override _step(): void {
    ...
    try {
      this.scope.runGrowth(this.scopeKey, this, (): void =>
  ...
  override visualize(): GraphicsObject {
    this.pushSolverState()
    return this.scope.runGrowth(
      this.scopeKey,
      this,
      (): GraphicsObject => this.binding.visualize(),
    )
  }
  ...
  disposeUnobserved(): boolean {
    if (
      this.observed ||
      this.dirty ||
      typeof this.constructorParams.growShrinkSolutionValidator === "function"
    )
      return false
    const supervisors = [...this.children.values()].map((child) =>
      child.getPortfolioAdapter(),
    )
    if (
      supervisors.some(
        (supervisor): boolean => !supervisor.canDisposeUnobserved(),
      )
    )
      return false
  ...
  releaseOrchestrationScope(): void {
    this.scope.releaseGrowth(this.scopeKey)
  }

  dispose(): void {
    this.releaseOrchestrationScope()
    this.binding.free()
  }
}
