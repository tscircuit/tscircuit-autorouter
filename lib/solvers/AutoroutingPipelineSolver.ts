...
} from "../utils/getTraceThicknessFromConnection"
...
export class AutoroutingPipelineSolver extends BaseSolver {
  ...
  constructor(
    ...
    if (opts.capacityDepth === undefined) {
      ...
      // Find the maximum trace thickness and via diameter from all connections
      let maxTraceThickness = 0.15 // Default standard thickness
      let maxViaDiameter = 0.6 // Default standard via diameter

      for (const connection of srj.connections) {
        const traceThickness = getTraceThicknessFromConnection(connection)
        const viaDiameter = getViaDiameterFromConnection(connection)

        maxTraceThickness = Math.max(maxTraceThickness, traceThickness)
        maxViaDiameter = Math.max(maxViaDiameter, viaDiameter)
      }
  ...
  _step() {
    ...
    // @ts-ignore - TypeScript can't infer the constructor parameter types properly
