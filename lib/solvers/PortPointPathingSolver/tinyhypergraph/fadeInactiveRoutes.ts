...
export function fadeInactiveRoutes(graphics: GraphicsObject, solver: TinyHyperGraphSolver, activeRouteId: number): void {
  ...
  for (const line of graphics.lines ?? []) {
    ...
    if (routeLabel?.startsWith("route: ") && routeLabel !== activeLabel && line.strokeColor) {
      line.strokeColor = line.strokeColor.replace(/,\s*[\d.]+\)$/, ", 0.15)")
    }
  }
  for (const point of graphics.points ?? []) {
    const routeLabel = point.label?.split("\n")[0]
    if (routeLabel?.startsWith("route: ") && routeLabel !== activeLabel && point.color) {
