...
export function visualizeCongestionRerouting(
  ...
  if (!node)
    throw new Error(`Missing visualization node for region ${attempt.regionId}`)
  const radius = Math.max(node.width * 0.9, node.height * 0.9, 2)
  const bounds: Bounds = {
    minX: node.center.x - radius,
    maxX: node.center.x + radius,
    minY: node.center.y - radius,
    maxY: node.center.y + radius,
  }
  const graphics: GraphicsObject = input.focus
    ? { rects: [], lines: [], points: [] }
    : solver.visualize()
  ...
  if (input.focus) {
    for (const region of input.nodesByRegionId.values()) {
      ...
      graphics.rects!.push({
        ...createRectFromCapacityNode(region),
        center: { x: (minX + maxX) / 2, y: (minY + maxY) / 2 },
        width: maxX - minX,
        height: maxY - minY,
        fill: region._containsObstacle
          ? "rgba(255,0,0,0.12)"
          : "rgba(0,0,0,0.015)",
  ...
  graphics.rects = (graphics.rects ?? []).filter(
    (rect) => !rect.label?.startsWith("TEMPORARY OBSTACLE:"),
  )
  graphics.lines ??= []
  graphics.rects.push({
  ...
  const affectedRoutes = new Set(
    input.previousSolver?.state.regionSegments[attempt.regionId].map(
      ([routeId]) => routeId,
    ),
  )
  const sources = input.previousSolver
    ? [input.previousSolver, solver]
    : [solver]
        ...
        graphics.lines.push({
          ...
          strokeColor: ghost
            ...
            : routeId === attempt.routeId
              ? "#2563eb"
              : affectedRoutes.has(routeId)
                ? "rgba(210,110,20,0.18)"
  ...
  if (input.focus && phase === "searching") {
    ...
    while (cursor?.prevCandidate) {
      ...
      const points = clipSegment(
        {
          x: solver.topology.portX[previous.portId],
          y: solver.topology.portY[previous.portId],
        },
        {
          x: solver.topology.portX[cursor.portId],
          y: solver.topology.portY[cursor.portId],
        },
        bounds,
      )
      if (points)
        graphics.lines.push({
          points,
          strokeColor: "#16a34a",
          strokeDash: node.availableZ.includes(
            solver.topology.portZ[cursor.portId],
          )
            ? undefined
            : "6 3",
          label: "Current search candidate",
          layer: `z${solver.topology.portZ[cursor.portId]}`,
        })
      cursor = previous
    }
  }
