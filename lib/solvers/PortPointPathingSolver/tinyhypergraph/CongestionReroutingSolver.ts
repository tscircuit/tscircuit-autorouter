...
type CongestionScore = { regionPf: Map<number, number>; maxPf: number; squaredPfSum: number }
export type ReroutingPhase = "ready" | "selected" | "blocked" | "searching" | "accepted" | "rejected" | "exhausted" | "complete"
...
export class CongestionReroutingSolver extends BaseSolver {
  ...
  constructor(private readonly input: CongestionReroutingInput) {
    ...
    if (!input.solver.solved || input.solver.failed) {
      throw new Error("CongestionReroutingSolver requires a complete pathing solution")
    }
    if (!Number.isInteger(this.maxAttempts) || this.maxAttempts < 0 ||
      !Number.isFinite(this.pfThreshold) || this.pfThreshold < 0 ||
      !Number.isInteger(this.maxIterationsPerAttempt) || this.maxIterationsPerAttempt < 1) {
      throw new Error("CongestionReroutingSolver: invalid search budget or PF threshold")
    }
    this.portSectionMask = new Int8Array(input.portSectionMask ?? input.solver.problem.portSectionMask)
    if (this.portSectionMask.length !== input.solver.topology.portCount ||
      this.portSectionMask.some(value => value !== 0 && value !== 1)) {
      throw new Error("CongestionReroutingSolver: invalid original graph permissions")
    }
    ...
    this.MAX_ITERATIONS = this.maxAttempts * (this.maxIterationsPerAttempt + 4) + 2
    this.selectAttempts()
    this.publishStats()
  }
  ...
  private measure(solver: TinyHyperGraphSolver): CongestionScore {
    ...
    for (const [regionId, node] of this.input.nodesByRegionId) {
      ...
      if (!segments) throw new Error(`CongestionReroutingSolver: missing region ${regionId}`)
      const portPoints = segments.flatMap(([routeId, fromPortId, toPortId]) =>
        [fromPortId, toPortId].map((portId) => ({
          x: solver.topology.portX[portId], y: solver.topology.portY[portId],
          z: solver.topology.portZ[portId], connectionName: String(routeId),
        })),
      )
      const crossings = getIntraNodeCrossingsUsingCircle({ ...node, portPoints })
      const pf = calculateNodeProbabilityOfFailure(node, crossings.numSameLayerCrossings,
        crossings.numEntryExitLayerChanges, crossings.numTransitionPairCrossings)
      if (!Number.isFinite(pf) || pf < 0) throw new Error(`Invalid PF in region ${regionId}`)
  ...
  private selectAttempts(): void {
    ...
    const hotRegions = [...this.score.regionPf].filter(([, pf]) => pf > this.pfThreshold)
      .sort(([a, pfA], [b, pfB]) => pfB - pfA || a - b)
    for (const [regionId] of hotRegions) {
      const routeIds = [...new Set(state.regionSegments[regionId].map(([routeId]) => routeId))].sort((a,b) => a-b)
      for (const routeId of routeIds) {
        if (this.input.preservedRouteIds?.has(routeId)) continue
        const startRegions = topology.incidentPortRegion[problem.routeStartPort[routeId]]
        const endRegions = topology.incidentPortRegion[problem.routeEndPort[routeId]]
        if (startRegions.includes(regionId) || endRegions.includes(regionId)) continue
  ...
  override _step(): void {
    if (this.candidate) {
      ...
      if (this.candidate.failed && !this.candidate.searchExhausted) {
        throw new Error(`Congestion rerouting invariant failure: ${this.candidate.error}`)
      }
      if (this.candidate.solved) {
        const score = this.measure(this.candidate)
        if (score.maxPf <= this.score.maxPf + 1e-9 && score.squaredPfSum < this.score.squaredPfSum - 1e-9) {
      ...
      } else if (this.candidate.searchExhausted || this.candidate.iterations >= this.maxIterationsPerAttempt) {
    ...
    if (this.phase !== "selected") {
      ...
      this.previousRegionPf = this.score.regionPf.get(this.selectedAttempt.regionId)!
      this.phase = "selected"
      this.publishStats()
      return
    }
    const { regionId, routeId } = this.pending.shift()!
    this.candidate = new RegionAvoidingRouteSolver(this.incumbent, routeId, regionId, this.maxIterationsPerAttempt, this.portSectionMask)
    this.phase = "blocked"
  ...
  override visualize(options: { focus?: boolean } = {}): GraphicsObject {
    return visualizeCongestionRerouting({
      ...
      regionPf: this.selectedAttempt ? this.score.regionPf.get(this.selectedAttempt.regionId)! : 0,
  ...
  private publishStats(): void {
    this.stats = { phase: this.phase, attempts: this.attempts, accepted: this.accepted, rejected: this.rejected,
      exhausted: this.exhausted, initialMaxPf: this.initialMaxPf, maxPf: this.score.maxPf,
      initialSquaredPfSum: this.initialSquaredPfSum, squaredPfSum: this.score.squaredPfSum,
      stopReason: this.solved ? (this.attempts >= this.maxAttempts ? "budget" : "no-improving-move") : null }
  }

  override getOutput(): TinyHyperGraphSolver {
    if (!this.solved || this.failed) {
      throw new Error("CongestionReroutingSolver: output requested before successful completion")
    }
