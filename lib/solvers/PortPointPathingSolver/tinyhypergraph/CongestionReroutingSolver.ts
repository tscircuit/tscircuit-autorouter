...
export class CongestionReroutingSolver extends BaseSolver {
  ...
  constructor(private readonly input: CongestionReroutingInput) {
    ...
    this.MAX_ITERATIONS = this.maxAttempts * (this.maxIterationsPerAttempt + 3) + 2
    this.selectAttempts()
    this.publishStats()
  }
  ...
  override visualize(): GraphicsObject {
    const visibleSolver = this.candidate ?? this.incumbent
    // Keep the established tiny-hypergraph rendering for accepted and trial paths.
    const graphics = visibleSolver.visualize()
    return graphics
  }
  ...
  private publishStats(): void {
    this.stats = { attempts: this.attempts, accepted: this.accepted, rejected: this.rejected,
