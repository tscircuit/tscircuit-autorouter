...
const isSerializedParams = (
  params:
    HgPortPointPathingSolverParams | SerializedHgPortPointPathingSolverParams,
...
const normalizeParams = (
  ...
  const regions = params.graph.regions.map((region): NormalizedRegion => ({
...
export class TinyHypergraphBfsPortPointPathingSolver extends BaseSolver {
  ...
  private getOrCreateActiveRouteBfs(solver: any): {
    ...
    const startingStates = startingIncidentRegions
      ...
      .map((regionId: number): TinyRouteBfsState => ({
  ...
  private visualizeActiveRouteBfs(): GraphicsObject {
    ...
    const routeMetadata =
      solver.problem.routeMetadata?.[activeRouteBfs.routeId];
    const startTarget =
      routeMetadata?.simpleRouteConnection?.pointsToConnect?.[0];
    const endTarget =
      routeMetadata?.simpleRouteConnection?.pointsToConnect?.[1];
