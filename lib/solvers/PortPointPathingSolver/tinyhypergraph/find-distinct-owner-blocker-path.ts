...
const compareLabels = <TState, TStateKey, TOwner, THopData>(
  a: SearchLabel<TState, TStateKey, TOwner, THopData>,
  b: SearchLabel<TState, TStateKey, TOwner, THopData>,
): number => {
  const ownerCountDifference = a.owners.size - b.owners.size
...
class SearchLabelQueue<TState, TStateKey, TOwner, THopData> {
  ...
  push(label: SearchLabel<TState, TStateKey, TOwner, THopData>): void {
    ...
    while (index > 0) {
      ...
      if (compareLabels(this.heap[parentIndex], this.heap[index]) <= 0) break
...
const labelDominates = <TState, TStateKey, TOwner, THopData>(
  ...
  if (!isOwnerSubset(a.owners, b.owners)) return false
