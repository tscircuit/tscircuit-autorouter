...
export function buildHyperGraph(params: {
  ...
  const connectivityMap = params.connectivityMap

  for (const cmnNode of params.capacityMeshNodes) {
    const canonicalConnectedTo = connectivityMap
      ? cmnNode._connectedTo?.map((connectionAlias) =>
          getCanonicalNetIdOrThrow(connectivityMap, connectionAlias),
        )
      : undefined
    graph.regions.push({
  ...
  for (const connection of params.simpleRouteJsonConnections) {
    ...
    connections.push({
      ...
      mutuallyConnectedNetworkId: connectivityMap
        ? getCanonicalNetIdOrThrow(connectivityMap, connection.name)
