 ...
 */
export function createSrjWithHoleClearance(srj: SimpleRouteJson): SimpleRouteJson {
  const clearance = srj.minTraceToHoleClearance
  if (clearance === undefined) return srj
  if (!Number.isFinite(clearance) || clearance < 0) {
    throw new Error("minTraceToHoleClearance must be a finite non-negative distance in mm")
  }
  ...
  return {
    ...
    obstacles: srj.obstacles.map((obstacle, index): Obstacle => {
      ...
      if (obstacle.connectedTo.length > 0) {
        throw new Error(`Non-plated hole ${name} must have an empty connectedTo array`)
      }
      if (
        !Number.isFinite(obstacle.width) || obstacle.width <= 0 ||
        !Number.isFinite(obstacle.height) || obstacle.height <= 0 ||
        !Number.isFinite(obstacle.center.x) || !Number.isFinite(obstacle.center.y) ||
      ...
      return {
        ...
        ...(obstacle.shape === "circle" ? { ccwRotationDegrees: undefined } : {}),
      }
    }),
  }
}
