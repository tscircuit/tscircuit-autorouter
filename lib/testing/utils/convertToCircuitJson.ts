...
function convertHdRouteToCircuitJson(
  ...
  // Use the hdRoute's trace thickness if width is not provided
  const effectiveWidth = width ?? hdRoute.traceThickness;
      ...
      sourceTraces.push({
        ...
        connected_source_port_ids: connectedPortIds.concat(
          obstaclesContainingEndpoints.flatMap((o) => [
            `obstacle_${o.center.x.toFixed(3)}_${o.center.y.toFixed(
              3
            )}_${o.layers.join(".")}`,
            ...o.connectedTo,
          ])
