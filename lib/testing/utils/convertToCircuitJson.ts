...
function getBestObstaclePcbPortId(
  ...
  if (!obstacleCenter) return candidatePortIds[0]

  const scoredCandidateIds = candidatePortIds
    .map((pcbPortId) => {
      const position = portPositionMap.get(pcbPortId)
      if (!position) return null
      return {
        pcbPortId,
        distanceToObstacle: pointToBoxDistance(position, obstacle),
  ...
  if (scoredCandidateIds.length === 0) return candidatePortIds[0]

  scoredCandidateIds.sort((a, b) => {
    if (Math.abs(a.distanceToObstacle - b.distanceToObstacle) > 0.001) {
      return a.distanceToObstacle - b.distanceToObstacle
    }
    return a.distanceToCenter - b.distanceToCenter
  })

  return scoredCandidateIds[0].pcbPortId
}
...
function createPcbPadElements(srj: SimpleRouteJson): AnyCircuitElement[] {
  ...
  for (const obstacle of srj.obstacles as any[]) {
    ...
    if (!padId && !platedHoleId && !pcbPortId) continue
