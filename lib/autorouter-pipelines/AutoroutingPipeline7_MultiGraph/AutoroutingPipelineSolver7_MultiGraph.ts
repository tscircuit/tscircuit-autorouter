...
import { ComponentPortPointBridgeSolver } from "../../solvers/ComponentPortPointBridgeSolver/ComponentPortPointBridgeSolver"
...
function isComponentCapacityMeshNodeId(
  nodeId: string,
  componentCapacityMeshNodeIds: Set<string>,
) {
  if (componentCapacityMeshNodeIds.has(nodeId)) return true

  for (const componentCapacityMeshNodeId of componentCapacityMeshNodeIds) {
    if (nodeId.startsWith(`${componentCapacityMeshNodeId}__sub_`)) {
      return true
    }
  }
...
function isComponentSharedEdgeSegment(
  ...
  return segment.nodeIds.some((nodeId) =>
    isComponentCapacityMeshNodeId(nodeId, componentCapacityMeshNodeIds),
  )
}
...
export class AutoroutingPipelineSolver7_MultiGraph extends BaseSolver {
  ...
  componentPortPointBridgeSolver?: ComponentPortPointBridgeSolver
  ...
  pipelineDef = [
    ...
    definePipelineStep(
      ...
      ],
      {
        onSolved: (cms) => {
          ...
          cms.componentBaseCapacityMeshNodeIds = componentMeshNodes.map(
    ...
    definePipelineStep(
      "componentPortPointBridgeSolver",
      ComponentPortPointBridgeSolver,
      (cms) => [
        {
          capacityMeshNodes: cms.capacityNodes!,
          componentBaseCapacityMeshNodeIds:
            cms.componentBaseCapacityMeshNodeIds,
        ...
        return [
          {
            ...
            sharedEdgeSegments: getNonComponentSharedEdgeSegments({
              sharedEdgeSegments:
                cms.componentPortPointBridgeSolver!.getOutput(),
              componentCapacityMeshNodeIds,
            }),
  ...
  visualize(): GraphicsObject {
    ...
    const componentPortPointBridgeViz =
      this.componentPortPointBridgeSolver?.visualize()
