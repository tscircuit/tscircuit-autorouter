...
export class AutoroutingPipelineSolver7_MultiGraph extends BaseSolver {
  ...
  pipelineDef = [
    ...
    definePipelineStep(
      ...
      (cms) => {
        const componentCapacityMeshNodeIds: Set<string> = new Set(
          (cms.componentTopologyGeneratorSolver?.getOutput() ?? []).map(
        ...
        return [
          {
            ...
            sharedEdgeSegments: cms
              .availableSegmentPointSolver!.getOutput()
              .filter(
                (segment) =>
                  !segment.nodeIds.some((nodeId) =>
      ...
      },
      {
        onSolved: (cms) => {
          ...
          const filteredSharedEdgeSegmentsByEdgeId: Map<
            string,
            SharedEdgeSegment
          > = new Map(
            cms
              .necessaryCrampedPortPointSolver!.getOutput()
              .map((segment) => [segment.edgeId, segment]),
          )

          cms.sharedEdgeSegmentsWithNecessaryCrampedPortPoints = cms
            .availableSegmentPointSolver!.getOutput()
            .map((segment) => {
              if (
                segment.nodeIds.some((nodeId) =>
              ...
              return (
                filteredSharedEdgeSegmentsByEdgeId.get(segment.edgeId) ??
                segment
              )
            })
        },
  ...
  visualize(): GraphicsObject {
    ...
    const errorViz: GraphicsObject | null =
      this.failed && this.error
        ? createConnectionRegionErrorVisualization({
            srj: this.srjWithPointPairs ?? this.srj,
            baseSrj: this.originalSrj,
            error: this.error,
          })
        : null
