...
/** Scores via-in-pad repair candidates against the complete DRC result. */
export const createViaInPadDrcEvaluator = (
  ...
  return ({ routes, hdRoutes }) => {
    ...
    if (!evaluatedRoutes) {
      throw new Error("Via-in-pad DRC evaluation requires HD routes")
    }
