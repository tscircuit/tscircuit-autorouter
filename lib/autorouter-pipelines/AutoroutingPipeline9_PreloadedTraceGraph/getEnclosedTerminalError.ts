...
export const getEnclosedTerminalError = (
  ...
  for (const connection of srj.connections) {
    ...
    const sameNetObstacles = srj.obstacles.filter((obstacle) =>
      obstacle.connectedTo.some((id) => connMap.getNetConnectedToId(id) === net),
    )
    for (const point of connection.pointsToConnect) {
      ...
      if (
        !connection.pointsToConnect.some((other) =>
          other.x < bounds.minX || other.x > bounds.maxX ||
          other.y < bounds.minY || other.y > bounds.maxY,
        )
      ) {
        continue
      }
      ...
      if (
        srj.layerCount > 1 &&
        maximumViaDistance >= viaDiameter / 2 - 1e-6
      ) {
        continue
      }
