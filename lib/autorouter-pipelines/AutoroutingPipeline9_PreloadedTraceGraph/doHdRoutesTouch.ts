...
export const doHdRoutesTouch = (a: HighDensityRoute, b: HighDensityRoute): boolean => {
  for (let i = 1; i < a.route.length; i++) {
    ...
    for (let j = 1; j < b.route.length; j++) {
      ...
      if (minimumDistanceBetweenSegments(p, q, u, v) <= (aDiameter + bDiameter) / 2 + CONTACT_TOLERANCE_MM) return true
    }
  }
  return false
}
