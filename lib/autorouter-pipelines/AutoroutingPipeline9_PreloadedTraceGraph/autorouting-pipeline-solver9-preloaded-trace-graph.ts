...
import {
  getPipeline9TopologyCacheKey,
  getPipeline9TopologyFromCache,
  savePipeline9TopologyToCache,
} from "./pipeline9-topology-cache"
...
type PipelineStep<T extends new (...args: any[]) => BaseSolver> = {
  solverName: string
  solverClass: T
  skipWhenTopologyCacheHit?: boolean
...
>(
  ...
  return {
    ...
    solverClass,
    getConstructorParams,
    skipWhenTopologyCacheHit: opts.skipWhenTopologyCacheHit,
...
export class AutoroutingPipelineSolver9_PreloadedTraceGraph extends BaseSolver {
  ...
  /** Raw topology port points before target-dependent cramped-point filtering. */
  sharedEdgeSegmentsBeforeNecessaryCrampedPortPoints?: SharedEdgeSegment[]
  ...
  pipelineDef = [
    ...
    definePipelineStep(
      ...
      },
      {
        onSolved: (cms) => {
          ...
          cms.attemptToUseTopologyCacheSync()
        },
      },
    ),
    definePipelineStep(
    ...
    definePipelineStep(
      ...
      ],
      {
        skipWhenTopologyCacheHit: true,
        onSolved: (cms) => {
          cms.sharedEdgeSegmentsBeforeNecessaryCrampedPortPoints =
            cms.availableSegmentPointSolver!.getOutput()
          cms.saveTopologyToCacheSync()
        },
      },
    ),
    definePipelineStep(
  ...
  private getTopologyCacheKey(): string {
    return getPipeline9TopologyCacheKey({
      inputSrj: this.srjWithPointPairs!,
      componentDetectionOutput: this.componentDetectionSolver!.getOutput(),
      viaDiameter: this.viaDiameter,
      obstacleMargin: this.srj.defaultObstacleMargin ?? 0.15,
  ...
  private attemptToUseTopologyCacheSync(): void {
    if (!this.cacheProvider?.isSyncCache) return

    try {
      const cacheKey = this.getTopologyCacheKey()
      this.topologyCacheKey = cacheKey
      const cachedTopology = getPipeline9TopologyFromCache({
        cacheProvider: this.cacheProvider,
        cacheKey,
      })
      if (!cachedTopology) return

      this.capacityNodes = cachedTopology.capacityNodes
      this.capacityEdges = cachedTopology.capacityEdges
      this.sharedEdgeSegmentsBeforeNecessaryCrampedPortPoints =
        cachedTopology.sharedEdgeSegments
      this.topologyCacheHit = true
    } catch (error) {
      console.error("Error loading cached Pipeline9 topology:", error)
    }
  }

  private saveTopologyToCacheSync(): void {
    if (
      !this.cacheProvider?.isSyncCache ||
      !this.capacityNodes ||
      !this.capacityEdges ||
      !this.sharedEdgeSegmentsBeforeNecessaryCrampedPortPoints
    ) {
    ...
    try {
      const cacheKey = this.topologyCacheKey ?? this.getTopologyCacheKey()
      this.topologyCacheKey = cacheKey
      savePipeline9TopologyToCache({
        cacheProvider: this.cacheProvider,
        cacheKey,
      ...
      console.error("Error caching Pipeline9 topology:", error)
    }
  }
  ...
  _step() {
    let pipelineStepDef = this.pipelineDef[this.currentPipelineStepIndex]
    while (pipelineStepDef?.skipWhenTopologyCacheHit && this.topologyCacheHit) {
      this.timeSpentOnPhase[pipelineStepDef.solverName] = 0
      this.currentPipelineStepIndex++
      pipelineStepDef = this.pipelineDef[this.currentPipelineStepIndex]
    }
    if (!pipelineStepDef) {
      this.solved = true
      return
