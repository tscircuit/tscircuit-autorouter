 ...
 * the affected node, with an explicit shared-handoff force family for seams.
...
export class Pipeline9HighDensityDrcRepairSolver extends BaseSolver {
  ...
  override readonly activeSubSolver: null = null
  ...
  private startNodeRepair(node: NodeWithPortPoints): void {
    ...
    this.activeForceCandidates = getPipeline9HighDensityForceCandidates({
      ...
      traceRouteIndexById,
      // Only actual board pads are passed to the force obstacle projection.
  ...
  private acceptSeamForceCandidate(
    ...
    // immutable, and subsequent force visits use both ends of the new seam.
  ...
  private finishAcceptedNodeRepair(source: "force" | "seam"): void {
    ...
    this.stats.acceptedForceRepairCount =
      Number(this.stats.acceptedForceRepairCount) + 1
  ...
  override _step(): void {
    ...
    if (this.activeSeamForceCandidates) {
      ...
      if (candidate.done) {
        this.activeSeamForceCandidates = null
        // Exhausting the fixed candidate families is not a solver failure.
        // Keep the incumbent and its visible DRCs; do not start another router.
        this.finishExhaustedNodeRepair(
          `Force and seam candidates did not improve DRCs for node "${this.activeNode.capacityMeshNodeId}"`,
        )
        return
      }
