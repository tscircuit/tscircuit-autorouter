...
import { AutoroutingDrcEngine, type DrcEvaluator, type DrcError } from "high-density-repair03/lib"
...
export class Pipeline9Repair04Solver extends BaseSolver {
  ...
  constructor(private readonly input: Pipeline9Repair04SolverParams) {
    ...
    this.MAX_ITERATIONS = (input.maxRegions ?? 32) * ((input.maxCandidatesPerRegion ?? 8000) * 2 + 6) + 2
  }

  private evaluateReference(routes: HighDensityRoute[]): DrcError[] {
    const result = this.input.referenceDrcEvaluator({ hdRoutes: routes, routes,
  ...
  private evaluate(routes: HighDensityRoute[]): DrcError[] {
    return this.engine.evaluate([
      ...(this.input.srj.traces ?? []).map((trace) => normalizeRepairTrace(trace as any, this.input.srj.minTraceWidth)),
      ...convertRepairRoutesToTraces(routes, this.input.srj.layerCount),
    ] as any).errorsWithCenters
  }

  override _step(): void {
    if (!this.input.enabled) { this.solved = true; return }
    if (!this.issues) {
      this.issues = this.evaluate(this.routes)
      this.referenceErrors = this.evaluateReference(this.routes)
      this.fixedViolations = new Map(getFixedObstacleViolations({srj:this.input.srj as any,routes:this.routes})
        .map((violation) => [violation.key,violation.severity]))
    }
    if (this.localSolver) {
      this.localSolver.step()
      if (this.localSolver.failed) throw new Error(`repair04 failed: ${this.localSolver.error}`)
      if (!this.localSolver.solved) return
      if (!this.region) throw new Error("repair04: completed region is missing")
      const candidate = mergeRepairRegion({ routes: this.routes, region: this.region,
        repairedRoutes: this.localSolver.getOutput() }) as HighDensityRoute[]
      const candidateIssues = this.evaluate(candidate)
      const fixedViolations = getFixedObstacleViolations({srj:this.input.srj as any,routes:candidate})
      const preservesFixedObstacles = fixedViolations.every(({key,severity}) =>
        this.fixedViolations!.has(key) && severity <= this.fixedViolations!.get(key)! + 1e-8)
      if (preservesFixedObstacles && candidateIssues.length <= this.issues.length) {
        const candidateReferenceErrors = this.evaluateReference(candidate)
        if (candidateReferenceErrors.length <= this.referenceErrors!.length &&
          (candidateIssues.length < this.issues.length || candidateReferenceErrors.length < this.referenceErrors!.length)) {
          this.routes = candidate
          this.issues = candidateIssues
          this.referenceErrors = candidateReferenceErrors
          this.fixedViolations = new Map(fixedViolations.map((violation) => [violation.key,violation.severity]))
          this.acceptedRegionCount++
        }
      }
      this.localSolver = null
      this.region = null
      this.stats = { regions: this.regionCount, acceptedRegions: this.acceptedRegionCount,
        indexedErrors: this.issues.length, referenceErrors: this.referenceErrors!.length }
    }
    if (this.referenceErrors!.length === 0 || this.issues.length === 0 || this.regionCount >= (this.input.maxRegions ?? 32)) {
      this.solved = true
      return
    }
    ...
    for (const error of [...this.referenceErrors!, ...this.issues]) {
      for (const size of [10, 16, 24]) {
        ...
        if (!center || typeof center !== "object" || !("x" in center) || !("y" in center)) continue
        ...
        const region = extractRepairRegion({ srj: this.input.srj as any, routes: this.routes,
          bounds: { minX: x - size/2, minY: y - size/2, maxX: x + size/2, maxY: y + size/2 } })
        this.region = region
        this.localSolver = new Repair04Solver({
          srj: region.srj, routes: region.routes, bounds: region.bounds,
          boundaryMargin: region.boundaryMargin, lockedPointIndices: region.lockedPointIndices,
  ...
  getOutput(): HighDensityRoute[] {
    if (!this.solved || this.failed) throw new Error("repair04: output requested before completed stage")
    return this.routes
  }
