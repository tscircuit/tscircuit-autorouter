...
export const applyPipeline9TraceShortcuts = ({
  routes,
  otherHdRoutes,
  srj,
  connMap,
  colorMap,
  drcEvaluator,
}: ShortcutInput): HighDensityRoute[] => {
  let accepted = [...routes]
  let beforeErrors = evaluateRoutes(accepted, drcEvaluator)
  if (beforeErrors.length === 0) return routes
  ...
  for (let index = 0; index < accepted.length; index++) {
    ...
    solver.solve()
    if (solver.failed)
      throw new Error(solver.error ?? "Trace shortcut solver failed")
    const candidate = solver.simplifiedRoute
    if (stringify(candidate.route) === stringify(inputRoute.route)) continue
    if (
      stringify(candidate.vias) !== stringify(inputRoute.vias) ||
    ...
    const attachments = others
      .flatMap((route) => [route.route[0]!, route.route.at(-1)!])
      .filter((point) => isPointOnRoute(point, inputRoute))
    if (attachments.some((point) => !isPointOnRoute(point, candidate))) continue
    if (
      others.some(
        (route) =>
          doHdRoutesTouch(inputRoute, route) &&
          !doHdRoutesTouch(candidate, route),
      )
    )
      continue
