...
function projectElement(element: AnyCircuitElement): object {
  ...
  if (element.type === "pcb_port") {
    return { type: element.type, pcb_port_id: element.pcb_port_id, source_port_id: element.source_port_id, x: element.x, y: element.y }
  }
  if (element.type === "source_trace") {
    return { type: element.type, source_trace_id: element.source_trace_id, connected_source_port_ids: element.connected_source_port_ids?.map((id) => id) }
  }
  if (element.type === "pcb_smtpad" || element.type === "pcb_plated_hole") {
    if (!element.pcb_port_id) return { type: element.type, pcb_port_id: element.pcb_port_id }
    const projected: Record<string, unknown> = { type: element.type }
    for (const field of ["pcb_port_id", "shape", "x", "y", "radius", "width", "height", "ccw_rotation", "outer_diameter", "outer_width", "outer_height", "rect_pad_width", "rect_pad_height", "rect_ccw_rotation"]) {
      if (field in source) {
        const value = source[field]
        projected[field] = value === undefined && (field === "rect_pad_width" || field === "rect_pad_height")
          ? { $traceNumber: "NaN" }
          : value
      }
    }
    if (element.type === "pcb_smtpad" && element.shape === "polygon") {
      projected.points = element.points.map((point) => ({ x: point.x, y: point.y }))
    }
    return projected
  }
...
export const checkTracesAreContiguous = (circuitJson: AnyCircuitElement[]): PcbTraceError[] => {
  ...
  return descriptors.map((descriptor): PcbTraceError => {
    ...
    const sourceTraceId = sourceTrace?.type === "source_trace" ? sourceTrace.source_trace_id : undefined
    const traceName = getReadableNameForPcbTrace(circuitJson, trace.pcb_trace_id)
    if (descriptor.kind === "misalignedVia") {
      const point = trace.route[descriptor.pointIndex!]!
      if (point.route_type !== "via") throw new Error("Native contiguity error must refer to a via")
      return {
        type: "pcb_trace_error",
        message: `Via in trace [${traceName}] is misaligned at position {x: ${point.x}, y: ${point.y}}.`,
        source_trace_id: sourceTraceId || trace.source_trace_id || `!${trace.pcb_trace_id}`,
    ...
    if (descriptor.kind === "missingConnection") {
      ...
      const portName = getReadableNameForPcbPort(circuitJson, port.pcb_port_id).replace("pcb_port", "")
    ...
    const point = trace.route[descriptor.endpoint === "start" ? 0 : trace.route.length - 1]!
    if (point.route_type !== "wire") throw new Error("Native disconnected endpoint must refer to a wire")
    return {
      type: "pcb_trace_error",
