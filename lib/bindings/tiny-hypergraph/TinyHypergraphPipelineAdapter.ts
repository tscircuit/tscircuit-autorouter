import { BaseSolver, BasePipelineSolver, type PipelineStep } from "@tscircuit/solver-utils"
...
function loadGraph(graph: SerializedHyperGraph): LoadedTinyHypergraph {
  ...
  return {
    topology: {
      ...
      regionMetadata: t.regionMetadata && [...t.regionMetadata] as TinyRegionMetadata[],
      portMetadata: t.portMetadata && [...t.portMetadata] as TinyPortMetadata[],
    ...
    problem: {
      ...p,
      routeMetadata: p.routeMetadata && [...p.routeMetadata] as RouteMetadata[],
...
class TinyHypergraphSearchStage extends BaseSolver implements TinyHypergraphSolverView {
  ...
  private snapshot: ReturnType<TinyHyperGraphSolver["getRoutingSnapshot"]> | undefined
  ...
  constructor(loaded: LoadedTinyHypergraph, options: TinyHypergraphRoutingInput["solveGraphOptions"], private readonly selective: boolean) {
    super()
    this.topology = loaded.topology
    this.problem = loaded.problem
    this.MAX_ITERATIONS = options?.MAX_ITERATIONS ?? 1_000_000
    this.solver = new TinyHyperGraphSolver(loaded.topology, loaded.problem, options, {
  ...
  captureStatsSnapshot(): Record<string, unknown> {
    if (Object.getOwnPropertyDescriptor(this, "stats")?.get !== this.statsGetter) {
      return { ...this.stats }
    }
    this.refreshStatsSnapshot()
    ...
    return this.exposedStatsSnapshot ? { ...this.exposedStatsSnapshot } : this.statsSnapshot
  }
  ...
  override _step(): void {
    ...
    this.error = status.failed && status.error === "Maximum iterations reached"
      ? `${this.getSolverName()} ran out of iterations` : status.error
...
class EmptySectionStage extends BaseSolver {
  ...
  constructor(loaded: LoadedTinyHypergraph, options: TinyHypergraphRoutingInput["sectionSolverOptions"]) {
    super()
    this.MAX_ITERATIONS = options?.MAX_ITERATIONS ?? 50_000
    if (loaded.problem.portSectionMask.some((mask) => mask !== 0)) {
      throw new Error("Autorouter empty-section stage requires a zero section mask")
    }
    const replay = new TinyHyperGraphSolver(loaded.topology, loaded.problem, options)
    try {
      replay.replaySolution(loaded.solution)
      const state = replay.getRoutingSnapshot()
      this.baselineSolver = {
        topology: loaded.topology, problem: loaded.problem, state,
        iterations: replay.iterations, stats: replay.getStats(),
        solved: replay.solved, failed: replay.failed,
      }
  ...
  override _setup(): void {
    ...
    this.stats = {
      ...this.stats, activeRouteCount: 0, initialMaxRegionCost: this.maxRegionCost,
      finalMaxRegionCost: this.maxRegionCost, optimized: false,
    }
    this.solved = true
  }

  getSolvedSolver(): TinyHypergraphSolverView {
    if (!this.optimizedSolver) throw new Error("Empty section has not completed setup")
    return this.optimizedSolver
  }

  override getOutput(): SerializedHyperGraph { return this.output }
  override visualize(): GraphicsObject { return this.graphics }
}
...
export class TinyHypergraphPipelineAdapter extends BasePipelineSolver<TinyHypergraphRoutingInput> {
  ...
  constructor(
    ...
    this.MAX_ITERATIONS = (input.solveGraphOptions?.MAX_ITERATIONS ?? 1_000_000) +
      (input.sectionSolverOptions?.MAX_ITERATIONS ?? 1_000_000) + 1_000_000
  }
  ...
  private loadHyperGraph(graph: SerializedHyperGraph): LoadedTinyHypergraph {
    ...
    this.duplicatePortPenaltyCount = Math.max(this.duplicatePortPenaltyCount, counts.duplicatePortPenaltyCount)
    this.metadataPortPenaltyCount = Math.max(this.metadataPortPenaltyCount, counts.metadataPortPenaltyCount)
    this.crampedPortPenaltyCount = Math.max(this.crampedPortPenaltyCount, counts.crampedPortPenaltyCount)
  ...
  override pipelineDef: PipelineStep<any>[] = [
    {
      solverName: "solveGraph", solverClass: TinyHypergraphSearchStage,
      getConstructorParams: (instance: TinyHypergraphPipelineAdapter) => [
        ...
        { RIP_THRESHOLD_RAMP_ATTEMPTS: 5, ...instance.inputProblem.solveGraphOptions },
        instance.selectiveRerip,
      ],
    },
    {
      solverName: "optimizeSection", solverClass: EmptySectionStage,
      getConstructorParams: (instance: TinyHypergraphPipelineAdapter) => {
        const solved = instance.getStageOutput<SerializedHyperGraph>("solveGraph")
        if (!solved) throw new Error("solveGraph did not produce a solved serialized hypergraph")
        ...
        return [loaded, {
          DISTANCE_TO_COST: 0.05, RIP_THRESHOLD_RAMP_ATTEMPTS: 16,
          RIP_CONGESTION_REGION_COST_FACTOR: 0.1, MAX_ITERATIONS: 50_000,
  ...
  getSolvedTinySolver(): TinyHypergraphSolverView {
    if (this.optimizeSection?.solved && !this.optimizeSection.failed) return this.optimizeSection.getSolvedSolver()
    if (this.solveGraph?.solved && !this.solveGraph.failed) return this.solveGraph
    throw new Error("WASM tiny-hypergraph pipeline has no solved graph")
  }

  override getOutput(): SerializedHyperGraph | null {
    return this.getStageOutput<SerializedHyperGraph>("optimizeSection") ??
  ...
  override tryFinalAcceptance(): void {
    if (this.getStageOutput<SerializedHyperGraph>("solveGraph")) {
      this.stats = { ...this.stats, acceptedSolveGraphOutputOnSectionPipelineTimeout: true }
  ...
  override initialVisualize(): GraphicsObject {
    if (!this.initialVisualizationSolver) {
      this.initialVisualizationSolver = new TinyHypergraphSearchStage(
        ...
        { RIP_THRESHOLD_RAMP_ATTEMPTS: 5, ...this.inputProblem.solveGraphOptions },
        this.selectiveRerip,
      )
    }
