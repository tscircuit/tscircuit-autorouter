...
import {
  encodeTraceSimplificationIndex,
  decodeTraceSimplificationIndex,
} from "lib/bindings/trace-simplification/TraceSimplificationIndexCodec"
...
export class TraceSimplificationGraphCodec {
  ...
  private readonly pointMetadata = new WeakMap<
    object,
    { id: number; snapshot: JsonRecord }
  >()
  ...
  private readonly typedMaps = new WeakMap<
    object,
    "terminal" | "net" | "layers"
  >()
  ...
  private readonly connectivityBaselines = new WeakMap<
    object,
    { map: object; keys: string[]; values: unknown[] }
  >()
  ...
  private shallowValues(object: JsonRecord): unknown[] {
    if (this.sourcePointObjects.has(object))
      return [
        object.x,
        object.y,
        object.z,
        object.traceThickness,
        object.toNextSegmentType,
        object.insideJumperPad,
        object.pcb_port_id,
        object.viaDiameter,
        object.toNextSegmentCircuitJsonMetadata,
      ]
  ...
  private sourceUnchanged(watch: SourceWatch): boolean {
    const point = watch.object,
      previous = watch.values
    if (watch.isPoint)
      return (
        Object.is(point.x, previous[0]) &&
        Object.is(point.y, previous[1]) &&
        Object.is(point.z, previous[2]) &&
        Object.is(point.traceThickness, previous[3]) &&
        Object.is(point.toNextSegmentType, previous[4]) &&
        Object.is(point.insideJumperPad, previous[5]) &&
        Object.is(point.pcb_port_id, previous[6]) &&
        Object.is(point.viaDiameter, previous[7]) &&
        Object.is(point.toNextSegmentCircuitJsonMetadata, previous[8])
      )
    if (Array.isArray(point)) {
      if (point.length !== previous.length) return false
      for (let index = 0; index < point.length; index++)
        if (!Object.is(point[index], previous[index])) return false
      return true
    }
    let index = 0
    if (watch.object instanceof Map) {
      for (const [key, value] of watch.object) {
        if (
          !Object.is(key, watch.values[index++]) ||
          !Object.is(value, watch.values[index++])
        )
          return false
      }
    } else if (watch.object instanceof Set) {
      for (const value of watch.object)
        if (!Object.is(value, watch.values[index++])) return false
    } else if (previous.length > 64) {
      const keys = Object.keys(point),
        values = Object.values(point)
      if (keys.length * 2 !== previous.length) return false
      for (let offset = 0; offset < keys.length; offset++) {
        if (
          keys[offset] !== previous[offset * 2] ||
          !Object.is(values[offset], previous[offset * 2 + 1])
        )
          return false
      }
      return true
      ...
      for (const key in watch.object) {
        ...
        if (
          key !== watch.values[index++] ||
          !Object.is(watch.object[key], watch.values[index++])
        )
          return false
      }
    }
  ...
  trackSources(params: JsonRecord, kind: string): void {
    ...
    for (const obstacle of params.__normalizedObstacles ?? [])
      this.privateNormalizedObstacles.add(obstacle)
    ...
    if (["path-base", "path", "vertex"].includes(kind))
      referenceFields.add("obstacles")
    const walk = (
      value: any,
      owner: JsonRecord | undefined,
      ...
      routingRequired =
        routingRequired &&
        field !== "colorMap" &&
        !(value.netMap && value.idToNetMap)
      const geometry =
        (Array.isArray(value.route) &&
          typeof value.connectionName === "string") ||
        this.sourcePointObjects.has(value) ||
        (value.center && Array.isArray(value.connectedTo))
      if (
        this.jumperArrays.has(value) ||
        this.typedMaps.has(value) ||
        this.colorMaps.has(value)
      )
        owner = value
      if (
        geometry &&
      ...
      const privateNormalized =
        !this.normalizedObstaclesExposed &&
        this.privateNormalizedObstacles.has(value)
      if (!privateNormalized) {
        let watch = this.sourceWatches.get(value)
        if (!watch) {
          watch = {
            object: value,
            isPoint: this.sourcePointObjects.has(value),
            routingRequired,
            deferred,
            values: this.shallowValues(value),
            owners: new Set(),
            fields: new Set(),
          }
          this.sourceWatches.set(value, watch)
        }
        ...
        if (
          field &&
          field !== "colorMap" &&
          field !== "terminalLayerIndicesByPcbPortId" &&
          field !== "netByConnectionName" &&
          (!owner ||
            [
              "connMap",
              "outline",
      ...
      if (
        ...
        for (const entry of value.values())
          walk(entry, owner, field, seen, routingRequired, deferred)
      } else {
        for (const [name, entry] of Object.entries(value)) {
          if (privateNormalized && name === "__zLayers") continue
          if (
            name === "circuitJsonMetadata" &&
            value.center &&
            Array.isArray(value.connectedTo)
          )
            continue
          const deferEntry =
            deferred || (name === "connectedTo" && value.center !== undefined)
          walk(
            entry,
            owner,
            field,
            seen,
            routingRequired && !deferEntry,
            deferEntry,
          )
        }
      }
    ...
    for (const [key, value] of Object.entries(params)) {
      if (key === "obstacles" && Object.hasOwn(params, "__normalizedObstacles"))
        continue
      if (kind === "trace" && key === "hdRoutes") {
        for (const route of value as JsonRecord[]) {
          for (const via of route.vias)
            walk(via, undefined, undefined, new Set())
          walk(route.jumpers, route, undefined, new Set())
        }
        continue
      }
      const deferred =
        key === "terminalLayerIndicesByPcbPortId" ||
        key === "netByConnectionName"
      walk(
        value,
        undefined,
        referenceFields.has(key) ? key : undefined,
        new Set(),
        !deferred,
        deferred,
      )
  ...
  trackArguments(args: unknown[]): void {
    this.trackSources(
      { ...this.sourceFields, __callArgs: args },
      this.sourceKind,
    )
  }

  sourceChanges(routingOnly: boolean | "read" = false): Graph | undefined {
    const owners = new Set<JsonRecord>(),
      fields: JsonRecord = {}
    for (const watch of this.sourceWatches.values()) {
      ...
      for (const field of watch.fields) {
        if (field === "connMap")
          fields.__connectivityUpdates = [this.sourceFields.connMap]
    ...
    for (const owner of owners) {
      if (this.typedMaps.has(owner) || this.colorMaps.has(owner))
        (fields.__inputUpdates ??= []).push(owner)
  ...
  beginConnectivityOperation(): void {
    this.connectivityOperations.push(new Set())
  }
  ...
  connectivityForRead(identity: number): TraceConnectivityUpdate | undefined {
        ...
        const unchanged =
          ...
          keys.every(
            (key, index) =>
              ...
              Object.is(values[index], previous.values[index]),
          )
        if (!unchanged) {
    ...
    return graph === undefined && updated === undefined
      ? undefined
      : { connectivity: updated, graph }
  }

  refreshSourceBaseline(): void {
    for (const watch of this.sourceWatches.values())
      if (watch.routingRequired) watch.values = this.shallowValues(watch.object)
  }
  ...
  private encodePoint(point: JsonRecord): unknown {
    ...
    if (
      !metadata ||
      keys.length !== Object.keys(metadata.snapshot).length ||
      keys.some(
        (key) =>
          !Object.hasOwn(metadata!.snapshot, key) ||
          !Object.is(point[key], metadata!.snapshot[key]),
      )
    ) {
  ...
  private encodeRoute(route: JsonRecord): unknown {
    ...
    delete value.route
    delete value.vias
    const jumpersArrayId = Array.isArray(route.jumpers)
      ? this.encodeJumpers(route.jumpers)
      : undefined
    this.routes.set(id, {
      id,
      value,
      jumpersArrayId,
      propertyOrder: Object.keys(route),
      route: this.encodePointArray(route.route),
      vias: this.encodePointArray(route.vias),
    })
    return { $ref: ["route", id] }
  }
  ...
  private encodePointArray(points: JsonRecord[]): unknown {
    return {
      $array: this.identity(points),
      items: points.map((point) => this.encodePoint(point)),
    }
  }
  ...
  encode(value: any, key = ""): any {
    ...
    if (this.decodedObjects.has(value)) {
      ...
      for (const [name, entry] of Object.entries(value))
        fields[name] = this.encode(entry, name)
      return { $object: this.identity(value), fields }
    }
    if (this.decodedMaps.has(value))
      return {
        $map: this.identity(value),
        entries: [...value].map(([name, entry]) => [name, this.encode(entry)]),
      }
    if (this.jumperArrays.has(value))
      return { $jumpers: this.encodeJumpers(value) }
    if (key === "outline" || key === "filteredJumperPads") return value
    const index = encodeTraceSimplificationIndex(
      value,
      key,
      (entry, name) => this.encode(entry, name),
      (object) => this.identity(object),
    )
    if (index !== undefined) return index
    const mapKind =
      this.typedMaps.get(value) ??
      (key === "terminalLayerIndicesByPcbPortId"
        ? "terminal"
        : key === "netByConnectionName"
          ? "net"
          : key === "__terminalLayerSet"
            ? "layers"
            : undefined)
    if (mapKind) {
      ...
      if (mapKind === "net")
        return { $netNamesId: id, $raw: Object.fromEntries(entries) }
      return {
        $terminalLayersId: id,
        $raw: Object.fromEntries(
          entries.map(([name, layers]) => [
            name,
            this.encode(layers, "__terminalLayerSet"),
          ]),
        ),
      }
    }
    if (key === "filteredVias")
      return value.map((entry: unknown) => ({ $raw: entry }))
    if (key === "newRoute" || key === "newVias" || key === "lastValidPath")
      return this.encodePointArray(value)
    if (
      key === "inputHdRoutes" ||
      key === "unsimplifiedHdRoutes" ||
      key === "hdRoutes"
    )
      return {
        $array: this.identity(value),
        items: value.map((route: JsonRecord) => this.encodeRoute(route)),
      }
    ...
    if (value instanceof Map) {
      if (key === "buckets")
        return [...value].map(([name, entry]) => [name, this.encode(entry)])
      return Object.fromEntries(
        [...value].map(([name, entry]) => [name, this.encode(entry)]),
      )
    }
    if (Array.isArray(value.route) && typeof value.connectionName === "string")
      return this.encodeRoute(value)
    if (typeof value.x === "number" && typeof value.y === "number")
      return this.encodePoint(value)
    if (
      value.center &&
      ...
      this.obstacles.set(id, {
        id,
        zLayersArrayId: Array.isArray(value.__zLayers)
          ? this.identity(value.__zLayers)
          : undefined,
        value,
      })
      return { $ref: ["obstacle", id] }
    }
    if (value.netMap && value.idToNetMap)
      return {
        $connectivityId: this.identity(value),
        $raw: { netMap: value.netMap, idToNetMap: value.idToNetMap },
      }
    const fields: JsonRecord = {}
    for (const [name, entry] of Object.entries(value))
      fields[name] = this.encode(entry, name)
  ...
  private reconcile(current: any, incoming: any): any {
    ...
    if (Array.isArray(incoming)) {
      ...
      incoming.forEach((entry, index) => {
        result[index] = this.reconcile(result[index], entry)
      })
    ...
    for (const key of Object.keys(result))
      if (!(key in incoming) && result[key] !== undefined) delete result[key]
    for (const [key, value] of Object.entries(incoming))
      result[key] = this.reconcile(result[key], value)
    return result
  }
  ...
  decode(value: any, current?: any, key = ""): any {
    if (value === null || value === undefined || typeof value !== "object")
      return value
    if (value.$index)
      return decodeTraceSimplificationIndex(
        value,
        current,
        (entry, previous) => this.decode(entry, previous),
        (id) => this.objects.get(id),
        (id, object) => this.register(id, object),
      )
    if (value.$ref) {
      const object = this.objects.get(value.$ref[1])
      if (!object)
        throw new Error(
          `Unknown trace simplification identity ${value.$ref[1]}`,
        )
      return object
    }
    const mapKind = Object.hasOwn(value, "$terminalLayersId")
      ? "terminal"
      : Object.hasOwn(value, "$netNamesId")
        ? "net"
        : Object.hasOwn(value, "$layerSetId")
          ? "layers"
          : undefined
    if (mapKind) {
      const id =
      ...
      const object =
        mapKind === "layers"
          ? new Set(value.$raw)
          : new Map(
              Object.entries(value.$raw).map(([name, entry]) => [
                name,
                mapKind === "terminal" ? this.decode(entry) : entry,
              ]),
            )
    ...
    if (Object.hasOwn(value, "$colorMapId")) {
      const object =
        this.objects.get(value.$colorMapId) ??
        this.register(value.$colorMapId, value.$raw)
    ...
    if (Object.hasOwn(value, "$connectivityId")) {
      ...
      const map = this.register(
        ...
        new ConnectivityMap(value.$raw.netMap),
      )
      this.reconcile(map.idToNetMap, value.$raw.idToNetMap)
      return map
    }
    if (Object.hasOwn(value, "$raw")) {
      if (key === "connMap" && current === undefined && value.$raw?.netMap)
        current = new ConnectivityMap(value.$raw.netMap)
      return this.reconcile(current, value.$raw)
    }
    if (Object.hasOwn(value, "$array")) {
      const array =
        this.objects.get(value.$array) ?? this.register(value.$array, [])
      value.items.forEach((entry: unknown, index: number) => {
        array[index] = this.decode(entry, array[index])
      })
    ...
    if (Object.hasOwn(value, "$map")) {
      const map =
        this.objects.get(value.$map) ?? this.register(value.$map, new Map())
      this.decodedMaps.add(map)
      const entries = value.entries.map(([name, entry]: [unknown, unknown]) => [
        name,
        this.decode(entry, map.get(name)),
      ])
      map.clear()
    ...
    if (Object.hasOwn(value, "$object")) {
      const object =
        this.objects.get(value.$object) ?? this.register(value.$object, {})
      if (typeof object.acceptSnapshotFields === "function") {
        object.acceptSnapshotFields(value.fields)
        return object
      }
    ...
    if (key === "preservedRouteEndpoints") {
      ...
      for (const [name, entry] of Object.entries(value))
        map.set(name, this.decode(entry, map.get(name)))
      for (const name of map.keys())
        if (!Object.hasOwn(value, name)) map.delete(name)
      return map
    }
    if (key === "terminalLayerIndicesByPcbPortId") {
      ...
      for (const [name, entries] of Object.entries(value))
        map.set(name, new Set(entries as number[]))
      return map
    }
    ...
    if (key === "traceThicknessByObstacleSegmentId" || key === "buckets") {
      const map = current instanceof Map ? current : new Map()
      map.clear()
      for (const [name, entry] of Array.isArray(value)
        ? value
        : Object.entries(value))
        map.set(name, this.decode(entry))
      return map
    }
    ...
    const result =
      current && typeof current === "object"
        ...
        : key === "segmentTree"
          ...
          : {}
    for (const [name, entry] of Object.entries(value))
      result[name] = this.decode(entry, result[name], name)
    return result
  }
  ...
  hydrate(graph: Graph, current?: any): any {
    ...
    for (const record of graph.points)
      records.set(record.id, { record, kind: "point" })
    for (const record of graph.obstacles)
      records.set(record.id, { record, kind: "obstacle" })
    for (const record of graph.routes)
      records.set(record.id, { record, kind: "route" })
    const groups = new Map(
      (graph.cloneGroups ?? []).map((group) => [group.id, group]),
    )
    ...
    const ensureReferences = (value: any): void => {
      ...
      if (value.$ref) {
        ensureRecord(value.$ref[1])
        return
      }
      if (Array.isArray(value)) {
        for (const item of value) ensureReferences(item)
        return
      }
      if (Object.hasOwn(value, "$array"))
        for (const item of value.items) ensureReferences(item)
    }
      ...
      const associate = (source: any, clone: any): void => {
        ...
        if (source instanceof Map) {
          const sourceEntries = [...source],
            cloneEntries = [...clone]
          sourceEntries.forEach(([key, value], index) => {
            associate(key, cloneEntries[index][0])
            associate(value, cloneEntries[index][1])
          })
        } else if (source instanceof Set) {
          const cloneEntries = [...clone]
          ;[...source].forEach((value, index) =>
            associate(value, cloneEntries[index]),
          )
        } else
          for (const key of Object.keys(source))
            associate(source[key], clone[key])
      }
    ...
    const ensureRecord = (id: number): void => {
      ...
      if (!entry) {
        if (!this.objects.has(id))
          throw new Error(`Unknown trace simplification source ${id}`)
        return
      }
      const { record, kind } = entry
      if (record.sourceId !== null && record.sourceId !== undefined)
        ensureRecord(record.sourceId)
      const clone =
        record.cloneGroupId === null || record.cloneGroupId === undefined
          ? undefined
          : ensureGroup(record.cloneGroupId).get(record.sourceId)
      const source =
        kind === "point" &&
        record.metadataSourceId !== null &&
        record.metadataSourceId !== undefined
      ...
      if (!object) {
        ...
        if (kind === "route" && !clone) {
          ...
          for (const key of record.propertyOrder ?? [
            ...Object.keys(record.value),
            "route",
            "vias",
          ])
            object[key] =
              metadataSource && Object.hasOwn(metadataSource, key)
                ? metadataSource[key]
                : source?.[key]
          if (
            record.metadataSourceId !== null &&
      ...
      if (kind === "route") {
        if (clone) {
          for (const key of ["route", "vias"]) {
            ...
            if (arrayId !== undefined && !this.objects.has(arrayId))
              this.register(arrayId, clone[key])
          }
        }
        for (const [key, value] of Object.entries(record.value)) {
          if (
            key === "jumpers" &&
            record.jumpersArrayId !== null &&
            record.jumpersArrayId !== undefined
          )
            continue
        ...
        if (
          ...
          const array =
            this.objects.get(record.jumpersArrayId) ??
            this.register(
              record.jumpersArrayId,
              Array.isArray(object.jumpers) ? object.jumpers : [],
            )
          object.jumpers = array
        }
        ensureReferences(record.route)
        ensureReferences(record.vias)
        ...
        if (!retained && !clone) {
          for (const key of Object.keys(object)) {
            if (!(key in record.value) && object[key] !== undefined)
              delete object[key]
          }
          for (const [key, value] of Object.entries(record.value)) {
            if (
        ...
        for (const [key, value] of Object.entries(record.value))
          if (
            ...
            !(
              key === "__zLayers" &&
              record.zLayersArrayId !== null &&
              record.zLayersArrayId !== undefined
            )
          )
            object[key] = this.reconcile(object[key], value)
      ...
      if (
        ...
        const layers =
          this.objects.get(record.zLayersArrayId) ??
          this.register(record.zLayersArrayId, [])
        object.__zLayers = this.reconcile(layers, record.value.__zLayers)
      }
      if (kind === "point") {
        for (const key of record.removedProperties ?? []) delete object[key]
        if (
          record.segmentMetadataObstacleId !== null &&
          record.segmentMetadataObstacleId !== undefined
        ) {
          ensureRecord(record.segmentMetadataObstacleId)
          object.toNextSegmentCircuitJsonMetadata = this.objects.get(
            record.segmentMetadataObstacleId,
          ).circuitJsonMetadata
        }
      }
    }
    ...
    if (graph.captureCloneGroups)
      for (const id of groups.keys()) ensureGroup(id)
    const result = this.decode(graph.fields, current)
    if (this.normalizedObstaclesExposed && graph.obstacles.length > 0) {
      for (const record of graph.obstacles)
        this.exposedObstacles.add(this.objects.get(record.id))
      this.trackSources(
        {
          ...this.sourceFields,
          __exposedObstacles: [...this.exposedObstacles],
        },
        this.sourceKind,
      )
    }
