...
type SolverAdapterClass = {
  prototype: TraceSimplificationSolverAdapter
  solverKind: string
...
function withQueryGraph<T>(
  graph: TraceSimplificationGraphCodec,
  run: () => T,
): T {
  let id = queryGraphIds.get(graph)
  if (id === undefined) {
    id = nextQueryGraphId++
    queryGraphIds.set(graph, id)
  }
  ...
  try {
    ...
    graph.endConnectivityOperation()
    if (previous) activeQueryGraphs.set(id, previous)
    else activeQueryGraphs.delete(id)
  }
}
...
export class TraceSimplificationSolverAdapter extends BaseSolver {
  ...
  static register(kind: string, ctor: SolverAdapterClass): void {
    childConstructors.set(kind, ctor)
  }
  ...
  constructor(params: Record<string, any>) {
    ...
    this.binding = TraceSimplificationSolverAdapter.createBinding(
      ctor.solverKind,
      paramsGraph,
      this.graph,
    )
    const normalizedObstacles = [
      ...
      "via-merger",
      "trace",
    ].includes(ctor.solverKind)
      ? this.hydrateSolver(this.binding.obstacles())
      : undefined
    if (["via-merger", "crossing"].includes(ctor.solverKind))
      this.hydrateSolver(this.binding.output())
    this.graph.trackSources(
      normalizedObstacles
        ? { ...params, __normalizedObstacles: normalizedObstacles }
        : params,
      ctor.solverKind,
    )
    if (normalizedObstacles && ctor.stateFields.includes("obstacles"))
      this.stateValues.obstacles = normalizedObstacles
    if (ctor.solverKind === "trace")
      this.stateValues.simplificationConfig = {
        ...params,
        obstacles: normalizedObstacles,
      }
    for (const field of ctor.stateFields)
      if (
        Object.hasOwn(params, field) &&
        !(field === "hdRoutes" && ctor.solverKind === "trace") &&
        !(
          field === "obstacles" &&
          ["multi-path", "via-removal", "via-merger", "trace"].includes(
            ctor.solverKind,
          )
        )
      )
        this.stateValues[field] = params[field]
  ...
  private static createBinding(
    kind: string,
    params: bindings.TraceGraphPacket,
    graph: TraceSimplificationGraphCodec,
  ): bindings.TraceSimplificationDispatcher {
    return withQueryGraph(
      graph,
      () =>
        new bindings.TraceSimplificationDispatcher(
          kind,
          params,
          TraceSimplificationSolverAdapter.createQueryCallback(
            queryGraphIds.get(graph)!,
          ),
          TraceSimplificationSolverAdapter.createConnectivityCallback(
            queryGraphIds.get(graph)!,
          ),
        ),
    )
  ...
  private static createConnectivityCallback(
    graphId: number,
  ): (identity: number) => bindings.TraceConnectivityUpdate | undefined {
    return (identity: number): bindings.TraceConnectivityUpdate | undefined => {
      const graph = activeQueryGraphs.get(graphId)
      if (!graph)
        throw new Error(
          "Connectivity read called outside native solver execution",
        )
      return graph.connectivityForRead(identity)
    }
  }

  protected runSolver<T>(run: () => T): T {
    return withQueryGraph(this.graph, run)
  }

  private static createQueryCallback(
    graphId: number,
  ): (request: bindings.TraceObstacleQuery) => bindings.TraceGraphPacket {
    return (
      request: bindings.TraceObstacleQuery,
    ): bindings.TraceGraphPacket => {
      const graph = activeQueryGraphs.get(graphId)
      if (!graph)
        throw new Error("Obstacle query called outside native solver execution")
      graph.exposeNormalizedObstacles()
      const index = graph.objects.get(request.indexId) as
      ...
      if (request.method === "search") {
        ...
        try {
          return graph.graph(index.search({ minX, minY, maxX, maxY }))
        } finally {
          graph.invalidateConnectivityReads()
        }
      }
      if (request.method === "searchArea") {
        const [x, y, width, height] = request.args
        try {
          return graph.graph(index.searchArea(x, y, width, height))
        } finally {
          graph.invalidateConnectivityReads()
        }
      }
  ...
  private installAccessors(ctor: SolverAdapterClass): void {
    for (const field of [
      ...
      Object.defineProperty(this, field, {
        ...
        set: (value: any): void => {
          this.baseValues[field] = value
          if (!this.syncingSolver) {
            this.dirtyBase = true
    ...
    if (!Object.hasOwn(this.stateValues, "stats"))
      this.stateValues.stats = this.stats
    const diagnosticFields = ctor.stateFields.includes("stats")
      ? ctor.stateFields
      : [...ctor.stateFields, "stats"]
  ...
  private syncBase(): void {
    ...
    try {
      Object.assign(this, this.binding.state())
    } finally {
      this.syncingSolver = false
  ...
  private wrapChild(
    binding: bindings.TraceSimplificationDispatcher,
  ): TraceSimplificationSolverAdapter {
    ...
    if (!ctor)
      throw new Error(
        `Unregistered binding simplification child ${binding.kind()}`,
      )
    const child = Object.create(
      ctor.prototype,
    ) as TraceSimplificationSolverAdapter
    Object.assign(child, new BaseSolver(), {
      binding: binding,
      graph: this.graph,
      stateValues: {},
      outputValues: {},
  ...
  private registerChildTree(): void {
    ...
    if (!binding) {
      ...
      if (
        (
          this.constructor as unknown as SolverAdapterClass
        ).stateFields.includes("activeSubSolver")
      )
        this.stateValues.activeSubSolver = null
      return
    }
    const child = this.wrapChild(binding)
    if (
      previous instanceof TraceSimplificationSolverAdapter &&
      previous !== child
    )
      previous.sync()
  ...
  sync(): void {
    ...
    try {
      ...
      for (const [key, value] of Object.entries(fields)) {
        if (Object.hasOwn(this.baseValues, key)) {
          if (!this.dirtyBase) this.baseValues[key] = value
  ...
  acceptSnapshotFields(fields: Record<string, unknown>): void {
    ...
    try {
      ...
      for (const key of Object.keys(this.baseValues))
        if (Object.hasOwn(fields, key) && !this.dirtyBase)
          this.baseValues[key] = this.stateValues[key]
      this.hasSnapshot = true
    } finally {
      this.syncingSolver = false
    }
  ...
  push(stepArguments = false, routingOnly = stepArguments): void {
    ...
    if (sourceChanges) this.binding.restore(sourceChanges)
    const dirtyFields = [...this.dirtyFields].filter(
      (field) =>
        !stepArguments ||
    ...
    if (
      ...
      Object.keys(this.outputValues).length === 0
    )
      return
    ...
    if (this.observed) {
      ...
      for (const field of dirtyFields) {
        fields[field] = Object.hasOwn(this.baseValues, field)
          ? this.baseValues[field]
          : this.stateValues[field]
      }
    ...
    if (fields.activeSubSolver instanceof TraceSimplificationSolverAdapter) {
      ...
      fields.activeSubSolver = {
        $object: fields.activeSubSolver.binding.identity(),
        fields: {},
      }
    }
  ...
  protected readSolverField(key: string): any {
    if (key === "stats" && !this.observed) {
      if (!this.statsObserved) {
        this.push()
        this.hydrateSolver(this.binding.statistics(), this.stateValues)
  ...
  protected canSolveInBindings(): boolean {
    return true
  }
  ...
  protected callSolver<A extends unknown[], R>(
    method: (
      args: bindings.TraceMethodGraph<A>,
    ) => bindings.TraceMethodGraph<R>,
    args: NoInfer<A>,
  ): R {
    this.graph.exposeNormalizedObstacles()
    this.push()
    ...
    const value = this.hydrateSolver(
      withQueryGraph(this.graph, () => method.call(this.binding, argsGraph)),
    )
    if (this.observed) this.sync()
    else if (this.statsObserved)
      this.hydrateSolver(this.binding.statistics(), this.stateValues)
    return value
  }
  ...
  private readOutput(track: boolean): any {
    ...
    switch ((this.constructor as unknown as SolverAdapterClass).solverKind) {
      ...
      case "path":
      case "vertex":
        this.outputValues = { newRoute: value.route, newVias: value.vias }
        break
      case "multi-path":
        this.outputValues = { simplifiedHdRoutes: value }
        break
      case "via-removal":
        this.outputValues = { optimizedHdRoutes: value }
        break
      case "via-merger":
        this.outputValues = { mergedViaHdRoutes: value }
        break
      case "trace":
        this.outputValues = { hdRoutes: value }
        break
      case "crossing":
        this.outputValues = { reducedHdRoutes: value }
        break
    }
  ...
  override _step(): void {
    ...
    let completed = false
    try {
      let status = withQueryGraph(this.graph, () =>
        this.binding.step(this.iterations, this.MAX_ITERATIONS),
      )
      status = this.resolveSolverStep(status)
      this.baseValues.solved = (status & 1) !== 0
      this.baseValues.failed = (status & 2) !== 0
      if (this.baseValues.failed)
        this.baseValues.error = this.binding.error() ?? null
      this.dirtyFields.clear()
      this.dirtyBase = false
      completed = true
    } finally {
      ...
      if (this.observed) this.sync()
        ...
        this.hydrateSolver(this.binding.statistics(), this.stateValues)
      if (!this.observed && Object.keys(this.outputValues).length)
  ...
  override solve(): void {
    if (
      !this.canSolveInBindings() ||
      this.step !== BaseSolver.prototype.step ||
      this._step !== TraceSimplificationSolverAdapter.prototype._step ||
    ...
    this.push()
    try {
      withQueryGraph(this.graph, () => this.binding.solve())
    } finally {
      this.syncBase()
      if (this.observed) this.sync()
      else {
        if (this.statsObserved)
          this.hydrateSolver(this.binding.statistics(), this.stateValues)
        if (Object.keys(this.outputValues).length) this.readOutput(false)
      }
    }
