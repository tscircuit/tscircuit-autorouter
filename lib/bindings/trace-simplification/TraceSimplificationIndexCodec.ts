...
type ObstacleIndexState = {
  ...
  idx: {
    ...
    shi?: {
      CELL_SIZE: number
      obstacles: unknown[]
      buckets: Map<string, Array<[unknown, number]>>
    }
  }
}

export function encodeTraceSimplificationIndex(
  value: unknown,
  key: string,
  encode: Encode,
  ...
  if (key === "hdRouteSHI" && value instanceof HighDensityRouteSpatialIndex) {
    ...
    return {
      ...
      segmentBuckets: [...index.segmentBuckets].map(([name, segments]) => [
        name,
      ...
      viaBuckets: [...index.viaBuckets].map(([name, vias]) => [
        name,
        vias.map((via) => ({
          viaId: via.viaId,
          x: via.x,
          y: via.y,
          parentRoute: encode(via.parentRoute),
        })),
      ]),
    }
  }
  if (key === "obstacleSHI" && value instanceof ObstacleSpatialHashIndex) {
    ...
    if (
      value.search !== ObstacleSpatialHashIndex.prototype.search ||
      value.searchArea !== ObstacleSpatialHashIndex.prototype.searchArea
    ) {
      return {
        kind: "host",
        hostQuery: true,
    ...
    if (!tree) {
      if (index.idx.tree)
        return {
          kind: "rbush",
          indexId: identity(value),
          tree: encode(index.idx.tree.data),
          storage: index.storage.map((obstacle) => encode(obstacle)),
        }
      const flat = index.idx.index
      if (!flat || !index.idx.items)
        throw new Error("Unrecognized obstacle spatial index")
      if (flat._pos !== flat._boxes.length)
        throw new Error("Data not yet indexed - call index.finish().")
      return {
        kind: "flatbush",
        indexId: identity(value),
        boxes: Array.from(flat._boxes),
        indices: Array.from(flat._indices),
        levelBounds: flat._levelBounds,
        nodeSize: flat.nodeSize,
        numItems: flat.numItems,
        items: index.idx.items.map((obstacle) => encode(obstacle)),
        storage: index.storage.map((obstacle) => encode(obstacle)),
      }
    }
    return {
      kind: "native",
      indexId: identity(value),
      CELL_SIZE: tree.CELL_SIZE,
      storage: index.storage.map((obstacle) => encode(obstacle)),
      obstacles: tree.obstacles.map((obstacle) => encode(obstacle)),
      buckets: [...tree.buckets].map(([name, entries]) => [
        name,
        entries.map(([obstacle, index]) => [encode(obstacle), index]),
      ]),
    }
  }
  return undefined
...
export function decodeTraceSimplificationIndex(
  value: any,
  current: any,
  decode: Decode,
  ...
  if (value.$index === "route") {
    const result =
      existing ?? register(value.indexId, new HighDensityRouteSpatialIndex([]))
    ...
    for (const name of ["segmentBuckets", "viaBuckets"]) {
      ...
      buckets.clear()
      for (const [key, entries] of value[name])
        buckets.set(
          key,
          entries.map((entry: any) => decode(entry)),
        )
    }
    return result
  }
  const result =
    existing ?? register(value.indexId, new ObstacleSpatialHashIndex())
  result.storage = decode(value.storage, result.storage)
  if (value.kind === "native") {
    if (!result.idx.shi)
      result.idx = (new ObstacleSpatialHashIndex() as any).idx
    const tree = result.idx.shi
    tree.CELL_SIZE = value.CELL_SIZE
    tree.obstacles = decode(value.obstacles, tree.obstacles)
    tree.buckets.clear()
    for (const [key, entries] of value.buckets)
      tree.buckets.set(
        key,
        entries.map((entry: any) => [decode(entry[0]), entry[1]]),
      )
  } else if (value.kind === "rbush") {
    if (!result.idx.tree)
      result.idx = (new ObstacleSpatialHashIndex("rbush") as any).idx
    ...
    throw new Error(`Unrecognized native obstacle index kind ${value.kind}`)
  return result
}
