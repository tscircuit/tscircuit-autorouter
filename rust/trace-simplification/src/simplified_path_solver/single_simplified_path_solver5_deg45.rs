...
fn segment_id(a:&PointRef,b:&PointRef)->String {
    use intra_node_routing::js_number::js_number_to_string as n;
...
impl SingleSimplifiedPathSolver5 {
    ...
    pub fn append_original_route_slice(&mut self,distance:f64,end:usize)->Result<(),String>{
        let index=self.path_segments.iter().position(|s|distance>=s.start_distance&&distance<=s.end_distance).ok_or_else(||format!("Could not find path segment containing distance {}",intra_node_routing::js_number::js_number_to_string(distance)))?;
