use super::{
    Bounds, Circle, Point, distance, do_segments_intersect, js_max, js_min,
...
pub fn segment_to_circle_min_distance(a: &Point, b: &Point, circle: &Circle) -> f64 {
    ...
    if a.x == b.x && a.y == b.y {
        return js_max(0.0, distance(a, &circle_center) - circle.radius);
    }
    let ab = Point {
        x: b.x - a.x,
        y: b.y - a.y,
    ...
    let ac = Point {
        x: circle_center.x - a.x,
        y: circle_center.y - a.y,
    };
