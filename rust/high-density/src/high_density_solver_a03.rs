...
impl TypedMinHeap {
    fn push(&mut self, f: f64, seq: u32, id: usize) {
        if self.n == self.f.len() {
            let next = (self.f.len() * 2).max(1024);
            self.f.resize(next, 0.0);
            self.seq.resize(next, 0);
            self.id.resize(next, 0);
        }
        let mut i = self.n;
        self.n += 1;
        self.f[i] = f;
    ...
    fn pop(&mut self) -> usize {
        ...
        if self.n > 0 {
            ...
            self.seq[0] = self.seq[self.n];
            self.id[0] = self.id[self.n];
            let mut i = 0;
            loop {
                let l = i * 2 + 1;
                let r = l + 1;
    ...
    fn less(&self, i: usize, j: usize) -> bool {
        if self.f[i] != self.f[j] { return self.f[i] < self.f[j]; }
...
pub struct HighDensitySolverA03 {
    ...
    usedCellsFlat: Vec<i32>,
    portOwnerFlat: Vec<i32>,
    sharedCellsFlat: Vec<Option<Vec<usize>>>,
...
impl HighDensitySolverA03 {
    ...
    pub fn setup(&mut self) {
        ...
        self.usedCellsFlat = vec![-1; totalCells];
        self.sharedCellsFlat = vec![None; totalCells];
    ...
    fn computeMoveCostAndRips(&mut self, activeConn: usize, toZ: usize, toCellId: usize, isVia: bool, rippedHead: i32, currentRipCount: usize, lateralCost: f64) {
        ...
        if isVia {
            ...
            let occs = self.fillViaOccupants(toCellId, activeConn);
            for occ in occs {
            ...
            let occs = self.fillTraceOccupants(toFlatIdx, activeConn);
            for occ in occs {
                if !self.ripChain.contains(head, occ) {
                    cost += self.ripCost;
                    head = self.ripChain.append(head, occ);
    ...
    fn fillViaOccupants(&self, cellId: usize, activeConn: usize) -> Vec<usize> {
        let mut occs = Vec::new();
        let cx = self.cellCenterX[cellId];
        let cy = self.cellCenterY[cellId];
        for occCellId in self.cellsNearCircle(cx, cy, self.viaKeepoutRadius) {
            ...
            for z in 0..self.layers {
                self.pushFlatOccupants(z * self.planeSize + occCellId, activeConn, &mut occs);
            }
        }
        occs
    }
    ...
    fn pushFlatOccupants(&self, flatIdx: usize, activeConn: usize, out: &mut Vec<usize>) {
        let primaryOcc = self.usedCellsFlat[flatIdx];
        if primaryOcc != -1 && primaryOcc != activeConn as i32 && !self.allowSharedUse(activeConn, primaryOcc) {
            pushUnique(out, primaryOcc as usize);
        }
        let Some(sharedOccs) = &self.sharedCellsFlat[flatIdx] else { return; };
        for &occ in sharedOccs {
    ...
    fn addSharedOccupant(&mut self, flatIdx: usize, connId: usize) {
        if self.usedCellsFlat[flatIdx] == connId as i32 { return; }
        let sharedOccs = self.sharedCellsFlat[flatIdx].get_or_insert_with(Vec::new);
        pushUnique(sharedOccs, connId);
    }

    fn replaceOccupants(&mut self, flatIdx: usize, connId: usize) {
        self.usedCellsFlat[flatIdx] = connId as i32;
        self.sharedCellsFlat[flatIdx] = None;
    }

    fn removeOccupant(&mut self, flatIdx: usize, connId: usize) {
        if self.usedCellsFlat[flatIdx] == connId as i32 {
            if let Some(sharedOccs) = &mut self.sharedCellsFlat[flatIdx] {
                if !sharedOccs.is_empty() {
                    self.usedCellsFlat[flatIdx] = sharedOccs.pop().unwrap() as i32;
                    if sharedOccs.is_empty() { self.sharedCellsFlat[flatIdx] = None; }
                    return;
                }
            }
            self.usedCellsFlat[flatIdx] = -1;
            return;
        }
        let Some(sharedOccs) = &mut self.sharedCellsFlat[flatIdx] else { return; };
    ...
    fn markTraceFootprint(&mut self, connId: usize, z: usize, sourceCellId: usize, indices: &mut Vec<usize>) {
        ...
        for cellId in self.cellsNearCircle(cx, cy, self.traceKeepoutRadius) {
            ...
            let existing = self.usedCellsFlat[flatIdx];
    ...
    pub fn visualize(&self) -> Value {
        ...
        if self.showUsedCellMap {
            for z in 0..self.layers {
                for cellId in 0..self.planeSize {
                    if self.usedCellsFlat[z * self.planeSize + cellId] == -1 { continue; }
                    rects.push(json!({
                        "center": self.transformPoint(self.cellCenterX[cellId], self.cellCenterY[cellId]),
                        "width": self.cellWidth[cellId] * self.gridToBoundsTransform[0],
                        "height": self.cellHeight[cellId] * self.gridToBoundsTransform[4],
