...
fn ripped_contains(r: &Option<Rc<RippedNode>>, id: ConnId) -> bool {
...
pub struct HighDensitySolverA01 {
    ...
    move_cost: f64,
    move_ripped: Option<Rc<RippedNode>>,
}
...
impl HighDensitySolverA01 {
    pub fn new(props: Value) -> Self {
        ...
        Self {
            ...
            conn_id_to_root_net: Vec::new(), overlap_friendly_root_nets: HashSet::new(), plane_size: 0,
            ...
            move_cost: 0.0, move_ripped: None,
        }
    }
    ...
    pub fn setup(&mut self) {
        ...
        for pp in &port_points {
            ...
            let root_net = &self.conn_id_to_root_net[conn_id as usize];
            if let Some(existing_root) = root_by_port_flat.get(&flat_idx) {
                ...
                root_by_port_flat.insert(flat_idx, root_net.clone());
            }
    ...
    fn step_once(&mut self) {
        if self.active_conn_seg.is_none() {
            ...
            self.node_pool.push(SearchNode { z: next.start_z, row: next.start_row, col: next.start_col, g: 0.0, f, parent_idx: -1, ripped: None });
        ...
        let node = self.node_pool[node_idx].clone();
        ...
        for d in 0..8 {
            ...
            self.compute_move_cost_and_rips(active_conn, z, row, col, z, nr, nc, ripped.clone());
            ...
            self.node_pool.push(SearchNode { z, row: nr, col: nc, g: g2, f: f2, parent_idx: node_idx as isize, ripped: self.move_ripped.clone() });
        ...
        if can_via {
            for nz in 0..self.layers {
                ...
                if self.visited_stamp[n_idx] == self.stamp { continue; }
                self.compute_move_cost_and_rips(active_conn, z, row, col, nz, row, col, ripped.clone());
                if self.move_cost < 0.0 { continue; }
                let g2 = g + self.move_cost;
                let f2 = g2 + self.compute_h(nz, row, col, end_z, end_row, end_col) * self.hyper_parameters.greedy_multiplier;
                let new_node_idx = self.node_pool.len();
                self.node_pool.push(SearchNode { z: nz, row, col, g: g2, f: f2, parent_idx: node_idx as isize, ripped: self.move_ripped.clone() });
                self.heap.push(f2, self.seq_counter, new_node_idx);
                self.seq_counter += 1;
            }
        }
    ...
    fn compute_move_cost_and_rips(&mut self, active_conn: ConnId, from_z: i32, from_row: i32, from_col: i32, to_z: i32, to_row: i32, to_col: i32, ripped: Option<Rc<RippedNode>>) {
        let mut cost = 0.0;
        let mut r = ripped;
        let cols = self.cols;
        if from_z != to_z {
            ...
            let fixed_same_root = self.conn_id_to_root_net.get(fixed_owner as usize) == self.conn_id_to_root_net.get(active_conn as usize);
            let allow_fixed_overlap = fixed_same_root && self.overlap_friendly_root_nets.contains(&self.conn_id_to_root_net[active_conn as usize]);
            ...
            for &occ in &self.via_occs {
                if !ripped_contains(&r, occ) {
                    cost += self.hyper_parameters.rip_cost;
                    r = Some(Rc::new(RippedNode { id: occ, prev: r }));
                }
            ...
            let same_root = self.conn_id_to_root_net.get(occ as usize) == self.conn_id_to_root_net.get(active_conn as usize);
            let allow_same_root_overlap = same_root && self.overlap_friendly_root_nets.contains(&self.conn_id_to_root_net[active_conn as usize]);
            ...
            if dr == 1 && dc == 1 {
                ...
                let crossing_same_root = self.conn_id_to_root_net.get(crossing_occ as usize) == self.conn_id_to_root_net.get(active_conn as usize);
                let allow_crossing_overlap = crossing_same_root && self.overlap_friendly_root_nets.contains(&self.conn_id_to_root_net[active_conn as usize]);
    ...
    fn fill_via_occupants(&mut self, row: i32, col: i32, active_conn: ConnId) {
        self.via_occs.clear();
        for z in 0..self.layers {
            ...
            for i in 0..self.via_offsets_len {
                ...
                if occ == -1 || occ == active_conn { continue; }
                let same_root = self.conn_id_to_root_net.get(occ as usize) == self.conn_id_to_root_net.get(active_conn as usize);
                if same_root && self.overlap_friendly_root_nets.contains(&self.conn_id_to_root_net[active_conn as usize]) { continue; }
                let mut seen = false;
    ...
    fn should_skip_fixed_port_halo(&self, flat_idx: usize, conn_id: ConnId) -> bool {
        ...
        let same_root = self.conn_id_to_root_net[fixed_owner as usize] == self.conn_id_to_root_net[conn_id as usize];
        !(same_root && self.overlap_friendly_root_nets.contains(&self.conn_id_to_root_net[conn_id as usize]))
    }
    ...
    fn finalize_route(&mut self, goal_node_idx: usize) {
        ...
        let mut cur = self.node_pool[goal_node_idx].ripped.as_ref();
        while let Some(node) = cur {
        ...
        for cell in &cells {
            for dr in -margin_cells..=margin_cells {
                for dc in -margin_cells..=margin_cells {
                    ...
                    let same_root = self.conn_id_to_root_net.get(existing as usize) == self.conn_id_to_root_net.get(conn_id as usize);
                    let allow_same_root_overlap = same_root && self.overlap_friendly_root_nets.contains(&self.conn_id_to_root_net[conn_id as usize]);
        ...
        for i in 1..cells.len() {
            ...
            let crossing_same_root = self.conn_id_to_root_net.get(crossing_occ as usize) == self.conn_id_to_root_net.get(conn_id as usize);
            let allow_crossing_overlap = crossing_same_root && self.overlap_friendly_root_nets.contains(&self.conn_id_to_root_net[conn_id as usize]);
