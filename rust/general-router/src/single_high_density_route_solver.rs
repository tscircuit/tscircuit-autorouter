...
use crate::geometry::{distance, do_segments_intersect, point_to_segment_distance};
use crate::single_route_candidate_priority_queue::{Node, SingleRouteCandidatePriorityQueue};
use crate::single_high_density_route_solver6_vert_horz_layer_future_cost::FutureCost;
use crate::types::{Bounds, Point, Point2, Route, RoutePoint};
pub use crate::types::FutureConnection;
...
impl SingleHighDensityRouteSolver {
    ...
    pub fn new(opts: Value) -> Self {
        let bounds: Bounds = serde_json::from_value(opts["bounds"].clone()).expect("bounds");
        let a: Point = serde_json::from_value(opts["A"].clone()).expect("A");
        let b: Point = serde_json::from_value(opts["B"].clone()).expect("B");
        let hyper_parameters = opts.get("hyperParameters").cloned().unwrap_or_else(|| json!({}));
        let layer_count = opts["layerCount"].as_f64().unwrap_or(2.0) as usize;
        let mut available_z: Vec<f64> = opts.get("availableZ").and_then(Value::as_array)
            .map(|values| values.iter().map(|v| v.as_f64().expect("availableZ number")).collect()).unwrap_or_default();
        ...
        let mut solver = Self {
            ...
            obstacle_routes: serde_json::from_value(opts["obstacleRoutes"].clone()).expect("obstacleRoutes"),
            bounds,
            ...
            endpoint_a: serde_json::from_value(opts["A"].clone()).expect("A"),
            endpoint_b: serde_json::from_value(opts["B"].clone()).expect("B"),
            ...
            nearby_segment_clearance: opts["nearbySegmentClearance"].as_f64().unwrap_or(0.15),
            explored_nodes: HashSet::new(), vias_in_path_by_node: RefCell::new(HashMap::new()),
            ...
            region_id: opts["regionId"].as_str().map(str::to_owned), solved_path: None,
            future_connections: opts.get("futureConnections").map(|v| serde_json::from_value(v.clone()).expect("futureConnections")).unwrap_or_default(),
            hyper_parameters, conn_map: opts.get("connMap").filter(|v| !v.is_null()).map(|v| serde_json::from_value(v.clone()).expect("connMap")),
            obstacle_segments: vec![], obstacle_segment_index: None,
            obstacle_segments_by_layer: HashMap::new(), obstacle_segment_index_by_layer: HashMap::new(),
            obstacle_vias: vec![], obstacle_via_index: None,
            debug_explored_nodes_ordered: vec![], debug_nodes_too_close_to_obstacle: HashSet::new(),
            debug_node_path_to_parent_intersects_obstacle: HashSet::new(),
            debug_enabled: opts["captureSearchDebug"].as_bool().unwrap_or(true),
        ...
        let min_dist = opts["minDistBetweenEnteringPoints"].as_f64().expect("minDistBetweenEnteringPoints");
    ...
    pub fn is_node_too_close_to_obstacle_base(&self, node: &Node, margin: Option<f64>, is_via: bool, query: Option<&PlanarObstacleQuery>) -> bool {
        ...
        let ids = if let Some(query) = query { query.segment_ids.clone() } else {
            let index = if !is_via { self.obstacle_segment_index_by_layer.get(&(node.z as i64)) } else { self.obstacle_segment_index.as_ref() };
            index.map(|index| index.search(node.x - trace_proximity, node.y - trace_proximity, node.x + trace_proximity, node.y + trace_proximity)).unwrap_or_default()
        };
        ...
        if let Some(index) = &self.obstacle_via_index {
            for id in index.search(node.x - via_proximity, node.y - via_proximity, node.x + via_proximity, node.y + via_proximity) {
    ...
    pub fn does_path_to_parent_intersect_obstacle(&self, node: &Node, query: Option<&PlanarObstacleQuery>) -> bool {
        ...
        let ids = query.map(|q| q.segment_ids.clone()).unwrap_or_else(|| self.obstacle_segment_index_by_layer.get(&(node.z as i64))
            .map(|index| index.search(min_x - clearance, min_y - clearance, max_x + clearance, max_y + clearance)).unwrap_or_default());
        for id in ids {
            let Some(segment) = segments.get(id) else { continue; };
    ...
    pub fn get_planar_obstacle_query(&self, node: &Node) -> Option<PlanarObstacleQuery> {
        ...
        let index = self.obstacle_segment_index_by_layer.get(&layer)?;
        self.obstacle_segments_by_layer.get(&layer)?;
        let trace_proximity = self.trace_thickness + self.obstacle_margin;
        let clearance = if node.z == parent.z && !self.obstacle_segments.is_empty() { self.nearby_segment_clearance } else { 0.0 };
        Some(PlanarObstacleQuery { layer, segment_ids: index.search(
            (node.x - trace_proximity).min(parent.x - clearance), (node.y - trace_proximity).min(parent.y - clearance),
            (node.x + trace_proximity).max(parent.x + clearance), (node.y + trace_proximity).max(parent.y + clearance)) })
    }
    ...
    pub fn get_neighbors(&mut self, node: Rc<Node>) -> Vec<Rc<Node>> {
        ...
        for x in -1..=1 {
            for y in -1..=1 {
                ...
                let query = self.get_planar_obstacle_query(&neighbor);
                if self.is_node_too_close_to_obstacle(&neighbor, None, false, query.as_ref()) {
    ...
    pub fn get_vias_in_node_path(&self, node: &Rc<Node>) -> Rc<Vec<Point2>> {
        ...
        self.vias_in_path_by_node.borrow_mut().insert(key, (Rc::downgrade(node), vias.clone()));
        vias
    }
    ...
    pub fn step_inner(&mut self) {
        ...
        for neighbor in self.get_neighbors(node) { self.candidates.enqueue(neighbor); }
    }
