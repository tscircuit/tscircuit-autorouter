        ...
        let number = |key: &str| -> String {
            ryu_js::Buffer::new()
                .format(
                    point[key]
                        .as_f64()
                        .expect("Port point coordinate must be a number"),
                )
                .to_owned()
        };
        format!(
            ...
            point["connectionName"]
                .as_str()
                .expect("Port point connectionName must be a string"),
            number("x"),
            number("y"),
            number("z")
        )
...
fn append_pair_if_unique<'a>(
    pair: PortPointPair<'a>,
    seen_pair_keys: &mut HashSet<String>,
...
pub fn get_connection_port_point_pairs(port_points: &[Value]) -> Vec<PortPointPair<'_>> {
    ...
    for port_point in port_points {
        if let Some(prev_id) = port_point
            .get("prevPortPointId")
            .and_then(Value::as_str)
            .filter(|id| !id.is_empty())
            && let Some(prev) = port_points_by_id.get(prev_id)
            && prev["connectionName"] == port_point["connectionName"]
        {
            append_pair_if_unique([prev, port_point], &mut seen_pair_keys, &mut pairs);
        }
        if let Some(next_id) = port_point
            .get("nextPortPointId")
            .and_then(Value::as_str)
            .filter(|id| !id.is_empty())
            && let Some(next) = port_points_by_id.get(next_id)
            && next["connectionName"] == port_point["connectionName"]
        {
            append_pair_if_unique([port_point, next], &mut seen_pair_keys, &mut pairs);
        }
    }
    if pairs.is_empty() {
        append_adjacent_pairs(
            &port_points.iter().collect::<Vec<_>>(),
            &mut seen_pair_keys,
            &mut pairs,
        );
        return pairs;
    ...
    let unlinked_port_points: Vec<_> = port_points
        .iter()
        .filter(
            |port_point| match port_point.get("portPointId").and_then(Value::as_str) {
