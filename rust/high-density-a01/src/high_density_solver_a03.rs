    ...
    fn pop(&mut self) -> usize {
        ...
        if self.n > 0 {
            ...
            loop {
                ...
                if l >= self.n { break; }
                let mut m = l;
                if r < self.n && !self.less(self.entries[l], self.entries[r]) { m = r; }
                if self.less(held, self.entries[m]) { break; }
...
impl TypedNodePool {
    fn push(&mut self, z: usize, cellId: usize, g: f64, parent: i32, ripHead: i32, ripCount: usize) -> usize {
...
impl TypedRipChain {
    ...
    fn contains(&self, mut head: i32, connId: usize) -> bool {
        while head >= 0 {
            if self.connId[head as usize] == connId { return true; }
...
fn pushUnique(arr: &mut Vec<usize>, value: usize) {
    if !arr.contains(&value) { arr.push(value); }
}

fn circleIntersectsRect(cx: f64, cy: f64, r: f64, minX: f64, minY: f64, maxX: f64, maxY: f64) -> bool {
...
impl HighDensitySolverA03 {
    pub fn new(props: Value) -> Self {
        ...
        Self {
            ...
            highResolutionCellThickness: number(&props, "highResolutionCellThickness", 8.0).floor().max(1.0) as usize,
            lowResolutionCellSize: number(&props, "lowResolutionCellSize", 0.4),
            viaDiameter: props["viaDiameter"].as_f64().expect("viaDiameter is required"),
    ...
    pub fn setup(&mut self) {
        let width = self.nodeWithPortPoints["width"].as_f64().expect("width is required");
        let height = self.nodeWithPortPoints["height"].as_f64().expect("height is required");
        let cx = self.nodeWithPortPoints["center"]["x"].as_f64().expect("center.x is required");
        let cy = self.nodeWithPortPoints["center"]["y"].as_f64().expect("center.y is required");
        ...
        let points = self.nodeWithPortPoints["portPoints"].as_array().expect("portPoints is required").clone();
        self.availableZ = if let Some(zs) = self.nodeWithPortPoints["availableZ"].as_array() {
            zs.iter().map(|z| z.as_f64().expect("availableZ must be numeric")).collect()
        } else {
            let mut zs = Vec::new();
            for pp in &points {
        ...
        if let Some(max) = self.maxCellCount {
            if totalCells > max {
                self.error = Some(format!("Cell count {totalCells} exceeds maxCellCount {max}"));
                self.failed = true;
                return;
            }
        }
        ...
        if let Some(penalties) = self.penaltyMap.as_array() {
            assert_eq!(penalties.len(), self.planeSize, "penaltyMap must match cell count");
        ...
        for pp in &points {
            ...
            let Some(&connId) = self.connNameToId.get(name) else { continue; };
            ...
            if let Some(existingRoot) = rootByPortFlat.get(&flatIdx) {
                if existingRoot != rootNet { self.sharedCrossRootPortFlat[flatIdx] = 1; }
            } else {
                rootByPortFlat.insert(flatIdx, rootNet.clone());
            }
            let existing = self.portOwnerFlat[flatIdx];
            self.portOwnerFlat[flatIdx] = if existing == -1 || existing == connId as i32 { connId as i32 } else { -2 };
        }
    ...
    fn buildFiveRegionGrid(&mut self, width: f64, height: f64) {
        ...
        self.regions = vec![
            RegionDef { id: REGION_LEFT, fineOriginRow: 0, fineOriginCol: 0, fineRows: self.fineRows, fineCols: self.bandCols, cellScale: 1, rows: self.fineRows, cols: self.bandCols, offset: 0 },
            RegionDef { id: REGION_TOP, fineOriginRow: 0, fineOriginCol: self.bandCols, fineRows: self.bandRows, fineCols: middleFineCols, cellScale: 1, rows: self.bandRows, cols: middleFineCols, offset: 0 },
            RegionDef { id: REGION_RIGHT, fineOriginRow: 0, fineOriginCol: self.fineCols - self.bandCols, fineRows: self.fineRows, fineCols: self.bandCols, cellScale: 1, rows: self.fineRows, cols: self.bandCols, offset: 0 },
            RegionDef { id: REGION_BOTTOM, fineOriginRow: self.fineRows - self.bandRows, fineOriginCol: self.bandCols, fineRows: self.bandRows, fineCols: middleFineCols, cellScale: 1, rows: self.bandRows, cols: middleFineCols, offset: 0 },
            RegionDef { id: REGION_MIDDLE, fineOriginRow: self.bandRows, fineOriginCol: self.bandCols, fineRows: middleFineRows, fineCols: middleFineCols, cellScale: self.lowScale, rows: middleFineRows.div_ceil(self.lowScale), cols: middleFineCols.div_ceil(self.lowScale), offset: 0 },
        ];
        let mut offset = 0;
        ...
        for region in &self.regions {
            for row in 0..region.rows {
                ...
                let fineRow1 = (region.fineOriginRow + region.fineRows).min(fineRow0 + region.cellScale);
                let minY = self.boundsMinY + fineRow0 as f64 * self.highResolutionCellSize;
                let maxY = self.boundsMaxY.min(self.boundsMinY + fineRow1 as f64 * self.highResolutionCellSize);
                for col in 0..region.cols {
                    let fineCol0 = region.fineOriginCol + col * region.cellScale;
                    let fineCol1 = (region.fineOriginCol + region.fineCols).min(fineCol0 + region.cellScale);
                    let minX = self.boundsMinX + fineCol0 as f64 * self.highResolutionCellSize;
                    let maxX = self.boundsMaxX.min(self.boundsMinX + fineCol1 as f64 * self.highResolutionCellSize);
        ...
        for region in &self.regions {
            for row in 0..region.rows {
                for col in 0..region.cols {
                    ...
                    if row + 1 < region.rows { addBidirectionalEdge(cellId, self.cellIdFor(region.id, row + 1, col)); }
                    if col + 1 < region.cols { addBidirectionalEdge(cellId, self.cellIdFor(region.id, row, col + 1)); }
                }
            }
        }
        ...
        if hasLeft && hasTop {
            for globalRow in 0..self.bandRows {
                addBidirectionalEdge(self.cellIdFor(REGION_LEFT, globalRow, left.cols - 1), self.cellIdFor(REGION_TOP, globalRow, 0));
            }
        }
        if hasTop && hasRight {
            for globalRow in 0..self.bandRows {
                addBidirectionalEdge(self.cellIdFor(REGION_TOP, globalRow, top.cols - 1), self.cellIdFor(REGION_RIGHT, globalRow, 0));
            }
        }
        if hasLeft && hasBottom {
            for globalRow in self.fineRows - self.bandRows..self.fineRows {
                addBidirectionalEdge(self.cellIdFor(REGION_LEFT, globalRow, left.cols - 1), self.cellIdFor(REGION_BOTTOM, globalRow - (self.fineRows - self.bandRows), 0));
            }
        }
        if hasBottom && hasRight {
            for globalRow in self.fineRows - self.bandRows..self.fineRows {
                addBidirectionalEdge(self.cellIdFor(REGION_BOTTOM, globalRow - (self.fineRows - self.bandRows), bottom.cols - 1), self.cellIdFor(REGION_RIGHT, globalRow, 0));
            }
        }
        if hasLeft && hasMiddle {
            for globalRow in self.bandRows..self.fineRows - self.bandRows {
                addBidirectionalEdge(self.cellIdFor(REGION_LEFT, globalRow, left.cols - 1), self.cellIdFor(REGION_MIDDLE, (globalRow - self.bandRows) / self.lowScale, 0));
            }
        }
        if hasRight && hasMiddle {
            for globalRow in self.bandRows..self.fineRows - self.bandRows {
                addBidirectionalEdge(self.cellIdFor(REGION_MIDDLE, (globalRow - self.bandRows) / self.lowScale, middle.cols - 1), self.cellIdFor(REGION_RIGHT, globalRow, 0));
            }
        }
        if hasTop && hasMiddle {
            for globalCol in self.bandCols..self.fineCols - self.bandCols {
                addBidirectionalEdge(self.cellIdFor(REGION_TOP, top.rows - 1, globalCol - self.bandCols), self.cellIdFor(REGION_MIDDLE, 0, (globalCol - self.bandCols) / self.lowScale));
            }
        }
        if hasBottom && hasMiddle {
            for globalCol in self.bandCols..self.fineCols - self.bandCols {
                addBidirectionalEdge(self.cellIdFor(REGION_MIDDLE, middle.rows - 1, (globalCol - self.bandCols) / self.lowScale), self.cellIdFor(REGION_BOTTOM, 0, globalCol - self.bandCols));
            }
        }
        ...
        if !hasMiddle && !hasTop && !hasBottom && hasLeft && hasRight {
            for row in 0..left.rows.min(right.rows) {
                addBidirectionalEdge(self.cellIdFor(REGION_LEFT, row, left.cols - 1), self.cellIdFor(REGION_RIGHT, row, 0));
            }
        }
        if !hasMiddle && !hasLeft && !hasRight && hasTop && hasBottom {
            for col in 0..top.cols.min(bottom.cols) {
                addBidirectionalEdge(self.cellIdFor(REGION_TOP, top.rows - 1, col), self.cellIdFor(REGION_BOTTOM, 0, col));
            }
        }
    ...
    fn stepOnce(&mut self) {
        if self.activeConnSeg.is_none() {
            ...
            let startIdx = self.nodePool.push(next.startZ, next.startCellId, 0.0, -1, -1, 0);
        ...
        let connRips = self.ripCount.get(self.activeConnId as usize).copied().unwrap_or(0);
        let budget = (self.baseSearchBudgetIters as f64 * (1.0 + connRips.min(10) as f64 * 0.25) + 0.5).floor() as usize;
        if self.searchIterations > budget {
            for p in &mut self.penalty2d { *p *= 0.9; }
            ...
            if self.consecutiveSkips >= 3.max(self.unsolvedSegs.len() * 3) {
                self.error = Some(format!("Convergence failure: {} connections stuck", self.unsolvedSegs.len()));
                self.failed = true;
            }
            return;
        }
        if self.heap.n == 0 {
            self.error = Some(format!("No path found for {}", self.connIdToName[self.activeConnId as usize]));
            self.failed = true;
            return;
        }
        ...
        if self.visitedStamp[searchStateIdx] == self.stamp { return; }
        ...
        for i in neighborStart..neighborEnd {
            ...
            self.computeMoveCostAndRips(activeConn, z, neighborCellId, false, rippedHead, ripCount, self.neighborCosts[i] as f64);
            if self._moveCost < 0.0 { continue; }
            let nextStateIdx = self.getSearchStateIdx(nextFlatIdx, self._moveRipCount);
            if self.visitedStamp[nextStateIdx] == stamp { continue; }
            let g2 = g + self._moveCost;
            if self.bestGStamp[nextStateIdx] == stamp && g2 >= self.bestGValue[nextStateIdx] { continue; }
            ...
            let newNodeIdx = self.nodePool.push(z, neighborCellId, g2, nodeIdx as i32, self._moveRippedHead, self._moveRipCount);
        ...
        if self.viaAllowed[cellId] != 0 {
            for nz in 0..self.layers {
                ...
                self.computeMoveCostAndRips(activeConn, nz, cellId, true, rippedHead, ripCount, 0.0);
                if self._moveCost < 0.0 { continue; }
                let nextStateIdx = self.getSearchStateIdx(nextFlatIdx, self._moveRipCount);
                if self.visitedStamp[nextStateIdx] == stamp { continue; }
                let g2 = g + self._moveCost;
                ...
                let newNodeIdx = self.nodePool.push(nz, cellId, g2, nodeIdx as i32, self._moveRippedHead, self._moveRipCount);
                self.heap.push(f2, self.seqCounter, newNodeIdx);
                self.seqCounter = self.seqCounter.wrapping_add(1);
            }
        }
    ...
    fn computeMoveCostAndRips(&mut self, activeConn: usize, toZ: usize, toCellId: usize, isVia: bool, rippedHead: i32, currentRipCount: usize, lateralCost: f64) {
        let mut cost = 0.0;
        ...
        if isVia {
            ...
            let isSegEnd = self.activeConnSeg.as_ref().is_some_and(|seg| toZ == seg.endZ && toCellId == seg.endCellId);
            ...
            self.pushFlatOccupantsWithMembership(toFlatIdx, activeConn, self.connIdToRootNetId[activeConn], &mut occs, &mut membership);
            for &occ in &occs {
                if !self.ripChain.contains(head, occ) {
                    cost += self.ripCost;
                    head = self.ripChain.append(head, occ);
    ...
    fn fillViaOccupants(&self, cellId: usize, activeConn: usize, occs: &mut Vec<usize>, cells: &mut Vec<usize>, membership: &mut OccupantMembership) {
        occs.clear();
        membership.begin(self.connIdToRootNetId.len());
        ...
        for &occCellId in cells.iter() {
            if !circleIntersectsRect(cx, cy, self.viaKeepoutRadius, self.cellMinX[occCellId], self.cellMinY[occCellId], self.cellMaxX[occCellId], self.cellMaxY[occCellId]) { continue; }
            for z in 0..self.layers {
                self.pushFlatOccupantsWithMembership(z * self.planeSize + occCellId, activeConn, activeRoot, occs, membership);
            }
        }
    }
    ...
    fn pushFlatOccupants(&self, flatIdx: usize, activeConn: usize, out: &mut Vec<usize>) {
        ...
        if primaryOcc != -1 && primaryOcc != activeConn as i32 && !self.allowSharedUse(activeConn, primaryOcc) {
            pushUnique(out, primaryOcc as usize);
        }
        let Some(sharedOccs) = &self.cellOccupants[flatIdx].shared else { return; };
        for &occ in sharedOccs.iter() {
            if occ == activeConn || self.allowSharedUse(activeConn, occ as i32) { continue; }
            pushUnique(out, occ);
        }
    }

    fn pushFlatOccupantsWithMembership(&self, flatIdx: usize, activeConn: usize, activeRoot: usize, out: &mut Vec<usize>, membership: &mut OccupantMembership) {
        let cell = &self.cellOccupants[flatIdx];
        let primaryOcc = cell.primary;
        if primaryOcc != -1 && primaryOcc != activeConn as i32 && !(primaryOcc >= 0 && self.connIdToRootNetId[primaryOcc as usize] == activeRoot) {
            membership.push(out, primaryOcc as usize);
        }
        let Some(sharedOccs) = &cell.shared else { return; };
        for &occ in sharedOccs.iter() {
            if occ == activeConn || self.connIdToRootNetId[occ] == activeRoot { continue; }
            membership.push(out, occ);
        }
    }

    fn addSharedOccupant(&mut self, flatIdx: usize, connId: usize) {
        if self.cellOccupants[flatIdx].primary == connId as i32 { return; }
        let sharedOccs = self.cellOccupants[flatIdx].shared.get_or_insert_with(Box::default);
    ...
    fn removeOccupant(&mut self, flatIdx: usize, connId: usize) {
        ...
        if cell.primary == connId as i32 {
            if let Some(sharedOccs) = &mut cell.shared {
                if !sharedOccs.is_empty() {
                    ...
                    if sharedOccs.is_empty() { cell.shared = None; }
                    return;
                }
            }
            cell.primary = -1;
        ...
        let Some(sharedOccs) = &mut cell.shared else { return; };
        let Some(idx) = sharedOccs.iter().position(|&id| id == connId) else { return; };
    ...
    fn shouldSkipFixedPortHalo(&self, flatIdx: usize, connId: usize) -> bool {
        ...
        if fixedOwner == connId as i32 { return false; }
    ...
    fn internConn(&mut self, name: &str, rootNetName: Option<&str>) -> usize {
        if let Some(&existing) = self.connNameToId.get(name) { return existing; }
        ...
        let root = rootNetName.map(str::to_owned).unwrap_or_else(|| {
            if let Some((prefix, suffix)) = name.rsplit_once("_mst") {
                if !suffix.is_empty() && suffix.bytes().all(|b| b.is_ascii_digit()) { return prefix.into(); }
            }
            name.into()
        });
    ...
    fn buildConnectionSegs(&mut self) -> Vec<ConnectionSeg> {
        let points = self.nodeWithPortPoints["portPoints"].as_array().expect("portPoints is required");
        let mut byName: Vec<(String, Vec<Value>, Option<String>)> = Vec::new();
        for pp in points {
            let name = pp["connectionName"].as_str().expect("connectionName is required");
            let idx = if let Some(i) = byName.iter().position(|c| c.0 == name) { i } else {
                byName.push((name.into(), Vec::new(), pp["rootConnectionName"].as_str().map(str::to_owned)));
                byName.len() - 1
            };
        ...
        for (name, pts, rootConnectionName) in byName {
            let pointPairs = crate::get_connection_port_point_pairs::get_connection_port_point_pairs(&pts);
            if pointPairs.is_empty() { continue; }
            ...
            for [startPoint, endPoint] in pointPairs {
                ...
                let orderedEndpoints = if endpointA < endpointB { format!("{endpointA}|{endpointB}") } else { format!("{endpointB}|{endpointA}") };
                ...
                segs.push(ConnectionSeg { connId, startZ, startCellId, startPoint: startPoint.clone(), endZ, endCellId, endPoint: endPoint.clone() });
            }
        }
        segs
    }
    ...
    fn pointToCell(&self, pt: &Value) -> (usize, usize) {
        ...
        let fineCol = clamp(((x - self.boundsMinX) / self.highResolutionCellSize).floor(), 0.0, (self.fineCols - 1) as f64) as usize;
        let fineRow = clamp(((y - self.boundsMinY) / self.highResolutionCellSize).floor(), 0.0, (self.fineRows - 1) as f64) as usize;
        let regionId = if fineCol < self.bandCols { REGION_LEFT }
            else if fineCol >= self.fineCols - self.bandCols { REGION_RIGHT }
            else if fineRow < self.bandRows { REGION_TOP }
            else if fineRow >= self.fineRows - self.bandRows { REGION_BOTTOM }
            else { REGION_MIDDLE };
        let region = &self.regions[regionId];
        let localFineRow = fineRow as f64 - region.fineOriginRow as f64;
        let localFineCol = fineCol as f64 - region.fineOriginCol as f64;
        let row = clamp((localFineRow / region.cellScale as f64).floor(), 0.0, region.rows.saturating_sub(1) as f64) as usize;
        let col = clamp((localFineCol / region.cellScale as f64).floor(), 0.0, region.cols.saturating_sub(1) as f64) as usize;
        (self.availableZ.iter().rposition(|&v| v == z).unwrap_or(0), self.cellIdFor(regionId, row, col))
    }
    ...
    fn finalizeRoute(&mut self, goalNodeIdx: usize) {
        ...
        states.reverse();
        while states.len() > 1 {
            if self.sharedCrossRootPortFlat[states[0]] == 0 { break; }
            states.remove(0);
        }
        while states.len() > 1 {
            if self.sharedCrossRootPortFlat[*states.last().unwrap()] == 0 { break; }
            states.pop();
        }
        ...
        if self.usedIndicesByConn.len() <= connId { self.usedIndicesByConn.resize(connId + 1, None); }
        self.usedIndicesByConn[connId].get_or_insert_with(Vec::new).extend(indices);
        if self.solvedRoutes.len() <= connId { self.solvedRoutes.resize(connId + 1, None); }
        let seg = self.activeConnSeg.as_ref().unwrap();
        self.solvedRoutes[connId].get_or_insert_with(Vec::new).push(SolvedRouteInternal {
            states,
            viaCellIds,
        ...
        if !rippedIds.is_empty() || !displacedByVias.is_empty() {
            if self.totalRipEvents > 50 {
                for p in &mut self.penalty2d { *p *= 0.99; }
            } else {
                for p in &mut self.penalty2d {
                    if *p > self.penaltyCap { *p *= 0.5; }
                }
            }
        }
    }
    ...
    fn extractViaCellIds(&self, states: &[usize]) -> Vec<usize> {
        ...
        for i in 1..states.len() {
            ...
            if prevZ != nextZ { viaCellIds.push(states[i] - nextZ * self.planeSize); }
        }
        viaCellIds
    }

    fn markTraceFootprint(&mut self, connId: usize, z: usize, sourceCellId: usize, indices: &mut Vec<usize>) {
        ...
        for cellId in self.cellsNearCircle(cx, cy, self.traceKeepoutRadius) {
            if !circleIntersectsRect(cx, cy, self.traceKeepoutRadius, self.cellMinX[cellId], self.cellMinY[cellId], self.cellMaxX[cellId], self.cellMaxY[cellId]) { continue; }
            let flatIdx = z * self.planeSize + cellId;
            if cellId != sourceCellId && self.shouldSkipFixedPortHalo(flatIdx, connId) { continue; }
            let existing = self.cellOccupants[flatIdx].primary;
            let allowSameRootOverlap = self.allowSharedUse(connId, existing);
            if existing != -1 && existing != connId as i32 && !allowSameRootOverlap { continue; }
    ...
    fn markViaFootprint(&mut self, connId: usize, sourceCellId: usize, indices: &mut Vec<usize>, displacedByVias: &mut Vec<usize>) {
        let cx = self.cellCenterX[sourceCellId];
        let cy = self.cellCenterY[sourceCellId];
        for cellId in self.cellsNearCircle(cx, cy, self.viaKeepoutRadius) {
            if !circleIntersectsRect(cx, cy, self.viaKeepoutRadius, self.cellMinX[cellId], self.cellMinY[cellId], self.cellMaxX[cellId], self.cellMaxY[cellId]) { continue; }
            for z in 0..self.layers {
                let flatIdx = z * self.planeSize + cellId;
                if cellId != sourceCellId && self.shouldSkipFixedPortHalo(flatIdx, connId) { continue; }
                let occs = self.fillTraceOccupants(flatIdx, connId);
                if !occs.is_empty() {
                    for id in occs { pushUnique(displacedByVias, id); }
    ...
    fn cellsNearCircleInto(&self, cx: f64, cy: f64, radius: f64, cells: &mut Vec<usize>) {
        cells.clear();
        let minFineCol = clamp(((cx - radius - self.boundsMinX) / self.highResolutionCellSize).floor(), 0.0, (self.fineCols - 1) as f64) as usize;
        let maxFineCol = clamp(((cx + radius - self.boundsMinX) / self.highResolutionCellSize).floor(), 0.0, (self.fineCols - 1) as f64) as usize;
        let minFineRow = clamp(((cy - radius - self.boundsMinY) / self.highResolutionCellSize).floor(), 0.0, (self.fineRows - 1) as f64) as usize;
        let maxFineRow = clamp(((cy + radius - self.boundsMinY) / self.highResolutionCellSize).floor(), 0.0, (self.fineRows - 1) as f64) as usize;
        for region in &self.regions {
            ...
            if regionFineRowMin > regionFineRowMax || regionFineColMin > regionFineColMax { continue; }
    ...
    fn ripTrace(&mut self, connId: usize) {
        if self.ripCount.len() <= connId { self.ripCount.resize(connId + 1, 0); }
        ...
        for route in &routes {
            ...
            for &cellId in &route.viaCellIds { self.penalty2d[cellId] += self.ripViaPenalty; }
        }
        if let Some(indices) = self.usedIndicesByConn.get_mut(connId).and_then(Option::take) {
            for idx in indices { self.removeOccupant(idx, connId); }
        }
        if !routes.is_empty() {
    ...
    pub fn get_output(&self) -> Value {
        ...
        for connId in 0..self.solvedRoutes.len() {
            ...
            for route in routes {
                let mut points: Vec<Value> = route.states.iter().map(|&state| {
                    let z = state / self.planeSize;
                    let cellId = state - z * self.planeSize;
                    let mut tp = self.transformPoint(self.cellCenterX[cellId], self.cellCenterY[cellId]);
                ...
                let vias: Vec<Value> = route.viaCellIds.iter().map(|&cellId| self.transformPoint(self.cellCenterX[cellId], self.cellCenterY[cellId])).collect();
                let mut output = json!({"connectionName": connName});
                if let Some(regionId) = self.nodeWithPortPoints.get("capacityMeshNodeId") { output["regionId"] = regionId.clone(); }
    ...
    fn getSolvedRoutesForConn(&self, connId: usize) -> &[SolvedRouteInternal] {
        self.solvedRoutes.get(connId).and_then(Option::as_deref).unwrap_or(&[])
    }

    pub fn solved_segment_count(&self) -> usize {
        (0..self.solvedRoutes.len()).map(|id| self.getSolvedRoutesForConn(id).len()).sum()
    }
    ...
    fn computeGridToBoundsTransform(&self) -> [f64; 6] {
        ...
        let c = if xSpan > 0.0 { self.boundsMinX - a * minCenterX } else { (self.boundsMinX + self.boundsMaxX) / 2.0 - minCenterX };
        let f = if ySpan > 0.0 { self.boundsMinY - e * minCenterY } else { (self.boundsMinY + self.boundsMaxY) / 2.0 - minCenterY };
        [a, 0.0, c, 0.0, e, f]
    }

    pub fn visualize(&self) -> Value {
        let layerColors = ["red", "blue", "orange", "green"];
        let traceColors = ["rgba(255,0,0,0.75)", "rgba(0,0,255,0.75)", "rgba(255,165,0,0.75)", "rgba(0,128,0,0.75)"];
        ...
        if self.showUsedCellMap {
            for z in 0..self.layers {
                for cellId in 0..self.planeSize {
                    if self.cellOccupants[z * self.planeSize + cellId].primary == -1 { continue; }
                    rects.push(json!({
                        "center": self.transformPoint(self.cellCenterX[cellId], self.cellCenterY[cellId]),
                        "width": self.cellWidth[cellId] * self.gridToBoundsTransform[0],
                        "height": self.cellHeight[cellId] * self.gridToBoundsTransform[4],
        ...
        for pp in self.nodeWithPortPoints["portPoints"].as_array().expect("portPoints is required") {
        ...
        for route in transformedRoutes.as_array().unwrap() {
            ...
            for i in 1..path.len() {
                ...
                if curr["z"] != prev["z"] {
                    if i - segStart >= 2 {
                        let ps: Vec<Value> = path[segStart..i].iter().map(|p| json!({"x": p["x"], "y": p["y"]})).collect();
            ...
            if path.len() - segStart >= 2 {
                ...
                let ps: Vec<Value> = path[segStart..].iter().map(|p| json!({"x": p["x"], "y": p["y"]})).collect();
                lines.push(json!({"points": ps, "strokeColor": traceColors.get(z).copied().unwrap_or("rgba(128,128,128,0.75)"), "strokeWidth": self.traceThickness}));
            }
        }
        for route in transformedRoutes.as_array().unwrap() {
        ...
        if self.activeConnSeg.is_some() {
            for z in 0..self.layers {
                for cellId in 0..self.planeSize {
                    if self.visitedFlatStamp[z * self.planeSize + cellId] != self.stamp { continue; }
                    let mut tc = self.transformPoint(self.cellCenterX[cellId], self.cellCenterY[cellId]);
...
mod occupancy_tests {
    ...
    fn sparse_shared_occupants_preserve_order_promotion_and_removal() {
        ...
        solver.cellOccupants = vec![CellOccupants { primary: 0, shared: None }, CellOccupants::default()];
        ...
        assert_eq!(std::mem::size_of::<Option<Box<Vec<usize>>>>(),std::mem::size_of::<usize>());
        assert_eq!(std::mem::size_of::<Option<Vec<usize>>>(),3*std::mem::size_of::<usize>());
        solver.cellOccupants = vec![CellOccupants { primary: 0, shared: Some(Box::new(vec![1,2,1])) }, CellOccupants { primary: 1, shared: Some(Box::new(vec![3,2])) }, CellOccupants { primary: 2, shared: None }];
        solver.connIdToRootNetId = vec![0,1,2,4,4];
        let mut membership = OccupantMembership::default();
        for overflow in [false,true] {
            if overflow { membership.generation = u32::MAX; }
            membership.begin(5);
            let mut expected = Vec::new();
            let mut actual = Vec::new();
            for cell in [0,1,0,2,1] {
                solver.pushFlatOccupants(cell,4,&mut expected);
                solver.pushFlatOccupantsWithMembership(cell,4,solver.connIdToRootNetId[4],&mut actual,&mut membership);
            }
            assert_eq!(actual,expected);
            assert_eq!(actual,vec![0,1,2]);
            if overflow { assert_eq!(membership.generation,1); }
        }
    }
}
