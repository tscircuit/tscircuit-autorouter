...
pub fn compute_max_iterations_by_node_size_and_connection_count(
    ...
    let requested_max_iterations = if input.max_iterations.is_nan() {
        f64::NAN
    } else {
        1.0_f64.max(input.max_iterations)
    };
    ...
    let max_iterations_iters = if requested_max_iterations.is_nan()
        || min_iteration_budget_iters.is_nan()
        || computed_max_iters.is_nan()
    {
        f64::NAN
    } else {
        1.0_f64
            .max(requested_max_iterations.min(min_iteration_budget_iters.max(computed_max_iters)))
    };
