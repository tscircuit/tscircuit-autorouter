...
impl StandaloneDrcEvaluator {
    pub fn new(
        ...
        let engine = AutoroutingDrcEngine::new_with_math(
            srj.clone(),
            conn_map,
            json!({
    ...
    pub fn traces(&self, routes: &Routes) -> (Vec<SimplifiedTrace>, IndexMap<String, usize>) {
        let mut grouped: IndexMap<
            ...
            Vec<(
                usize,
                &crate::solvers::global_drc_force_improve_solver::internal_types::MutableRoute,
            )>,
        > = IndexMap::new();
        ...
        for connection in self.srj["connections"].as_array().expect("Connections") {
            ...
            for (index, (route_index, route)) in group.iter().enumerate() {
                ...
                let width = route
                    .trace_thickness
                    .or_else(|| connection["nominalTraceWidth"].as_f64())
                    .or_else(|| self.srj["nominalTraceWidth"].as_f64())
                    .or_else(|| self.srj["minTraceWidth"].as_f64())
                    .unwrap_or(0.1);
                let diameter = route
                    .via_diameter
                    .or_else(|| self.srj["minViaDiameter"].as_f64());
                traces.push(SimplifiedTrace {
                    pcb_trace_id: id.clone(),
                    connection_name: connection["netConnectionName"]
                        .as_str()
                        .or_else(|| connection["rootConnectionName"].as_str())
                        .unwrap_or(name)
                        .to_owned(),
                    route: convert_hd_route_to_simplified_route(
                        route,
                        layer_count,
                        width,
                        diameter,
...
impl DrcEvaluator for StandaloneDrcEvaluator {
    fn snapshot(
        ...
        let errors = self
            .engine
            .evaluate_typed_trace_errors(&traces.iter().collect::<Vec<_>>());
        let count = errors
            .iter()
            .filter(|error| !legacy || !is_via_pad_drc_error(error))
            .count();
        let has_centered = errors.iter().any(|error| !error["center"].is_null());
        let errors: Vec<_> = errors
            .into_iter()
            .filter(|error| {
                (!legacy || !is_via_pad_drc_error(error))
                    && (!has_centered || !error["center"].is_null())
            })
            .map(|mut error| {
                if !topology {
                    if error["first_contact_center"].is_object()
                        || error
                            .get("first_contact_center")
                            .is_some_and(Value::is_null)
                    {
                        error["center"] = error["first_contact_center"].clone();
                    }
                    if error["first_contact_message"].is_string() {
                        error["message"] = error["first_contact_message"].clone();
                    }
                    if error["first_actual_clearance"].is_number() {
                        error["actual_clearance"] = error["first_actual_clearance"].clone();
                    }
                }
                error
            })
