use crate::drc::simplified_trace::{SimplifiedRoutePoint, SimplifiedTrace};
use crate::transformation_matrix::{
    Matrix, apply_to_point, compose, inverse, rotate_deg_with_math, translate,
};
use crate::utils::get_via_layers::get_via_layers_from_span;
...
impl<'a> NetIds<'a> {
    fn new() -> Self {
        Self {
            ...
            compiled: HashMap::with_hasher(FxBuildHasher),
        }
    }
...
impl SpatialItem for DynamicCollidable {
    fn slot(self) -> usize {
        match self {
            Self::Segment(index) => index * 2,
            Self::Via(index) => index * 2 + 1,
        }
    }
}
...
impl<T: SpatialItem> SpatialHash<T> {
    fn new(cell_size: f64) -> Self {
        Self {
            cells: IndexMap::with_hasher(FxBuildHasher),
            cell_size,
        }
    }
...
fn num(value: &Value) -> f64 {
    value.as_f64().expect("Expected a number")
}
fn string(value: &Value) -> &str {
    value.as_str().expect("Expected a string")
}
fn array(value: &Value) -> &[Value] {
    value.as_array().expect("Expected an array")
}
fn strings(value: &Value) -> Vec<String> {
    array(value).iter().map(|v| string(v).to_owned()).collect()
}
fn point_json(p: Point) -> Value {
    json!({"x": p.x, "y": p.y})
}
fn optional_num(value: &Value, default: f64) -> f64 {
    if value.is_null() { default } else { num(value) }
}
...
fn get_obstacle_bounds(o: &StaticObstacle) -> Bounds {
    ...
    get_bounds_from_points(
        &points
            .iter()
            .map(|p| apply_to_point(&o.local_to_world, p))
            .collect::<Vec<_>>(),
    )
    .expect("Obstacle corners must have bounds")
}
...
fn closest_between_segments(
    ...
    if let Some(p) = get_segment_intersection(&a.start, &a.end, &b.start, &b.end) {
        return p;
    }
    let candidates = [
        (
            a.start,
            point_to_segment_closest_point(&a.start, &b.start, &b.end),
        ),
        (
            a.end,
            point_to_segment_closest_point(&a.end, &b.start, &b.end),
        ),
        (
            point_to_segment_closest_point(&b.start, &a.start, &a.end),
            b.start,
        ),
        (
            point_to_segment_closest_point(&b.end, &a.start, &a.end),
            b.end,
        ),
    ];
    ...
    Point {
        x: (closest.0.x + closest.1.x) / 2.0,
        y: (closest.0.y + closest.1.y) / 2.0,
    }
}
...
fn closest_segment_bounds(start: &Point, end: &Point, b: Bounds) -> Point {
    ...
    let q = Point {
        x: b.min_x.max(b.max_x.min(p.x)),
        y: b.min_y.max(b.max_y.min(p.y)),
    };
    Point {
        x: (p.x + q.x) / 2.0,
        y: (p.y + q.y) / 2.0,
...
fn trace_error_message(trace_id: &str, other: &str, gap: f64) -> String {
    if gap < 0.0 {
        format!("PCB trace {trace_id} overlaps with {other} (accidental contact)")
    } else {
        format!(
            "PCB trace {trace_id} is too close to {other} (gap: {}mm)",
            js_to_fixed(gap, 3)
        )
    }
}
...
impl AutoroutingDrcEngine {
    ...
    pub fn new_with_math(
        ...
        let via_to_pad_clearance = optional_num(
            &options["viaToPadClearance"],
            optional_num(&srj["minViaEdgeToPadEdgeClearance"], 0.1),
        );
        ...
        let default_cell_size = 0.25f64.max(board_width.max(board_height) / 64.0).max(
            optional_num(&srj["minViaDiameter"], 0.3) + trace_clearance.max(via_to_pad_clearance),
        );
        let cell_size = optional_num(&options["spatialCellSize"], default_cell_size);
        if !trace_clearance.is_finite() || trace_clearance < 0.0 {
            return Err("traceClearance must be a non-negative finite number".into());
        }
        if !via_clearance.is_finite() {
            return Err("viaClearance must be a finite number".into());
        }
        if !via_to_pad_clearance.is_finite() || via_to_pad_clearance < 0.0 {
            return Err("viaToPadClearance must be a non-negative finite number".into());
        }
        if !cell_size.is_finite() || cell_size <= 0.0 {
            return Err("spatialCellSize must be a positive finite number".into());
        }
        let mut engine = Self {
            compiled: Rc::new(CompiledDrcData {
                math,
                srj,
                trace_clearance,
                via_clearance,
                via_to_pad_clearance,
                cell_size,
                include_trace_via_owner_metadata: options["includeTraceViaOwnerMetadata"]
                    .as_bool()
                    .unwrap_or(false),
                canonical_net_by_alias: IndexMap::with_hasher(FxBuildHasher),
                conn_map_net_by_canonical_net: IndexMap::with_hasher(FxBuildHasher),
                obstacles: Vec::new(),
                obstacle_indexes_by_layer: IndexMap::with_hasher(FxBuildHasher),
            }),
            conn_map,
            last_run_stats: AutoroutingDrcEngineRunStats::default(),
        };
        engine.compile_connection_aliases();
        let obstacles = engine.compile_static_obstacles();
        Rc::get_mut(&mut engine.compiled)
            .expect("Unshared constructor data")
            .obstacles = obstacles;
    ...
    fn conn_map_net(&self, id: &str) -> Option<&str> {
        self.conn_map
            .as_ref()
            .and_then(|m| m.id_to_net_map.get(id))
            .map(String::as_str)
            .filter(|s| !s.is_empty())
    }
    ...
    fn compile_connection_aliases(&mut self) {
        ...
        for connection in array(&self.compiled.srj["connections"]) {
            let canonical = connection["netConnectionName"]
                .as_str()
                .or_else(|| connection["rootConnectionName"].as_str())
                .unwrap_or_else(|| string(&connection["name"]));
            let mut aliases = vec![
                &connection["name"],
                &connection["rootConnectionName"],
                &connection["netConnectionName"],
            ];
            if let Some(merged) = connection["mergedConnectionNames"].as_array() {
                aliases.extend(merged);
            }
            for p in array(&connection["pointsToConnect"]) {
                aliases.push(&p["pointId"]);
                aliases.push(&p["pcb_port_id"]);
            }
            for alias in aliases {
                let Some(alias) = alias.as_str().filter(|s| !s.is_empty()) else {
                    continue;
                };
                aliases_by_name.insert(alias.to_owned(), canonical.to_owned());
                if let Some(net) = self.conn_map_net(alias) {
                    nets.entry(canonical.to_owned())
                        .or_default()
                        .insert(net.to_owned());
                }
            }
        ...
        for (canonical, values) in nets {
            if values.len() == 1 {
                canonical_nets.insert(canonical, values.into_iter().next().unwrap());
            }
        }
    ...
    fn resolve_net_id<'a>(&'a self, id: &'a str) -> &'a str {
        if let Some(net) = self.conn_map_net(id) {
            return net;
        }
        let Some(canonical) = self
            .compiled
            .canonical_net_by_alias
            .get(id)
            .filter(|s| !s.is_empty())
        else {
            return id;
        ...
        self.compiled
            .conn_map_net_by_canonical_net
            .get(canonical)
            .unwrap_or(canonical)
    }

    fn compile_net<'a>(&'a self, id: &'a str, ids: &mut NetIds<'a>) -> ResolvedNet {
        if let Some(net) = ids.compiled.get(id) {
            return *net;
        }
        let net = ResolvedNet {
            id: ids.intern(id),
    ...
    fn are_connected(&self, left: &ResolvedNet, right: &ResolvedNet) -> bool {
        ...
        if let (Some(a), Some(b)) = (left.conn_map_net, right.conn_map_net)
            && (a == b || b == left.id)
        {
            return true;
        }
    ...
    fn compile_static_obstacles(&self) -> Vec<StaticObstacle> {
        ...
        for o in array(&self.compiled.srj["obstacles"]) {
            ...
            let plated_hole_id = connected_to
                .iter()
                .find(|id| id.starts_with("pcb_plated_hole_"));
            let pcb_port_id = connected_to
                .iter()
                .find(|id| id.starts_with("pcb_port_"))
                .cloned();
            if smt_pad_id.is_none() && plated_hole_id.is_none() && pcb_port_id.is_none() {
                continue;
            }
            let multilayer = layers.len() > 1;
            let obstacle_type = if multilayer {
                "pcb_plated_hole"
            } else {
                "pcb_smtpad"
            };
            ...
            let added = if multilayer {
                &mut added_plated_hole_ids
            } else {
                &mut added_smt_pad_ids
            };
            if !added.insert(obstacle_id.clone()) {
                continue;
            }
            let rotation = o["ccwRotationDegrees"].as_f64().filter(|r| r.is_finite());
            let local_to_world = compose(&[
                translate(x, y),
                rotate_deg_with_math(
                    rotation.unwrap_or(0.0),
                    self.compiled.math.sin,
                    self.compiled.math.cos,
                ),
            ]);
            let width = num(&o["width"]);
            let height = num(&o["height"]);
            let radius = if rotation.is_none() && multilayer && (width - height).abs() < 0.001 {
                Some(width.max(height) / 2.0)
            } else {
                None
            };
            let world_to_local = inverse(&local_to_world);
            obstacles.push(StaticObstacle {
                obstacle_type: obstacle_type.into(),
                obstacle_id,
                connected_to,
                x,
                y,
    ...
    fn index_static_obstacles(&mut self) {
        ...
        for (i, obstacle) in self.compiled.obstacles.iter().enumerate() {
            let bounds = expand_bounds(
                get_obstacle_bounds(obstacle),
                self.compiled
                    .trace_clearance
                    .max(self.compiled.via_to_pad_clearance),
            );
            for layer in &obstacle.layers {
                indexes
                    .entry(layer.clone())
                    .or_insert_with(|| SpatialHash::new(self.compiled.cell_size))
                    .insert(i, bounds);
            }
        }
        Rc::get_mut(&mut self.compiled)
            .expect("Unshared constructor data")
            .obstacle_indexes_by_layer = indexes;
    }

    fn collect_dynamic_geometry<'a: 'n, 'n>(
        &'n self,
        traces: impl Iterator<Item = &'a SimplifiedTrace>,
        net_ids: &mut NetIds<'n>,
    ) -> (Vec<TraceSegment<'a>>, Vec<Via<'a>>) {
        ...
        for trace in traces {
            ...
            for pair in route.windows(2) {
                ...
                if (a.x - b.x).abs() <= POSITION_EPSILON && (a.y - b.y).abs() <= POSITION_EPSILON {
                    continue;
                }
                segments.push(TraceSegment {
                    ...
                    width: start_width.unwrap_or(end_width.unwrap_or(0.1)),
                    layer: start_layer.as_str(),
                    pcb_port_ids: pcb_port_ids.clone(),
                });
            }
            for point in route {
                let SimplifiedRoutePoint::Via {
                    x,
                    y,
                    from_layer,
                    to_layer,
                ...
                if !via_locations.insert(key) {
                    continue;
                }
                vias.push(Via {
                    order: vias.len(),
                    via_id: format!("via_{}", vias.len()),
                    trace_id: trace_id.as_str(),
                    net_id,
                    x: *x,
                    y: *y,
                    diameter: via_diameter
                        .unwrap_or(optional_num(&self.compiled.srj["minViaDiameter"], 0.3)),
                    layers: get_via_layers_from_span(
                        from_layer,
                        to_layer,
                        self.compiled.srj["layerCount"]
                            .as_u64()
                            .expect("Expected layerCount") as usize,
                    ),
                });
            }
        }
    ...
    fn build_dynamic_indexes(
        &self,
        segments: &[TraceSegment],
        vias: &[Via],
    ) -> IndexMap<String, SpatialHash<DynamicCollidable>, FxBuildHasher> {
        let mut indexes: IndexMap<String, SpatialHash<DynamicCollidable>, FxBuildHasher> =
            IndexMap::with_hasher(FxBuildHasher);
        for (i, s) in segments.iter().enumerate() {
            indexes
                .entry(s.layer.to_owned())
                .or_insert_with(|| SpatialHash::new(self.compiled.cell_size))
                .insert(
                    DynamicCollidable::Segment(i),
                    expand_bounds(get_segment_bounds(s), self.compiled.trace_clearance),
                );
        }
        for (i, v) in vias.iter().enumerate() {
            for layer in &v.layers {
                indexes
                    ...
                    .or_insert_with(|| SpatialHash::new(self.compiled.cell_size))
                    .insert(
                        DynamicCollidable::Via(i),
                        expand_bounds(get_via_bounds(v), self.compiled.trace_clearance),
                    );
            }
        }
        indexes
    ...
    fn check_trace_pair(&mut self, a: &TraceSegment, b: &TraceSegment) -> Option<Value> {
        if self.are_connected(&a.net_id, &b.net_id) {
            return None;
        }
        self.last_run_stats.exact_check_count += 1;
        let gap = segment_to_segment_min_distance(&a.start, &a.end, &b.start, &b.end)
            - a.width / 2.0
            - b.width / 2.0;
        if gap > self.compiled.trace_clearance - DRC_EPSILON {
            return None;
        }
        let ports: IndexSet<_> = a
            .pcb_port_ids
            .iter()
            .chain(b.pcb_port_ids.iter())
            .cloned()
            .collect();
        Some(json!({
    ...
    fn check_trace_via(&mut self, s: &TraceSegment, v: &Via) -> Option<Value> {
        if self.are_connected(&s.net_id, &v.net_id) {
            return None;
        }
        self.last_run_stats.exact_check_count += 1;
        let gap = segment_to_circle_min_distance(
            &s.start,
            &s.end,
            &Circle {
                x: v.x,
    ...
    fn check_trace_obstacle(
        &mut self,
        s: &TraceSegment,
        obstacle_index: usize,
        connected_to: &[ResolvedNet],
    ) -> Option<Value> {
        let o = &self.compiled.obstacles[obstacle_index];
        if self.obstacle_shares_net(&s.net_id, connected_to) {
            return None;
        }
        self.last_run_stats.exact_check_count += 1;
        ...
        let shape_distance = match o.radius {
            ...
            Some(radius) => segment_to_circle_min_distance(
                &s.start,
                &s.end,
                &Circle {
                    x: o.x,
        ...
        if gap + DRC_EPSILON >= self.compiled.trace_clearance {
            return None;
        }
        let mut ports: IndexSet<String> = s.pcb_port_ids.iter().cloned().collect();
        if let Some(id) = &o.pcb_port_id {
            ports.insert(id.clone());
        }
        let center = if o.radius.is_none() {
            apply_to_point(
                &o.local_to_world,
                &closest_segment_bounds(&local_start, &local_end, bounds),
            )
        } else {
            closest_segment_point(s, Point { x: o.x, y: o.y })
        };
        Some(json!({
            "type": "pcb_trace_error",
            "error_type": "pcb_trace_error",
    ...
    fn check_via_obstacle(
        ...
        let o = &self.compiled.obstacles[obstacle_index];
        if self.obstacle_shares_net(&v.net_id, connected_to) {
            return None;
        }
        self.last_run_stats.exact_check_count += 1;
        let bounds = get_obstacle_local_bounds(o);
        ...
        let distance = match o.radius {
            None => (self.compiled.math.hypot)(
                (bounds.min_x - local_via.x)
                    .max(0.0)
                    .max(local_via.x - bounds.max_x),
                (bounds.min_y - local_via.y)
                    .max(0.0)
                    .max(local_via.y - bounds.max_y),
            ),
            Some(radius) => (self.compiled.math.hypot)(v.x - o.x, v.y - o.y) - radius,
        };
        let gap = distance - v.diameter / 2.0;
        if gap + DRC_EPSILON >= self.compiled.via_to_pad_clearance {
            return None;
        }
        Some(json!({
    ...
    fn check_via_pairs(&mut self, vias: &[Via]) -> Vec<Value> {
        ...
        for (i, via) in vias.iter().enumerate() {
            index.insert(
                i,
                expand_bounds(get_via_bounds(via), self.compiled.via_clearance),
            );
        }
        ...
        for a in vias {
            ...
            for &i in &query.results {
                ...
                if center_distance <= POSITION_EPSILON {
                    continue;
                }
                let gap = center_distance - a.diameter / 2.0 - b.diameter / 2.0;
                if gap + DRC_EPSILON >= self.compiled.via_clearance {
                    continue;
                }
                ...
                if self.compiled.include_trace_via_owner_metadata {
                    error["pcb_trace_ids"] = json!([a.trace_id, b.trace_id]);
                }
                error["pcb_via_pair_net_relation"] = json!(if same_net {
                    "same_net"
                } else {
                    "different_net"
                });
    ...
    pub fn evaluate_trace_errors(&mut self, traces: &[&Value]) -> Vec<Value> {
        let traces: Vec<SimplifiedTrace> = traces
            .iter()
            .map(|trace| SimplifiedTrace::from_value(trace))
            .collect();
    ...
    fn evaluate_internal<'a>(
        &mut self,
        traces: impl ExactSizeIterator<Item = &'a Value>,
    ...
    fn evaluate_typed_internal<'a>(
        &mut self,
        traces: impl ExactSizeIterator<Item = &'a SimplifiedTrace>,
        include_via_pad_errors: bool,
    ) -> Value {
        let errors = self.evaluate_typed_errors_internal(traces, include_via_pad_errors);
        let errors_with_centers: Vec<_> = errors
            .iter()
            .filter(|error| !error["center"].is_null())
            .collect();
        json!({ "errors": errors, "errorsWithCenters": errors_with_centers, "locationAwareErrors": errors_with_centers })
    }

    fn evaluate_typed_errors_internal<'a>(
        &mut self,
        traces: impl ExactSizeIterator<Item = &'a SimplifiedTrace>,
        include_via_pad_errors: bool,
    ) -> Vec<Value> {
        ...
        let obstacle_nets: Vec<Vec<ResolvedNet>> = self
            .compiled
            .obstacles
            .iter()
            .map(|obstacle| {
                obstacle
                    .connected_to
                    .iter()
                    .map(|id| self.compile_net(id, &mut net_ids))
                    .collect()
            })
            .collect();
        drop(net_ids);
        ...
        self.last_run_stats = AutoroutingDrcEngineRunStats {
            trace_count,
            segment_count: segments.len(),
            via_count: vias.len(),
            obstacle_count: self.compiled.obstacles.len(),
        ...
        for s in &segments {
            ...
            if let Some(index) = dynamic_indexes.get(s.layer) {
                index.query(bounds, &mut dynamic_query);
            }
            if let Some(index) = self.compiled.obstacle_indexes_by_layer.get(s.layer) {
                index.query(bounds, &mut obstacle_query);
            }
            for &candidate in &dynamic_query.results {
                self.last_run_stats.broad_phase_candidate_count += 1;
                let error = match candidate {
                    DynamicCollidable::Segment(i) => {
                        if segments[i].order <= s.order {
                            continue;
                        }
                ...
                if let Some(error) = error {
                    detected_trace_errors.push(error);
                }
            }
            for &i in &obstacle_query.results {
                self.last_run_stats.broad_phase_candidate_count += 1;
                if let Some(error) = self.check_trace_obstacle(s, i, &obstacle_nets[i]) {
                    detected_trace_errors.push(error);
                }
            }
        }
        ...
        if include_via_pad_errors {
            for via in &vias {
                ...
                for layer in &via.layers {
                    if let Some(index) = self.compiled.obstacle_indexes_by_layer.get(layer) {
                        index.query(get_via_bounds(via), &mut obstacle_query);
                    }
                }
                for &i in &obstacle_query.results {
                    self.last_run_stats.broad_phase_candidate_count += 1;
                    if let Some(error) = self.check_via_obstacle(via, i, &obstacle_nets[i]) {
                        detected_via_pad_errors.push(error);
                    }
                }
            }
        }
        let mut first_trace_error_by_id: IndexMap<String, Value, FxBuildHasher> =
            IndexMap::with_hasher(FxBuildHasher);
        ...
        let mut errors: Vec<Value> = first_trace_error_by_id
            .into_values()
            .map(|mut error| {
                if error["worst_contact_center"].is_object() {
                    error["center"] = error["worst_contact_center"].clone();
                }
                if error["worst_contact_message"].is_string() {
                    error["message"] = error["worst_contact_message"].clone();
                }
                if error["worst_actual_clearance"].is_number() {
                    error["actual_clearance"] = error["worst_actual_clearance"].clone();
                }
                error
            })
            .collect();
...
mod compiled_fork_tests {
    ...
    fn compiled_fork_shares_only_immutable_constructor_data() {
        ...
        let original =
            AutoroutingDrcEngine::new(srj, Some(json!({"idToNetMap":{"a":"net"}})), json!({}))
                .unwrap();
        ...
        fork.conn_map
            ...
            .unwrap()
            .id_to_net_map
            .insert("a".into(), "other".into());
        assert_eq!(
            original.conn_map.as_ref().unwrap().id_to_net_map["a"],
            "net"
        );
        ...
        for mapping in [
            ...
            for left in names {
                for right in names {
                    ...
                    let expected = left == right
                        || match (fork.conn_map_net(left), fork.conn_map_net(right)) {
            ...
            assert!(ids.ids.keys().all(|key| {
                names.iter().any(|name| name.as_ptr() == key.as_ptr())
                    || fork
                        .conn_map
                        .as_ref()
                        .unwrap()
                        .id_to_net_map
                        .values()
                        .any(|name| name.as_ptr() == key.as_ptr())
            }));
        }
    }
}
