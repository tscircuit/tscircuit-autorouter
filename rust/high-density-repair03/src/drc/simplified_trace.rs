...
impl SimplifiedTrace {
    pub fn from_value(trace: &Value) -> Self {
        let route = trace["route"].as_array().expect("Expected trace route array");
        let route = route.iter().map(|point| {
            match point["route_type"].as_str() {
                Some("wire") => SimplifiedRoutePoint::Wire {
                    ...
                    layer: point["layer"].as_str().expect("Expected wire layer").to_owned(),
                ...
                Some("via") => SimplifiedRoutePoint::Via {
                    ...
                    from_layer: point["from_layer"].as_str().expect("Expected via from_layer").to_owned(),
                    to_layer: point["to_layer"].as_str().expect("Expected via to_layer").to_owned(),
        ...
        Self {
            pcb_trace_id: trace["pcb_trace_id"].as_str().expect("Expected pcb_trace_id").to_owned(),
            connection_name: trace["connection_name"].as_str().expect("Expected connection_name").to_owned(),
            route,
        }
    }
}
...
fn optional_number(value: &Value) -> Option<f64> {
    if value.is_null() { None } else { Some(value.as_f64().expect("Expected numeric trace field")) }
}
