use crate::drc::simplified_trace::SimplifiedRoutePoint;
use crate::solvers::global_drc_force_improve_solver::internal_types::{MutableRoute, RoutePoint};
...
fn layer_name(z: f64, layer_count: usize) -> String {
    ...
    if z == layer_count as f64 - 1.0 {
        return "bottom".to_owned();
    }
    format!("inner{}", json_bindings::js_json::js_json(&Value::from(z)))
}

fn endpoint_port(
    point: &RoutePoint,
    connection_points: &[Value],
    layer_count: usize,
) -> Option<String> {
    if let Some(id) = point.metadata["pcb_port_id"]
        .as_str()
    ...
    connection_points
        ...
        .find(|candidate| {
            candidate["pcb_port_id"]
                ...
                && if candidate.get("layer").is_some() {
                    candidate["layer"].as_str() == Some(&layer)
                } else {
                    candidate["layers"]
                        .as_array()
                        .expect("Connection point layers")
                        .iter()
                        .any(|value| value.as_str() == Some(&layer))
                }
        })
...
pub fn convert_hd_route_to_simplified_route(
    ...
    let mut result = vec![SimplifiedRoutePoint::Wire {
        ...
        y: first.y,
        width: Some(width),
        layer: layer_name(first.z, layer_count),
        start_pcb_port_id: start_port,
        end_pcb_port_id: if route.route.len() == 1 {
            end_port.clone()
        } else {
            None
        },
    ...
    for index in 1..route.route.len() {
        ...
        if previous.z != current.z && previous.x == current.x && previous.y == current.y {
            result.push(SimplifiedRoutePoint::Via {
                x: current.x,
                y: current.y,
                from_layer: layer_name(previous.z, layer_count),
                to_layer: layer_name(current.z, layer_count),
                via_diameter: via_diameter.filter(|value| *value != 0.0 && !value.is_nan()),
                via_hole_diameter: None,
            });
        }
        result.push(SimplifiedRoutePoint::Wire {
            ...
            y: current.y,
            width: Some(width),
            layer: layer_name(current.z, layer_count),
            start_pcb_port_id: None,
            end_pcb_port_id: if index == route.route.len() - 1 {
                end_port.clone()
            } else {
                None
            },
