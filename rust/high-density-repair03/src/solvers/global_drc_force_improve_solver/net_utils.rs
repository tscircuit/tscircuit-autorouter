...
pub fn resolve_net_id<'a>(
    id: &'a str,
    conn_map: Option<&'a RepairConnectivityMap>,
) -> ResolvedNetId<'a> {
    ResolvedNetId {
        id,
...
pub fn shares_resolved_net(left: ResolvedNetId<'_>, right: ResolvedNetId<'_>) -> bool {
    ...
    if let Some(left_net_id) = left.net_id.filter(|id| !id.is_empty())
        && (Some(left_net_id) == right.net_id || (!left.id.is_empty() && left_net_id == right.id))
    {
        return true;
    }
    if let Some(right_net_id) = right.net_id.filter(|id| !id.is_empty())
        && right_net_id == left.id
    {
        return true;
    }
    false
...
pub fn get_root_connection_name(route: &MutableRoute) -> &str {
    route
        ...
        .as_deref()
        .unwrap_or(&route.connection_name)
}

pub fn get_conn_map_net_id<'a>(
    conn_map: Option<&'a RepairConnectivityMap>,
    id: Option<&str>,
) -> Option<&'a str> {
...
pub fn get_conn_map_aware_srj(srj: &Value, conn_map: Option<&RepairConnectivityMap>) -> Value {
    ...
    let connections = srj["connections"]
        .as_array()
        .expect("SRJ connections are required")
        .iter()
        .map(|connection| {
            let net_connection_name =
                get_conn_map_net_id(Some(conn_map), connection["name"].as_str())
                    .or_else(|| {
                        get_conn_map_net_id(
                            Some(conn_map),
                            connection["rootConnectionName"].as_str(),
                        )
                    })
    ...
    let obstacles = srj["obstacles"]
        ...
        .iter()
        .map(|obstacle| {
            let original_connected_to = obstacle["connectedTo"]
                .as_array()
                .expect("Obstacle connectedTo is required");
            let mut connected_to = Vec::new();
            for id in original_connected_to {
                if !connected_to.contains(id) {
                    connected_to.push(id.clone());
                }
            }
            for connected_id in original_connected_to {
                if let Some(net_id) = get_conn_map_net_id(Some(conn_map), connected_id.as_str())
                    .filter(|net_id| !net_id.is_empty())
                {
                    let net_id = Value::from(net_id);
                    if !connected_to.contains(&net_id) {
                        connected_to.push(net_id);
                    }
                }
            }
...
pub fn shares_net(
    ...
    let Some(right) = right.filter(|right| !right.is_empty()) else {
        return false;
    };
    if left == right {
        return true;
    ...
    if let Some(left_net_id) = left_net_id.filter(|id| !id.is_empty())
        && (Some(left_net_id) == right_net_id || (!left.is_empty() && left_net_id == right))
    {
        return true;
    }
...
pub fn obstacle_shares_net(
    root_connection_name: &str,
    obstacle: &Value,
    conn_map: Option<&RepairConnectivityMap>,
) -> bool {
    let Some(connected_to) = obstacle.get("connectedTo").filter(|value| !value.is_null()) else {
        return false;
    };
    connected_to
        .as_array()
...
impl ResolvedNetInterner {
    ...
    pub(crate) fn resolve(
        ...
        let id = match ids.get(resolved.id) {
            ...
            None => {
                let id = ids.len();
                ids.insert(resolved.id.to_owned(), id);
                id
            }
        };
...
pub(crate) fn shares_interned_resolved_net(
    left: InternedResolvedNetId,
    right: InternedResolvedNetId,
) -> bool {
    if right.id == 0 {
        return false;
    }
    ...
    if let Some(left_net_id) = left.net_id.filter(|id| *id != 0)
        && (Some(left_net_id) == right.net_id || (left.id != 0 && left_net_id == right.id))
    {
        return true;
    }
...
mod interned_net_tests {
    ...
    fn interned_membership_preserves_empty_alias_and_update_semantics() {
        ...
        for entries in [
            ...
            let map: RepairConnectivityMap =
                serde_json::from_value(serde_json::json!({"idToNetMap":entries})).unwrap();
            for conn in [None, Some(&map)] {
                for left in ids {
                    for right in ids {
                        assert_eq!(
                            shares_interned_resolved_net(
                                interner.resolve(left, conn),
                                interner.resolve(right, conn)
                            ),
