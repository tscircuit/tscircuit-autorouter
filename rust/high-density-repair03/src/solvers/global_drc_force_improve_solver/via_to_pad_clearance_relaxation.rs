use crate::solvers::global_drc_force_improve_solver::internal_types::{
    MutableRoute, Point, ViaNode,
};
use crate::solvers::global_drc_force_improve_solver::net_utils::{
    RepairConnectivityMap, obstacle_shares_net,
};
use crate::solvers::global_drc_force_improve_solver::solver_helpers::{
    RepairMath, RepairObstacle, collect_via_nodes, get_obstacle_z_layers, get_rect_repulsion,
    materialize_routes,
};
...
fn get_via_pad_blockers<'a>(
    ...
    for obstacle in srj["obstacles"].as_array().expect("Obstacles required") {
        let same_net = obstacle_shares_net(&via.root_connection_name, obstacle, conn)
            || obstacle_shares_net(&route.connection_name, obstacle, conn);
        if obstacle["isCopperPour"].as_bool() == Some(true)
            || (same_net && point_obstacle_distance(point, obstacle, math) <= CLEARANCE_EPSILON)
        {
            continue;
        }
        let layers = get_obstacle_z_layers(
            obstacle,
            srj["layerCount"].as_u64().expect("Layer count required") as usize,
        );
        if !via.z_layers.iter().any(|z| layers.contains(z)) {
            continue;
        }
...
fn get_via_clearance_penalty(
    ...
    for obstacle in get_via_pad_blockers(srj, routes, via, conn, math) {
        let clearance = point_obstacle_distance(point, obstacle, math)
            - (via.radius
                + srj["minViaEdgeToPadEdgeClearance"].as_f64().unwrap()
        ...
        if point_obstacle_distance(point, obstacle, math) < CLEARANCE_EPSILON {
            let center = Point {
                x: obstacle["center"]["x"].as_f64().unwrap(),
                y: obstacle["center"]["y"].as_f64().unwrap(),
            };
...
fn get_route_via_clearance_penalty(
    srj: &Value,
    routes: &[MutableRoute],
    index: usize,
    conn: Option<&RepairConnectivityMap>,
    ...
    collect_via_nodes(routes, srj["minViaDiameter"].as_f64().unwrap_or(0.3))
        .iter()
        .filter(|via| via.route_index == index)
        .fold(0.0, |penalty, via| {
            penalty + get_via_clearance_penalty(srj, routes, via, conn, math)
        })
}

fn compute_via_nudge_forces(
    srj: &Value,
    routes: &[MutableRoute],
    index: usize,
    conn: Option<&RepairConnectivityMap>,
    ...
    for via in vias.iter().filter(|via| via.route_index == index) {
        ...
        for obstacle in get_via_pad_blockers(srj, routes, via, conn, math) {
            let required = via.radius
                + srj["minViaEdgeToPadEdgeClearance"].as_f64().unwrap()
                + RELAXATION_CLEARANCE_SLACK;
            let Some((direction, penetration)) =
                rect_repulsion(Point { x: via.x, y: via.y }, obstacle, required, math)
            else {
                continue;
            };
...
fn apply_nudge_forces(
    ...
    let indices: HashSet<_> = collect_via_nodes(
        std::slice::from_ref(route),
        srj["minViaDiameter"].as_f64().unwrap_or(0.3),
    )
    .into_iter()
    .filter(|via| via.movable)
    .flat_map(|via| via.point_indexes)
    .collect();
...
fn route_stays_inside_bounds(srj: &Value, route: &MutableRoute) -> bool {
    ...
    route.route.iter().all(|point| {
        ...
        point.x >= bounds["minX"].as_f64().unwrap() - CLEARANCE_EPSILON
            && point.x <= bounds["maxX"].as_f64().unwrap() + CLEARANCE_EPSILON
            && point.y >= bounds["minY"].as_f64().unwrap() - CLEARANCE_EPSILON
            && point.y <= bounds["maxY"].as_f64().unwrap() + CLEARANCE_EPSILON
    })
}
...
pub fn apply_via_to_pad_clearance_relaxation(
    srj: &Value,
    routes: &Routes,
    conn: Option<&RepairConnectivityMap>,
    math: RepairMath,
    ...
    if srj
        .get("minViaEdgeToPadEdgeClearance")
    ...
    for _ in 0..RELAXATION_PASSES {
        for index in 0..relaxed.len() {
            changed = nudge_route_vias(srj, &mut relaxed, index, conn, math) || changed;
        }
    }
    if changed {
        materialize_routes(&mut relaxed);
        Routes::new(relaxed)
    } else {
        routes.clone()
    }
...
fn rect_repulsion(
    ...
    let obstacle = RepairObstacle {
        ...
        width: obstacle["width"].as_f64().unwrap(),
        height: obstacle["height"].as_f64().unwrap(),
        is_copper_pour: false,
        z_layers: Vec::new(),
        connected_to: Vec::new(),
        connected_to_net_ids: Vec::new(),
        interned_connected_to: Vec::new(),
    };
    get_rect_repulsion(point, &obstacle, required, math)
        .map(|repulsion| (repulsion.direction, repulsion.penetration))
}
