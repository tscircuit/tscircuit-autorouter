...
use crate::solvers::global_drc_force_improve_solver::drc_snapshot::{is_via_pad_drc_error, is_trace_obstacle_drc_error};
...
pub(crate) fn materialize_native(mut routes: Vec<MutableRoute>, indexes: Option<&[usize]>) -> Routes {
    for (index, route) in routes.iter_mut().enumerate() {
        if indexes.is_none_or(|indexes| indexes.contains(&index)) { route.identity = crate::solvers::global_drc_force_improve_solver::internal_types::next_identity(); route.via_array_identity = crate::solvers::global_drc_force_improve_solver::internal_types::next_identity(); route.vias = derive_vias(route); }
    }
    Routes::new(routes)
}
