...
use crate::solvers::global_drc_force_improve_solver::internal_types::{
    Bounds2D as Bounds, MutableRoute, Point,
};
use crate::solvers::global_drc_force_improve_solver::net_utils::{
    InternedResolvedNetId, RepairConnectivityMap, ResolvedNetInterner, get_root_connection_name,
    shares_interned_resolved_net,
};
use crate::solvers::global_drc_force_improve_solver::solver_config::{
    POSITION_EPSILON, get_via_edge_to_pad_edge_clearance,
};
use crate::solvers::global_drc_force_improve_solver::solver_helpers::RepairMath;
use crate::transformation_matrix::{
    Matrix, apply_to_point, apply_to_points, compose, inverse, rotate_deg_with_math, translate,
};
use math_utils::{
    get_bounds_from_points, get_segment_intersection, point_to_segment_closest_point,
};
...
impl Boundary {
    pub fn bounds(&self) -> &Bounds {
        match self {
            Self::Line { bounds, .. } | Self::Circle { bounds, .. } => bounds,
        }
    }
}
...
fn project_to_bounds(point: &Point, bounds: &Bounds) -> Point {
    Point {
        x: clamp(point.x, bounds.min_x, bounds.max_x),
        y: clamp(point.y, bounds.min_y, bounds.max_y),
    }
}
...
fn distance_to_pad(point: &Point, pad: &PadRegion, math: &RepairMath) -> f64 {
    ...
    if pad.circular {
        return js_max(0.0, (math.hypot)(dx, dy) - pad.half_width);
    }
    point_to_bounds_distance(
        &apply_to_point(&pad.world_to_local, point),
        &Bounds {
            min_x: -pad.half_width,
            max_x: pad.half_width,
            min_y: -pad.half_height,
            max_y: pad.half_height,
        },
    )
}
...
pub fn create_line(start: Point, end: Point) -> Boundary {
    Boundary::Line {
        ...
        bounds: get_bounds_from_points(&[start, end]).expect("Two points have bounds"),
    }
}
...
pub fn create_circle(center: Point, radius: f64) -> Boundary {
    Boundary::Circle {
        center,
        radius,
        bounds: get_bound_from_centered_rect(&center, radius * 2.0, radius * 2.0),
    }
}
...
fn get_pad_boundaries(pad: &PadRegion) -> Vec<Boundary> {
    ...
    if pad.circular {
        return vec![create_circle(pad.center, pad.half_width + margin)];
    }
...
pub fn boundary_projections(point: &Point, boundary: &Boundary, math: &RepairMath) -> Vec<Point> {
    match boundary {
        Boundary::Circle { center, radius, .. } => {
            ...
            vec![Point {
                x: center.x + (dx * radius) / length,
                y: center.y + (dy * radius) / length,
            }]
        }
        Boundary::Line { start, end, .. } => vec![
            *start,
            *end,
            point_to_segment_closest_point(point, start, end),
        ],
    }
}
...
pub fn boundary_intersections(left: &Boundary, right: &Boundary, math: &RepairMath) -> Vec<Point> {
    match (left, right) {
        ...
        | (Boundary::Circle { center, radius, .. }, Boundary::Line { start, end, .. }) => {
        ...
        (
            ...
            if d == 0.0 || d > left_radius + right_radius || d < (left_radius - right_radius).abs()
            {
                return vec![];
            }
            let along =
                (left_radius * left_radius - right_radius * right_radius + d * d) / (2.0 * d);
            let perpendicular = js_max(0.0, left_radius * left_radius - along * along).sqrt();
            let x = left.x + (dx * along) / d;
            let y = left.y + (dy * along) / d;
            vec![
                Point {
                    x: x - (dy * perpendicular) / d,
                    y: y + (dx * perpendicular) / d,
                },
                Point {
                    x: x + (dy * perpendicular) / d,
                    y: y - (dx * perpendicular) / d,
...
impl PadClearanceContext {
    pub fn new(srj: &Value, math: &RepairMath, conn_map: Option<&RepairConnectivityMap>) -> Self {
        ...
        let obstacles = srj["obstacles"]
            .as_array()
            .expect("SRJ obstacles are required")
            .iter()
            .filter(|obstacle| !obstacle["isCopperPour"].as_bool().unwrap_or(false))
            .map(|obstacle| {
                let layers: Vec<String> = obstacle["layers"]
                    .as_array()
                    .expect("Obstacle layers are required")
                    .iter()
                    .map(|layer| {
                        layer
                            .as_str()
                            .expect("Obstacle layer must be a string")
                            .to_owned()
                    })
                    .collect();
                let connected_to: Vec<String> = obstacle
                    ...
                    .unwrap_or_default();
                let rotation = obstacle["ccwRotationDegrees"]
                    .as_f64()
                    .filter(|angle| angle.is_finite());
                let width = obstacle["width"]
                    .as_f64()
                    .expect("Obstacle width is required");
                let height = obstacle["height"]
                    .as_f64()
                    .expect("Obstacle height is required");
                let circular =
                    rotation.is_none() && layers.len() > 1 && (width - height).abs() < 0.001;
                let center = Point {
                    x: obstacle["center"]["x"]
                        .as_f64()
                        .expect("Obstacle center x is required"),
                    y: obstacle["center"]["y"]
                        .as_f64()
                        .expect("Obstacle center y is required"),
                };
                let local_to_world = compose(&[
                    translate(center.x, center.y),
                    rotate_deg_with_math(rotation.unwrap_or(0.0), math.sin, math.cos),
                ]);
                let half_width = if circular {
                    js_max(width, height) / 2.0
                } else {
                    width / 2.0
                };
                let half_height = if circular { half_width } else { height / 2.0 };
                let bounds = get_bounds_from_points(&apply_to_points(
                    &local_to_world,
                    &[
                        Point {
                            x: -half_width,
                ...
                let connected_net_ids = connected_to
                    .iter()
        ...
        Self {
            ...
            board,
            obstacles,
            layer_count: srj["layerCount"]
                .as_f64()
                .expect("SRJ layerCount is required"),
    ...
    pub fn set_connectivity(&mut self, conn_map: Option<&RepairConnectivityMap>) {
        for obstacle in &mut self.obstacles {
            obstacle.connected_net_ids = obstacle
                .connected_to
                .iter()
    ...
    fn get_pad_regions(
        ...
        let layers: Vec<String> = range(min_z, Some(max_z + 1.0), 1.0)
            .iter()
            .map(|&z| map_z_to_layer_name(z, self.layer_count))
            .collect();
        let root = self
            .resolved_nets
        ...
        self.obstacles
            .iter()
            .filter(|obstacle| obstacle.layers.iter().any(|layer| layers.contains(layer)))
            .map(|obstacle| {
                let same_net = obstacle
                    .connected_net_ids
                    .iter()
                    ...
                    || obstacle
                        .connected_net_ids
                        .iter()
                        .any(|&id| shares_interned_resolved_net(connection, id));
                let clearance = via_radius
                    + if same_net {
                        0.0
                    } else {
                        self.via_edge_to_pad_edge_clearance
                    };
                ...
                PadRegion {
                    ...
                    local_to_world: obstacle.local_to_world,
                    world_to_local: obstacle.world_to_local,
                    circular: obstacle.circular,
                    clearance,
                    bounds: Bounds {
                        min_x: obstacle.bounds.min_x - margin,
                        max_x: obstacle.bounds.max_x + margin,
                        min_y: obstacle.bounds.min_y - margin,
                        max_y: obstacle.bounds.max_y + margin,
                    },
                }
            })
            .collect()
...
pub fn find_pad_clearance_via_position(
    srj: &Value,
    route: &MutableRoute,
    preferred: Point,
    via_radius: f64,
    ...
    find_pad_clearance_via_position_with_identity(
        srj, route, preferred, via_radius, z_layers, conn_map, math,
    )
    .point
}

pub fn find_pad_clearance_via_position_with_identity(
    srj: &Value,
    route: &MutableRoute,
    preferred: Point,
    via_radius: f64,
    ...
    PadClearanceContext::new(srj, math, conn_map)
        .find_with_identity(route, preferred, via_radius, z_layers, conn_map, math)
}
...
impl PadClearanceContext {
    ...
    pub fn find_with_identity(
        ...
        if board.min_x > board.max_x || board.min_y > board.max_y {
            return PadPlacement {
                point: None,
                is_preferred: false,
            };
        ...
        let is_feasible = |point: &Point| {
            is_point_inside_bounds(point, &board)
                && pads
                    .iter()
                    .all(|pad| distance_to_pad(point, pad, math) >= pad.clearance)
        };
        ...
        if is_feasible(&projected) {
            ...
            return PadPlacement {
                point: Some(if is_preferred { preferred } else { projected }),
                is_preferred,
            };
        }

        let mut component: Vec<usize> = pads
            .iter()
            .enumerate()
            .filter(|(_, pad)| distance_to_pad(&projected, pad, math) < pad.clearance)
            .map(|(i, _)| i)
            .collect();
        ...
        let mut boundaries: Vec<Boundary> = component
            .iter()
            .flat_map(|&i| get_pad_boundaries(&pads[i]))
            .collect();
        ...
        boundaries.extend([
            create_line(bottom_left, bottom_right),
            create_line(bottom_right, top_right),
            create_line(top_right, top_left),
            create_line(top_left, bottom_left),
        ]);
        let mut best = None;
        ...
        for boundary in &boundaries {
            for point in boundary_projections(&preferred, boundary, math) {
                consider(point, &mut best, &mut best_distance);
            }
        }
        for left in 0..boundaries.len() {
            let a = &boundaries[left];
            if dist_sq(&preferred, &project_to_bounds(&preferred, a.bounds())) > best_distance {
                continue;
            }
            for b in &boundaries[left + 1..] {
                ...
                if dist_sq(&preferred, &project_to_bounds(&preferred, b.bounds())) > best_distance {
                    continue;
                }
