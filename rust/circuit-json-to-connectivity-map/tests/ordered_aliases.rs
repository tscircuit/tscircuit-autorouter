...
fn merging_preserves_order_stale_aliases_and_self_spread_snapshot(){
    ...
    let ids:Vec<_>=["a","b","c","d"].iter().map(|name|map.intern(name.encode_utf16().collect())).collect();
    assert!(map.add_connections(&[vec![ids[0],ids[1]],vec![ids[2],ids[3]],vec![ids[0],ids[2]]]));
    ...
    assert_eq!(map.arrays[map.net_map[&net0]],ids.iter().chain(ids.iter()).copied().collect::<Vec<_>>());
}
