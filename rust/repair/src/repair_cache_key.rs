...
fn write_string(value: &str, output: &mut Vec<u8>) {
    ...
    let token = match value {
        "x" => Some(0), "y" => Some(1), "z" => Some(2),
        "traceThickness" => Some(3), "pcb_port_id" => Some(4),
        "connectionName" => Some(5), "rootConnectionName" => Some(6),
        "viaDiameter" => Some(7), "route" => Some(8), "vias" => Some(9),
        "jumpers" => Some(10), "portPointId" => Some(11),
        _ => None,
    };
...
pub(crate) fn write_mutable_routes_key(routes: &[MutableRoute], output: &mut Vec<u8>) {
    ...
    for route in routes {
        ...
        for key in ["route", "vias"] {
            if metadata.is_some_and(|object| object.contains_key(key)) { continue; }
            write_string(key, output);
            if key == "route" { write_points(route, output); }
            else { write_values(&route.vias, output); }
        }
        output.push(b'}');
    }
    output.push(b']');
...
mod tests {
    ...
    use serde_json::json;
    use crate::filter_pipeline9_drc_errors_against_baseline::js_json_slice;
    use crate::mutable_route_json::js_json_routes;

    #[test]
    fn binary_keys_preserve_json_equality_and_typed_value_cache_order() {
        let mut corpus = vec![vec![], vec![Value::Null], vec![json!(true)], vec![json!(false)],
            vec![json!("")], vec![json!("n")], vec![json!("[{}]ds\u{0000}")],
            vec![json!("é漢字😀\u{2028}\u{2029}")], vec![json!([])], vec![json!({})],
            vec![json!([null])], vec![json!({"a": 1, "b": 2})], vec![json!({"b": 2, "a": 1})],
            vec![json!(9007199254740992_u64)], vec![json!(9007199254740993_u64)]];
        for token in ["x", "y", "z", "traceThickness", "pcb_port_id", "connectionName",
            "rootConnectionName", "viaDiameter", "route", "vias", "jumpers", "portPointId"] {
        ...
        if let Ok(path) = std::env::var("REPAIR_CACHE_KEY_FIXTURE") {
            ...
            eprintln!("Captured routes: {}, binary key bytes: {}, JSON bytes: {}", routes.len(), key.len(), js_json_slice(values).len());
        }
        let mut typed_keys = Vec::new();
        for number in [0.0, -0.0, 1e-7, 1e-6, 1e20, 1e21, f64::MIN_POSITIVE,
            f64::from_bits(1), f64::MAX, -f64::MAX, f64::NAN, f64::INFINITY, f64::NEG_INFINITY] {
        ...
        let encoded: Vec<_> = corpus.iter().map(|values| (route_values_key(values), js_json_slice(values))).collect();
        ...
        let mut sequence: Vec<_> = encoded.iter().map(|(key, json)| (key.clone(), json.clone())).collect();
        sequence.extend(typed_keys.iter().map(|(key, values)| (key.clone(), js_json_slice(values))));
        ...
        for (key, json) in sequence {
            ...
            if !json_cache.contains_key(&json) {
                if json_cache.len() >= 64 { json_cache.shift_remove_index(0); binary_cache.shift_remove_index(0); }
                evaluation_count += 1;
                json_cache.insert(json, evaluation_count);
                binary_cache.insert(key, evaluation_count);
            }
            assert_eq!(binary_cache.len(), json_cache.len());
            assert_eq!(binary_cache.values().collect::<Vec<_>>(), json_cache.values().collect::<Vec<_>>());
        }
    }
}
