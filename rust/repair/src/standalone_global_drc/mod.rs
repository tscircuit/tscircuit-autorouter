...
use autorouting_drc::{autorouting_drc_engine::{AutoroutingDrcEngine, DrcMath}, simplified_trace::SimplifiedTrace};
use indexmap::IndexMap;
use serde_json::{Value, json};
use crate::{types::{DrcEvaluator, DrcSnapshot, Routes}, drc_snapshot::{get_drc_error_severity, get_topology_repair_drc_error_severity, is_via_pad_drc_error}};
use convert_hd_route_to_simplified_route::convert_hd_route_to_simplified_route;
...
impl StandaloneDrcEvaluator {
    ...
    pub fn traces(&self, routes: &Routes) -> (Vec<SimplifiedTrace>, IndexMap<String, usize>) {
        let mut grouped: IndexMap<&str, Vec<(usize, &crate::internal_types::MutableRoute)>> = IndexMap::new();
