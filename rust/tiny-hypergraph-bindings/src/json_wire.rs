...
pub fn encode_loaded_numbers(loaded: &mut tiny_hypergraph::compat::load_serialized_hyper_graph::LoadedHyperGraph) -> Vec<SpecialNumber> {
        ...
        for (index, number) in values.iter_mut().enumerate() {
            if !number.is_finite() {
                numbers.push(SpecialNumber {
                    ...
                    value: if number.is_nan() { "NaN" } else if number.is_sign_positive() { "Infinity" } else { "-Infinity" }.into(),
                });
                *number = 0.0;
            }
        }
