...
assert.throws(() => new TinyHyperGraphSolver(topology, problem), /must be initialized before constructing a solver/)
assert.throws(() => loadSerializedHyperGraph({ regions: [], ports: [] }), /must be initialized before constructing a solver/)
await assert.rejects(loadTinyHypergraphBindings(new Uint8Array([0])))
assert.throws(() => new TinyHyperGraphSolver(topology, problem), /must be initialized before constructing a solver/)

const bytes = await readFile(new URL(import.meta.resolve("@tscircuit/tiny-hypergraph-bindings/wasm")))
let resolveSource
const source = new Promise((resolve) => { resolveSource = resolve })
const firstInit = loadTinyHypergraphBindings(source)
assert.strictEqual(loadTinyHypergraphBindings(bytes), firstInit)
assert.throws(() => new TinyHyperGraphSolver(topology, problem), /must be initialized before constructing a solver/)
resolveSource(bytes)
await firstInit
assert.strictEqual(loadTinyHypergraphBindings(), firstInit)
