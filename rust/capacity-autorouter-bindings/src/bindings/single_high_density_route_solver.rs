...
impl SingleHighDensityRouteSolver {
    ...
    pub fn call(
        &mut self,
        method: &str,
        args: Ts<HighDensityValue>,
    ) -> Result<Ts<HighDensityValue>, JsValue> {
        ...
        let query = a
            .get("query")
            .filter(|q| !q.is_null())
            .map(|q| PlanarObstacleQuery {
                layer: q["layer"].as_i64().expect("Query layer required"),
                segment_ids: q["segmentIds"]
                    .as_array()
                    .expect("Query ids required")
                    .iter()
        ...
        let result = match method {
            "step" => {
                e.iterations = a["iterations"].as_u64().unwrap() as usize;
                e.step_inner();
                Value::Null
            }
            "handleSimpleCases" => {
                e.handle_simple_cases();
                Value::Null
            }
            "buildObstacleIndexes" => {
                e.obstacle_routes = serde_json::from_value(a["routes"].clone())
                    .map_err(|error| JsValue::from_str(&error.to_string()))?;
                e.build_obstacle_indexes();
                Value::Null
            }
            "viaPenaltyDistance" => json!(e.via_penalty_distance()),
            "isNodeTooCloseToObstacle" => json!(e.is_node_too_close_to_obstacle(
                &node(&a["node"]),
                a["margin"].as_f64(),
                a["isVia"].as_bool().unwrap_or(false),
                query.as_ref()
            )),
            "isNodeTooCloseToEdge" => json!(e.is_node_too_close_to_edge(
                &node(&a["node"]),
                a["isVia"].as_bool().unwrap_or(false)
            )),
            "doesPathToParentIntersectObstacle" => {
                json!(e.does_path_to_parent_intersect_obstacle(&node(&a["node"]), query.as_ref()))
            }
            "queryBounds" => {
                let n = node(&a["node"]);
            ...
            "search" => {
                let index = match a["kind"].as_str().unwrap() {
                    "segments" => e.obstacle_segment_index.as_ref(),
                    "vias" => e.obstacle_via_index.as_ref(),
                    "layer" => e
                        .obstacle_segment_index_by_layer
                        .get(&a["layer"].as_i64().unwrap()),
                    _ => return Err(JsValue::from_str("Unknown index kind")),
                };
                let bounds = a["bounds"].as_array().unwrap();
                json!(index.expect("Obstacle index required").search(
                    bounds[0].as_f64().unwrap(),
                    bounds[1].as_f64().unwrap(),
                    bounds[2].as_f64().unwrap(),
                    bounds[3].as_f64().unwrap()
                ))
            }
            "computeG" => json!(e.compute_g(&node(&a["node"]))),
            "computeH" => json!(e.compute_h(&node(&a["node"]))),
            "computeF" => json!(e.compute_f(
                a["g"].as_f64().unwrap_or(f64::NAN),
                a["h"].as_f64().unwrap_or(f64::NAN)
            )),
            "setNodeCosts" => {
                let mut n = (*node(&a["node"])).clone();
                e.set_node_costs(&mut n);
                json!({"g":n.g,"h":n.h,"f":n.f})
            }
            "getNodeKey" => json!(e.get_node_key(&node(&a["node"]))),
            "getNeighbors" => json!(
                e.get_neighbors(node(&a["node"]))
                    .iter()
                    .map(|n| node_value(n))
                    .collect::<Vec<_>>()
            ),
            "getNodePath" => json!(
                e.get_node_path(node(&a["node"]))
                    .iter()
                    .map(|n| node_value(n))
                    .collect::<Vec<_>>()
            ),
            ...
            "setSolvedPath" => {
                e.set_solved_path(node(&a["node"]));
                Value::Null
            }
            "computeProgress" => {
                json!(e.compute_progress(a["goalDist"].as_f64(), a["isOnLayer"].as_bool()))
            }
            "closestFuturePointIndex" => {
                let closest = e.get_closest_future_connection_point(&node(&a["node"]));
                json!(closest.and_then(|point| {
                    e.future_cost
                        .as_ref()
                        .unwrap()
            ...
            "getFutureConnectionSegments" => json!(
                e.get_future_connection_segments()
                    .iter()
                    .map(
                        |s| json!({"connectionName":s.connection_name,"start":s.start,"end":s.end})
                    )
                    .collect::<Vec<_>>()
            ),
            "isViaTooCloseToFutureConnectionTrace" => {
                json!(e.is_via_too_close_to_future_connection_trace(&node(&a["node"])))
            }
            "diminishCloseToGoal" => json!(e.diminish_close_to_goal(&node(&a["node"]))),
            "getFutureConnectionPenalty" => json!(e.get_future_connection_penalty(
                &node(&a["node"]),
                a["isVia"].as_bool().unwrap_or(false)
            )),
            "queuePeek" => e
            ...
            "queueDequeue" => e
                ...
                .unwrap_or(Value::Null),
            "queueEnqueue" => {
                e.candidates.enqueue(node(&a["node"]));
                Value::Null
            }
            "queueHeapifyUp" => {
                e.candidates.heapify_up();
                Value::Null
            }
            "queueHeapifyDown" => {
                e.candidates.heapify_down();
                Value::Null
            }
            "queueTop" => json!(
                e.candidates
                    .get_top_n(a["n"].as_u64().unwrap() as usize)
                    .iter()
                    .map(|n| node_value(n))
                    .collect::<Vec<_>>()
            ),
            ...
            _ => {
                return Err(JsValue::from_str(&format!(
                    "Unknown single-route operation {method}"
                )));
            }
        };
        HighDensityValue(result)
            .into_ts()
            .map_err(|error| JsError::new(&error.to_string()).into())
    }
}
