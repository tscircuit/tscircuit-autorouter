...
pub struct SpecializedRouterContext {
    ...
    connectivity: Option<Rc<capacity_autorouter::solvers::high_density_solver::connectivity_map::ConnectivityMap>>,
...
impl SpecializedIntraNodeDispatcher {
    ...
    pub fn is_applicable(kind: &str, params: Ts<HighDensityMappedValue>) -> Result<bool, JsValue> {
        let params: Value = read_mapped_value(params)?;
        capacity_autorouter::bindings::high_density::specialized_simple_dispatch::is_applicable(kind, params)
            .map_err(|error| JsValue::from_str(&error))
    }
