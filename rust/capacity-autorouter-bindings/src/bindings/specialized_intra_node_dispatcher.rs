...
impl SpecializedIntraNodeDispatcher {
    ...
    pub fn invoke(
        &mut self,
        method: &str,
        args: Ts<HighDensityMappedValue>,
    ) -> Result<Ts<SpecializedInvocation>, JsValue> {
        let args: Vec<Value> = serde_json::from_value(read_mapped_value(args)?)
            .map_err(|error| JsValue::from_str(&error.to_string()))?;
        let (result, args) = self
            .engine
            .borrow_mut()
            .invoke(method, args)
            .map_err(|error| JsValue::from_str(&error))?;
        SpecializedInvocation {
            result,
            args,
            identity: None,
        }
    ...
    pub fn is_applicable(kind: &str, params: Ts<HighDensityMappedValue>) -> Result<bool, JsValue> {
        let params: Value = read_mapped_value(params)?;
        capacity_autorouter::bindings::high_density::specialized_simple_dispatch::is_applicable(
            kind, params,
        )
        .map_err(|error| JsValue::from_str(&error))
    }
...
impl SpecializedIntraNodeDispatcher {
    ...
    pub fn invoke_identity(
        ...
        let args: Vec<Value> = serde_json::from_value(read_mapped_value(args)?)
            .map_err(|error| JsValue::from_str(&error.to_string()))?;
        let identity: Value = read_mapped_value(identity)?;
        let (result,args,identity) = capacity_autorouter::solvers::high_density_solver::multi_head_poly_line_intra_node_solver::types1::invoke_with_identity(&mut self.engine.borrow_mut(),method,args,identity)
            .map_err(|error|JsValue::from_str(&error))?;
        SpecializedInvocation {
            result,
            args,
