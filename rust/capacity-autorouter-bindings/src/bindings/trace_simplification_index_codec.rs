...
use capacity_autorouter::bindings::trace_simplification::high_density_route_spatial_index::{
    HighDensityRouteSpatialIndex, StoredSegment, StoredVia,
};
use capacity_autorouter::data_structures::obstacle_tree::{
    ImportedObstacleIndex, NativeObstacleTree, ObstacleSpatialHashIndex, RbushNode,
};
...
fn bucket_key(value: &Value) -> Result<(i64, i64), String> {
    let value = value
        .as_str()
        .ok_or("Spatial bucket key must be a string")?;
    let (x, y) = value
        .split_once('x')
        .ok_or("Spatial bucket key must contain x")?;
    Ok((
        x.parse().map_err(|_| "Invalid spatial bucket x")?,
        y.parse().map_err(|_| "Invalid spatial bucket y")?,
    ))
}

pub(crate) fn decode_obstacle_index(
    value: &Value,
    codec: &mut GraphCodec,
) -> Result<Rc<RefCell<ObstacleSpatialHashIndex>>, String> {
    if value["hostQuery"].as_bool() == Some(true) {
        let id = value["indexId"].as_u64().ok_or("Host indexId missing")?;
        let query = codec
            .obstacle_query
            .clone()
            .ok_or("Obstacle query callback missing")?;
    ...
    if value["kind"] != "native" {
        let imported = match value["kind"].as_str() {
            ...
            Some("flatbush") => ImportedObstacleIndex::Flatbush {
                ...
                indices: serde_json::from_value(value["indices"].clone())
                    .map_err(|e| e.to_string())?,
                level_bounds: serde_json::from_value(value["levelBounds"].clone())
                    .map_err(|e| e.to_string())?,
        ...
        return Ok(Rc::new(RefCell::new(ObstacleSpatialHashIndex {
            identity: value["indexId"].as_u64().unwrap_or_else(
                capacity_autorouter::bindings::trace_simplification::types::next_identity,
            ),
            storage: codec.read_obstacles(&value["storage"])?,
    ...
    for bucket in value["buckets"]
        .as_array()
        .ok_or("Obstacle buckets must be an array")?
    {
        let mut entries = Vec::new();
        for entry in bucket[1]
            .as_array()
            .ok_or("Obstacle bucket entries must be an array")?
        {
            entries.push((
                codec.read_obstacle(&entry[0])?,
                entry[1]
                    .as_u64()
                    .ok_or("Obstacle bucket index must be unsigned")? as usize,
            ));
        }
    ...
    let native = NativeObstacleTree {
        ...
        cell_size: value["CELL_SIZE"]
            .as_f64()
            .ok_or("Obstacle CELL_SIZE missing")?,
...
pub(crate) fn decode_hd_route_index(
    value: &Value,
    codec: &mut GraphCodec,
) -> Result<Rc<RefCell<HighDensityRouteSpatialIndex>>, String> {
    let mut segment_buckets = IndexMap::new();
    let mut via_buckets = IndexMap::new();
    for bucket in value["segmentBuckets"]
        .as_array()
        .ok_or("Segment buckets must be an array")?
    {
        let mut entries = Vec::new();
        for entry in bucket[1]
            .as_array()
            .ok_or("Segment bucket entries must be an array")?
        {
            entries.push(Rc::new(StoredSegment {
                segment_id: entry["segmentId"]
                    .as_str()
                    .ok_or("Missing segmentId")?
                    .to_owned(),
                segment: [
                    codec.read_point(&entry["segment"][0])?,
                    codec.read_point(&entry["segment"][1])?,
                ],
    ...
    for bucket in value["viaBuckets"]
        .as_array()
        .ok_or("Via buckets must be an array")?
    {
        let mut entries = Vec::new();
        for entry in bucket[1]
            .as_array()
            .ok_or("Via bucket entries must be an array")?
        {
            entries.push(Rc::new(StoredVia {
                via_id: entry["viaId"].as_str().ok_or("Missing viaId")?.to_owned(),
                x: entry["x"].as_f64().ok_or("Missing via x")?,
                y: entry["y"].as_f64().ok_or("Missing via y")?,
                parent_route: codec.read_route(&entry["parentRoute"])?,
            }));
        }
    ...
    Ok(Rc::new(RefCell::new(HighDensityRouteSpatialIndex {
        ...
        cell_size: value["CELL_SIZE"]
            .as_f64()
            .ok_or("Missing route CELL_SIZE")?,
        maximum_copper_radius: value["maximumCopperRadius"]
            .as_f64()
            .ok_or("Missing maximumCopperRadius")?,
    })))
}
...
fn decode_rbush(value: &Value, codec: &mut GraphCodec) -> Result<RbushNode, String> {
    ...
    if let Some(values) = value["children"].as_array() {
        for child in values {
            children.push(decode_rbush(child, codec)?);
        }
    }
    let data = if value.get("data").is_some() {
        Some(codec.read_obstacle(&value["data"])?)
    } else {
        None
    };
    Ok(RbushNode {
        bounds: capacity_autorouter::bindings::trace_simplification::types::Bounds {
            min_x: value["minX"].as_f64().unwrap_or(f64::INFINITY),
            min_y: value["minY"].as_f64().unwrap_or(f64::INFINITY),
            max_x: value["maxX"].as_f64().unwrap_or(f64::NEG_INFINITY),
            max_y: value["maxY"].as_f64().unwrap_or(f64::NEG_INFINITY),
        },
        leaf: value["leaf"].as_bool().unwrap_or(false),
        height: value["height"].as_u64().map(|v| v as usize),
        children,
        data,
    })
}

pub(crate) fn encode_obstacle_index(
    index: &ObstacleSpatialHashIndex,
    id: u64,
    codec: &mut GraphCodec,
) -> Value {
    let mut value = match &index.imported {
        Some(ImportedObstacleIndex::Rbush(root)) => {
            serde_json::json!({"kind":"rbush","tree":encode_rbush(root,codec)})
        }
        Some(ImportedObstacleIndex::Flatbush {
            boxes,
            indices,
            level_bounds,
            node_size,
        ...
        None => {
            serde_json::json!({"kind":"native","CELL_SIZE":index.native.cell_size,"obstacles":codec.obstacles(&index.native.obstacles),"buckets":index.native.buckets.iter().map(|((x,y),entries)|serde_json::json!([format!("{x}x{y}"),entries.iter().map(|(o,i)|serde_json::json!([codec.obstacle(o),i])).collect::<Vec<_>>()])).collect::<Vec<_>>()})
        }
    };
...
fn encode_rbush(node: &RbushNode, codec: &mut GraphCodec) -> Value {
    ...
    if let Some(data) = &node.data {
        value["data"] = codec.obstacle(data);
    } else {
        ...
        value["children"] = Value::Array(
            node.children
                .iter()
                .map(|child| encode_rbush(child, codec))
                .collect(),
        );
    }
    value
...
pub(crate) fn encode_hd_route_index(
