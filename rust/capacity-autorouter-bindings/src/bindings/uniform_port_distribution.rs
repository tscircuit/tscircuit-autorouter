...
#[serde(transparent)]
pub struct UniformOptionalSharedEdge(
    #[tsify(type = "UniformSharedEdge | null")] Option<SharedEdge>,
);

#[derive(Serialize, Tsify)]
#[serde(transparent)]
pub struct UniformSharedEdges(
    #[tsify(type = "[UniformName, UniformSharedEdge][]")] Vec<(Name, SharedEdge)>,
);

#[wasm_bindgen(js_name = buildUniformPortDistribution)]
pub fn build_uniform_port_distribution(
    input: Ts<UniformPortDistributionInput>,
) -> Result<Ts<UniformPortDistributionConstructor>, JsError> {
...
pub fn determine_uniform_port_owner_pair(
    input: Ts<UniformOwnerPairInput>,
) -> Result<Ts<UniformOwnerPair>, JsError> {
    let input = input.to_rust()?;
    let pair = determine_owner_pair::determine_owner_pair(
        input.port_point_id.as_ref(),
...
pub fn normalize_uniform_port_owner_pair(
    node_a: Ts<Name>,
    node_b: Ts<Name>,
) -> Result<Ts<UniformOwnerPair>, JsError> {
...
pub fn get_uniform_port_owner_pair_key(
    owner_node_ids: Ts<UniformOwnerPair>,
) -> Result<Ts<Name>, JsError> {
...
pub fn get_uniform_shared_edge(
    input: Ts<UniformSharedEdgeInput>,
) -> Result<Ts<UniformOptionalSharedEdge>, JsError> {
    let input = input.to_rust()?;
    let bounds: IndexMap<_, _> = input.node_bounds.into_iter().collect();
    Ok(UniformOptionalSharedEdge(
        get_shared_edge_for_node_pair::get_shared_edge_for_node_pair(
            &input.node_a_id,
            &input.node_b_id,
            &bounds,
        ),
...
pub fn precompute_uniform_shared_edges(
    input: Ts<UniformSharedEdgesInput>,
) -> Result<Ts<UniformSharedEdges>, JsError> {
    let input = input.to_rust()?;
    let bounds: IndexMap<_, _> = input.node_bounds.into_iter().collect();
    let edges = precompute_shared_edges::precompute_shared_edges(&input.owner_pairs, &bounds)
        .into_iter()
        .collect();
