...
use capacity_autorouter::solvers::hyper_parameter_supervisor_solver::{
    Candidate, CandidateFactory, CandidateState,
};
...
enum SharedEngine {
    ...
    Specialized(
        Rc<
            RefCell<
                capacity_autorouter::bindings::high_density::specialized_solver::SpecializedEngine,
            >,
        >,
    ),
}
...
fn share(engine: SharedEngine) -> u32 {
    SHARED_ENGINES.with(|registry| {
        ...
        registry.0 = registry
            .0
            .checked_add(1)
            .expect("Native engine handle overflow");
...
pub(crate) fn share_specialized(
    engine: Rc<
        RefCell<capacity_autorouter::bindings::high_density::specialized_solver::SpecializedEngine>,
    >,
) -> u32 {
...
impl Hooks {
    ...
    fn external_state(
        ...
        let result = self.external.call3(
            &JsValue::UNDEFINED,
    ...
    fn cache_action(
        ...
        let state =
            callback_output(&serde_json::to_value(state).map_err(|error| error.to_string())?)?;
        let result = self.parse_call(self.cache.call3(
            &JsValue::UNDEFINED,
            &JsValue::from_f64(id as f64),
            &JsValue::from_str(action),
            &state,
        ...
        if let (Some(hits), Some(misses)) =
            (result["cacheHits"].as_f64(), result["cacheMisses"].as_f64())
        {
...
fn read_state(value: &Value) -> Result<CandidateState, String> {
    Ok(CandidateState {
        iterations: value["iterations"]
            .as_u64()
            .ok_or("Candidate iterations required")? as usize,
        max_iterations: value["maxIterations"]
            .as_f64()
            .ok_or("Candidate maxIterations required")?,
        solved: value["solved"]
            .as_bool()
            .ok_or("Candidate solved required")?,
        failed: value["failed"]
            .as_bool()
...
impl PortfolioCandidate {
    fn step_general(&mut self) -> Result<(), String> {
        if self.has_cache && !self.cache_checked {
            ...
            if result["hit"] == true {
                ...
                let routes = result["routes"]
                    .as_array()
                    .ok_or("Cached General routes required")?;
                if let SharedEngine::General(engine) = &self.engine {
                    let routes = serde_json::from_value(Value::Array(routes.clone()))
                        .map_err(|error| error.to_string())?;
                    engine.borrow_mut().set_cached_output(
                        routes,
                        self.state.solved,
                        self.state.failed,
        ...
        if matches!(&self.engine, SharedEngine::PendingGeneral) {
            let result = self.hooks.external.call3(
                &JsValue::UNDEFINED,
                &JsValue::from_f64(self.id as f64),
                &JsValue::from_str("attach-general"),
                &JsValue::from_f64(0.0),
            );
            let descriptor = self.hooks.parse_call(result)?;
            let handle = descriptor["handle"]
                .as_u64()
                .ok_or("Attached General handle required")? as u32;
            let engine = SHARED_ENGINES
                .with(|registry| registry.borrow_mut().1.remove(&handle))
                .ok_or("Attached General handle not found")?;
            if !matches!(&engine, SharedEngine::General(_)) {
                return Err("Attached General handle has wrong engine kind".into());
            }
            self.engine = engine;
        }
        let SharedEngine::General(engine) = &self.engine else {
            return Err("General candidate requires a General engine".into());
        };
        {
            ...
            self.state.progress =
                engine.solved_routes.len() as f64 / engine.total_connections as f64;
        }
...
impl Candidate for PortfolioCandidate {
    ...
    fn is_specialized(&self) -> bool {
        matches!(self.kind.as_str(), "a01" | "a03")
    }
    ...
    fn step(&mut self) -> Result<(), String> {
        if self.state.solved || self.state.failed {
            return Ok(());
        }
...
impl Candidate for SnapshotCandidate {
    ...
    fn step(&mut self) -> Result<(), String> {
        Err("Snapshot candidate cannot step".to_owned())
    }
}
...
impl CandidateFactory for Factory {
    fn generate(&mut self, hyper_parameters: &Value) -> Result<Box<dyn Candidate>, String> {
        ...
        let descriptor = self
            .hooks
            .parse_call(self.hooks.factory.call1(&JsValue::UNDEFINED, &parameters))?;
        let id = descriptor["id"].as_u64().ok_or("Candidate id required")? as usize;
        let state = read_state(&descriptor["state"])?;
        let kind = descriptor["kind"]
            .as_str()
            .ok_or("Candidate kind required")?;
        if kind == "external" {
            return Ok(Box::new(ExternalCandidate {
                id,
                state,
        ...
        let engine = if kind == "general" && descriptor.get("handle").is_none_or(Value::is_null) {
            ...
            let handle = descriptor["handle"]
                .as_u64()
                .ok_or("Native candidate handle required")? as u32;
            let engine = SHARED_ENGINES
                .with(|registry| registry.borrow_mut().1.remove(&handle))
                .ok_or("Native candidate handle not found")?;
            match (&engine, kind) {
                (SharedEngine::General(_), "general")
                | (SharedEngine::HighDensity(_), "a01" | "a03")
        ...
        let total_connections = if kind == "general" {
            descriptor["totalConnections"]
                .as_u64()
                .ok_or("General candidate totalConnections required")? as usize
        } else {
            0
        };
        Ok(Box::new(PortfolioCandidate {
            id,
            engine,
            state,
            hooks: self.hooks.clone(),
            total_connections,
            has_cache: descriptor["hasCache"].as_bool().unwrap_or(false),
            cache_checked: false,
            kind: kind.to_owned(),
        }))
    }
...
impl PortfolioCore {
    pub(crate) fn state(
        ...
        capacity_autorouter::bindings::high_density::specialized_base_solver::BaseSolverState {
            ...
            solved: self.solver.supervisor.solved,
            failed: self.solver.supervisor.failed,
            progress: self.solver.progress,
            error: self.solver.supervisor.error.clone(),
        }
    }

    pub(crate) fn step_inner(&mut self, iterations: usize) -> Result<(), JsValue> {
        self.solver.iterations = iterations;
        self.solver.step_inner(&mut self.factory).map_err(|error| {
            self.factory
                .hooks
                .error
                .borrow_mut()
    ...
    fn complete_terminal(&mut self) -> Result<(), JsValue> {
        if (self.solver.supervisor.solved || self.solver.supervisor.failed)
            ...
            && let Some(terminal) = &self.terminal
        {
            ...
            self.resolved_solver_type = result["solverType"]
                .as_str()
                .ok_or_else(|| JsValue::from_str("Portfolio terminal solverType required"))?
                .to_owned();
        }
        Ok(())
    }
    ...
    pub(crate) fn step(&mut self) -> Result<(), JsValue> {
        ...
        if let Err(error) = self.step_inner(iterations) {
            ...
            self.solver.supervisor.error = Some(format!(
                "portfolio_solver::PortfolioSingleIntraNodeSolver error: {text}"
            ));
        ...
        if !self.solver.supervisor.solved && iterations as f64 > self.solver.max_iterations {
            self.solver.supervisor.failed = true;
            self.solver.supervisor.error = Some(format!(
                "portfolio_solver::PortfolioSingleIntraNodeSolver ran out of iterations (MAX_ITERATIONS={})",
                capacity_autorouter::utils::js_number::js_number_to_string(
    ...
    pub(crate) fn snapshot(&self) -> Result<Ts<PortfolioSnapshot>, JsValue> {
        ...
        let candidates: Vec<_> = records
            ...
            .map(|record| {
                ...
                CandidateSnapshot {
                    id: record.id,
                    hyper_parameters: record.hyper_parameters.clone(),
                    g: record.g,
                    h: record.h,
                    f: record.f,
                    state: CandidateStateSnapshot {
        ...
        let snapshot = PortfolioSnapshot {
            ...
            error: supervisor.error.clone(),
            max_iterations: self.solver.max_iterations,
            stats: SnapshotStats {
                dynamic_expansion_work_budget: stats.get("dynamicExpansionWorkBudget").cloned(),
                dynamic_supervisor_iteration_limit: stats
                    .get("dynamicSupervisorIterationLimit")
                    .cloned(),
                adaptive_search_expanded: stats.get("adaptiveSearchExpanded").cloned(),
                adaptive_search_expanded_at_iteration: stats
                    .get("adaptiveSearchExpandedAtIteration")
                    .cloned(),
...
impl PortfolioSingleIntraNodeSolver {
    ...
    pub fn new(
        node: Ts<HighDensityValue>,
        effort: f64,
        #[wasm_bindgen(
            unchecked_param_type = "(hyperParameters: Record<string, unknown>) => Record<string, unknown>"
        )]
        factory: js_sys::Function,
        #[wasm_bindgen(
            unchecked_param_type = "(id: number, action: 'setup' | 'step' | 'attach-general', count: number) => CandidateStateSnapshot | { handle: number }"
        )]
        external: js_sys::Function,
        #[wasm_bindgen(
            unchecked_param_type = "(id: number, action: 'lookup' | 'save', state: CandidateStateSnapshot) => Record<string, unknown>"
        )]
        cache: js_sys::Function,
        #[wasm_bindgen(
        ...
        Ok(Self {
            engine: Rc::new(RefCell::new(PortfolioCore {
                ...
                resolved_solver_type: "portfolio_solver::PortfolioSingleIntraNodeSolver".into(),
    ...
    pub fn initialize(&mut self) -> Result<(), JsValue> {
        ...
        let core = &mut *core;
        core.solver
            .initialize_solvers(&mut core.factory)
            .map_err(|error| {
                core.factory
                    .hooks
                    .error
    ...
    pub fn get_hyper_parameter_defs() -> Result<Ts<PortfolioHyperParameterDefinitions>, JsValue> {
        ...
        PortfolioHyperParameterDefinitions(defs)
            .into_ts()
    ...
    pub fn get_hyper_parameter_combinations(
        definitions: Ts<HighDensityValue>,
    ) -> Result<Ts<PortfolioHyperParameters>, JsValue> {
        fn combinations(defs: &[Value]) -> Vec<Value> {
            let Some((first, remaining)) = defs.split_first() else {
                return vec![serde_json::json!({})];
            };
            let sub = combinations(remaining);
            let mut output = Vec::new();
            for value in first["possibleValues"]
                .as_array()
                .expect("Hyperparameter values required")
            {
                for combination in &sub {
                    let mut result = combination.clone();
                    for (key, value) in value.as_object().expect("Hyperparameter object required") {
        ...
        let defs: Vec<Value> = serde_json::from_value(read_value(definitions)?)
            .map_err(|error| JsValue::from_str(&error.to_string()))?;
        PortfolioHyperParameters(combinations(&defs))
            .into_ts()
            .map_err(|error| JsError::new(&error.to_string()).into())
    }
    ...
    pub fn best_candidate_id(&self) -> Option<usize> {
        ...
        let index = core
            .solver
            .supervisor
            .get_supervised_solver_with_best_fitness()?;
    ...
    pub fn compute_candidate_g(
        state: Ts<HighDensityValue>,
        hyper_parameters: Ts<HighDensityValue>,
        specialized: bool,
    ) -> Result<f64, JsValue> {
        ...
        Ok(portfolio_solver::PortfolioSingleIntraNodeSolver::compute_g(
            &candidate,
            &hyper_parameters,
        ))
    }
    ...
    pub fn compute_h(&self, id: usize) -> Result<f64, JsValue> {
        ...
        if core.solver.adaptive_search_expanded {
            Ok(1.0
                - portfolio_solver::PortfolioSingleIntraNodeSolver::get_candidate_progress(
                    record.solver.as_ref(),
                    core.solver.get_node_segment_count(),
                ))
        } else {
    ...
    pub fn restore_state(&mut self, state: Ts<HighDensityValue>) -> Result<(), JsValue> {
        let state: Value = read_value(state)?;
        let mut core = self.engine.borrow_mut();
        if let Some(value) = state.get("iterations") {
            core.solver.iterations = value
                .as_u64()
                .ok_or_else(|| JsValue::from_str("Portfolio iterations required"))?
                as usize;
        }
        if let Some(value) = state.get("MAX_ITERATIONS") {
            core.solver.max_iterations = value.as_f64().unwrap_or(f64::NAN);
        }
        if let Some(value) = state.get("progress") {
            core.solver.progress = value.as_f64().unwrap_or(f64::NAN);
        }
        if let Some(value) = state.get("solved") {
            core.solver.supervisor.solved = value
                .as_bool()
                .ok_or_else(|| JsValue::from_str("Portfolio solved must be boolean"))?;
        }
        if let Some(value) = state.get("failed") {
            core.solver.supervisor.failed = value
                .as_bool()
                .ok_or_else(|| JsValue::from_str("Portfolio failed must be boolean"))?;
        }
        if let Some(value) = state.get("error") {
            core.solver.supervisor.error = value.as_str().map(str::to_owned);
        }
        if let Some(value) = state.get("stats") {
            core.solver.stats = value.clone();
        }
        if let Some(value) = state.get("GREEDY_MULTIPLIER") {
            core.solver.supervisor.greedy_multiplier = value.as_f64().unwrap_or(f64::NAN);
        }
        if let Some(value) = state.get("MIN_SUBSTEPS") {
            core.solver.supervisor.min_substeps =
                value.as_f64().unwrap_or(f64::NAN).ceil() as usize;
        }
        if let Some(value) = state.get("solvedRoutes") {
            core.output_routes = value
                .as_array()
                .ok_or_else(|| JsValue::from_str("Portfolio routes must be an array"))?
                .clone();
        }
        if !core.solver.supervisor.solved && !core.solver.supervisor.failed {
            core.output_completed = false;
        }
        Ok(())
    }
...
#[derive(Serialize, Tsify)]
#[serde(transparent)]
pub struct PortfolioHyperParameterDefinitions(
    #[tsify(type = "{ name: string; possibleValues: Record<string, unknown>[] }[]")] Vec<Value>,
);
