...
pub fn find_connected_networks(
    connections: &[[Id; 2]],
) -> indexmap::IndexMap<String, IndexSet<Id>> {
    ...
    networks
        ...
        .filter_map(|(index, network)| {
            network.map(|nodes| (format!("connectivity_net{index}"), nodes))
        })
        .collect()
}
fn get_or_create_network(networks: &mut Vec<Option<IndexSet<Id>>>, node: &Id) -> usize {
    for (index, network) in networks.iter().enumerate() {
        if network.as_ref().is_some_and(|n| n.contains(node)) {
            return index;
        }
    }
