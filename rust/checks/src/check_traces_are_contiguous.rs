...
use crate::is_point_in_pad::{Pad, is_point_in_pad};
use crate::pcb_connectivity_map::PcbConnectivityMap;
use crate::types::{Id, Math, Point, Port, RoutePoint, Trace, encoded_number, point};
...
pub struct ErrorDescriptor {
    #[cfg_attr(
        feature = "wasm-types",
        tsify(type = "'misalignedVia' | 'missingConnection' | 'disconnectedEndpoint'")
    )]
    pub kind: &'static str,
    ...
    #[cfg_attr(
        feature = "wasm-types",
        tsify(
            type = "{ x: number | { $traceNumber: string }; y: number | { $traceNumber: string } }"
        )
    )]
...
impl ErrorDescriptor {
    fn new(kind: &'static str, trace: &Trace, source: Option<usize>) -> Self {
        Self {
            ...
            source_trace_index: source,
            point_index: None,
            port_index: None,
            pad_index: None,
            center_point_index: None,
            center: None,
            endpoint: None,
        }
    }
...
fn point_in_pads(point: Point, pads: &[IndexedPad], math: Math) -> Result<bool, String> {
    for pad in pads {
        if is_point_in_pad(point, &pad.pad, math)? {
            return Ok(true);
        }
    }
    Ok(false)
}
fn route_point_connects_to_another_expected_port(
    point: &RoutePoint,
    expected: &[&Port],
    missing: &Id,
    pads: &PadMap,
    ...
    for port in expected {
        if !port.id.truthy() || &port.id == missing {
            continue;
        }
        if let Some(port_pads) = pads.get(&port.id)
            && point_in_pads(point.position, port_pads, math)?
        {
            return Ok(true);
        }
    }
...
fn get_missing_connection_error_center(
    trace: &Trace,
    port: &Port,
    expected: &[&Port],
    pads: &PadMap,
    math: Math,
) -> Result<(Option<usize>, Option<Value>), String> {
    let first = &trace.route[0];
    let last_index = trace.route.len() - 1;
    let last = &trace.route[last_index];
    let location = if first.references(&port.id) && first.wire {
        Some(0)
    } else if (last.references(&port.id) && last.wire)
        || (route_point_connects_to_another_expected_port(first, expected, &port.id, pads, math)?
            && last.wire)
    {
        Some(last_index)
    } else if route_point_connects_to_another_expected_port(last, expected, &port.id, pads, math)?
        && first.wire
    {
        Some(0)
    } else if first.wire && last.wire {
        if (math.hypot)(
            first.position.x - port.position.x,
            first.position.y - port.position.y,
        ) <= (math.hypot)(
            last.position.x - port.position.x,
            last.position.y - port.position.y,
        ) {
            Some(0)
        } else {
            Some(last_index)
    ...
    Ok(match location {
        ...
        None => (
            None,
            Some(
                json!({"x":encoded_number((first.center.x + last.center.x) / 2.0), "y":encoded_number((first.center.y + last.center.y) / 2.0)}),
            ),
        ),
    })
}
...
pub fn check(circuit_json: &[Value], math: Math) -> Result<Vec<ErrorDescriptor>, String> {
    ...
    for (index, element) in circuit_json.iter().enumerate() {
        match element.get("type").and_then(Value::as_str) {
            Some("pcb_port") => ports.push(Port {
                index,
                id: Id::read(element.get("pcb_port_id")),
                source_id: Id::read(element.get("source_port_id")),
            ...
            Some("pcb_trace") => {
                let route = element
                    ...
                    .ok_or_else(|| "PCB trace route must be an array".to_string())?;
                traces.push(Trace {
                    index,
                    id: Id::read(element.get("pcb_trace_id")),
                    source_id: Id::read(element.get("source_trace_id")),
                    route: route.iter().map(RoutePoint::read).collect(),
                });
            }
            Some("source_trace") => sources.push(SourceTrace {
                index,
                id: Id::read(element.get("source_trace_id")),
                connected_ports: element
                    .get("connected_source_port_ids")
                    .and_then(Value::as_array)
                    .map(|values| values.iter().map(|v| Id::read(Some(v))).collect())
                    .unwrap_or_default(),
            }),
    ...
    for index in smt_pads.into_iter().chain(plated_holes) {
        ...
        if id.truthy() {
            pads.entry(id).or_default().push(IndexedPad {
                index,
                pad: Pad::read(element),
            });
        }
    }
    ...
    for trace in &traces {
        ...
        let expected: Vec<&Port> = match source {
            Some(source) => ports
                .iter()
                .filter(|port| source.connected_ports.contains(&port.source_id))
                .collect(),
            None => Vec::new(),
        };
        for i in 1..trace.route.len().saturating_sub(1) {
            ...
            if current.via && previous.wire && next.wire {
                let prev_aligned = (previous.position.x - current.position.x).abs() < 0.01
                    && (previous.position.y - current.position.y).abs() < 0.01;
                let next_aligned = (next.position.x - current.position.x).abs() < 0.01
                    && (next.position.y - current.position.y).abs() < 0.01;
        ...
        if let Some(source) = source
            && !expected.is_empty()
            && !checked_sources.insert(source.id.clone())
        {
            continue;
        }
        for port in &expected {
            ...
            let Some(port_pads) = pads.get(&port.id) else {
                continue;
            };
            if port_pads.is_empty() {
                continue;
            }
            ...
            candidates.extend(
                traces
                    .iter()
                    .enumerate()
                    .filter(|(_, candidate)| source.is_some_and(|s| candidate.source_id == s.id))
                    .map(|(index, _)| index),
            );
            for candidate_index in candidates {
                ...
                if !candidate.source_id.truthy() {
                    continue;
                }
                let candidate_source = sources.iter().find(|s| s.id == candidate.source_id);
                if source.is_none()
                    || (candidate.source_id != source.unwrap().id
                        && !candidate_source
                            .is_some_and(|s| s.connected_ports.contains(&port.source_id)))
                {
                    continue;
                }
            ...
            if !first_connected && !last_connected {
                let (center_point_index, center) =
                    get_missing_connection_error_center(trace, port, &expected, &pads, math)?;
        ...
        if expected.is_empty() {
            ...
            let same = first.wire
                && last.wire
                && (first.position.x - last.position.x).abs() < 0.01
                && (first.position.y - last.position.y).abs() < 0.01;
            for port_pads in pads.values() {
                if first.wire && point_in_pads(first.position, port_pads, math)? {
                    first_connected = true;
                }
                if last.wire && point_in_pads(last.position, port_pads, math)? {
                    last_connected = true;
                }
            }
            if !first_connected && first.wire {
                let mut error = ErrorDescriptor::new("disconnectedEndpoint", trace, source_index);
                error.endpoint = Some("start");
                error.point_index = Some(0);
                errors.push(error);
            }
            if !last_connected && last.wire && !(same && !first_connected) {
                let mut error = ErrorDescriptor::new("disconnectedEndpoint", trace, source_index);
                error.endpoint = Some("end");
                error.point_index = Some(trace.route.len() - 1);
                errors.push(error);
            }
        }
    }
