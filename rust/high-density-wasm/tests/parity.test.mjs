...
import { initializeHighDensityWasm, WasmHighDensitySolver } from "../dist/index.js"
...
test("A01 and A03 match TypeScript at every step and serialize identical routes", async () => {
  ...
  await initializeHighDensityWasm({ module_or_path: await readFile(new URL("../pkg/high_density_wasm_bg.wasm", import.meta.url)) })
  ...
  for (const [variant, ReferenceSolver] of [["a01", reference.HighDensitySolverA01], ["a03", reference.HighDensitySolverA03]]) {
    for (const fixture of fixtures) {
      ...
      const actual = new WasmHighDensitySolver(variant, makeProps(fixture, actualPenaltyCalls))
      try {
