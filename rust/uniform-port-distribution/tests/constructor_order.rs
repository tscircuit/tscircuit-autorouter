use serde_json::json;
use uniform_port_distribution::types::Name;
...
use uniform_port_distribution::determine_owner_pair::determine_owner_pair;
use uniform_port_distribution::get_owner_pair_key::normalize_owner_pair;
...
fn constructor_preserves_first_match_dedupe_sources_and_ordered_edges() {
    ...
    assert_eq!(result.node_bounds.iter().map(|entry|serde_json::to_value(&entry.0).unwrap()).collect::<Vec<_>>(),vec![json!("a"),json!("b")]);
    assert_eq!(result.owner_pair_port_points.iter().map(|entry|serde_json::to_value(&entry.0).unwrap()).collect::<Vec<_>>(),vec![json!("a|b"),json!("a|a")]);
    let family = &result.owner_pair_port_points[0].1;
    assert_eq!(family.iter().map(|point|(point.node_index,point.point_index)).collect::<Vec<_>>(),vec![(0,0),(1,1)]);
    assert_eq!(serde_json::to_value(&family[0].owner_node_ids).unwrap(),json!(["a","b"]));
    assert_eq!(serde_json::to_value(&result.owner_pairs_to_process).unwrap(),json!(["a|b"]));
    ...
    assert_eq!(determine_owner_pair(Some(&invalid),&current,&input.input_nodes_with_port_points),[current.clone(),current]);
    let bmp = Name::String("\u{e000}".into());
    let supplementary = Name::String("\u{10000}".into());
    assert_eq!(normalize_owner_pair(&bmp,&supplementary),[supplementary,bmp]);
    let lone = Name::Units { __utf16: vec![0xd800] };
    let replacement = Name::String("\u{fffd}".into());
    assert_eq!(normalize_owner_pair(&replacement,&lone),[lone.clone(),replacement.clone()]);
    let key = uniform_port_distribution::get_owner_pair_key::get_owner_pair_key(&[lone,replacement]);
    assert_eq!(key,Name::Units { __utf16: vec![0xd800,0x7c,0xfffd] });
}
