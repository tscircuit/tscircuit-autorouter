...
use crate::specialized_utils::math::{Point,Bounds,min,max};
use crate::specialized_utils::classify_point_in_bounds::BOUNDARY_COORDINATE_TOLERANCE_MM as TOL;
...
pub fn point_to_angle(point:Point,b:Bounds)->Result<f64,String>{
    ...
    }else{return Err(format!("Point ({}, {}) does not lie on the boundary defined by {}",crate::js_number::js_number_to_string(point.x),crate::js_number::js_number_to_string(point.y),serde_json::to_string(&b).expect("bounds")));};
