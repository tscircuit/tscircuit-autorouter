...
pub fn create_objects_with_z_layers_owned(objects: Vec<Value>, layer_count: f64) -> Vec<Value> {
    let all: Vec<f64> = (0..layer_count.max(0.0).floor() as usize).map(|index| index as f64).collect();
    objects.into_iter().map(|mut object| {
        let candidates = if let Some(layers) = object.get("__zLayers").filter(|value| !value.is_null()) {
            layers.as_array().expect("Z layers array").iter().map(|value| value.as_f64().expect("Numeric layer")).collect()
        } else if let Some(layers) = object.get("layers").filter(|value| !value.is_null() && **value != false) {
            let names: Vec<_> = layers.as_array().expect("Layers array").iter().map(|value| value.as_str().expect("Layer name")).collect();
