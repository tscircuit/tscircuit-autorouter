...
use crate::single_transition_intra_node_solver::SingleTransitionIntraNodeSolver;
use crate::single_transition_through_obstacle_intra_node_solver::SingleTransitionThroughObstacleIntraNodeSolver;
use crate::two_route_high_density_solver::two_crossing_routes_high_density_solver::TwoCrossingRoutesHighDensitySolver;
use crate::two_route_high_density_solver::single_transition_crossing_route_solver::SingleTransitionCrossingRouteSolver;
use crate::multi_head_poly_line_intra_node_solver::multi_head_poly_line_intra_node_solver::MultiHeadPolyLineIntraNodeSolver;
use crate::multi_head_poly_line_intra_node_solver::multi_head_poly_line_intra_node_solver2_optimized::MultiHeadPolyLineIntraNodeSolver2;
use crate::multi_head_poly_line_intra_node_solver::multi_head_poly_line_intra_node_solver3_via_possibilities_solver_integration::MultiHeadPolyLineIntraNodeSolver3;
use crate::via_possibilities_solver::via_possibilities_solver2::ViaPossibilitiesSolver2;
...
impl SpecializedEngine {
    pub fn new(kind: &str, params: Value, math: SpecializedMath) -> Result<Self, String> {
        match kind {
            "single-layer" => Ok(Self::SingleLayer(Box::new(SingleLayerNoDifferentRootIntersectionsIntraNodeSolver::new_with_math(params, math)?))),
            "single-transition" => Ok(Self::SingleTransition(Box::new(SingleTransitionIntraNodeSolver::new_with_math(params, math)?))),
            "through-obstacle" => Ok(Self::ThroughObstacle(Box::new(SingleTransitionThroughObstacleIntraNodeSolver::new_with_math(params, math)?))),
            "two-crossing" => Ok(Self::TwoCrossing(Box::new(TwoCrossingRoutesHighDensitySolver::new_with_math(params, math)?))),
            "transition-crossing" => Ok(Self::TransitionCrossing(Box::new(SingleTransitionCrossingRouteSolver::new_with_math(params, math)?))),
            "multi-head" => Ok(Self::MultiHead(Box::new(MultiHeadPolyLineIntraNodeSolver::new_with_math(params, math)?))),
            "multi-head2" => Ok(Self::MultiHead2(Box::new(MultiHeadPolyLineIntraNodeSolver2::new_with_math(params, math)?))),
            "multi-head3" => Ok(Self::MultiHead3(Box::new(MultiHeadPolyLineIntraNodeSolver3::new_with_math(params, math)?))),
            "via-possibilities2" => Ok(Self::ViaPossibilities2(Box::new(ViaPossibilitiesSolver2::new_with_math(params, math)?))),
    ...
    pub fn invoke(&mut self, method: &str, args: Vec<Value>) -> Result<(Value, Vec<Value>), String> {
        match self {
            Self::SingleLayer(_) | Self::SingleTransition(_) | Self::ThroughObstacle(_) =>
                crate::specialized_simple_dispatch::invoke(self, method, args),
            Self::TwoCrossing(_) | Self::TransitionCrossing(_) =>
                crate::specialized_crossing_dispatch::invoke(self, method, args),
            Self::MultiHead(_) | Self::MultiHead2(_) | Self::MultiHead3(_) | Self::ViaPossibilities2(_) =>
                crate::specialized_multi_head_dispatch::invoke(self, method, args),
        }
    }
    ...
    pub fn solved_routes(&self) -> Result<&[Value], String> {
        match self {
            ...
            Self::ViaPossibilities2(_) => Err("ViaPossibilitiesSolver2 exposes completedPaths, not solvedRoutes".into()),
        }
    }

    pub fn restore(&mut self, snapshot: Value, math: SpecializedMath) -> Result<(), String> {
        match self {
            Self::SingleLayer(solver) => **solver = serde_json::from_value(snapshot).map_err(|error| error.to_string())?,
            Self::SingleTransition(solver) => **solver = serde_json::from_value(snapshot).map_err(|error| error.to_string())?,
            Self::ThroughObstacle(solver) => **solver = serde_json::from_value(snapshot).map_err(|error| error.to_string())?,
