...
use crate::specialized_utils::{math::*,get_bounds_from_node_with_port_points::Bounds,
...
impl MultiHeadPolyLineIntraNodeSolver {
    ...
    pub fn setup_initial_poly_lines(&mut self)->Result<(),String> {
        ...
        for point in self.node_with_port_points["portPoints"].as_array().unwrap() {
            ...
            let point=MHPoint{diagnostic_id: crate::multi_head_poly_line_intra_node_solver::types1::next_diagnostic_id(),x:point["x"].as_f64().unwrap(),y:point["y"].as_f64().unwrap(),z1:z,z2:z,metadata};
        ...
        for (counts,positions) in reordered {
            ...
            for (index,(name,(start,end))) in pairs.iter().enumerate() {
                ...
                used+=count;
                lines.push(PolyLine{diagnostic_id: crate::multi_head_poly_line_intra_node_solver::types1::next_diagnostic_id(), m_points_id: crate::multi_head_poly_line_intra_node_solver::types1::next_diagnostic_id(),connection_name:name.clone(),start:start.clone(),end:end.clone(),m_points});
            }
