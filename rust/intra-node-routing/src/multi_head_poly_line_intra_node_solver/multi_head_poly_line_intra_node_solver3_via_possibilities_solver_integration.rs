...
use crate::specialized_utils::math::{SpecializedMath,distance};
...
impl MultiHeadPolyLineIntraNodeSolver {
    pub fn create_initial_candidate_from_seed(&mut self,seed:f64)->Result<Option<Candidate>,String> {
        ...
        for (name,path) in &via_solver.completed_paths {
            ...
            let start=MHPoint{diagnostic_id: crate::multi_head_poly_line_intra_node_solver::types1::next_diagnostic_id(),x:start_value.coordinates().0.unwrap(),y:start_value.coordinates().1.unwrap(),z1:start_z,z2:start_z,metadata:start_metadata};
            let end=MHPoint{diagnostic_id: crate::multi_head_poly_line_intra_node_solver::types1::next_diagnostic_id(),x:end_value.coordinates().0.unwrap(),y:end_value.coordinates().1.unwrap(),z1:end_z,z2:end_z,metadata:end_metadata};
            ...
            while index<middle.len() {
                ...
                points.push(MHPoint{diagnostic_id: crate::multi_head_poly_line_intra_node_solver::types1::next_diagnostic_id(),x:p.coordinates().0.unwrap(),y:p.coordinates().1.unwrap(),z1:last_z,z2,metadata:Map::new()});
            ...
            while segments<self.segments_per_polyline {
                ...
                let a=full[k];let b=full[k+1];
                let midpoint=MHPoint{diagnostic_id: crate::multi_head_poly_line_intra_node_solver::types1::next_diagnostic_id(),x:(a.x+b.x)/2.0,y:(a.y+b.y)/2.0,z1:a.z2,z2:a.z2,metadata:Map::new()};
                points.insert(k,midpoint);segments+=1;
            }
            poly_lines.push(PolyLine{diagnostic_id: crate::multi_head_poly_line_intra_node_solver::types1::next_diagnostic_id(), m_points_id: crate::multi_head_poly_line_intra_node_solver::types1::next_diagnostic_id(),connection_name:name.clone(),start,end,m_points:points});
        }
