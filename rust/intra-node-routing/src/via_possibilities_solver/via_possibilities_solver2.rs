...
impl ViaPossibilitiesSolver2 {
    ...
    pub fn new_with_math(params: Value, math: SpecializedMath) -> Result<Self, String> {
        Self::new_from_borrowed_inputs(
            &params["nodeWithPortPoints"], params.get("colorMap"),
            &params["hyperParameters"], &params["viaDiameter"], math,
        )
    }
    ...
    pub(crate) fn new_from_borrowed_inputs(
        ...
        if let Some(seed) = hyper_parameters["SHUFFLE_SEED"].as_f64().filter(|v| *v != 0.0 && !v.is_nan()) {
            unprocessed_connections = clone_and_shuffle_array(&unprocessed_connections, seed);
        }
        let mut solver = Self {
            base: BaseSolverState { max_iterations: 100_000.0, ..Default::default() },
            bounds, max_via_count: 5, port_pair_map,
            color_map: color_map.filter(|v| !v.is_null()).cloned().unwrap_or_else(|| generate_color_map_from_node_with_port_points(node)),
            node_width: bounds.max_x - bounds.min_x,
            available_z: node["availableZ"].as_array().map(|values| values.iter().map(|v| v.as_f64().expect("Layer number")).collect()).unwrap_or(vec![0.0,1.0]),
            hyper_parameters: if hyper_parameters.is_null() { json!({"SHUFFLE_SEED":0}) } else { hyper_parameters.clone() },
            via_intersection_buffer_distance: 0.05, placeholder_wall_buffer_distance: 0.1, new_head_wall_buffer_distance: 0.05,
            via_diameter: via_diameter.as_f64().unwrap_or(0.3), unprocessed_connections,
            completed_paths: IndexMap::new(), placeholder_paths: IndexMap::new(), current_head: Value::Null.into(),
            current_connection_name: String::new(), current_path: vec![], current_via_count: 0,
            stats: json!({"solutionsFound":0}), solved_routes: vec![], math,
        };
        for (name, pair) in &solver.port_pair_map {
            let start = &pair.start;
            let end = &pair.end;
            if end.is_null() { return Err(format!("Missing end port for {name}")); }
            let a = point(start);
            let b = point(end);
            let path = if start["z"].as_f64() == end["z"].as_f64() {
                if (a.x-b.x).abs() < 1e-9 || (a.y-b.y).abs() < 1e-9 {
                    vec![start.clone(), solver.pad_by_placeholder_wall_buffer(start), solver.pad_by_placeholder_wall_buffer(end), end.clone()]
                } else { vec![start.clone(), end.clone()] }
            } else {
                let x = (a.x+b.x)/2.0;
                let y = (a.y+b.y)/2.0;
                vec![start.clone(), solver.pad_by_placeholder_wall_buffer(start),
                    solver.pad_by_placeholder_wall_buffer(&json!({"x":x,"y":y,"z":start["z"]})),
                    solver.pad_by_placeholder_wall_buffer(&json!({"x":x,"y":y,"z":end["z"]})),
                    solver.pad_by_placeholder_wall_buffer(end), end.clone()]
            };
            solver.placeholder_paths.insert(name.clone(), path.into_iter().map(PathPoint::from).collect());
        }
        solver.current_connection_name = solver.unprocessed_connections.pop().ok_or("Missing initial connection")?;
        let start = solver.port_pair_map[&solver.current_connection_name].start.clone();
        ...
        solver.placeholder_paths.shift_remove(&solver.current_connection_name);
        Ok(solver)
    }
    ...
    pub fn visualize(&self, transparentize: &dyn Fn(&str, f64) -> String) -> Value {
        let mut points = vec![];
        let mut lines = vec![json!({"points":[{"x":self.bounds.min_x,"y":self.bounds.min_y},{"x":self.bounds.max_x,"y":self.bounds.min_y},
        ...
        for (prefix, paths) in [("Placeholder",&self.placeholder_paths),("Completed",&self.completed_paths)] {
            for (name, path) in paths {
                let color = self.color_map[name].as_str().unwrap_or("black");
                for pair in path.windows(2) {
                    let (a,b) = (&pair[0],&pair[1]);
                    let z = js_number_to_string(a["z"].as_f64().unwrap());
                    if a["x"].as_f64() == b["x"].as_f64() && a["y"].as_f64() == b["y"].as_f64() && a["z"].as_f64() != b["z"].as_f64() {
                        ...
                        let mut line = json!({"points":[a,b],"strokeColor":transparentize(color,0.5)});
                        if a["z"].as_f64() != Some(0.0) { line["strokeDash"] = json!([0.1,0.1]); }
...
impl SpecializedSolver for ViaPossibilitiesSolver2 {
    ...
    fn get_solver_name(&self) -> &'static str { "ViaPossibilitiesSolver2" }

    fn _step(&mut self) -> Result<(), String> {
        if self.base.solved { return Ok(()); }
        let end = self.port_pair_map[&self.current_connection_name].end.clone();
        ...
        for paths in [&self.completed_paths,&self.placeholder_paths] {
            for path in paths.values() {
                for pair in path.windows(2) {
                    ...
                    if a.z != head_z { continue; }
                    if let Some(intersection) = get_segment_intersection(head,target,a.point(),b.point()) {
                        let d = distance(head,intersection);
                        if d < 1e-6 { continue; }
                        if closest.as_ref().is_none_or(|(_,best,_)| d < *best) { closest = Some((intersection,d,a.z)); }
                    }
                }
            }
        }
        ...
        if let Some((intersection,d,intersected_z)) = closest {
            let (x,y) = if d <= self.via_intersection_buffer_distance + 1e-6 { ((head.x+intersection.x)/2.0,(head.y+intersection.y)/2.0) }
                else { let ratio = (d-self.via_intersection_buffer_distance)/d; (head.x+(intersection.x-head.x)*ratio,head.y+(intersection.y-head.y)*ratio) };
            let Some(next_z) = self.available_z.iter().find(|&&z| Some(z) != intersected_z).copied() else {
                self.base.error = Some("Could not determine next Z level for via placement!".into()); self.base.failed = true; return Ok(());
            };
            let first = PathPoint::from(json!({"x":x,"y":y,"z":self.current_head["z"]}));
            let second = PathPoint::from(json!({"x":x,"y":y,"z":next_z}));
            self.current_path.extend([first,second.clone()]); self.current_head = second;
        } else if needs_z_change {
            let d = distance(head,target);
            let (x,y) = if d < self.via_intersection_buffer_distance { ((head.x+target.x)/2.0,(head.y+target.y)/2.0) }
                else { let ratio = (d-self.via_intersection_buffer_distance)/d; (head.x+(target.x-head.x)*ratio,head.y+(target.y-head.y)*ratio) };
            let first = PathPoint::from(json!({"x":x,"y":y,"z":self.current_head["z"]}));
            let second = PathPoint::from(json!({"x":x,"y":y,"z":end["z"]}));
            self.current_path.extend([first,second.clone()]); self.current_head = second;
        } else {
            self.current_path.push(end.into());
            self.completed_paths.insert(self.current_connection_name.clone(),self.current_path.clone());
            if self.unprocessed_connections.is_empty() { self.base.solved = true; self.stats["solutionsFound"] = json!(1); }
            else {
                self.current_connection_name = self.unprocessed_connections.pop().unwrap();
                let start = self.port_pair_map[&self.current_connection_name].start.clone();
                self.current_head = self.pad_by_new_head_wall_buffer(&start).into();
                self.current_path = vec![start.into(),self.current_head.clone()]; self.current_via_count = 0;
                self.placeholder_paths.shift_remove(&self.current_connection_name);
            }
        }
        Ok(())
    }
...
mod tests {
    ...
    fn typed_paths_preserve_raw_metadata_restore_and_skipped_invalid_points() {
        let raw: Value = serde_json::from_str(r#"{"metadata":{"label":"retained"},"y":-0.0,"x":1.25,"z":0,"tail":[1,null]}"#).unwrap();
        let typed = PathPoint::from(raw.clone());
        assert_eq!(serde_json::to_string(&typed).unwrap(), serde_json::to_string(&raw).unwrap());
        ...
        let borrowed = ViaPossibilitiesSolver2::new_from_borrowed_inputs(
            &borrowed_node, None, &Value::Null, &Value::Null, SpecializedMath::default(),
        ).unwrap();
        assert_eq!(serde_json::to_value(&borrowed).unwrap(), serde_json::to_value(&solver).unwrap());
        ...
        assert_eq!(solver.current_head.coordinates(), (Some(-0.75),Some(0.0),Some(1.0)));
        ...
        let padded = solver.pad_by_new_head_wall_buffer(&json!({"x":-9,"y":9,"z":"raw-layer","extra":true}));
