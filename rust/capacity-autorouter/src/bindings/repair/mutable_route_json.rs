...
pub fn js_json_routes(routes: &[MutableRoute]) -> String {
    ...
    for (index, route) in routes.iter().enumerate() {
        ...
        for key in ["route", "vias"] {
            ...
            output.push(':');
            if key == "route" {
                write_points(route, &mut output);
            } else {
                write_vias(&route.vias, &mut output);
            }
        }
        output.push('}');
    }
...
mod tests {
    ...
    fn previous_writer(value: &Value, output: &mut String) {
        match value {
            Value::Number(number) => {
                output.push_str(&js_number_to_string(number.as_f64().unwrap()))
            }
            Value::Array(values) => {
                output.push('[');
    ...
    fn appended_json_matches_previous_bytes_for_metadata_and_numeric_overlays() {
        ...
        for number in numbers {
            ...
            for omit_overlay_keys in [false, true] {
                ...
                metadata.insert(
                    escaped.clone(),
                    json!([escaped, true, false, null, {"nested": [number]}]),
                );
