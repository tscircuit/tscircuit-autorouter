...
pub fn get_bounds_from_node_with_port_points(node: &Value) -> Bounds {
    let x = node["center"]["x"].as_f64().expect("Node center.x required");
    let y = node["center"]["y"].as_f64().expect("Node center.y required");
    let width = node["width"].as_f64().expect("Node width required");
    let height = node["height"].as_f64().expect("Node height required");
    let mut bounds = Bounds { min_x: x - width / 2.0, max_x: x + width / 2.0, min_y: y - height / 2.0, max_y: y + height / 2.0 };
