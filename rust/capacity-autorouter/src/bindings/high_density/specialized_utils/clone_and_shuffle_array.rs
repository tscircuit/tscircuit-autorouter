...
pub fn clone_and_shuffle_array<T: Clone>(array: &[T], seed: f64) -> Vec<T> {
    ...
    for index in 0..shuffled.len() {
        let first = (random.next_f64() * shuffled.len() as f64).floor() as usize;
        let second = (random.next_f64() * (index + 1) as f64).floor() as usize;
