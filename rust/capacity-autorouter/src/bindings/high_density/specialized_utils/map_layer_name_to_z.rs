pub fn map_layer_name_to_z(name: &str, layer_count: f64) -> f64 {
    if name == "top" { return 0.0; }
    if name == "bottom" { return layer_count - 1.0; }
    let suffix: String = String::from_utf16_lossy(&name.encode_utf16().skip(5).collect::<Vec<_>>());
    let mut value = suffix.trim_start();
    let sign = if let Some(rest) = value.strip_prefix('-') { value = rest; -1.0 }
        else { if let Some(rest) = value.strip_prefix('+') { value = rest; } 1.0 };
    let radix = if value.starts_with("0x") || value.starts_with("0X") { value = &value[2..]; 16 } else { 10 };
    let mut number = 0.0;
    let mut found = false;
...
pub fn get_unique_valid_z_layers_from_layer_names(names: &[&str], layer_count: f64) -> Vec<f64> {
    get_unique_valid_z_layers(&names.iter().map(|name| map_layer_name_to_z(name, layer_count)).collect::<Vec<_>>(), layer_count)
}
