pub use crate::bindings::high_density::specialized_utils::math::Bounds;
pub use crate::solvers::high_density_solver::connectivity_map::ConnectivityMap;
pub use crate::types::high_density_types::{Point, Point2};
...
pub struct CloneGroup {
    pub identity: u64,
    pub sources: RefCell<Vec<RouteRef>>,
}
...
pub fn reserve_identity(identity: u64) {
    ...
    if identity < (1_u64 << 40) {
        NEXT_ID.with(|next| next.set(next.get().max(identity + 1)));
    }
}
...
pub fn point2(p: &PointRef) -> Point2 {
    let p = p.borrow();
    Point2 { x: p.x, y: p.y }
}
pub fn point3(p: &PointRef) -> Point {
    let p = p.borrow();
    Point {
        x: p.x,
        y: p.y,
...
pub fn point_from_value(value: &Value) -> PointRef {
    point_from_owned_value(value.clone())
}
pub fn point_from_owned_value(value: Value) -> PointRef {
    Rc::new(RefCell::new(RoutePoint {
        x: value["x"].as_f64().unwrap_or(f64::NAN),
        y: value["y"].as_f64().unwrap_or(f64::NAN),
        z: value["z"].as_f64().unwrap_or(f64::NAN),
        identity: next_identity(),
        source_identity: None,
        source: None,
        metadata_source_identity: None,
        segment_metadata_obstacle: None,
        removed_segment_properties: 0,
        clone_group: None,
        metadata: Rc::new(value),
    }))
}
pub fn fresh_point(x: f64, y: f64, z: f64) -> PointRef {
    point_from_owned_value(json!({"x":x,"y":y,"z":z}))
}
pub fn fresh_via(x: f64, y: f64) -> PointRef {
    point_from_owned_value(json!({"x":x,"y":y}))
}
...
pub fn point_to_value(point: &PointRef) -> Value {
    ...
    value["x"] = json!(point.x);
    value["y"] = json!(point.y);
    if value.get("z").is_some() || !point.z.is_nan() {
        value["z"] = json!(point.z);
    }
    value
}
pub fn route_from_value(value: &Value) -> RouteRef {
    ...
    let metadata = value
        .as_object()
        .expect("Route metadata object")
        .iter()
        .map(|(key, entry)| {
            let entry = if key == "route" || key == "vias" {
                Value::Null
            } else {
                entry.clone()
            };
    ...
    Rc::new(RefCell::new(Route {
        ...
        route_array_identity: next_identity(),
...
pub fn route_to_value(route: &RouteRef) -> Value {
    ...
    if route.root_connection_name.is_some() || value.get("rootConnectionName").is_some() {
        value["rootConnectionName"] = json!(route.root_connection_name);
    }
    if !route.trace_thickness.is_nan() || value.get("traceThickness").is_some() {
        value["traceThickness"] = json!(route.trace_thickness);
    }
    if !route.via_diameter.is_nan() || value.get("viaDiameter").is_some() {
        value["viaDiameter"] = json!(route.via_diameter);
    }
    ...
    if let Some(jumpers) = &route.jumpers {
        value["jumpers"] = jumpers.borrow().clone();
    }
    value
}
pub fn structured_clone_routes(routes: &[RouteRef]) -> Vec<RouteRef> {
    ...
    let group = Rc::new(CloneGroup {
        identity: next_identity(),
        sources: RefCell::new(routes.to_vec()),
    });
    routes
        .iter()
        .map(|route| {
            ...
            let copy = spread_route(route);
            {
                ...
                copy.clone_group = Some(group.clone());
                copy.source = None;
                copy.source_metadata_identity = None;
                if let (Some(id), Some(value)) = (copy.jumpers_identity, copy.jumpers.clone()) {
                    copy.jumpers = Some(
                        jumpers
                            .entry(id)
                            .or_insert_with(|| Rc::new(RefCell::new(value.borrow().clone())))
                            .clone(),
                    );
                    copy.jumpers_identity = Some(*arrays.entry(id).or_insert_with(next_identity));
                }
                copy.route_array_identity = *arrays
                    .entry(copy.route_array_identity)
                    .or_insert_with(next_identity);
                copy.vias_array_identity = *arrays
                    .entry(copy.vias_array_identity)
                    .or_insert_with(next_identity);
                let Route {
                    route: route_points,
                    vias,
...
pub fn structured_clone_route(route: &RouteRef) -> RouteRef {
    structured_clone_routes(std::slice::from_ref(route)).remove(0)
}
pub fn obstacle_from_value(value: &Value) -> ObstacleRef {
    Rc::new(RefCell::new(Obstacle {
        center: Point2 {
            x: value["center"]["x"].as_f64().unwrap_or(f64::NAN),
            y: value["center"]["y"].as_f64().unwrap_or(f64::NAN),
        },
        width: value["width"].as_f64().unwrap_or(f64::NAN),
        height: value["height"].as_f64().unwrap_or(f64::NAN),
        layers: value["layers"]
            .as_array()
            .map(|v| {
                v.iter()
                    .map(|v| v.as_str().expect("Layer string").to_owned())
                    .collect()
            })
            .unwrap_or_default(),
        z_layers: value["__zLayers"]
            .as_array()
            .map(|v| v.iter().map(|v| v.as_f64().unwrap_or(f64::NAN)).collect())
            .unwrap_or_default(),
        z_layers_identity: value["__zLayers"].as_array().map(|_| next_identity()),
        connected_to: Rc::new(
            value["connectedTo"]
                .as_array()
                .expect("Obstacle connectedTo required")
                .iter()
                .map(|v| v.as_str().expect("Connection ID").to_owned())
                .collect(),
        ),
        identity: next_identity(),
        source_identity: None,
...
impl Route {
    pub fn has_jumpers(&self) -> bool {
        self.jumpers.as_ref().is_some_and(|value| {
            value
                .borrow()
                .as_array()
    ...
    pub fn jumpers_value(&self) -> Option<Value> {
        self.jumpers.as_ref().map(|value| value.borrow().clone())
    }
}
...
mod representation_tests {
    ...
    fn owned_point_construction_preserves_metadata_order_and_coordinate_presence() {
        ...
        assert_eq!(
            serde_json::to_string(&point_to_value(&borrowed)).unwrap(),
            serde_json::to_string(&point_to_value(&owned)).unwrap()
        );
        let point = fresh_point(1.0, 2.0, f64::NAN);
        let via = fresh_via(1.0, 2.0);
        assert_eq!(
            serde_json::to_string(&point_to_value(&point)).unwrap(),
            "{\"x\":1.0,\"y\":2.0,\"z\":null}"
        );
        assert_eq!(
            serde_json::to_string(&point_to_value(&via)).unwrap(),
            "{\"x\":1.0,\"y\":2.0}"
        );
    ...
    fn structured_clone_keeps_identity_and_cow_metadata_without_duplicate_point_graphs() {
        ...
        assert_eq!(
            serde_json::to_string(&route_to_value(&original)).unwrap(),
        ...
        assert!(!Rc::ptr_eq(&original, &cloned[0]));
        {
            ...
            assert!(Rc::ptr_eq(
                &source.route[0].borrow().metadata,
                &copy.route[0].borrow().metadata
            ));
            assert!(!Rc::ptr_eq(
                source.jumpers.as_ref().unwrap(),
                copy.jumpers.as_ref().unwrap()
            ));
        ...
        assert_eq!(
            original.borrow().route[0].borrow().metadata["label"]["name"],
            "a"
        );
        assert_eq!(original.borrow().metadata["extra"]["nested"], json!([1, 2]));
        assert_eq!(
            original.borrow().jumpers.as_ref().unwrap().borrow()[0]["id"],
            "j"
        );
    }
}
