...
use crate::bindings::trace_simplification::math_utils::{do_segments_intersect, min, max};
...
fn point_to_segment_distance_sq(p: Point2, a: Point2, b: Point2) -> f64 {
    ...
    if l2 == 0.0 { return compute_dist_sq(p, a); }
    let mut t = ((p.x - a.x) * (b.x - a.x) + (p.y - a.y) * (b.y - a.y)) / l2;
    t = max(0.0, min(1.0, t));
    let projection = Point2 { x: a.x + t * (b.x - a.x), y: a.y + t * (b.y - a.y) };
...
fn segment_to_segment_distance_sq(a: &Point, b: &Point, c: &Point, d: &Point) -> f64 {
    ...
    if do_segments_intersect(pa, pb, pc, pd) { return 0.0; }
    min(min(min(point_to_segment_distance_sq(pa, pc, pd),
...
impl HighDensityRouteSpatialIndex {
    pub fn new(routes: Vec<RouteRef>, cell_size: f64) -> Self {
        let mut index = Self { identity: crate::bindings::trace_simplification::types::next_identity(), segment_buckets: IndexMap::new(), via_buckets: IndexMap::new(), cell_size, maximum_copper_radius: 0.0 };
        for route in routes { index.add_route(route); }
        index
    }

    pub fn add_route(&mut self, route_ref: RouteRef) {
        let route = route_ref.borrow();
        if route.connection_name.is_empty() { return; }
        self.maximum_copper_radius = max(max(self.maximum_copper_radius, route.trace_thickness / 2.0), route.via_diameter / 2.0);
        let epsilon = 1e-9;
        for i in 0..route.route.len().saturating_sub(1) {
            let p1 = route.route[i].borrow();
            let p2 = route.route[i + 1].borrow();
            if p1.x == p2.x && p1.y == p2.y { continue; }
            if p1.metadata["insideJumperPad"].as_bool().unwrap_or(false) && p2.metadata["insideJumperPad"].as_bool().unwrap_or(false) { continue; }
        ...
        for (i, via) in route.vias.iter().enumerate() {
            ...
            let stored_via = Rc::new(StoredVia {
                via_id: format!("{}-via-{}", route.connection_name, i), x: via.x, y: via.y, parent_route: route_ref.clone(),
            });
    ...
    pub fn remove_route(&mut self, connection_name: &str) {
        self.segment_buckets.retain(|_, segments| {
            segments.retain(|segment| segment.parent_route.borrow().connection_name != connection_name);
            !segments.is_empty()
        });
    ...
    pub fn get_conflicting_routes_for_segment(&self, segment_start: &Point, segment_end: &Point, margin: f64) -> Vec<RouteConflict> {
            ...
            for iy in min_index_y..=max_index_y {
                if let Some(segment_bucket_list) = self.segment_buckets.get(&(ix, iy)) {
                    for segment_info in segment_bucket_list {
                        if !checked_segments.insert(&segment_info.segment_id) { continue; }
                        let route = segment_info.parent_route.borrow();
                        let p1 = point3(&segment_info.segment[0]);
                        let p2 = point3(&segment_info.segment[1]);
                        if segment_start.z != segment_end.z || p1.z != p2.z || p1.z != segment_start.z { continue; }
                        ...
                        if dist_sq < required_separation_sq {
                            ...
                            if existing.is_none() || dist_sq < existing.unwrap().1 {
                                conflicting_route_data.insert(route.connection_name.clone(), (segment_info.parent_route.clone(), dist_sq));
                            }
                        }
                    }
                }
                if let Some(via_bucket_list) = self.via_buckets.get(&(ix, iy)) {
                    for via_info in via_bucket_list {
                        if !checked_vias.insert(&via_info.via_id) { continue; }
                        ...
                        if dist_sq < required_separation_sq {
                            let existing = conflicting_route_data.get(&route.connection_name);
                            if existing.is_none() || dist_sq < existing.unwrap().1 {
                                conflicting_route_data.insert(route.connection_name.clone(), (via_info.parent_route.clone(), dist_sq));
                            }
                        }
                    }
                }
        ...
        conflicting_route_data.into_values().map(|(route, min_dist_sq)| RouteConflict { conflicting_route: route, distance: min_dist_sq.sqrt() }).collect()
    }
            ...
            for iy in min_index_y..=max_index_y {
                if let Some(segment_bucket_list) = self.segment_buckets.get(&(ix, iy)) {
                    for segment_info in segment_bucket_list {
                        ...
                        let p1 = point3(&segment_info.segment[0]);
                        let p2 = point3(&segment_info.segment[1]);
                        if p1.z != p2.z || p1.z != point.z { continue; }
                        let route = segment_info.parent_route.borrow();
                        let required_separation = margin + route.trace_thickness / 2.0;
                        let required_separation_sq = required_separation * required_separation;
                        let dist_sq = point_to_segment_distance_sq(query_point, Point2 { x: p1.x, y: p1.y }, Point2 { x: p2.x, y: p2.y });
                        if dist_sq < required_separation_sq {
                            let existing = conflicting_route_data.get(&route.connection_name);
                            if existing.is_none() || dist_sq < existing.unwrap().1 {
                                conflicting_route_data.insert(route.connection_name.clone(), (segment_info.parent_route.clone(), dist_sq));
...
pub trait RouteClearanceIndex {
    fn get_conflicting_routes_for_segment(&self,start:&Point,end:&Point,margin:f64)->Vec<RouteConflict>;
...
impl RouteClearanceIndex for HighDensityRouteSpatialIndex {
    fn get_conflicting_routes_for_segment(&self,start:&Point,end:&Point,margin:f64)->Vec<RouteConflict>{HighDensityRouteSpatialIndex::get_conflicting_routes_for_segment(self,start,end,margin)}
    fn get_conflicting_routes_near_point(&self,point:&Point,margin:f64)->Vec<RouteConflict>{HighDensityRouteSpatialIndex::get_conflicting_routes_near_point(self,point,margin)}
}
