type ObstacleQuery = Rc<dyn Fn(u64, &str, &[f64]) -> Result<Vec<ObstacleRef>, String>>;

use crate::bindings::trace_simplification::shared_maps::{
    LayerSet, NetByConnectionName, TerminalLayers,
};
...
pub struct GraphCodec {
    ...
    pub obstacle_query: Option<ObstacleQuery>,
}
...
fn reference_id(value: &Value, kind: &str) -> Result<u64, String> {
    ...
    reference
        ...
        .ok_or_else(|| format!("Invalid {kind} identity"))
}
...
fn array_items(value: &Value) -> Result<&[Value], String> {
    value
        ...
        .map(Vec::as_slice)
        .ok_or_else(|| "Expected identity array".to_owned())
}
...
impl GraphCodec {
    ...
    fn clone_group(&mut self, group: &Rc<CloneGroup>) {
        if self.acknowledged_clone_groups.contains(&group.identity)
            || self.clone_groups.contains_key(&group.identity)
        {
            return;
        }
        self.emitted_clone_groups.insert(group.identity);
        self.clone_group_refs
            .insert(group.identity, Rc::downgrade(group));
        ...
        let sources = Value::Array(
            group
                ...
                .borrow()
                .iter()
                .map(|route| self.route_with_source(route, true))
                .collect(),
        );
        self.clone_groups.insert(
            group.identity,
            json!({"id":group.identity,"sources":sources}),
        );
    }
    ...
    pub fn acknowledge_clone_groups(&mut self, identities: &[u64]) -> Result<(), String> {
        ...
        self.acknowledged_clone_groups
            .extend(identities.iter().copied());
    ...
    pub fn connectivity(&mut self, map: &Rc<ConnectivityMap>) -> Value {
        let identity = self
            .connectivity_refs
            .iter()
            .find_map(|(id, existing)| Rc::ptr_eq(existing, map).then_some(*id))
            .unwrap_or_else(next_identity);
    ...
    pub fn read_connectivity(&mut self, value: &Value) -> Result<Rc<ConnectivityMap>, String> {
        let incoming: ConnectivityMap =
            serde_json::from_value(self.read_raw(value)).map_err(|error| error.to_string())?;
        let identity = value["$connectivityId"]
            .as_u64()
            .unwrap_or_else(next_identity);
    ...
    pub fn net_names(&mut self, map: &Rc<NetByConnectionName>) -> Value {
        let id = map
            ...
            .or_else(|| {
                self.net_names_refs
                    .iter()
                    .find_map(|(id, item)| Rc::ptr_eq(item, map).then_some(*id))
            })
            .unwrap_or_else(next_identity);
    ...
    pub fn read_net_names(&mut self, value: &Value) -> Result<Rc<NetByConnectionName>, String> {
        let incoming: IndexMap<String, String> =
            serde_json::from_value(self.read_raw(value)).map_err(|e| e.to_string())?;
        let id = value["$netNamesId"].as_u64().unwrap_or_else(next_identity);
        if let Some(existing) = self.net_names_refs.get(&id) {
            if !existing.iter().eq(incoming.iter()) {
    ...
    fn layer_set(&mut self, set: &Rc<LayerSet>) -> Value {
        let id = set
            ...
            .or_else(|| {
                self.layer_set_refs
                    .iter()
                    .find_map(|(id, item)| Rc::ptr_eq(item, set).then_some(*id))
            })
            .unwrap_or_else(next_identity);
    ...
    pub fn read_layer_set(&mut self, value: &Value) -> Result<Rc<LayerSet>, String> {
        ...
        if let Some(existing) = self.layer_set_refs.get(&id) {
            if existing.as_slice() != incoming.as_slice() {
                existing.replace(incoming);
            }
            return Ok(existing.clone());
        }
    ...
    pub fn terminal_layers(&mut self, map: &Rc<TerminalLayers>) -> Value {
        ...
        let values: serde_json::Map<String, Value> = map
            .iter()
            .map(|(key, set)| (key.clone(), self.layer_set(set)))
            .collect();
    ...
    pub fn read_terminal_layers(&mut self, value: &Value) -> Result<Rc<TerminalLayers>, String> {
        ...
        let id = value["$terminalLayersId"]
            .as_u64()
            .unwrap_or_else(next_identity);
        if let Some(existing) = self.terminal_layers_refs.get(&id) {
            let unchanged = existing.len() == incoming.len()
                && existing
                    .iter()
                    .zip(incoming.iter())
    ...
    pub fn read_color_map(&mut self, value: &Value) -> Result<ColorMapRef, String> {
        ...
        if !incoming.is_object() {
            return Err("Color map must be an object".into());
        }
        if let Some(map) = self.color_map_refs.get(&identity) {
            *map.value.borrow_mut() = incoming;
            return Ok(map.clone());
        }
        let map = Rc::new(ColorMap {
            identity,
            value: RefCell::new(incoming),
        });
    ...
    fn import_connectivity(&mut self, value: &Value) -> Result<(), String> {
        if value.get("$colorMapId").is_some() {
            self.read_color_map(value)?;
        } else if value.get("$connectivityId").is_some() {
            self.read_connectivity(value)?;
        } else if value.get("$netNamesId").is_some() {
            self.read_net_names(value)?;
        } else if value.get("$terminalLayersId").is_some() {
            self.read_terminal_layers(value)?;
        } else if value.get("$layerSetId").is_some() {
            self.read_layer_set(value)?;
        } else if value.get("$raw").is_none() {
            if let Some(fields) = value.as_object() {
                for field in fields.values() {
                    self.import_connectivity(field)?;
                }
            } else if let Some(items) = value.as_array() {
                for item in items {
                    self.import_connectivity(item)?;
                }
            }
        }
        Ok(())
    ...
    pub fn point(&mut self, point: &PointRef) -> Value {
        self.point_with_source(point, false)
    }
    ...
    fn point_with_source(&mut self, point: &PointRef, source_only: bool) -> Value {
        ...
        if self
            .points
            .get(&id)
            .is_some_and(|record| source_only || record["sourceOnly"] == Value::Bool(false))
        {
            return json!({"$ref":["point",id]});
        }
        self.point_refs.insert(id, Rc::downgrade(point));
        if let Some(group) = &p.clone_group {
            self.clone_group(group);
        }
        if !p
            .clone_group
            .as_ref()
            .is_some_and(|group| self.acknowledged_clone_groups.contains(&group.identity))
            && let Some(source) = &p.source
        {
            self.point_with_source(source, true);
        }
        if let Some(obstacle) = &p.segment_metadata_obstacle {
            self.obstacle(obstacle);
        }
        let removed_properties: &[&str] = match p.removed_segment_properties {
            0 => &[],
            1 => &["toNextSegmentType"],
            2 => &["toNextSegmentCircuitJsonMetadata"],
            _ => &["toNextSegmentType", "toNextSegmentCircuitJsonMetadata"],
        };
    ...
    pub fn route(&mut self, route: &RouteRef) -> Value {
        self.route_with_source(route, false)
    }
    ...
    fn route_with_source(&mut self, route: &RouteRef, source_only: bool) -> Value {
        ...
        if self
            ...
            .is_some_and(|record| source_only || record["sourceOnly"] == Value::Bool(false))
        {
            return json!({"$ref":["route",id]});
        }
        self.route_refs.insert(id, Rc::downgrade(route));
        if let Some(group) = &r.clone_group {
            self.clone_group(group);
        }
        if !r
            .clone_group
            .as_ref()
            .is_some_and(|group| self.acknowledged_clone_groups.contains(&group.identity))
            && let Some(source) = &r.source
        {
            self.route_with_source(source, true);
        }
        ...
        if let (Some(id), Some(jumpers)) = (r.jumpers_identity, &r.jumpers) {
            ...
            self.jumpers
                .insert(id, json!({"id":id,"value":*jumpers.borrow()}));
            value.insert("jumpers".into(), jumpers.borrow().clone());
        }
        value.insert("connectionName".into(), json!(r.connection_name));
        if r.root_connection_name.is_some() || value.contains_key("rootConnectionName") {
            value.insert("rootConnectionName".into(), json!(r.root_connection_name));
        }
        if !r.trace_thickness.is_nan() || value.contains_key("traceThickness") {
            value.insert("traceThickness".into(), json!(r.trace_thickness));
        }
        if !r.via_diameter.is_nan() || value.contains_key("viaDiameter") {
            value.insert("viaDiameter".into(), json!(r.via_diameter));
        }
        let mut property_order: Vec<String> =
            r.metadata.as_object().unwrap().keys().cloned().collect();
        for key in value.keys().map(String::as_str).chain(["route", "vias"]) {
            if !property_order.iter().any(|existing| existing == key) {
                property_order.push(key.to_owned());
            }
        }
    ...
    pub fn raw(&self, value: Value) -> Value {
        json!({"$raw":value})
    }
    pub fn read_raw(&self, value: &Value) -> Value {
        value.get("$raw").unwrap_or(value).clone()
    }
    ...
    pub fn obstacle(&mut self, obstacle: &ObstacleRef) -> Value {
        ...
        if value.get("layers").is_some() {
            value["layers"] = json!(o.layers);
        }
        if value.get("__zLayers").is_some() {
            value["__zLayers"] = json!(o.z_layers);
        }
    ...
    pub fn obstacles(&mut self, obstacles: &[ObstacleRef]) -> Value {
        Value::Array(
            obstacles
                .iter()
                .map(|obstacle| self.obstacle(obstacle))
                .collect(),
        )
    }
    ...
    pub fn read_point(&mut self, value: &Value) -> Result<PointRef, String> {
        ...
        self.point_refs
            ...
            .ok_or_else(|| format!("Unknown point identity {id}"))
    }
    ...
    pub fn read_route(&mut self, value: &Value) -> Result<RouteRef, String> {
        ...
        self.route_refs
            .get(&id)
            .and_then(Weak::upgrade)
            .ok_or_else(|| format!("Unknown route identity {id}"))
    }

    pub fn read_points(&mut self, value: &Value) -> Result<Vec<PointRef>, String> {
        array_items(value)?
            .iter()
            .map(|entry| self.read_point(entry))
            .collect()
    }
    ...
    pub fn read_routes(&mut self, value: &Value) -> Result<Vec<RouteRef>, String> {
        array_items(value)?
            .iter()
            .map(|entry| self.read_route(entry))
            .collect()
    }
    ...
    pub fn read_obstacle(&mut self, value: &Value) -> Result<ObstacleRef, String> {
        ...
        self.obstacle_refs
            .get(&id)
            .cloned()
            .ok_or_else(|| format!("Unknown obstacle identity {id}"))
    }

    pub fn read_obstacles(&mut self, value: &Value) -> Result<Vec<ObstacleRef>, String> {
        array_items(value)?
            .iter()
            .map(|entry| self.read_obstacle(entry))
            .collect()
    }
    ...
    pub fn import_graph(&mut self, graph: &Value) -> Result<ImportedGraph, String> {
        let mut imported = ImportedGraph {
            points: Vec::new(),
            routes: Vec::new(),
        };
        ...
        if let Some(jumpers) = graph["jumpers"].as_array() {
            for record in jumpers {
                let id = record["id"]
                    .as_u64()
                    .ok_or("Invalid jumper array identity")?;
                let value = record["value"].clone();
                if let Some(existing) = self.jumper_refs.get(&id) {
                    *existing.borrow_mut() = value;
                } else {
                    self.jumper_refs.insert(id, Rc::new(RefCell::new(value)));
                }
            }
        }
        ...
        for record in points {
            ...
            point.identity = id;
            point.source_identity = record["sourceId"].as_u64();
            if point.source_identity.is_none() {
                point.source = None;
                point.clone_group = None;
            }
        ...
        for record in routes {
            ...
            let route_array_identity = record["route"]["$array"]
                .as_u64()
                .ok_or("Missing route array identity")?;
            let vias_array_identity = record["vias"]["$array"]
                .as_u64()
                .ok_or("Missing vias array identity")?;
            ...
            if let Some(order) = record["propertyOrder"].as_array() {
                for key in order {
                    ...
                    if key == "route" || key == "vias" {
                        value[key] = json!([]);
                    } else if let Some(entry) = record["value"].get(key) {
                        value[key] = entry.clone();
                    }
                }
            }
            for (key, entry) in record["value"]
                .as_object()
                .ok_or("Invalid route metadata")?
            {
            ...
            if let Some(id) = record["jumpersArrayId"].as_u64() {
                ...
                *cell.borrow_mut() = value;
                route.jumpers = Some(cell);
                route.jumpers_identity = Some(id);
            }
            route.identity = id;
            route.source_identity = record["sourceId"].as_u64();
            if route.source_identity.is_none() {
                route.source = None;
                route.clone_group = None;
            }
            route.source_metadata_identity = record["metadataSourceId"].as_u64();
            route.route_array_identity = route_array_identity;
            route.vias_array_identity = vias_array_identity;
        }
        ...
        for record in obstacles {
            ...
            incoming.z_layers_identity = record["zLayersArrayId"]
                .as_u64()
                .or(incoming.z_layers_identity);
            ...
            if let Some(existing) = self.obstacle_refs.get(&id) {
                ...
                *existing = incoming;
                ...
                self.obstacle_refs
                    .insert(id, Rc::new(std::cell::RefCell::new(incoming)));
            }
        }
        ...
        if !imported_layers.is_empty() {
            for obstacle in self.obstacle_refs.values() {
                let changed_layers = {
                    ...
                    obstacle
                        .z_layers_identity
                        .and_then(|id| imported_layers.get(&id))
...
mod shared_map_tests {
    ...
    fn deferred_new_set_keeps_source_identity_when_main_codec_observes_it() {
        ...
        let map = main
            .read_terminal_layers(
                &json!({"$terminalLayersId":100,"$raw":{"port":{"$layerSetId":101,"$raw":[0]}}}),
            )
            .unwrap();
        ...
        deferred
            .read_terminal_layers(
                &json!({"$terminalLayersId":100,"$raw":{"port":{"$layerSetId":102,"$raw":[0,1]}}}),
            )
            .unwrap();
        let observed = main.terminal_layers(&map);
        assert_eq!(observed["$raw"]["port"]["$layerSetId"], json!(102));
        deferred
            .read_layer_set(&json!({"$layerSetId":102,"$raw":[1]}))
            .unwrap();
        assert_eq!(
            main.terminal_layers(&map)["$raw"]["port"]["$raw"],
            json!([1.0])
        );
    }
}
...
mod geometry_ownership_tests {
    ...
    fn acknowledged_clone_releases_source_geometry_but_keeps_metadata() {
        let source = route_from_value(
            &json!({"connectionName":"n","traceThickness":0.1,"viaDiameter":0.3,"tag":{"value":1},"route":[{"x":0,"y":0,"z":0},{"x":1,"y":0,"z":0}],"vias":[]}),
        );
        let old_route = Rc::downgrade(&source);
        let old_point = Rc::downgrade(&source.borrow().route[0]);
        let copies = structured_clone_routes(std::slice::from_ref(&source));
    ...
    fn imported_guard_keeps_unattached_geometry_alive_and_dead_records_recreate() {
        let route = route_from_value(
            &json!({"connectionName":"n","traceThickness":0.1,"viaDiameter":0.3,"route":[{"x":0,"y":0,"z":0}],"vias":[]}),
        );
        ...
        assert!(Rc::ptr_eq(
            &first,
            &codec.read_route(&packet["fields"]).unwrap()
        ));
        ...
        assert_eq!(
            serde_json::to_string(&route_to_value(&recreated)).unwrap(),
...
mod normalized_obstacle_identity_tests {
    ...
    fn normalized_layers_are_fresh_except_shared_fallback_and_import_updates_aliases() {
        let source = obstacle_from_value(
            &json!({"center":{"x":0,"y":0},"width":1,"height":1,"connectedTo":[],"__zLayers":[0]}),
        );
        let empty = obstacle_from_value(
            &json!({"center":{"x":1,"y":0},"width":1,"height":1,"connectedTo":[],"__zLayers":[]}),
        );
        let normalized = normalize_obstacles(
            vec![source.clone(), source.clone(), empty.clone(), empty],
            2.0,
        );
        assert_ne!(
            normalized[0].borrow().z_layers_identity,
            source.borrow().z_layers_identity
        );
        assert_ne!(
            normalized[0].borrow().z_layers_identity,
            normalized[1].borrow().z_layers_identity
        );
        assert_eq!(
            normalized[2].borrow().z_layers_identity,
            normalized[3].borrow().z_layers_identity
        );
        let mut codec = GraphCodec::new();
        ...
        packet["obstacles"]
            .as_array_mut()
            .unwrap()
            .retain(|record| record["id"] == json!(normalized[2].borrow().identity));
...
mod connected_id_identity_tests {
    ...
    fn geometry_import_preserves_connectivity_cache_until_ids_change() {
        let obstacle = obstacle_from_value(
            &json!({"center":{"x":0,"y":0},"width":1,"height":1,"connectedTo":["a","a","b"]}),
        );
        let mut codec = GraphCodec::new();
        ...
        assert!(Rc::ptr_eq(
            &before_resolved,
            &context.resolve_obstacle(&obstacle)
        ));
...
mod color_map_identity_tests {
    ...
    fn color_map_updates_share_contents_while_field_replacement_detaches() {
        let mut codec = GraphCodec::new();
        let parent = codec
            .read_color_map(&json!({"$colorMapId":100,"$raw":{"n":"red"}}))
            .unwrap();
        ...
        let parent = codec
            .read_color_map(&json!({"$colorMapId":101,"$raw":{"n":"green"}}))
            .unwrap();
        assert!(!Rc::ptr_eq(&parent, &child));
        codec
            .read_color_map(&json!({"$colorMapId":100,"$raw":{"n":"orange"}}))
            .unwrap();
