use crate::bindings::trace_simplification::types::{Point, PointRef};
...
impl SegmentTree {
    pub fn new(segments: Vec<Segment>, segment_margin: f64) -> Self {
        ...
        for segment in &tree.segments {
            ...
            if !segments_by_id.insert(segment_key.clone()) {
                continue;
            }
            ...
            for ix in min_index_x..=max_index_x {
                for iy in min_index_y..=max_index_y {
                    tree.buckets.entry((ix, iy)).or_default().push((
                        segment[0].clone(),
                        segment[1].clone(),
                        segment_key.clone(),
                    ));
            ...
            for iy in min_index_y..=max_index_y {
                if let Some(bucket) = self.buckets.get(&(ix, iy)) {
                    for segment in bucket {
                        if already_added_segments.insert(&segment.2) {
