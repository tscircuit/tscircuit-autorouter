...
use crate::bindings::trace_simplification::types::{RouteRef, ObstacleRef, Point2, ConnectivityMap, Math};
...
use super::single_route_useless_via_removal_solver::{SingleRouteUselessViaRemovalSolver, SingleRouteUselessViaRemovalSolverParams};
...
impl UselessViaRemovalSolver {
    pub fn new(mut input: UselessViaRemovalSolverInput) -> Self {
        ...
        let obstacle_shi = Rc::new(RefCell::new(ObstacleSpatialHashIndex::new_flatbush(input.obstacles.clone())));
        let hd_route_shi = Rc::new(RefCell::new(HighDensityRouteSpatialIndex::new(
            input.unsimplified_hd_routes.iter().chain(input.other_hd_routes.iter()).cloned().collect(), 1.0,
        )));
        Self {
            identity: crate::bindings::trace_simplification::types::next_identity(), unsimplified_routes_array_identity: crate::bindings::trace_simplification::types::next_identity(),
            optimized_routes_array_identity: crate::bindings::trace_simplification::types::next_identity(), unprocessed_routes_array_identity: crate::bindings::trace_simplification::types::next_identity(),
            ...
            optimized_hd_routes: Vec::new(), active_sub_solver: None, obstacle_shi, hd_route_shi, input,
        }
    }
...
impl SpecializedSolver for UselessViaRemovalSolver {
    ...
    fn get_solver_name(&self) -> &'static str { "UselessViaRemovalSolver" }
    ...
    fn _step(&mut self) -> Result<(), String> {
        if let Some(child_ref) = self.active_sub_solver.clone() {
            ...
            if child.base.solved {
                ...
                self.hd_route_shi.borrow_mut().remove_route(&optimized.borrow().connection_name);
                self.hd_route_shi.borrow_mut().add_route(optimized.clone());
                self.optimized_hd_routes.push(optimized);
                self.active_sub_solver = None;
            } else if child.base.failed || child.base.error.as_ref().is_some_and(|error| !error.is_empty()) {
                self.base.error = child.base.error.clone();
                self.base.failed = true;
            }
            return Ok(());
        }
        let Some(route) = self.unprocessed_routes.pop_front() else { self.base.solved = true; return Ok(()); };
        self.active_sub_solver = Some(Rc::new(RefCell::new(SingleRouteUselessViaRemovalSolver::new(SingleRouteUselessViaRemovalSolverParams {
            obstacle_shi: self.obstacle_shi.clone(), hd_route_shi: self.hd_route_shi.clone(), unsimplified_route: route,
            conn_map: self.input.conn_map.clone(), outline: self.input.outline.clone(), terminal_layers: self.input.terminal_layers.clone(),
            options: self.input.options.clone(), math: self.input.math,
        }))));
        Ok(())
    }
}
...
impl UselessViaRemovalSolver {
    pub fn snapshot(&self, codec: &mut crate::bindings::trace_simplification::graph_codec::GraphCodec) -> Value {
        ...
        fields["unsimplifiedHdRoutes"] = codec.route_array(self.unsimplified_routes_array_identity, &self.unsimplified_hd_routes);
        fields["optimizedHdRoutes"] = codec.route_array(self.optimized_routes_array_identity, &self.optimized_hd_routes);
        fields["unprocessedRoutes"] = codec.route_array(self.unprocessed_routes_array_identity, &self.unprocessed_routes.iter().cloned().collect::<Vec<_>>());
    ...
    pub fn restore(&mut self, fields: &Value, codec: &mut crate::bindings::trace_simplification::graph_codec::GraphCodec) -> Result<(), String> {
        ...
        if let Some(value) = fields.get("terminalLayerIndicesByPcbPortId") { self.input.terminal_layers = if value.is_null() { None } else { Some(codec.read_terminal_layers(value)?) }; }
        if let Some(value) = fields.get("unsimplifiedHdRoutes") {
            self.unsimplified_hd_routes = codec.read_routes(value)?;
            if let Some(id) = value["$array"].as_u64() { self.unsimplified_routes_array_identity = id; }
        }
        if let Some(value) = fields.get("optimizedHdRoutes") {
            self.optimized_hd_routes = codec.read_routes(value)?;
            if let Some(id) = value["$array"].as_u64() { self.optimized_routes_array_identity = id; }
        }
        if let Some(value) = fields.get("unprocessedRoutes") {
            self.unprocessed_routes = codec.read_routes(value)?.into_iter().collect();
            if let Some(id) = value["$array"].as_u64() { self.unprocessed_routes_array_identity = id; }
        }
        if let Some(value) = fields.get("activeSubSolver") {
            if value.is_null() { self.active_sub_solver = None; }
                ...
                if self.active_sub_solver.is_none() {
                    ...
                    self.active_sub_solver = Some(Rc::new(RefCell::new(SingleRouteUselessViaRemovalSolver::new(SingleRouteUselessViaRemovalSolverParams {
                        obstacle_shi:self.obstacle_shi.clone(), hd_route_shi:self.hd_route_shi.clone(), unsimplified_route:route,
                        conn_map:self.input.conn_map.clone(), outline:self.input.outline.clone(), terminal_layers:self.input.terminal_layers.clone(), options:self.input.options.clone(), math:self.input.math,
                    }))));
                }
                let mut child = self.active_sub_solver.as_ref().unwrap().borrow_mut();
                if let Some(id) = child_fields["identity"].as_u64() { child.identity = id; }
    ...
    pub fn invoke(&mut self, method: &str, _args: &Value, codec: &mut crate::bindings::trace_simplification::graph_codec::GraphCodec) -> Result<Value, String> {
        match method {
            "getOptimizedHdRoutes" => Ok(codec.route_array(self.optimized_routes_array_identity, &self.optimized_hd_routes)),
