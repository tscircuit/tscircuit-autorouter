...
pub fn create_obstacle_detour_path_validator<'a>(
    ...
    ObstacleDetourPathValidator {
        ...
        next_point_id: 0,
        segment_clearance_cache: HashMap::new(),
        obstacle_same_net_cache: HashMap::new(),
        route_same_net_cache: HashMap::new(),
    }
}
...
impl ObstacleDetourPathValidator<'_> {
    ...
    fn get_segment_clearance_cache_key(&mut self, start: &Point, end: &Point) -> SegmentKey {
        ...
        let (a, b) = if start_first {
            (start, end)
        } else {
            (end, start)
        };
        SegmentKey::Text(format!(
            "{}:{}|{}:{}",
            js_number_to_string(a.x),
            js_number_to_string(a.y),
            js_number_to_string(b.x),
            js_number_to_string(b.y)
        ))
    }

    fn is_connected_to_route(&self, id: &str) -> bool {
        id == self.connection_name
            || self.conn_map.are_ids_connected(id, &self.connection_name)
            || self
                .root_connection_name
                .as_ref()
                .is_some_and(|root| id == root || self.conn_map.are_ids_connected(id, root))
    }
    ...
    pub fn validate(
        ...
        for order_index in 0..count {
            let index = if first_segment_index == 0 {
                order_index
            } else if order_index == 0 {
                first_segment_index
            } else if order_index > first_segment_index {
                order_index
            } else {
                order_index - 1
            };
            ...
            for obstacle in self.obstacles.search_area(
                (a.x + b.x) / 2.0,
                (a.y + b.y) / 2.0,
                ...
                let same = if let Some(same) = self.obstacle_same_net_cache.get(&identity) {
                    *same
                } else {
                    let same = obstacle
                        .connected_to
                        .iter()
                        .any(|id| self.is_connected_to_route(id));
                ...
                if obstacle.z_layers.contains(&self.target_z)
                    && (((a.x - obstacle.center.x).abs() < 0.01
                        && (a.y - obstacle.center.y).abs() < 0.01)
                        || ((b.x - obstacle.center.x).abs() < 0.01
                            && (b.y - obstacle.center.y).abs() < 0.01))
                {
                    continue;
                }
            ...
            for conflict in
                self.hd_routes
                    .get_conflicting_routes_for_segment(&a, &b, self.route_search_margin)
            {
                let identity = Rc::as_ptr(&conflict.conflicting_route) as usize;
                let route = conflict.conflicting_route.borrow();
                let same = if let Some(same) = self.route_same_net_cache.get(&identity) {
                    *same
                } else {
                    let same = self.is_connected_to_route(&route.connection_name)
                        || route
                            .root_connection_name
                            .as_deref()
                            .is_some_and(|id| self.is_connected_to_route(id));
                ...
                let radius = crate::bindings::trace_simplification::math_utils::max(
                    other_thickness / 2.0,
