use crate::bindings::trace_simplification::types::RouteRef;
...
pub fn break_route_into_sections(route: &RouteRef) -> Vec<RouteSection> {
    ...
    let mut current = RouteSection { identity: crate::bindings::trace_simplification::types::next_identity(), points_array_identity: crate::bindings::trace_simplification::types::next_identity(),
    ...
    for (index, point) in points.iter().enumerate().skip(1) {
        if point.borrow().z == current.z {
            ...
            current = RouteSection { identity: crate::bindings::trace_simplification::types::next_identity(), points_array_identity: crate::bindings::trace_simplification::types::next_identity(),
