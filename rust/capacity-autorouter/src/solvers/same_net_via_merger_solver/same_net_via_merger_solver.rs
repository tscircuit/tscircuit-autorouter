...
use crate::bindings::trace_simplification::types::{ColorMapRef, Point, Point2, RouteRef, PointRef, ObstacleRef, ConnectivityMap, structured_clone_routes, spread_point, fresh_via, next_identity};
...
impl SameNetViaMergerSolver {
    ...
    pub fn get_offending_via_groups_batch(&self) -> Result<Vec<ViaGroup>, String> {
        ...
        for vias in self.vias_by_net.values() {
            ...
            let cell_size = vias.iter().fold(1e-6_f64, |max, via| crate::bindings::trace_simplification::math_utils::max(max, via.diameter));
...
impl Via {
    fn snapshot(&self, codec: &mut crate::bindings::trace_simplification::graph_codec::GraphCodec) -> Value {
