...
use crate::bindings::trace_simplification::types::{
    ColorMapRef, ConnectivityMap, ObstacleRef, Point, Point2, PointRef, RouteRef, fresh_via,
    next_identity, spread_point, structured_clone_routes,
};
...
pub fn via_transition_cluster_touches_route_endpoint(route: &RouteRef, via: &PointRef) -> bool {
    ...
    for index in 0..route.route.len().saturating_sub(1) {
        ...
        if start == 0 || end == route.route.len() - 1 {
            return true;
        }
    }
    false
...
pub fn try_get_net_for_route(
    conn: &ConnectivityMap,
    route: &RouteRef,
    explicit: Option<&NetByConnectionName>,
) -> Option<String> {
    let route = route.borrow();
    explicit
        .and_then(|map| map.get(&route.connection_name))
        .or_else(|| conn.id_to_net_map.get(&route.connection_name))
        .or_else(|| {
            route
                .root_connection_name
                .as_ref()
                .filter(|root| !root.is_empty())
                .and_then(|root| conn.id_to_net_map.get(root))
        })
        .cloned()
}

pub fn get_net_for_route(
    conn: &ConnectivityMap,
    route: &RouteRef,
    explicit: Option<&NetByConnectionName>,
    ...
    match net.filter(|net| !net.is_empty()) {
        Some(net) => Ok(net),
        None => Err(format!(
            "SameNetViaMergerSolver could not find net for route \"{}\"",
            route.borrow().connection_name
        )),
    }
}
...
pub fn obstacle_is_same_net(conn: &ConnectivityMap, obstacle: &ObstacleRef, via: &Via) -> bool {
    ...
    for id in obstacle.connected_to.iter() {
        if id == &via.net
            || conn.id_to_net_map.get(id) == Some(&via.net)
            || conn.are_ids_connected(id, &via.net)
        {
            return true;
        }
    }
...
pub struct SameNetViaMergerSolver {
    pub identity: u64,
    pub input_routes_array_identity: u64,
    pub merged_routes_array_identity: u64,
    pub unprocessed_routes_array_identity: u64,
    pub vias_by_net_identity: u64,
    pub vias_array_identity: u64,
    pub offending_vias_array_identity: u64,
    pub current_via_routes_array_identity: u64,
    pub base: BaseSolverState,
    pub stats: Value,
    pub input: SameNetViaMergerSolverInput,
    pub input_hd_routes: Vec<RouteRef>,
    pub merged_via_hd_routes: Vec<RouteRef>,
    pub unprocessed_routes: Vec<RouteRef>,
    pub vias: Vec<Rc<Via>>,
    pub offending_vias: Vec<(Rc<Via>, Rc<Via>)>,
    pub current_via_routes: Vec<RouteRef>,
    pub conn_map: Rc<ConnectivityMap>,
    pub color_map: ColorMapRef,
    pub outline: Option<Vec<Point2>>,
    pub obstacles: Vec<ObstacleRef>,
    pub vias_by_net: IndexMap<String, Vec<Rc<Via>>>,
    pub net_by_connection_name: Option<Rc<NetByConnectionName>>,
    pub obstacle_shi: ObstacleSpatialHashIndex,
    pub hd_route_shi: HighDensityRouteSpatialIndex,
}
...
impl SameNetViaMergerSolver {
    pub fn new(mut input: SameNetViaMergerSolverInput) -> Result<Self, String> {
        ...
        let hd_route_shi = HighDensityRouteSpatialIndex::new(
            merged
                .iter()
        ...
        let mut solver = Self {
            ...
            input_routes_array_identity: next_identity(),
            merged_routes_array_identity: next_identity(),
            unprocessed_routes_array_identity: next_identity(),
            vias_array_identity: next_identity(),
            vias_by_net_identity: next_identity(),
            offending_vias_array_identity: next_identity(),
            current_via_routes_array_identity: next_identity(),
            base: BaseSolverState {
                max_iterations: 1e6,
                ..Default::default()
            },
            stats: json!({}),
            input_hd_routes: input.input_hd_routes.clone(),
            merged_via_hd_routes: merged,
            unprocessed_routes: input.input_hd_routes.clone(),
            color_map: input.color_map.clone(),
            outline: input.outline.clone(),
            obstacles: input.obstacles.clone(),
            obstacle_shi,
            hd_route_shi,
            vias: Vec::new(),
            offending_vias: Vec::new(),
            current_via_routes: Vec::new(),
            conn_map: input.conn_map.clone(),
            net_by_connection_name: input.net_by_connection_name.clone(),
            vias_by_net: IndexMap::new(),
            input,
        };
        solver.rebuild_vias()?;
        Ok(solver)
    ...
    pub fn create_hd_route_spatial_index(&self) -> HighDensityRouteSpatialIndex {
        HighDensityRouteSpatialIndex::new(
            self.merged_via_hd_routes
                .iter()
                .chain(self.input.other_hd_routes.iter())
                .cloned()
                .collect(),
            1.0,
        )
    ...
    pub fn rebuild_vias(&mut self) -> Result<(), String> {
        self.vias = Vec::new();
        self.vias_array_identity = next_identity();
        self.vias_by_net = IndexMap::new();
        self.vias_by_net_identity = next_identity();
        let routes: Vec<_> = self
            .merged_via_hd_routes
            .iter()
            .enumerate()
            .map(|(i, r)| (r.clone(), i, true))
            .chain(
                self.input
                    .other_hd_routes
                    .iter()
                    .enumerate()
                    .map(|(i, r)| (r.clone(), self.merged_via_hd_routes.len() + i, false)),
            )
            .collect();
        for (route_ref, index, mutable) in routes {
            if route_ref.borrow().vias.is_empty() {
                continue;
            }
            let net = if mutable {
                Some(get_net_for_route(
                    &self.conn_map,
                    &route_ref,
                    self.net_by_connection_name.as_deref(),
                )?)
            } else {
                try_get_net_for_route(
                    &self.conn_map,
                    &route_ref,
                    self.net_by_connection_name.as_deref(),
                )
            };
            let Some(net) = net.filter(|net| !net.is_empty()) else {
                continue;
            };
            ...
            for point in &route.vias {
                ...
                for p in &route.route {
                    ...
                    if !layers
                        .iter()
                        .any(|old: &f64| *old == z || (old.is_nan() && z.is_nan()))
                    {
                        layers.push(z);
                    }
                }
                if layers.is_empty() {
                    return Err(format!(
                        "SameNetViaMergerSolver found via on route \"{}\" with no route points",
                ...
                let via = Rc::new(Via {
                    ...
                    route_index: index,
                    mutable: mutable
                        && !(self.input.preserve_route_endpoints
                            && via_transition_cluster_touches_route_endpoint(&route_ref, point)),
                });
                self.vias.push(via.clone());
                self.vias_by_net.entry(net.clone()).or_default().push(via);
            }
        }
        Ok(())
    }
    ...
    pub fn get_via_key(&self, via: &Via) -> String {
        format!(
            "{}:{}:{}:{}:{}:{}",
            if via.mutable { "mutable" } else { "immutable" },
            via.route_index,
            js_number_to_string(via.x),
            js_number_to_string(via.y),
            via.layers
                .iter()
                .map(|z| js_number_to_string(*z))
                .collect::<Vec<_>>()
                .join(","),
            via.net
        )
    ...
    pub fn can_move_via_to(&self, remove: &Via, keep: &Via) -> Result<bool, String> {
        let route = self
            .merged_via_hd_routes
            .get(remove.route_index)
            .ok_or_else(|| {
                format!(
                    "SameNetViaMergerSolver could not find route for via at index {}",
                    remove.route_index
                )
            })?
            .borrow();
        ...
        if layers.is_empty() {
            return Err(format!(
                "SameNetViaMergerSolver could not find transition layers for via at ({}, {})",
                js_number_to_string(remove.x),
                js_number_to_string(remove.y)
            ));
        }
        for z in layers {
            ...
            for conflict in
                self.hd_route_shi
                    .get_conflicting_routes_for_segment(&start, &end, thickness / 2.0)
            {
                if conflict.conflicting_route.borrow().connection_name == route.connection_name {
                    continue;
                }
                if try_get_net_for_route(
                    &self.conn_map,
                    &conflict.conflicting_route,
                    self.net_by_connection_name.as_deref(),
                )
                .as_deref()
                    == Some(remove.net.as_str())
                {
                    continue;
                }
                if conflict.distance
                    < thickness / 2.0 + conflict.conflicting_route.borrow().trace_thickness / 2.0
                {
                    return Ok(false);
                }
            }
            let margin = thickness / 2.0 + OBSTACLE_MARGIN;
            for obstacle in self.obstacle_shi.search_area(
                (start.x + end.x) / 2.0,
                (start.y + end.y) / 2.0,
                (start.x - end.x).abs() + margin * 2.0,
                (start.y - end.y).abs() + margin * 2.0,
            )? {
                if obstacle
                    .borrow()
                    .metadata
                ...
                {
                    return Err(format!(
                        "SameNetViaMergerSolver found obstacle without zLayers near via at ({}, {})",
                        js_number_to_string(remove.x),
                        js_number_to_string(remove.y)
                    ));
                }
                if !obstacle.borrow().z_layers.contains(&z) {
                    continue;
                }
                if obstacle_is_same_net(&self.conn_map, &obstacle, remove) {
                    continue;
                }
                let obstacle = obstacle.borrow();
    ...
    pub fn get_offending_via_groups_batch(&self) -> Result<Vec<ViaGroup>, String> {
        ...
        for vias in self.vias_by_net.values() {
            ...
            let cell_size = vias.iter().fold(1e-6_f64, |max, via| {
                crate::bindings::trace_simplification::math_utils::max(max, via.diameter)
            });
            let mut buckets: IndexMap<String, Vec<usize>> = IndexMap::new();
            for (index, via) in vias.iter().enumerate() {
                let x = (via.x / cell_size).floor();
                let y = (via.y / cell_size).floor();
                buckets
                    .entry(format!(
                        "{}:{}",
                        js_number_to_string(x),
                        js_number_to_string(y)
                    ))
                    .or_default()
                    .push(index);
            }
            for (index, keep) in vias.iter().enumerate() {
                let x = (keep.x / cell_size).floor();
                let y = (keep.y / cell_size).floor();
                ...
                for dx in -radius..=radius {
                    for dy in -radius..=radius {
                        let key = format!(
                            "{}:{}",
                            js_number_to_string(x + dx as f64),
                            js_number_to_string(y + dy as f64)
                        );
                        ...
                        for candidate_index in bucket {
                            ...
                            let pair_dx = keep.x - candidate.x;
                            let pair_dy = keep.y - candidate.y;
                            ...
                            if squared <= overlap * overlap {
                                remove.push(candidate.clone());
                                continue;
                            }
                            if squared <= near * near && self.can_move_via_to(candidate, keep)? {
                                remove.push(candidate.clone());
                            }
                        }
                    }
        ...
        for candidate in candidates {
            ...
            let remove: Vec<_> = candidate
                .remove
                .into_iter()
                .filter(|via| !touched.contains(&self.get_via_key(via)))
                .collect();
            if remove.is_empty() {
                continue;
            }
            touched.insert(key);
            for via in &remove {
                touched.insert(self.get_via_key(via));
            }
            groups.push(ViaGroup {
                keep: candidate.keep,
                remove,
            });
        }
    ...
    pub fn move_via_to(&mut self, remove: &Via, keep: &Via, rebuild: bool) -> Result<(), String> {
        if !remove.mutable {
            return Err("SameNetViaMergerSolver cannot mutate an immutable via anchor".into());
        }
        let route_ref = self
            .merged_via_hd_routes
            .get(remove.route_index)
        ...
        for j in (1..route.route.len()).rev() {
            let prev = route.route[j - 1].borrow();
            let current = route.route[j].borrow();
            if prev.z == current.z
                || prev.x != remove.x
                || prev.y != remove.y
                || current.x != remove.x
        ...
        if indexes.is_empty() {
            return Err(format!(
                "SameNetViaMergerSolver could not find route transition for via at ({}, {}) on route \"{}\"",
                js_number_to_string(remove.x),
                js_number_to_string(remove.y),
                route.connection_name
            ));
        ...
        for index in indexes {
            ...
            point.borrow_mut().x = keep.x;
            point.borrow_mut().y = keep.y;
        ...
        if !replaced {
            return Err(format!(
                "SameNetViaMergerSolver could not find via at ({}, {}) on route \"{}\"",
                js_number_to_string(remove.x),
                js_number_to_string(remove.y),
                route.connection_name
            ));
    ...
    fn _step(&mut self) -> Result<(), String> {
        ...
        for group in &groups {
            for remove in &group.remove {
                self.move_via_to(remove, &group.keep, false)?;
                count += 1;
            }
        }
        self.rebuild_vias()?;
...
impl Via {
    fn snapshot(
        ...
        codec.object(
            self.identity,
            json!({"x":self.x,"y":self.y,"diameter":self.diameter,"net":self.net,
    ...
    fn restore(value: &Value) -> Result<Rc<Self>, String> {
        ...
        Ok(Rc::new(Self {
            ...
            x: fields["x"].as_f64().ok_or("Via x required")?,
            y: fields["y"].as_f64().ok_or("Via y required")?,
            diameter: fields["diameter"].as_f64().ok_or("Via diameter required")?,
            net: fields["net"].as_str().ok_or("Via net required")?.into(),
            layers: serde_json::from_value(fields["layers"].clone())
                .map_err(|error| error.to_string())?,
            route_index: fields["routeIndex"]
                .as_u64()
                .ok_or("Via routeIndex required")? as usize,
...
impl SameNetViaMergerSolver {
    pub fn snapshot(
        ...
        fields["inputHdRoutes"] =
            codec.route_array(self.input_routes_array_identity, &self.input_hd_routes);
        fields["mergedViaHdRoutes"] = codec.route_array(
            self.merged_routes_array_identity,
            &self.merged_via_hd_routes,
        );
        fields["unprocessedRoutes"] = codec.route_array(
            self.unprocessed_routes_array_identity,
            &self.unprocessed_routes,
        );
        fields["currentViaRoutes"] = codec.route_array(
            self.current_via_routes_array_identity,
            &self.current_via_routes,
        );
        ...
        fields["netByConnectionName"] = self
    ...
    pub fn restore(
        ...
        macro_rules! routes {
            ($key:literal, $field:ident, $id:ident) => {
                if let Some(value) = fields.get($key) {
        ...
        routes!(
            "inputHdRoutes",
            input_hd_routes,
            input_routes_array_identity
        );
        routes!(
            "mergedViaHdRoutes",
            merged_via_hd_routes,
            merged_routes_array_identity
        );
        routes!(
            "unprocessedRoutes",
            unprocessed_routes,
        ...
        routes!(
            "currentViaRoutes",
            current_via_routes,
            current_via_routes_array_identity
        );
        ...
        if let Some(value) = fields.get("vias") {
            self.vias = value
                ...
                .unwrap_or(value)
                .as_array()
                .ok_or("Vias array required")?
                .iter()
                .map(&mut read_via)
                .collect::<Result<_, _>>()?;
            if let Some(id) = value["$array"].as_u64() {
                self.vias_array_identity = id;
            }
        }
        if let Some(value) = fields.get("offendingVias") {
            self.offending_vias = value
                .get("items")
                .unwrap_or(value)
                .as_array()
                .ok_or("Offending vias array required")?
                .iter()
                .map(|pair| Ok((read_via(&pair[0])?, read_via(&pair[1])?)))
                .collect::<Result<_, String>>()?;
            if let Some(id) = value["$array"].as_u64() {
                self.offending_vias_array_identity = id;
            }
        }
        if let Some(value) = fields.get("viasByNet") {
            let entries = value
                ...
                .unwrap_or(value)
                .as_array()
                .ok_or("viasByNet entries required")?;
            self.vias_by_net = entries
                .iter()
                .map(|entry| {
                    let net = entry[0].as_str().ok_or("Via net required")?.to_owned();
                    let vias = entry[1]
                        .as_array()
                        .ok_or("Net vias required")?
                        .iter()
                        .map(&mut read_via)
                        .collect::<Result<_, _>>()?;
                    Ok((net, vias))
                ...
                .collect::<Result<_, String>>()?;
            if let Some(id) = value["$map"].as_u64() {
                self.vias_by_net_identity = id;
            }
        }
        ...
        if let Some(value) = fields.get("netByConnectionName") {
            self.net_by_connection_name = if value.is_null() {
                None
            } else {
        ...
        if let Some(value) = fields.get("colorMap") {
            self.color_map = codec.read_color_map(value)?;
        }
        if let Some(value) = fields.get("obstacles") {
            self.obstacles = codec.read_obstacles(value)?;
        }
        Ok(())
    }
