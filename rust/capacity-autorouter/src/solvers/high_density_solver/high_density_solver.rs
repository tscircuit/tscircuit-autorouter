type ParentObserver = Box<dyn Fn(Option<Value>) -> Result<(), String>>;
...
pub type NodeFactory<'a> =
    dyn FnMut(Value, &HighDensitySolver) -> Result<Box<dyn HighDensityNodeSolver>, String> + 'a;
...
pub struct HighDensitySolver {
    ...
    #[serde(skip)]
    pub failed_solvers: Vec<Box<dyn HighDensityNodeSolver>>,
    #[serde(skip)]
    pub active_sub_solver: Option<Box<dyn HighDensityNodeSolver>>,
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    pub stats: Value,
    #[serde(skip)]
    pub observe_parent: Option<ParentObserver>,
}

impl HighDensitySolver {
    pub fn new(mut params: Value) -> Result<Self, String> {
        let effort = params["effort"].as_f64().unwrap_or(1.0);
        let grow = params["useGrowShrinkHighDensityIntraNodeSolver"]
            .as_bool()
            .unwrap_or(false);
        let nodes = params
            .get_mut("nodePortPoints")
            .ok_or("nodePortPoints required")?
            .take();
        let unsolved_node_port_points =
            serde_json::from_value(nodes).map_err(|error| error.to_string())?;
        let node_pf_by_id = serde_json::from_value(
            params
                .get("nodePfById")
                .filter(|v| !v.is_null())
                .cloned()
        ...
        Ok(Self {
            base: BaseSolverState {
                max_iterations: 10e6
                    * effort
                    * if grow {
                        DEFAULT_MAX_GROWTH_ATTEMPTS + 1.0
                    } else {
                        1.0
                    },
                ..Default::default()
            },
            unsolved_node_port_points,
            routes: Vec::new(),
            color_map: Rc::new(
                params
                    .get_mut("colorMap")
            ...
            default_via_diameter: 0.3,
            default_trace_thickness: 0.15,
            ...
            preserve_terminal_pcb_port_ids: params["preserveTerminalPcbPortIds"]
                .as_bool()
                .unwrap_or(false),
            grow_shrink_max_inner_iterations_per_growth_attempt:
                params["growShrinkMaxInnerIterationsPerGrowthAttempt"].as_f64(),
            grow_shrink_fallback_to_invalid_geometry_on_failure:
                params["growShrinkFallbackToInvalidGeometryOnFailure"]
                    .as_bool()
                    .unwrap_or(false),
            ...
            conn_map: params
                ...
                .map(|v| Rc::new(v.take())),
            node_pf_by_id,
            node_solve_metadata_by_id: IndexMap::new(),
            stats: json!({"solverNodeCount":{},"difficultNodePfs":{},"highDensityResizeCount":0}),
            observe_parent: None,
        })
    }

    pub fn record_node_solve_metadata(
        &mut self,
        solver: &dyn HighDensityNodeSolver,
        status: &str,
    ) -> Result<(), String> {
        let node = solver.node_with_port_points();
        let id = node["capacityMeshNodeId"]
            .as_str()
        ...
        self.node_solve_metadata_by_id
    ...
    pub fn record_solved_node_stats(&mut self, solver: &dyn HighDensityNodeSolver) {
        ...
        let id = solver.node_with_port_points()["capacityMeshNodeId"]
            .as_str()
            .expect("Node ID");
        if let Some(pf) = self
            .node_pf_by_id
            ...
            .filter(|pf| *pf > 0.05)
        {
            if self.stats["difficultNodePfs"].get(&name).is_none() {
                self.stats["difficultNodePfs"][&name] = json!([]);
            }
            self.stats["difficultNodePfs"][&name]
                .as_array_mut()
                .unwrap()
                .push(json!(pf));
        }
    ...
    pub fn record_resize_stats(&mut self, solver: &dyn HighDensityNodeSolver) {
        if let Some(attempts) = solver.growth_attempts() {
            self.stats["highDensityResizeCount"] =
                json!(self.stats["highDensityResizeCount"].as_f64().unwrap_or(0.0) + attempts);
        }
    }

    pub fn get_solved_routes_with_terminal_pcb_port_ids(
        &self,
        solver: &dyn HighDensityNodeSolver,
    ...
    pub fn attach_terminal_pcb_port_ids(
        node: &Value,
        mut routes: Vec<Value>,
    ) -> Result<Vec<Value>, String> {
        let terminals: Vec<_> = node["portPoints"]
            .as_array()
            .ok_or("Port points required")?
            .iter()
            .filter(|point| point.get("pcb_port_id").is_some())
            .collect();
        ...
        for route in &mut routes {
            ...
            for point in [
                ...
                if let Some(point) = point {
                    let matches: Vec<_> = terminals
                        ...
                        .filter(|terminal| {
                            terminal["connectionName"] == route["connectionName"]
                                && ["x", "y", "z"]
                                    .iter()
                                    .all(|key| terminal[*key].as_f64() == point[*key].as_f64())
                        })
                        .collect();
                    if matches.len() > 1 {
                        return Err(format!(
                            "HighDensitySolver found multiple PCB terminals at an endpoint of \"{}\"",
                    ...
                    {
                        id = Some(terminal["pcb_port_id"].clone());
                    }
                }
                endpoints.push(id);
            }
            for (key, id) in ["startPcbPortId", "endPcbPortId"].iter().zip(endpoints) {
                if let Some(id) = id {
                    ...
                    route.as_object_mut().unwrap().shift_remove(*key);
                }
            }
        }
        Ok(routes)
    ...
    pub fn _step(
        ...
        if let Some(mut solver) = self.active_sub_solver.take() {
            ...
            if let Err(error) = solver.step(Some(&mut observe)) {
                self.active_sub_solver = Some(solver);
                return Err(error);
            }
            let state = solver.state();
            if state.solved {
                let routes = if self.preserve_terminal_pcb_port_ids {
                    self.get_solved_routes_with_terminal_pcb_port_ids(solver.as_ref())
                } else {
        ...
        if self.unsolved_node_port_points.is_empty() {
            if !self.failed_solvers.is_empty() {
                ...
                let ids = self
                    ...
                    .take(5)
                    .map(|solver| {
                        solver.node_with_port_points()["capacityMeshNodeId"]
                            .as_str()
                            .unwrap()
                            .to_owned()
                    })
                    .collect::<Vec<_>>()
                    .join(",");
                self.base.error = Some(format!(
                    "Failed to solve {} nodes, {}. err0: {}.",
                    self.failed_solvers.len(),
                    ids,
                    self.failed_solvers[0]
                        .state()
                        .error
                        .as_deref()
                        .unwrap_or("null")
    ...
    pub fn step(
        ...
        if !self.base.solved && self.base.iterations as f64 > self.base.max_iterations {
            self.base.error = Some(format!(
                "HighDensitySolver ran out of iterations (MAX_ITERATIONS={})",
                js_number_to_string(self.base.max_iterations)
            ));
            self.base.failed = true;
        }
        Ok(())
    ...
    pub fn visualize(&self, transparentize: &dyn Fn(&str, f64) -> String) -> Result<Value, String> {
        ...
        for route in &self.routes {
            let name = route["connectionName"]
                .as_str()
                .ok_or("Connection name required")?;
            let mut label = name.to_owned();
            if let Some(root) = route["rootConnectionName"]
                .as_str()
            ...
            for segment in merge_route_segments(
                route["route"].as_array().ok_or("Route points required")?,
                name,
                color,
            ) {
                ...
                } else if let Some(color) = segment.color.as_ref().and_then(Value::as_str) {
                    line["strokeColor"] = json!(transparentize(color, 0.5));
                }
                line["layer"] = json!(format!("z{}", js_number_to_string(segment.z)));
                if let Some(width) = route.get("traceThickness") {
                    line["strokeWidth"] = width.clone();
                }
                if segment.z != 0.0 {
                    line["strokeDash"] = json!([0.1, 0.3]);
                }
        ...
        if self.base.solved || self.base.failed {
            for (id, metadata) in &self.node_solve_metadata_by_id {
                ...
                let mut label = format!(
                    ...
                    id,
                    status,
                    metadata["solverType"].as_str().unwrap(),
                    metadata["iterations"],
                    metadata["routeCount"],
                    pf,
                    node["portPoints"].as_array().unwrap().len()
                );
                if let Some(error) = metadata["error"].as_str().filter(|error| !error.is_empty()) {
                    label.push_str(&format!("\nerror: {error}"));
                }
                ...
                if status == "solved" {
                    graphics["points"].as_array_mut().unwrap().push(
                        json!({"x":x,"y":y,"color":color,"layer":"hd_node_markers","label":label}),
                    );
                } else {
                ...
                for key in [
                    ...
                    if let Some(items) = visualization[key].as_array() {
                        for item in items {
                            ...
                            item["step"] = json!(index);
                            combined[key].as_array_mut().unwrap().push(item);
                        }
                    }
                }
            }
    ...
    pub fn snapshot(&self) -> Result<Value, String> {
        ...
        snapshot["activeId"] = self
            .active_sub_solver
            .as_ref()
            .map(|solver| json!(solver.id()))
            .unwrap_or(Value::Null);
        snapshot["failedIds"] = json!(
            self.failed_solvers
                .iter()
...
mod tests {
    ...
    impl HighDensityNodeSolver for Child {
        ...
        fn solved_routes(&self) -> Result<Vec<Value>, String> {
            Ok(vec![
                json!({"connectionName":"a","route":[{"x":0,"y":0,"z":0},{"x":1,"y":0,"z":0}],"vias":[],"traceThickness":0.15,"viaDiameter":0.3}),
            ])
        }
        fn node_with_port_points(&self) -> &Value {
            &self.node
    ...
    fn board_pops_nodes_and_records_routes_between_cache_updates() {
        ...
        let mut factory = move |node: Value,
                                _: &HighDensitySolver|
              -> Result<Box<dyn HighDensityNodeSolver>, String> {
            factory_events
                .borrow_mut()
                .push(node["capacityMeshNodeId"].as_str().unwrap().to_owned());
            Ok(Box::new(Child {
                id: 1,
                node,
