type ViasByNode = RefCell<HashMap<usize, (Weak<Node>, Rc<Vec<Point2>>)>>;
...
impl SingleRouteSolverOptions {
    pub fn from_value(mut opts: Value) -> Self {
        Self {
            bounds: serde_json::from_value(
                opts.get_mut("bounds")
                    .map(Value::take)
                    .unwrap_or(Value::Null),
            )
            .expect("bounds"),
            endpoint_a: serde_json::from_value(
                opts.get_mut("A").map(Value::take).unwrap_or(Value::Null),
            )
            .expect("A"),
            endpoint_b: serde_json::from_value(
                opts.get_mut("B").map(Value::take).unwrap_or(Value::Null),
            )
            .expect("B"),
            obstacle_routes: serde_json::from_value(
                opts.get_mut("obstacleRoutes")
                    .map(Value::take)
                    .unwrap_or(Value::Null),
            )
            .expect("obstacleRoutes"),
            future_connections: opts
                .get_mut("futureConnections")
                .map(|value| serde_json::from_value(value.take()).expect("futureConnections"))
                .unwrap_or_default(),
            hyper_parameters: opts
            ...
            available_z: opts
                ...
                .map(|values| {
                    values
                        .iter()
                        .map(|value| value.as_f64().expect("availableZ number"))
                        .collect()
                })
                .unwrap_or_default(),
            ...
            nearby_segment_clearance: opts
                .get("nearbySegmentClearance")
                .filter(|value| !value.is_null())
                .map(|value| value.as_f64().unwrap_or(0.15)),
            min_dist_between_entering_points: opts["minDistBetweenEnteringPoints"]
                .as_f64()
...
pub struct SingleHighDensityRouteSolver {
    ...
    pub vias_in_path_by_node: ViasByNode,
...
impl SingleHighDensityRouteSolver {
    ...
    pub fn new_typed(opts: SingleRouteSolverOptions) -> Self {
        ...
        let mut solver = Self {
            ...
            a,
            b,
            straight_line_distance: distance(&a, &b),
            ...
            explored_nodes: HashSet::default(),
            vias_in_path_by_node: RefCell::new(HashMap::default()),
            via_cache_sweep_threshold: Cell::new(1024),
            grid_min_x_index: 0.0,
            grid_min_y_index: 0.0,
            grid_width: 0.0,
            grid_height: 0.0,
            ...
            hyper_parameters,
            conn_map: opts.conn_map,
            obstacle_segments: vec![],
            obstacle_segment_index: None,
            obstacle_segments_by_layer: HashMap::default(),
            obstacle_segment_index_by_layer: HashMap::default(),
            obstacle_vias: vec![],
            obstacle_via_index: None,
            debug_explored_nodes_ordered: vec![],
            debug_nodes_too_close_to_obstacle: HashSet::default(),
            debug_node_path_to_parent_intersects_obstacle: HashSet::default(),
            debug_enabled: opts.capture_search_debug,
            initial_node_grid_offset: Point2::default(),
            future_cost: None,
        };
        ...
        while num_x_cells * num_y_cells > best_row_or_column_count.powi(2) {
            if solver.cell_step * 2.0 > min_dist {
                break;
            }
        ...
        solver.grid_width =
            js_round(bounds.max_x / solver.cell_step) + 1.0 - solver.grid_min_x_index + 1.0;
        solver.grid_height =
            js_round(bounds.max_y / solver.cell_step) + 1.0 - solver.grid_min_y_index + 1.0;
        let is_on_same_edge = ((a.x - bounds.min_x).abs() < 0.001
            && (b.x - bounds.min_x).abs() < 0.001)
        ...
        if solver.future_connections.is_empty()
            && solver.obstacle_routes.is_empty()
            && !is_on_same_edge
        {
        ...
        let initial_parent = Rc::new(Node {
            x: a.x,
            y: a.y,
            z: a.z,
            g: 0.0,
        ...
        let rounded = Rc::new(Node {
            ...
            h: 0.0,
            f: 0.0,
            parent: Some(initial_parent.clone()),
        });
        let differs = (rounded.x - a.x).abs() > 1e-9 || (rounded.y - a.y).abs() > 1e-9;
        let exact_start = differs
            && (solver.is_node_too_close_to_obstacle(&rounded, None, false, None)
                || solver.is_node_too_close_to_edge(&rounded, false)
                || solver.does_path_to_parent_intersect_obstacle(&rounded, None));
        solver.candidates = SingleRouteCandidatePriorityQueue::new(vec![if exact_start {
            initial_parent
        } else {
            rounded
        }]);
    ...
    pub fn handle_simple_cases(&mut self) {
        self.solved = true;
        let route = if self.a.z == self.b.z {
            vec![self.endpoint_a.clone(), self.endpoint_b.clone()]
        } else {
            vec![
                self.endpoint_a.clone(),
                route_point(Point {
                    x: self.bounds_center.x,
                    y: self.bounds_center.y,
                    z: self.a.z,
                }),
                route_point(Point {
                    x: self.bounds_center.x,
        ...
        self.solved_path = Some(Route {
            ...
            vias: if self.a.z == self.b.z {
                vec![]
            } else {
                vec![self.bounds_center]
            },
            ..Default::default()
        });
    }
    ...
    pub fn is_node_too_close_to_obstacle(
        ...
        margin: Option<f64>,
        is_via: bool,
        query: Option<&PlanarObstacleQuery>,
    ) -> bool {
        if self.is_node_too_close_to_obstacle_base(node, margin, is_via, query) {
            return true;
        }
        is_via
            && self.future_cost.is_some()
            && self.is_via_too_close_to_future_connection_trace(node)
    }

    pub fn is_node_too_close_to_obstacle_base(
        &self,
        node: &Node,
        margin: Option<f64>,
        is_via: bool,
        ...
        if is_via && let Some(parent) = &node.parent {
        ...
        let indexed_segments = if let Some(query) = query {
            self.obstacle_segments_by_layer.get(&query.layer)
        } else if !is_via {
            self.obstacle_segments_by_layer.get(&(node.z as i64))
        } else {
            Some(&self.obstacle_segments)
        };
        let searched_ids;
        let ids = if let Some(query) = query {
            query.segment_ids.as_slice()
        } else {
            let index = if !is_via {
                self.obstacle_segment_index_by_layer.get(&(node.z as i64))
            } else {
                self.obstacle_segment_index.as_ref()
            };
            searched_ids = index.map(|index| {
                index.search_reusing(
                    node.x - trace_proximity,
                    node.y - trace_proximity,
                    node.x + trace_proximity,
                    node.y + trace_proximity,
                )
            });
        ...
        if let Some(segments) = indexed_segments {
            for &id in ids {
                ...
                if segment.connected_to_current_connection || (!is_via && segment.z != node.z) {
                    continue;
                }
                if query.is_some()
                    && (node.x + trace_proximity < segment.min_x
                        || node.y + trace_proximity < segment.min_y
                        || node.x - trace_proximity > segment.max_x
                        || node.y - trace_proximity > segment.max_y)
                {
                    continue;
                }
                if point_to_segment_distance(&position, &segment.a, &segment.b) < trace_proximity {
                    return true;
                }
            }
        }
        ...
        if let Some(index) = &self.obstacle_via_index {
            for &id in index
                .search_reusing(
                    node.x - via_proximity,
                    node.y - via_proximity,
                    node.x + via_proximity,
                    node.y + via_proximity,
                )
                .iter()
    ...
    pub fn is_node_too_close_to_edge(&self, node: &Node, is_via: bool) -> bool {
        let margin = if is_via {
            self.via_diameter / 2.0 + self.obstacle_margin / 2.0
        } else {
            self.obstacle_margin / 2.0
        };
        let too_close = node.x < self.bounds.min_x + margin
            || node.x > self.bounds.max_x - margin
            || node.y < self.bounds.min_y + margin
            || node.y > self.bounds.max_y - margin;
        if too_close
            && !is_via
            && (distance(&node_point(node), &self.b) < margin * 2.0
                || distance(&node_point(node), &self.a) < margin * 2.0)
        {
            return false;
        }
        too_close
    ...
    pub fn does_path_to_parent_intersect_obstacle(
        ...
        let Some(segments) = self
            .obstacle_segments_by_layer
            .get(&query.map(|q| q.layer).unwrap_or(node.z as i64))
        else {
            return false;
        };
        let clearance = if node.z == parent.z && !self.obstacle_segments.is_empty() {
        ...
        let ids = if let Some(query) = query {
            ...
            searched_ids = self
                .obstacle_segment_index_by_layer
                .get(&(node.z as i64))
                .map(|index| {
                    index.search_reusing(
                        min_x - clearance,
        ...
        for &id in ids {
            ...
            if segment.connected_to_current_connection || segment.z != node.z {
                continue;
            }
            if query.is_some()
                && (max_x + clearance < segment.min_x
                    || max_y + clearance < segment.min_y
                    || min_x - clearance > segment.max_x
                    || min_y - clearance > segment.max_y)
            {
                continue;
            }
    ...
    pub fn get_planar_obstacle_query(&self, node: &Node) -> Option<PlanarObstacleQuery> {
        let mut query = PlanarObstacleQuery {
            layer: 0,
            segment_ids: Vec::new(),
        };
        self.fill_planar_obstacle_query(node, &mut query)
            .then_some(query)
    }
    ...
    fn fill_planar_obstacle_query(&self, node: &Node, query: &mut PlanarObstacleQuery) -> bool {
        ...
        let Some(index) = self.obstacle_segment_index_by_layer.get(&layer) else {
            return false;
        };
        if !self.obstacle_segments_by_layer.contains_key(&layer) {
            return false;
        }
        ...
        query.layer = layer;
        index.search_into(
            (node.x - trace_proximity).min(parent.x - clearance),
            (node.y - trace_proximity).min(parent.y - clearance),
            (node.x + trace_proximity).max(parent.x + clearance),
            (node.y + trace_proximity).max(parent.y + clearance),
            &mut query.segment_ids,
        );
        true
    }
    ...
    pub fn build_obstacle_indexes(&mut self) {
        ...
        for route in &self.obstacle_routes {
            let connected = self.conn_map.as_ref().is_some_and(|map| {
                map.are_ids_connected(&self.connection_name, &route.connection_name)
            });
            for pair in route.route.windows(2) {
                ...
                let a = pair[0].point();
                let b = pair[1].point();
                segments.push(IndexedObstacleSegment {
                    z: a.z,
                    a,
                    b,
                    min_x: a.x.min(b.x),
                    ...
                    max_x: a.x.max(b.x),
                    max_y: a.y.max(b.y),
                    connected_to_current_connection: connected,
                });
            }
        ...
        self.obstacle_segments_by_layer.clear();
        self.obstacle_segment_index_by_layer.clear();
        self.obstacle_segment_index = if self.obstacle_segments.is_empty() {
            None
        } else {
            ...
            for segment in &self.obstacle_segments {
                ...
                if !segment.connected_to_current_connection {
                    self.obstacle_segments_by_layer
                        .entry(segment.z as i64)
                        .or_default()
                        .push(segment.clone());
                }
            }
        ...
        self.obstacle_via_index = if self.obstacle_vias.is_empty() {
            None
        } else {
            let mut index = Flatbush::new(self.obstacle_vias.len());
            for via in &self.obstacle_vias {
                index.add(via.x, via.y, via.x, via.y);
            }
            index.finish();
            Some(index)
        };
    ...
    pub fn compute_h(&self, node: &Node) -> f64 {
        if self.future_cost.is_some() {
            return self.compute_h_future(node);
        }
        distance(&node_point(node), &self.b)
            + if node.z != self.b.z {
    ...
    pub fn compute_g(&self, node: &Node) -> f64 {
        if self.future_cost.is_some() {
            return self.compute_g_future(node);
        }
        ...
        parent.g
            ...
            + distance(&node_point(node), &node_point(parent))
    }
    ...
    pub fn set_node_costs(&self, node: &mut Node) {
        if self.future_cost.is_some() {
            self.set_node_costs_future(node);
            return;
        }
    ...
    fn fill_neighbors(&mut self, node: Rc<Node>, neighbors: &mut Vec<Node>) {
        neighbors.clear();
        let mut query_buffer = PlanarObstacleQuery {
            layer: 0,
            segment_ids: std::mem::take(&mut self.planar_query_scratch),
        };
        for x in -1..=1 {
            for y in -1..=1 {
                if x == 0 && y == 0 {
                ...
                let mut neighbor = Node {
                    x: clamp(
                        node.x + x as f64 * self.cell_step,
                        self.bounds.min_x,
                        self.bounds.max_x,
                    ),
                    y: clamp(
                        node.y + y as f64 * self.cell_step,
                        self.bounds.min_y,
                        self.bounds.max_y,
                    ),
                    z: node.z,
                ...
                let query = self
                    .fill_planar_obstacle_query(&neighbor, &mut query_buffer)
                    .then_some(&query_buffer);
                if self.is_node_too_close_to_obstacle(&neighbor, None, false, query) {
                    if self.debug_enabled {
                        self.debug_nodes_too_close_to_obstacle.insert(key);
                    }
                    self.explored_nodes.insert(key);
                    continue;
                }
                if self.is_node_too_close_to_edge(&neighbor, false) {
                    self.explored_nodes.insert(key);
                    continue;
                }
                if self.does_path_to_parent_intersect_obstacle(&neighbor, query) {
                    if self.debug_enabled {
                        self.debug_node_path_to_parent_intersects_obstacle
                            .insert(key);
                    }
                    self.explored_nodes.insert(key);
                    continue;
        ...
        for &new_z in &self.available_z {
            ...
            if !self.explored_nodes.contains(&self.get_node_key(&neighbor))
                && !self.is_node_too_close_to_obstacle(
                    &neighbor,
                    Some(self.via_diameter / 2.0 + self.obstacle_margin / 2.0),
                    true,
                    None,
                )
    ...
    pub fn get_vias_in_node_path(&self, node: &Rc<Node>) -> Rc<Vec<Point2>> {
        ...
        let parent_vias = node
            .parent
            .as_ref()
            .map(|parent| self.get_vias_in_node_path(parent))
            .unwrap_or_else(|| Rc::new(vec![]));
        let vias = if node
            .parent
            .as_ref()
            .is_some_and(|parent| node.z != parent.z)
        {
            let mut vias = vec![Point2 {
                x: node.x,
                y: node.y,
            }];
        ...
        if cache.len() >= self.via_cache_sweep_threshold.get() {
            ...
            self.via_cache_sweep_threshold
                .set(cache.len().saturating_mul(2).max(1024));
        }
        vias
    }
    ...
    pub fn set_solved_path(&mut self, node: Rc<Node>) {
        ...
        let mut route: Vec<RoutePoint> = path
            .iter()
            .map(|node| route_point(node_point(node)))
            .collect();
    ...
    pub fn compute_progress(&self, goal_dist: Option<f64>, is_on_layer: Option<bool>) -> f64 {
        let (Some(mut goal_dist), Some(is_on_layer)) = (goal_dist, is_on_layer) else {
            return f64::NAN;
        };
        if !is_on_layer {
            goal_dist += self.via_penalty_distance();
        }
        let goal_dist_percent = 1.0 - goal_dist / self.straight_line_distance;
        let prior = if self.progress.is_nan() || self.progress == 0.0 {
            0.0
        } else {
            self.progress
        };
        let next = (2.0 / std::f64::consts::PI)
            * ((0.112 * goal_dist_percent) / (1.0 - goal_dist_percent)).atan();
        if next.is_nan() {
            f64::NAN
        } else {
            prior.max(next)
    ...
    pub fn step_inner(&mut self) {
        ...
        while let Some(node) = &current {
            ...
            if key == 0 || !self.explored_nodes.contains(&key) {
                break;
            }
        ...
        self.explored_nodes.insert(key);
        if self.debug_enabled {
            self.debug_explored_nodes_ordered.push((
                key,
                Point {
                    x: js_round(node.x / self.cell_step) * self.cell_step
                        + self.initial_node_grid_offset.x,
                    y: js_round(node.y / self.cell_step) * self.cell_step
                        + self.initial_node_grid_offset.y,
                    z: node.z,
                },
            ));
        }
        ...
        if goal_dist <= self.cell_step * std::f64::consts::SQRT_2
            && node.z == self.b.z
            && !self.does_path_to_parent_intersect_obstacle(&last, None)
        {
            self.solved = true;
        ...
        for neighbor in neighbors.drain(..) {
            self.candidates.enqueue_owned(neighbor);
        }
    ...
    pub fn visualize(&self) -> Value {
        ...
        for (i, (key, point)) in self.debug_explored_nodes_ordered.iter().enumerate() {
            if self.debug_nodes_too_close_to_obstacle.contains(key)
                || self
                    .debug_node_path_to_parent_intersects_obstacle
                    .contains(key)
            {
                continue;
            }
        ...
        for route in &self.obstacle_routes {
            for via in &route.vias {
                circles.push(json!({"center":via,"radius":self.via_diameter/2.0,"fill":"rgba(255, 0, 0, 0.5)","label":"Via"}));
            }
        }
        if let Some(route) = &self.solved_path {
            ...
            for via in &route.vias {
                circles.push(json!({"center":via,"radius":self.via_diameter/2.0,"fill":"green","label":connection_label(&route.connection_name,route.root_connection_name.as_deref(),&["Via".into()])}));
            }
        }
...
mod via_cache_tests {
    ...
    fn dead_via_cache_entries_are_reclaimed_without_changing_live_path_identity() {
        ...
        assert!(Rc::ptr_eq(
            &ancestor_path,
            &solver.get_vias_in_node_path(&ancestor)
        ));
