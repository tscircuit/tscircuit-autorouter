...
impl ConnectivityMap {
    fn from_data(data: ConnectivityMapData) -> Self {
        Self(Rc::new(ConnectivityVersion {
            data,
            revision: 0,
            next: OnceCell::new(),
        }))
    ...
    pub fn replace_from(&self, incoming: &Self) {
        ...
        if current.data.net_map.iter().eq(incoming.net_map.iter())
            && current
                .data
                .id_to_net_map
                .iter()
                .eq(incoming.id_to_net_map.iter())
        {
            return;
        }
        ...
        current
            .next
            .set(Box::new(ConnectivityVersion {
                data: incoming.current().data.clone(),
    ...
    pub fn new(net_map: IndexMap<String, Vec<String>>) -> Self {
        ...
        Self::from_data(ConnectivityMapData {
            net_map,
            id_to_net_map,
        })
    }
...
impl Clone for ConnectivityMap {
    fn clone(&self) -> Self {
        Self::from_data(self.current().data.clone())
    }
}

impl Serialize for ConnectivityMap {
    fn serialize<S: Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        self.current().data.serialize(serializer)
    }
}
