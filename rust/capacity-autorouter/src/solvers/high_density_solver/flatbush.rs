...
impl Flatbush {
    ...
    pub fn finish(&mut self) {
        assert_eq!(
            self.pos >> 2,
            self.num_items,
            "Incorrect number of Flatbush items"
        );
        if self.num_items <= self.node_size {
            self.boxes[self.pos..self.pos + 4]
                .copy_from_slice(&[self.min_x, self.min_y, self.max_x, self.max_y]);
            self.pos += 4;
            return;
        }
        ...
        loop {
            ...
            if contained {
                ...
                for (bounds, &index) in boxes.iter().zip(indices) {
                    ...
                    if is_node {
                        let c =
                            usize::from(min_x <= x0 && min_y <= y0 && max_x >= x1 && max_y >= y1);
            ...
            let Some((next_index, next_level)) = queue.pop() else {
                break;
            };
    ...
    fn collect_contained(
        ...
        for _ in (1..=level).rev() {
            pos = self.indices[pos >> 2];
        }
        let leaf_end =
            (pos + (end - node_index) * self.node_size.pow(level as u32)).min(num_items4);
        while pos < leaf_end {
...
fn sort(
    ...
    while let Some((l, r)) = stack.pop() {
        if r - l <= node_size && l / node_size >= r / node_size {
            continue;
        }
        let a = values[l];
        let b = values[(l + r) >> 1];
        ...
        loop {
            ...
            for offset in 0..4 {
                boxes.swap(4 * i as usize + offset, 4 * j + offset);
            }
