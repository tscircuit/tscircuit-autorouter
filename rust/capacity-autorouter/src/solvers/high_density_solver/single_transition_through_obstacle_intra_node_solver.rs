...
use crate::bindings::high_density::specialized_utils::create_objects_with_z_layers::create_objects_with_z_layers_owned;
...
pub fn obstacle_is_connected_to_route(
    obstacle: &Value,
    name: &str,
...
pub struct SingleTransitionThroughObstacleIntraNodeSolver {
    ...
    pub conn_map: Option<Rc<ConnectivityMap>>,
...
impl SingleTransitionThroughObstacleIntraNodeSolver {
    ...
    pub fn new_with_math(mut params: Value, math: SpecializedMath) -> Result<Self, String> {
        ...
        let conn_map = fields
            ...
            .transpose()?;
        if !fields.contains_key("nodeWithPortPoints") {
    ...
    pub fn new_with_context(
        ...
        let node_with_port_points = params
            .as_object_mut()
            .ok_or("Solver parameters required")?
            .remove("nodeWithPortPoints")
            .ok_or("Node required")?;
        let mut solver = Self {
        ...
        if solver
            .routes
            .iter()
            .any(|route| route["A"].get("z").is_none() || route["B"].get("z").is_none())
        {
            solver.base.failed = true;
        ...
        if !solver
            .routes
            .iter()
            .any(|route| route["A"]["z"] != route["B"]["z"])
        {
            solver.base.failed = true;
            solver.base.error = Some("No route transitions through an obstacle".into());
            return Ok(solver);
        }
        let containing: Vec<_> = solver
            .routes
            .iter()
            .map(|route| solver.get_containing_through_obstacle(route))
            .collect();
        ...
        let solved_routes = solver
            ...
            .map(|(route, obstacle)| {
                ...
                if a["z"] != b["z"] {
                    ...
                    if truthy(metadata) {
                        start["toNextSegmentCircuitJsonMetadata"] = metadata.clone();
                    }
                }
                let mut output = json!({"connectionName":route["connectionName"]});
                if let Some(root) = route.get("rootConnectionName") {
                    output["rootConnectionName"] = root.clone();
                }
    ...
    pub fn is_applicable(params: Value) -> Result<bool, String> {
        Ok(Self::new(params)?.base.solved)
    }
        ...
        for (name, points) in groups {
            for [a, b] in get_connection_port_point_pairs(&points) {
                ...
                if let Some(root) = a
                    ...
                    .filter(|v| !v.is_null())
                    .or_else(|| b.get("rootConnectionName"))
                {
    ...
    pub fn get_containing_through_obstacle(&self, route: &Value) -> Option<&Value> {
        ...
        self.obstacles.iter().find(|obstacle| {
            ...
            if layers.len() < 2
                || !layers.iter().any(|z| z.as_f64() == a.as_f64())
                || !layers.iter().any(|z| z.as_f64() == b.as_f64())
            {
                return false;
            }
            if !obstacle_is_connected_to_route(
                obstacle,
                route["connectionName"].as_str().expect("Connection name"),
                self.conn_map.as_deref(),
            ) {
                return false;
            }
            point_inside_obstacle(&route["A"], obstacle)
                && point_inside_obstacle(&route["B"], obstacle)
        })
    }
    ...
    pub fn visualize(&self) -> Value {
        ...
        for obstacle in self.obstacles.iter() {
            let layers = obstacle["__zLayers"]
                .as_array()
                .unwrap()
                .iter()
                .map(|z| js_number_to_string(n(z)))
                .collect::<Vec<_>>()
                .join(",");
