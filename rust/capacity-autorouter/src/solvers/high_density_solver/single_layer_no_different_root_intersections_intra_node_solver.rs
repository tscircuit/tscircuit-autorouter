...
use crate::bindings::high_density::specialized_utils::math::{
    Point, SpecializedMath, distance, do_segments_intersect, point_to_segment_distance,
};
...
fn root(value: &Value) -> &str {
    value
        .get("rootConnectionName")
        .filter(|v| !v.is_null())
        .unwrap_or(&value["connectionName"])
        .as_str()
        .expect("Root connection name")
}

pub fn point_key(point: Point) -> String {
    format!("{},{}", js_to_fixed(point.x, 6), js_to_fixed(point.y, 6))
}
...
pub fn unique_available_z(node: &Value) -> Vec<f64> {
    let mut result = Vec::new();
    if let Some(layers) = node["availableZ"]
        .as_array()
...
pub fn get_edge(point: Point, bounds: Bounds) -> Option<&'static str> {
    if (point.y - bounds.min_y).abs() < 1e-3 {
        return Some("top");
    }
    if (point.x - bounds.max_x).abs() < 1e-3 {
        return Some("right");
    }
    if (point.y - bounds.max_y).abs() < 1e-3 {
        return Some("bottom");
    }
    if (point.x - bounds.min_x).abs() < 1e-3 {
        return Some("left");
    }
    None
}
...
pub fn is_inside_bounds(point: Point, bounds: Bounds) -> bool {
    point.x >= bounds.min_x - EPS
        && point.x <= bounds.max_x + EPS
        && point.y >= bounds.min_y - EPS
        && point.y <= bounds.max_y + EPS
}

pub fn segment_intersects_foreign_port(a: Point, b: Point, ports: &[Point]) -> bool {
    for &port in ports {
        if same_point(port, a) || same_point(port, b) {
            continue;
        }
        if point_to_segment_distance(port, a, b) < 1e-4 {
            return true;
        }
    }
    false
...
pub fn segment_intersects_obstacles(a: Point, b: Point, segments: &[ObstacleSegment]) -> bool {
    for segment in segments {
        if same_point(a, segment.a)
            || same_point(a, segment.b)
            || same_point(b, segment.a)
            || same_point(b, segment.b)
        {
            continue;
        }
        if do_segments_intersect(a, b, segment.a, segment.b) {
            return true;
        }
    }
    false
    ...
    for route in routes {
        ...
        for pair in route["route"].as_array().expect("Route points").windows(2) {
            ...
            segments.push(ObstacleSegment {
                a: point(&pair[0]),
                b: point(&pair[1]),
                root_connection_name: route_root.to_owned(),
            });
        }
    }
    segments
...
fn permutations(
    items: &mut [Value],
    count: usize,
    visit: &mut impl FnMut(&[Value]) -> Option<Vec<Value>>,
) -> Option<Vec<Value>> {
    if count <= 1 {
        return visit(items);
    }
...
pub fn find_path(
    ...
    let base: Vec<_> = obstacles
        .iter()
        .flat_map(|s| [s.a, s.b])
        .chain(foreign.iter().copied())
        .chain([a_point, b_point])
        .collect();
    for p in base {
        for dx in [-POINT_OFFSET, 0.0, POINT_OFFSET] {
            for dy in [-POINT_OFFSET, 0.0, POINT_OFFSET] {
                ...
                if !is_inside_bounds(candidate, bounds) {
                    continue;
                }
    ...
    for p in nodes {
        ...
        distances.insert(
            key.clone(),
            if key == start_key { 0.0 } else { f64::INFINITY },
        );
    ...
    while !queue.is_empty() {
        ...
        for key in &queue {
            ...
            if d < current_distance - EPS
                || ((d - current_distance).abs() <= EPS
                    && current.as_ref().is_none_or(|current| key < current))
            {
                current_distance = d;
                current = Some(key.clone());
            }
        }
        let Some(current) = current.filter(|key| !key.is_empty()) else {
            break;
        };
        ...
        for next in &queue {
            ...
            if same_point(current_node, next_node) {
                continue;
            }
            if segment_intersects_obstacles(current_node, next_node, obstacles) {
                continue;
            }
            if segment_intersects_foreign_port(current_node, next_node, foreign) {
                continue;
            }
            ...
            if candidate_distance < next_distance - EPS
                || ((candidate_distance - next_distance).abs() <= EPS
                    && next_previous.is_none_or(|prior| &current < prior))
            {
    ...
    if distances.get(&end_key).copied().unwrap_or(f64::INFINITY) == f64::INFINITY {
        return None;
    }
...
impl SingleLayerNoDifferentRootIntersectionsIntraNodeSolver {
    ...
    pub fn new_with_math(params: Value, _math: SpecializedMath) -> Result<Self, String> {
        let base = BaseSolverState {
            max_iterations: 1.0,
            ..BaseSolverState::default()
        };
        Ok(Self {
            base,
    ...
    pub fn is_applicable(node: &Value) -> bool {
        ...
        if ports.iter().any(|p| get_edge(point(p), bounds).is_none()) {
            return false;
        }
        let mut counts: IndexMap<&str, usize> = IndexMap::new();
        for p in ports {
            *counts
                .entry(p["connectionName"].as_str().expect("Connection name"))
                .or_default() += 1;
        }
    ...
    pub fn try_solve_node(&self) -> Option<Vec<Value>> {
        ...
        for (name, points) in groups {
            for [a, b] in get_connection_port_point_pairs(&points.iter().collect::<Vec<_>>()) {
                let root = a
                    .get("rootConnectionName")
                    .filter(|v| !v.is_null())
                    .or_else(|| b.get("rootConnectionName").filter(|v| !v.is_null()))
        ...
        let count = tasks.len();
        if count <= 6 {
            permutations(&mut tasks, count, &mut try_order)
        } else {
            try_order(&tasks)
        }
    }
    ...
    pub fn visualize(&self) -> Value {
        ...
        let points: Vec<_> = self.node_with_port_points["portPoints"]
            .as_array()
            .expect("Port points")
            .iter()
            .map(|p| {
                json!({"x":p["x"],"y":p["y"],"color":"blue",
            "label":format!("{}\nroot: {}",p["connectionName"].as_str().unwrap(),root(p))})
            })
            .collect();
