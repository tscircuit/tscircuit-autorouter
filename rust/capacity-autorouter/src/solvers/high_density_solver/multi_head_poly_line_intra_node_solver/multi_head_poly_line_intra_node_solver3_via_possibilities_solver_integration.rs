use super::{
    ...
    multi_head_poly_line_intra_node_solver2_optimized::MultiHeadPolyLineIntraNodeSolver2,
    types1::*,
};
...
pub struct MultiHeadPolyLineIntraNodeSolver3 {
    #[serde(flatten)]
    pub inner: MultiHeadPolyLineIntraNodeSolver2,
}
impl Deref for MultiHeadPolyLineIntraNodeSolver3 {
    type Target = MultiHeadPolyLineIntraNodeSolver2;
    fn deref(&self) -> &Self::Target {
        &self.inner
    }
}
impl DerefMut for MultiHeadPolyLineIntraNodeSolver3 {
        ...
        .map(|line| {
            let positions: Vec<_> = line
                ...
                .map(|p| {
                    ...
                    format!(
                        "{},{},{},{}",
                        x.format_to_fixed(p.x, 2),
                        y.format_to_fixed(p.y, 2),
                        js_number_to_string(p.z1),
                        js_number_to_string(p.z2)
                    )
                })
                .collect();
...
impl MultiHeadPolyLineIntraNodeSolver {
    pub fn create_initial_candidate_from_seed(
        ...
        let mut via_solver = ViaPossibilitiesSolver2::new_from_borrowed_inputs(
            ...
            Some(&self.color_map),
            &json!({"SHUFFLE_SEED":seed}),
            &json!(self.via_diameter),
            self.math,
        )?;
        via_solver.solve()?;
        if via_solver.base.failed || !via_solver.base.solved {
            ...
            self.base.error = Some(format!(
                "ViaPossibilitiesSolver2 failed with: {}",
                via_solver.base.error.as_deref().unwrap_or("null")
            ));
            return Ok(None);
        }
        ...
        for (name, path) in &via_solver.completed_paths {
            ...
            let start_value = &path[0];
            let end_value = path.last().unwrap();
            ...
            while index < middle.len() {
                ...
                let is_via = index + 1 < middle.len()
                    && p.coordinates().0 == next.coordinates().0
                    && p.coordinates().1 == next.coordinates().1
                    && p.coordinates().2 != next.coordinates().2;
                let z2 = if is_via {
                    next.coordinates().2.unwrap()
                } else {
                    z
                };
            ...
            while segments < self.segments_per_polyline {
                let full: Vec<_> = std::iter::once(&start)
                    .chain(points.iter())
                ...
                for k in 0..full.len() - 1 {
                    ...
                    if length > longest {
                        longest = length;
                        longest_index = Some(k);
                    }
                }
                let Some(k) = longest_index else {
                    break;
                };
                let a = full[k];
                let b = full[k + 1];
        ...
        if poly_lines.is_empty() {
            self.base.failed = true;
            self.base.error =
                Some("No valid polylines generated from ViaPossibilitiesSolver2.".into());
            return Ok(None);
        }
        ...
        candidate.f = 0.0 + candidate.h;
        candidate.g = self.compute_g(&candidate.poly_lines, &candidate);
        candidate.f = candidate.g + candidate.h;
        Ok(Some(candidate))
    }
}

pub fn setup_initial_poly_lines(
    solver: &mut MultiHeadPolyLineIntraNodeSolver,
) -> Result<(), String> {
    solver.candidates.clear();
    let count = factorial(solver.unique_connections).min(2000.0) as usize;
    let mut hashes = IndexSet::new();
    for seed in 0..count {
        let Some(candidate) = solver.create_initial_candidate_from_seed(seed as f64)? else {
            continue;
        };
        if !hashes.insert(hash_poly_lines(&candidate.poly_lines)) {
            continue;
        }
        solver.candidates.push(candidate);
    }
    solver.candidates.sort_by(|a, b| {
        (a.f - b.f)
            .partial_cmp(&0.0)
