use super::{
    compute_via_count_variants::{PortPairsEntries, compute_via_count_variants},
...
use crate::bindings::high_density::specialized_utils::{
    ...
    get_bounds_from_node_with_port_points::Bounds,
    get_intra_node_crossings::get_intra_node_crossings, math::*,
};
...
pub struct MultiHeadPolyLineIntraNodeSolver {
    ...
    pub conn_map: Option<Rc<Value>>,
    pub candidates: Vec<Candidate>,
    pub bounds: Bounds,
    pub solved_routes: Vec<Value>,
    pub unsolved_connections: Vec<Value>,
    #[serde(rename = "SEGMENTS_PER_POLYLINE")]
    pub segments_per_polyline: usize,
    pub cell_size: f64,
    #[serde(rename = "MAX_CANDIDATES")]
    ...
    pub variant: u8,
    #[serde(skip)]
    pub has_candidate_aliases: bool,
}
...
impl MultiHeadPolyLineIntraNodeSolver {
    ...
    pub fn new_with_math(mut params: Value, math: SpecializedMath) -> Result<Self, String> {
        ...
        let x = node["center"]["x"]
            .as_f64()
            .ok_or("Node center.x required")?;
        let y = node["center"]["y"]
            .as_f64()
            .ok_or("Node center.y required")?;
        let via_diameter = params["viaDiameter"].as_f64().unwrap_or(0.3);
        let obstacle_margin = 0.1;
        let trace_width = 0.15;
        let segments_per_polyline = params["hyperParameters"]["SEGMENTS_PER_POLYLINE"]
            .as_u64()
            .unwrap_or(3) as usize;
        let unique_connections = node["portPoints"]
            .as_array()
            .ok_or("Port points required")?
            .iter()
            .map(|p| p["connectionName"].as_str().expect("Connection name"))
            .collect::<IndexSet<_>>()
            .len();
        let crossings = get_intra_node_crossings(&node);
        let min_via_count =
            crossings.num_same_layer_crossings * 2 + crossings.num_entry_exit_layer_changes;
        let area_per_via = (math.pow)(
            via_diameter + obstacle_margin * 2.0 + trace_width / 2.0,
            2.0,
        );
        let max_via_count = min(
            (width * height / area_per_via).floor(),
            (unique_connections as f64 * 1.5).ceil(),
        ) as usize;
        let mut result = Self {
            ...
            solved_routes: vec![],
            unsolved_connections: vec![],
            segments_per_polyline,
            cell_size: width / 1024.0,
            ...
            unique_connections,
            boundary_padding: params["hyperParameters"]["BOUNDARY_PADDING"]
                .as_f64()
                .unwrap_or(0.05),
            last_candidate: None,
            max_via_count,
        ...
        if min_via_count as f64 > segments_per_polyline as f64 * (unique_connections as f64 / 2.0) {
            ...
            result.base.error = Some(format!(
                "Not possible to solve problem with given SEGMENTS_PER_POLYLINE ({segments_per_polyline}), atleast {min_via_count} vias are required"
            ));
            return Ok(result);
        }
        if result.max_via_count > segments_per_polyline {
            result.max_via_count = segments_per_polyline;
        }
        if result.max_via_count < min_via_count {
            result.max_via_count = min_via_count;
        }
        Ok(result)
    }

    pub fn compute_min_gap_btw_poly_lines(&self, lines: &[PolyLine]) -> Vec<f64> {
        let paths: Vec<Vec<&MHPoint>> = lines
            .iter()
            .map(|line| {
                std::iter::once(&line.start)
                    .chain(line.m_points.iter())
        ...
        for i in 0..lines.len() {
            for j in i + 1..lines.len() {
                if let Some(map) = &self.conn_map {
                    ...
                    let first = map["idToNetMap"][name1]
                        .as_str()
                        .filter(|name| !name.is_empty());
                    let second = map["idToNetMap"][name2]
                        .as_str()
                        .filter(|name| !name.is_empty());
                    if let (Some(first), Some(second)) = (first, second)
                        && (first == second || second == name1)
                    {
                        continue;
                    }
                }
                ...
                for &z in &self.available_z {
                    ...
                    for a in &segments1 {
                        for b in &segments2 {
                            min_gap = min(
                                min_gap,
                                segment_to_segment_min_distance(
                                    xy(a[0]),
                                    xy(a[1]),
                                    xy(b[0]),
                                    xy(b[1]),
                    ...
                    for via in &vias1 {
                        for s in &segments2 {
                            min_gap = min(
                                min_gap,
                                point_to_segment_distance(xy(via), xy(s[0]), xy(s[1]))
                    ...
                    for a in &vias1 {
                        for b in &vias2 {
                            min_gap = min(min_gap, distance(xy(a), xy(b)) - self.via_diameter);
                        }
                    }
                }
                result.push(min_gap);
    ...
    pub fn insert_candidate(&mut self, candidate: Candidate) {
        ...
        while low <= high {
            let mid = (low + high) / 2;
            if self.candidates[mid as usize].f < candidate.f {
                low = mid + 1;
            } else {
                high = mid - 1;
            }
    ...
    pub fn setup_initial_poly_lines(&mut self) -> Result<(), String> {
        if self.variant == 3 {
            return super::multi_head_poly_line_intra_node_solver3_via_possibilities_solver_integration::setup_initial_poly_lines(self);
        }
        ...
        for point in self.node_with_port_points["portPoints"].as_array().unwrap() {
            ...
            if let Some(pair) = pairs.get_mut(name) {
                ...
                pairs.insert(name.into(), (point, None));
            }
        }
        let pairs: PortPairsEntries = pairs
            .into_iter()
            .filter_map(|(name, (start, end))| end.map(|end| (name, (start, end))))
            .collect();
        if pairs.is_empty() {
            self.base.failed = true;
            self.base.error = Some("No port pairs found, can't solve".into());
            return Ok(());
        }
        let counts = compute_via_count_variants(
            &pairs,
            self.segments_per_polyline,
            self.max_via_count,
            self.min_via_count,
        );
        let possible =
            get_possible_initial_via_positions(&pairs, &self.bounds, &counts, self.math)?;
        let mut reordered = Vec::new();
        for variant in possible {
            for positions in get_every_possible_ordering(&variant.via_positions) {
                reordered.push((variant.via_count_variant.clone(), positions));
            }
        }
        for (counts, positions) in reordered {
            ...
            for (index, (name, (start, end))) in pairs.iter().enumerate() {
                ...
                let m_points = construct_middle_points_with_via_positions(
                    start,
                    end,
                    self.segments_per_polyline,
                    count,
                    &self.available_z,
                    &positions[used..used + count],
                );
                used += count;
            ...
            if detect_multi_connection_closed_faces_without_vias(&lines, &self.bounds, self.math) {
                continue;
            }
            ...
            candidate.h = self.compute_h(&candidate);
            candidate.f = candidate.h;
            if self.check_if_solved(&candidate) {
    ...
    pub fn compute_h(&self, candidate: &Candidate) -> f64 {
        if self.variant >= 2 {
            let mut score = 0.0;
            for &gap in &candidate.min_gaps {
                if gap < 0.0 {
                    score += self.obstacle_margin;
                }
                if gap < self.obstacle_margin {
                    score += self.obstacle_margin - gap;
                }
            }
            return score * 0.011;
        }
        let mut total = 0.0;
        if let Some(forces) = &candidate.forces {
            for force in forces {
                for map in force {
                    for force in map.values() {
                        total += force.fx * force.fx + force.fy * force.fy;
                    }
                }
            }
        }
    ...
    pub fn check_if_solved(&self, candidate: &Candidate) -> bool {
        candidate
            .min_gaps
            .iter()
            .all(|&gap| gap >= self.obstacle_margin)
            && candidate.poly_lines.iter().all(|line| {
                line.m_points.iter().all(|p| {
                    let padding = if p.z1 != p.z2 {
                        self.via_diameter / 2.0
                    } else {
                        self.trace_width / 2.0
                    };
    ...
    pub fn set_solved_routes(&mut self) -> Vec<Value> {
        if !self.base.solved || self.last_candidate.is_none() {
            return vec![];
        }
        let mut routes = vec![];
        for line in &self.last_candidate.as_ref().unwrap().poly_lines {
            let mut route = vec![];
            let mut vias = vec![];
            for p in std::iter::once(&line.start)
                .chain(line.m_points.iter())
                .chain(std::iter::once(&line.end))
            {
                route.push(json!({"x":p.x,"y":p.y,"z":p.z1}));
                if p.z1 != p.z2 {
                    vias.push(json!({"x":p.x,"y":p.y}));
                    route.push(json!({"x":p.x,"y":p.y,"z":p.z2}));
                }
            }
            let mut output = json!({"connectionName":line.connection_name});
            if let Some(id) = self.node_with_port_points.get("capacityMeshNodeId") {
                output["regionId"] = id.clone();
            }
            output["traceThickness"] = json!(self.trace_width);
            output["viaDiameter"] = json!(self.via_diameter);
            output["route"] = json!(route);
...
impl SpecializedSolver for MultiHeadPolyLineIntraNodeSolver {
    ...
    fn get_solver_name(&self) -> &'static str {
        match self.variant {
            2 => "MultiHeadPolyLineIntraNodeSolver2",
            3 => "MultiHeadPolyLineIntraNodeSolver3",
            _ => "MultiHeadPolyLineIntraNodeSolver",
        }
    }

    fn try_final_acceptance(&mut self) -> Result<(), String> {
        let Some(target) = self.hyper_parameters["MINIMUM_FINAL_ACCEPTANCE_GAP"].as_f64() else {
            return Ok(());
        };
        let Some(candidate) = &self.last_candidate else {
            return Ok(());
        };
        if candidate.min_gaps.is_empty() {
            return Ok(());
        }
        let min = candidate.min_gaps.iter().copied().fold(f64::INFINITY, min);
        if min >= target {
            self.base.solved = true;
            self.set_solved_routes();
        }
        Ok(())
    }
    ...
    fn _step(&mut self) -> Result<(), String> {
        if self.phase == "setup" {
            self.setup_initial_poly_lines()?;
            self.phase = "solving".into();
            return Ok(());
        }
        if self.variant >= 2 {
            return super::multi_head_poly_line_intra_node_solver2_optimized::step_optimized(self);
        }
        if self.candidates.is_empty() {
            self.try_final_acceptance()?;
            if !self.base.solved {
                self.base.failed = true;
                self.base.error = Some("No candidates left".into());
            }
            return Ok(());
        }
        ...
        for neighbor in self.get_neighbors(&candidate) {
            self.insert_candidate(neighbor);
        }
        Ok(())
    }
}
...
fn endpoint_force(
    ...
    let dist = d_sq.sqrt();
    let magnitude =
        SEGMENT_FORCE_MULTIPLIER * FORCE_MAGNITUDE * (math.exp)(-FORCE_DECAY_RATE * dist);
...
impl MultiHeadPolyLineIntraNodeSolver {
    pub fn get_neighbors(&self, candidate: &Candidate) -> Vec<Candidate> {
        ...
        let mut forces: Forces = lines
            .iter()
            .map(|line| line.m_points.iter().map(|_| IndexMap::new()).collect())
            .collect();
        for i in 0..num_lines {
            for j in i + 1..num_lines {
                let line1 = &lines[i];
                let line2 = &lines[j];
                let points1: Vec<&MHPoint> = std::iter::once(&line1.start)
                    .chain(line1.m_points.iter())
                    .chain(std::iter::once(&line1.end))
                    .collect();
                let points2: Vec<&MHPoint> = std::iter::once(&line2.start)
                    .chain(line2.m_points.iter())
                    .chain(std::iter::once(&line2.end))
                    .collect();
                let mut segments1 = Vec::new();
                let mut vias1 = Vec::new();
                for k in 0..points1.len() - 1 {
                ...
                for (k, p) in points1.iter().enumerate() {
                    if p.z1 != p.z2 {
                        vias1.push(Via {
                            point: xy(p),
                            layers: [p.z1, p.z2],
                ...
                let mut vias2 = Vec::new();
                for k in 0..points2.len() - 1 {
                ...
                for (k, p) in points2.iter().enumerate() {
                    if p.z1 != p.z2 {
                        vias2.push(Via {
                            point: xy(p),
                            layers: [p.z1, p.z2],
                ...
                for seg1 in &segments1 {
                    for seg2 in &segments2 {
                        if seg1.layer == seg2.layer {
                            let min_dist =
                                segment_to_segment_min_distance(seg1.p1, seg1.p2, seg2.p1, seg2.p2);
                            if min_dist < EPSILON {
                                continue;
                            }
                            let center1 = Point {
                                x: (seg1.p1.x + seg1.p2.x) / 2.0,
                                y: (seg1.p1.y + seg1.p2.y) / 2.0,
                            };
                            let center2 = Point {
                                x: (seg2.p1.x + seg2.p2.x) / 2.0,
                                y: (seg2.p1.y + seg2.p2.y) / 2.0,
                            };
                            ...
                            if d_sq > EPSILON {
                                let dist = d_sq.sqrt();
                                let magnitude = SEGMENT_FORCE_MULTIPLIER
                                    * FORCE_MAGNITUDE
                                    * (self.math.exp)(-FORCE_DECAY_RATE * dist);
                                let _fx = (dx / dist) * magnitude;
                                let _fy = (dy / dist) * magnitude;
                ...
                for via in &vias1 {
                    for segment in &segments2 {
                        if via.layers.contains(&segment.layer) {
                            let closest =
                                point_to_segment_closest_point(via.point, segment.p1, segment.p2);
                            let dx = via.point.x - closest.x;
                            let dy = via.point.y - closest.y;
                            let d_sq = dx * dx + dy * dy;
                            if d_sq > EPSILON {
                                let dist = d_sq.sqrt();
                                let mut multiplier = VIA_FORCE_MULTIPLIER;
                                let effective_distance = if dist < self.via_diameter / 2.0 {
                                    multiplier *= INSIDE_VIA_FORCE_MULTIPLIER;
                                    max(EPSILON, dist)
                                } else {
                                    max(EPSILON, dist - self.via_diameter / 2.0)
                                };
                                let magnitude = multiplier
                                    * FORCE_MAGNITUDE
                                    * (self.math.exp)(-FORCE_DECAY_RATE * effective_distance);
                                let fx = (dx / dist) * magnitude;
                                let fy = (dy / dist) * magnitude;
                                let source_segment =
                                    format!("seg:{j}:{}:{}", segment.p1_idx, segment.p2_idx);
                                add_force_contribution(
                                    &mut forces,
                                    lines,
                                    i,
                    ...
                    for segment in &segments1 {
                        if via.layers.contains(&segment.layer) {
                            ...
                            if d_sq > EPSILON {
                                ...
                                let fy = (dy / dist) * magnitude;
                                let source_segment =
                                    format!("seg:{i}:{}:{}", segment.p1_idx, segment.p2_idx);
                                add_force_contribution(
                                    &mut forces,
                                    lines,
                                    j,
                ...
                for via1 in &vias1 {
                    for via2 in &vias2 {
                        let common: Vec<_> = via1
                            .layers
                            .iter()
                            .filter(|z| via2.layers.contains(z))
                            .collect();
                        if !common.is_empty() {
                            let dx = via1.point.x - via2.point.x;
                            let dy = via1.point.y - via2.point.y;
                            let d_sq = dx * dx + dy * dy;
                            if d_sq > EPSILON {
                                let dist = d_sq.sqrt();
                                let mut multiplier = VIA_FORCE_MULTIPLIER;
                                let effective_distance = if dist < self.via_diameter {
                                    multiplier *= INSIDE_VIA_FORCE_MULTIPLIER;
                                    max(EPSILON, dist)
                                } else {
                                    max(EPSILON, dist - self.via_diameter)
                                };
                                let magnitude = multiplier
                                    * FORCE_MAGNITUDE
                                    * (self.math.exp)(-FORCE_DECAY_RATE * effective_distance);
                                let fx = (dx / dist) * magnitude;
                                let fy = (dy / dist) * magnitude;
                                let source2 = format!("via:{j}:{}", via2.index);
                                let source1 = format!("via:{i}:{}", via1.index);
                                add_force_contribution(
                                    &mut forces,
                                    lines,
                                    i,
        ...
        for i in 0..num_lines {
            let line = &lines[i];
            let points: Vec<_> = std::iter::once(&line.start)
                .chain(line.m_points.iter())
                .chain(std::iter::once(&line.end))
                .collect();
            ...
            for v1 in 0..vias.len() {
                for v2 in v1 + 1..vias.len() {
                    let via1 = &vias[v1];
                    let via2 = &vias[v2];
                    let dx = via1.point.x - via2.point.x;
                    let dy = via1.point.y - via2.point.y;
                    ...
                    if d_sq > EPSILON {
                        ...
                        let fx = (dx / dist) * magnitude;
                        let fy = (dy / dist) * magnitude;
                        let source2 = format!("via:{i}:{}", via2.index);
                        let source1 = format!("via:{i}:{}", via1.index);
        ...
        for line in &mut new_lines {
            ...
            for point in &mut line.m_points {
                point.diagnostic_id = next_diagnostic_id();
            }
        }
        let mut points_moved = false;
        for i in 0..num_lines {
            for (k, p) in new_lines[i].m_points.iter_mut().enumerate() {
                let mut net = Force { fx: 0.0, fy: 0.0 };
                for force in forces[i][k].values() {
                    net.fx += force.fx;
                    net.fy += force.fy;
                }
                let is_via = p.z1 != p.z2;
                let mut new_x = p.x + net.fx;
                let mut new_y = p.y + net.fy;
                if is_via {
                    let radius = self.via_diameter / 2.0;
                    let mut boundary_x = 0.0;
                    let mut boundary_y = 0.0;
                    let base_margin = self.via_diameter / 2.0;
                    let margin = base_margin + self.boundary_padding;
                    let min_x = self.bounds.min_x + margin;
                    let max_x = self.bounds.max_x - margin;
                    let min_y = self.bounds.min_y + margin;
                    let max_y = self.bounds.max_y - margin;
                    let outside_min_x = min_x + radius - p.x;
                    let outside_max_x = p.x - (max_x - radius);
                    let outside_min_y = min_y + radius - p.y;
                    let outside_max_y = p.y - (max_y - radius);
                    if outside_min_x > 0.0 {
                        boundary_x = BOUNDARY_FORCE_STRENGTH
                            * ((self.math.exp)(outside_min_x / (self.obstacle_margin * 2.0)) - 1.0);
                    } else if outside_max_x > 0.0 {
                        boundary_x = -BOUNDARY_FORCE_STRENGTH
                            * ((self.math.exp)(outside_max_x / (self.obstacle_margin * 2.0)) - 1.0);
                    }
                    if outside_min_y > 0.0 {
                        boundary_y = BOUNDARY_FORCE_STRENGTH
                            * ((self.math.exp)(outside_min_y / (self.obstacle_margin * 2.0)) - 1.0);
                    } else if outside_max_y > 0.0 {
                        boundary_y = -BOUNDARY_FORCE_STRENGTH
                            * ((self.math.exp)(outside_max_y / (self.obstacle_margin * 2.0)) - 1.0);
                    }
                    net.fx += boundary_x;
                    net.fy += boundary_y;
                    new_x = p.x + net.fx;
                    ...
                    let base_padding = self.trace_width / 2.0;
                    let padding = base_padding + self.boundary_padding;
                    new_x = max(
                        self.bounds.min_x + padding,
                        min(self.bounds.max_x - padding, new_x),
                    );
                    new_y = max(
                        self.bounds.min_y + padding,
                        min(self.bounds.max_y - padding, new_y),
                    );
                }
                if net.fx.abs() < EPSILON && net.fy.abs() < EPSILON {
                    continue;
                }
                if (p.x - new_x).abs() > EPSILON || (p.y - new_y).abs() > EPSILON {
                    p.x = new_x;
                    p.y = new_y;
                    points_moved = true;
                }
        ...
        neighbor.h = self.compute_h(&neighbor);
        neighbor.f = (self.math.round)(g * 5.0) / 5.0 + neighbor.h;
        vec![neighbor]
    }
}
...
impl MultiHeadPolyLineIntraNodeSolver {
    pub fn visualize(&self, transparentize: &dyn Fn(&str, f64) -> String) -> Value {
        ...
        if candidate.is_some_and(|c| c.has_closed_same_layer_face.unwrap_or(false)) {
            let width = (self.bounds.max_x - self.bounds.min_x) * 0.1;
            let height = (self.bounds.max_y - self.bounds.min_y) * 0.1;
            graphics["rects"].as_array_mut().unwrap().push(
                json!({"center":{"x":self.bounds.max_x+width*0.6,"y":self.bounds.max_y+height*0.6},
        ...
        if let Some(candidate) = candidate {
            for (line_index, line) in candidate.poly_lines.iter().enumerate() {
                let color = self.color_map[&line.connection_name]
                    .as_str()
                    .unwrap_or("purple");
                let points: Vec<_> = std::iter::once(&line.start)
                    .chain(line.m_points.iter())
                ...
                for pair in points.windows(2) {
                    let segment_layer = pair[0].z2;
                    let is_layer_zero = segment_layer == 0.0;
                    let segment_color = if is_layer_zero {
                        color.to_owned()
                    } else {
                    ...
                    if !is_layer_zero {
                        graphic["strokeDash"] = json!([0.15, 0.15]);
                    }
                    graphic["label"] = json!(format!(
                        "{} segment (z={})",
                        line.connection_name,
                        js_number_to_string(segment_layer)
                    ));
                    graphics["lines"].as_array_mut().unwrap().push(graphic);
                }
                for (point_index, point) in points.iter().enumerate() {
                    let is_via = point.z1 != point.z2;
                    let point_layer = point.z1;
                    let is_middle = point_index > 0 && point_index < points.len() - 1;
                    let mut force_label = String::new();
                    if is_middle {
                        let middle_index = point_index - 1;
                        let force_map = candidate
                            .forces
                            .as_ref()
                            .and_then(|forces| forces.get(line_index))
                            .and_then(|forces| forces.get(middle_index));
                        if let Some(force_map) = force_map.filter(|map| !map.is_empty()) {
                            ...
                            for (source, force) in force_map {
                                ...
                                if force.fx.abs() > 1e-6 || force.fy.abs() > 1e-6 {
                                    ...
                                    let applying_index: usize =
                                        parts[1].parse().expect("Force source line index");
                                    let applying = &candidate.poly_lines[applying_index];
                                    let applying_color = self.color_map[&applying.connection_name]
                                        .as_str()
                                        .unwrap_or("gray");
                                    ...
                                    if source_type == "via" {
                                        let point_index: usize =
                                            parts[2].parse().expect("Force source point index");
                                        source_label.push_str(&format!(" Via {point_index}"));
                                    } else if source_type == "seg" {
                                        let p1: usize =
                                            parts[2].parse().expect("Force segment start index");
                                        let p2: usize =
                                            parts[3].parse().expect("Force segment end index");
                            ...
                            if net_x.abs() > 1e-6 || net_y.abs() > 1e-6 {
                                force_label = format!(
                                    "\nNet Force: ({}, {})",
                                    js_to_fixed(net_x, 3),
                                    js_to_fixed(net_y, 3)
                    ...
                    if is_via {
                        let label = format!(
                            "Via ({} z={} -> z={}){force_label}",
                            line.connection_name,
                            js_number_to_string(point.z1),
                            js_number_to_string(point.z2)
                        );
                        ...
                        let point_color = if point_layer == 0.0 {
                            color.to_owned()
                        } else {
                            transparentize(color, 0.5)
                        };
                        let label = format!(
                            "mPoint ({} z={}){force_label}",
                            line.connection_name,
                            js_number_to_string(point_layer)
                        );
