use super::{
    multi_head_poly_line_intra_node_solver::{MultiHeadPolyLineIntraNodeSolver, xy},
    types1::*,
};
...
use crate::bindings::high_density::specialized_utils::math::{
    SpecializedMath, max, min, point_to_segment_closest_point, segment_to_segment_min_distance,
};
...
pub struct MultiHeadPolyLineIntraNodeSolver2 {
    #[serde(flatten)]
    pub inner: MultiHeadPolyLineIntraNodeSolver,
}
impl Deref for MultiHeadPolyLineIntraNodeSolver2 {
    type Target = MultiHeadPolyLineIntraNodeSolver;
    fn deref(&self) -> &Self::Target {
        &self.inner
    }
}
impl DerefMut for MultiHeadPolyLineIntraNodeSolver2 {
...
impl SpecializedSolver for MultiHeadPolyLineIntraNodeSolver2 {
    ...
    fn try_final_acceptance(&mut self) -> Result<(), String> {
        self.inner.try_final_acceptance()
    }
}
...
pub fn step_optimized(solver: &mut MultiHeadPolyLineIntraNodeSolver) -> Result<(), String> {
    ...
    let steps = if candidate.mag_force_applied.is_none() {
        1
    } else {
        10
    };
    ...
    if solver.check_if_solved(&candidate) {
        if solver.has_candidate_aliases {
            for queued in &mut solver.candidates {
                if queued.diagnostic_id == candidate.diagnostic_id {
    ...
    if last_step_moved {
        solver.insert_candidate(candidate.clone());
    }
    solver.last_candidate = Some(candidate);
    Ok(())
}
...
impl MultiHeadPolyLineIntraNodeSolver {
    pub fn apply_forces_to_poly_lines(&self, lines: &mut [PolyLine]) -> ForceResult {
        ...
        let mut forces: Vec<Vec<Force>> = lines
            .iter()
            .map(|line| vec![Force { fx: 0.0, fy: 0.0 }; line.m_points.len()])
            .collect();
        ...
        for i in 0..lines.len() {
            for j in i + 1..lines.len() {
                let points1 = &paths[i];
                let points2 = &paths[j];
                let vias1: Vec<_> = points1
                    .iter()
                    .enumerate()
                    .filter(|(_, p)| p.z1 != p.z2)
                    .collect();
                let vias2: Vec<_> = points2
                    .iter()
                    .enumerate()
                    .filter(|(_, p)| p.z1 != p.z2)
                    .collect();
                for (k, a) in points1.windows(2).enumerate() {
                    for (l, b) in points2.windows(2).enumerate() {
                        if a[0].z2 == b[0].z2 {
                            let _min_distance = segment_to_segment_min_distance(
                                xy(a[0]),
                                xy(a[1]),
                                xy(b[0]),
                                xy(b[1]),
                ...
                for &(index, via) in &vias1 {
                    for (k, segment) in points2.windows(2).enumerate() {
                        if via.z1 == segment[0].z2 || via.z2 == segment[0].z2 {
                            let closest = point_to_segment_closest_point(
                                xy(via),
                                xy(segment[0]),
                                xy(segment[1]),
                            );
                            let dx = via.x - closest.x;
                            let dy = via.y - closest.y;
                            let sq = dx * dx + dy * dy;
                            if sq > 1e-6 {
                                let distance = sq.sqrt();
                                let mut multiplier = 2.0;
                                let effective = if distance < self.via_diameter / 2.0 {
                                    multiplier *= 4.0;
                                    max(1e-6, distance)
                                } else {
                                    max(1e-6, distance - self.via_diameter / 2.0)
                                };
                                let magnitude =
                                    multiplier * 0.02 * (self.math.exp)(-6.0 * effective);
                                let fx = (dx / distance) * magnitude;
                                let fy = (dy / distance) * magnitude;
                ...
                for &(index, via) in &vias2 {
                    for (k, segment) in points1.windows(2).enumerate() {
                        if via.z1 == segment[0].z2 || via.z2 == segment[0].z2 {
                            let closest = point_to_segment_closest_point(
                                xy(via),
                                xy(segment[0]),
                ...
                for &(a_index, a) in &vias1 {
                    for &(b_index, b) in &vias2 {
                        if [a.z1, a.z2].iter().any(|&z| z == b.z1 || z == b.z2) {
                            ...
                            if sq > 1e-6 {
                                let distance = sq.sqrt();
                                let mut multiplier = 2.0;
                                let effective = if distance < self.via_diameter {
                                    multiplier *= 4.0;
                                    max(1e-6, distance)
                                } else {
                                    max(1e-6, distance - self.via_diameter)
                                };
                                let magnitude =
                                    multiplier * 0.02 * (self.math.exp)(-6.0 * effective);
                                let fx = (dx / distance) * magnitude;
        ...
        for (i, path) in paths[..lines.len()].iter().enumerate() {
            let vias: Vec<_> = path
                .iter()
                .enumerate()
                .filter(|(_, p)| p.z1 != p.z2)
            ...
            for a_index in 0..vias.len() {
                for b_index in a_index + 1..vias.len() {
                    let (ai, a) = vias[a_index];
                    let (bi, b) = vias[b_index];
                    let dx = a.x - b.x;
                    let dy = a.y - b.y;
                    let sq = dx * dx + dy * dy;
                    if sq > 1e-6 {
        ...
        for (i, line) in lines.iter_mut().enumerate() {
            for (k, p) in line.m_points.iter_mut().enumerate() {
                let net = &forces[i][k];
                let mut fx = net.fx;
                let mut fy = net.fy;
                let mut x = p.x + fx;
                ...
                if p.z1 != p.z2 {
                    ...
                    let d_min_x = min_x + radius - p.x;
                    let d_max_x = p.x - (max_x - radius);
                    let d_min_y = min_y + radius - p.y;
                    let d_max_y = p.y - (max_y - radius);
                    let mut bx = 0.0;
                    let mut by = 0.0;
                    if d_min_x > 0.0 {
                        bx =
                            0.008 * ((self.math.exp)(d_min_x / (self.obstacle_margin * 2.0)) - 1.0);
                    } else if d_max_x > 0.0 {
                        bx = -0.008
                            * ((self.math.exp)(d_max_x / (self.obstacle_margin * 2.0)) - 1.0);
                    }
                    if d_min_y > 0.0 {
                        by =
                            0.008 * ((self.math.exp)(d_min_y / (self.obstacle_margin * 2.0)) - 1.0);
                    } else if d_max_y > 0.0 {
                        by = -0.008
                            * ((self.math.exp)(d_max_y / (self.obstacle_margin * 2.0)) - 1.0);
                    }
                    fx += bx;
                    fy += by;
                    x = p.x + fx;
                    ...
                    x = max(
                        ...
                        min(self.bounds.max_x - padding, x),
                    );
                    y = max(
                        self.bounds.min_y + padding,
                        min(self.bounds.max_y - padding, y),
                    );
                }
