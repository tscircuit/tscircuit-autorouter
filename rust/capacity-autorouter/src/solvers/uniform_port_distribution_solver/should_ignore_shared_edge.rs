...
pub fn should_ignore_shared_edge(
    shared_edge: &JsValue,
    obstacles: &JsValue,
    ...
    for obstacle in values(obstacles)? {
        ...
        let input: Float64Array = read_obstacle
            .call1(&JsValue::UNDEFINED, &obstacle)?
            .unchecked_into();
        ...
        if get(shared_edge, "orientation")? == property_key("vertical") {
            if (number(get(shared_edge, "x1")?) - min_x).abs() < EPSILON
                || (number(get(shared_edge, "x1")?) - max_x).abs() < EPSILON
            {
        ...
        if (number(get(shared_edge, "y1")?) - min_y).abs() < EPSILON
            || (number(get(shared_edge, "y1")?) - max_y).abs() < EPSILON
        {
