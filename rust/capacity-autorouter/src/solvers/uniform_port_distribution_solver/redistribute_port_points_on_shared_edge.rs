...
pub fn redistribute_port_points_on_shared_edge(
    shared_edge: &JsValue,
    port_points: &JsValue,
    spread_point: &Function,
) -> Result<JsValue, JsValue> {
    if array(port_points).length() == 0 {
    ...
    for point in values(port_points)? {
        ...
        if z.is_null() || z.is_undefined() {
            z = JsValue::from_f64(0.0);
        }
        let existing = ports_by_z.get(&z);
        let existing: Array = if existing.is_null() || existing.is_undefined() {
            Array::new()
        } else {
    ...
    let compare_layers =
        Closure::wrap(
            Box::new(|a: JsValue, b: JsValue| -> f64 { number(a) - number(b) })
                as Box<dyn FnMut(JsValue, JsValue) -> f64>,
        );
    ...
    for layer in values(&layers)? {
        ...
        let comparator = Closure::wrap(Box::new(
            move |a: JsValue, b: JsValue| -> Result<f64, JsValue> {
                let axis = if get(&edge, "orientation")? == property_key("horizontal") {
                    "x"
                } else {
                    "y"
                };
        ...
        for index in 0..count {
            ...
            let x = if get(shared_edge, "orientation")? == property_key("horizontal") {
                ...
                number(get(shared_edge, "x1")?)
            };
            let y = if get(shared_edge, "orientation")? == property_key("horizontal") {
                number(get(shared_edge, "y1")?)
            } else {
                number(get(shared_edge, "y1")?) + number(get(shared_edge, "length")?) * fraction
            };
            redistributed.push(&spread_point.call3(
                &JsValue::UNDEFINED,
                &array(&points).get(index),
                &JsValue::from_f64(x),
                &JsValue::from_f64(y),
            )?);
        }
