...
use crate::solvers::uniform_port_distribution_solver::types::{
    Bounds, Name, OwnerPair, SharedEdge,
};
use indexmap::IndexMap;

pub fn precompute_shared_edges(
    owner_pairs: &[OwnerPair],
    node_bounds: &IndexMap<Name, Bounds>,
) -> IndexMap<Name, SharedEdge> {
    ...
    for owner_pair in owner_pairs {
        ...
        let Some(shared_edge) = get_shared_edge_for_node_pair(node_a_id, node_b_id, node_bounds)
        else {
            continue;
        };
