use crate::solvers::uniform_port_distribution_solver::get_owner_pair_key::{
    get_owner_pair_key, normalize_owner_pair,
};
use crate::solvers::uniform_port_distribution_solver::types::{
    Bounds, EdgeOrientation, Name, Point, SharedEdge, Side,
};
use indexmap::IndexMap;

const EPSILON: f64 = 1e-6;

pub fn get_shared_edge_for_node_pair(
    node_a_id: &Name,
    node_b_id: &Name,
    node_bounds: &IndexMap<Name, Bounds>,
) -> Option<SharedEdge> {
    ...
    if vertical_touch {
        ...
        if length > EPSILON {
            ...
            node_side_by_owner_id.insert(
                node_a_id.to_owned(),
                if a_on_left { Side::Right } else { Side::Left },
            );
            node_side_by_owner_id.insert(
                node_b_id.to_owned(),
                if a_on_left { Side::Left } else { Side::Right },
            );
    ...
    if horizontal_touch {
        ...
        if length > EPSILON {
            ...
            node_side_by_owner_id.insert(
                node_a_id.to_owned(),
                if a_below { Side::Top } else { Side::Bottom },
            );
            node_side_by_owner_id.insert(
                node_b_id.to_owned(),
                if a_below { Side::Bottom } else { Side::Top },
            );
            return Some(SharedEdge {
                owner_node_ids,
                owner_pair_key,
