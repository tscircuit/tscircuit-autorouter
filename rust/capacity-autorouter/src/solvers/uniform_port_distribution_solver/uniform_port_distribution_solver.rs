...
use crate::solvers::uniform_port_distribution_solver::types::{
    Bounds, InputNodeWithPortPoints, Name, NodeWithPortPoints, OwnerPair, SharedEdge,
};
...
pub struct UniformPortDistributionConstructor {
    ...
    #[cfg_attr(
        feature = "wasm-types",
        tsify(type = "[UniformName, UniformSharedEdge][]")
    )]
...
impl UniformPortDistributionConstructor {
    pub fn new(input: &UniformPortDistributionInput) -> Self {
        ...
        for node in &input.node_with_port_points {
            map_of_node_id_to_bounds.insert(
                node.capacity_mesh_node_id.clone(),
                get_bounds_from_node_with_port_points(node),
            );
        }
        let mut map_of_owner_pair_to_port_points: IndexMap<Name, Vec<FamilyPoint>> =
            IndexMap::new();
        ...
        for (node_index, node) in input.node_with_port_points.iter().enumerate() {
            for (point_index, port_point) in node.port_points.iter().enumerate() {
                let Some(port_point_id) = port_point
                    .port_point_id
                    .as_ref()
                ...
                let owner_node_ids = determine_owner_pair(
                    Some(port_point_id),
                    &node.capacity_mesh_node_id,
                    &input.input_nodes_with_port_points,
                );
                let owner_pair_key = get_owner_pair_key(&owner_node_ids);
                let existing = map_of_owner_pair_to_port_points
                    .entry(owner_pair_key.clone())
                    .or_default();
                let already_present = existing.iter().any(|point| {
                    let id = input.node_with_port_points[point.node_index].port_points
                        [point.point_index]
                        .port_point_id
                        .as_ref();
                ...
                if !already_present {
                    existing.push(FamilyPoint {
                        node_index,
                        point_index,
                        owner_node_ids: owner_node_ids.clone(),
                        owner_pair_key: owner_pair_key.clone(),
                    });
                }
        ...
        let map_of_owner_pair_to_shared_edge =
            precompute_shared_edges(&owner_pairs, &map_of_node_id_to_bounds);
        let mut owner_pairs_to_process: Vec<_> =
            map_of_owner_pair_to_shared_edge.keys().cloned().collect();
        owner_pairs_to_process.sort_by(|a, b| {
            ...
            let difference = if x != 0.0 && !x.is_nan() {
                x
            } else {
                edge_a.center.y - edge_b.center.y
            };
            difference
                .partial_cmp(&0.0)
        ...
        Self {
            node_bounds: map_of_node_id_to_bounds.into_iter().collect(),
            owner_pair_port_points: map_of_owner_pair_to_port_points.into_iter().collect(),
            shared_edges: map_of_owner_pair_to_shared_edge.into_iter().collect(),
            owner_pairs_to_process,
        }
    }
}
...
pub mod live {
    ...
    pub fn step(
        ...
        let shared_edge = call(
            &get(solver, "mapOfOwnerPairToSharedEdge")?,
            "get",
            &args(std::slice::from_ref(&owner_pair)),
        )?;
        if !truthy(&shared_edge) {
            return Ok(());
        }
        if should_ignore_shared_edge(&shared_edge, &get(input, "obstacles")?, read_obstacle)? {
            return Ok(());
        }
        let family_raw = call(
            &get(solver, "mapOfOwnerPairToPortPoints")?,
            "get",
            &args(std::slice::from_ref(&owner_pair)),
        )?;
        let family_raw = if family_raw.is_null() || family_raw.is_undefined() {
            Array::new().into()
        } else {
            family_raw
        };
        ...
        for point in values(&family_raw)? {
            let point = point?;
            if !should_ignore_port_point(
                &point,
                &get(&point, "ownerNodeIds")?,
                &get(input, "inputNodesWithPortPoints")?,
                find_node,
                find_point,
            )? {
                family.push(&point);
        ...
        let redistributed =
            redistribute_port_points_on_shared_edge(&shared_edge, &family, spread_point)?;
        call(
            &get(solver, "mapOfOwnerPairToPortPoints")?,
            "set",
            &args(&[owner_pair, redistributed]),
        )?;
        Ok(())
    }

    fn update_port_point_position(
        point: JsValue,
        positions: &Map,
        spread_point: &Function,
    ...
    pub fn rebuild_nodes(
            ...
            Closure::wrap(Box::new(move |node: JsValue| -> Result<JsValue, JsValue> {
                ...
                let pairs = if original_pairs.is_null() || original_pairs.is_undefined() {
                    ...
                    let pair_mapper =
                        ...
                        .into_js_value();
                    let pair_map =
                        js_sys::Reflect::get(&original_pairs, &map_key)?.dyn_into::<Function>()?;
        ...
        let node_map =
            js_sys::Reflect::get(&nodes, &JsValue::from_str("map"))?.dyn_into::<Function>()?;
