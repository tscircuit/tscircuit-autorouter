...
use math_utils::{js_number_to_string, js_to_fixed};
use serde_json::Value;

fn point_key(point: &Value) -> String {
    format!(
        "{},{},{}",
        js_to_fixed(point["x"].as_f64().expect("Point x required"), 6),
        js_to_fixed(point["y"].as_f64().expect("Point y required"), 6),
        js_number_to_string(point["z"].as_f64().expect("Point z required"))
    )
}

fn get_connected_point_keys_for_connection(
    routes: &[Value],
    name: &str,
    start: String,
) -> IndexSet<String> {
    ...
    for route in routes {
        ...
        if route["connectionName"].as_str() != Some(name) || points.is_empty() {
            continue;
        }
        if points.len() == 1 {
            adjacency.entry(point_key(&points[0])).or_default();
            continue;
        }
        for pair in points.windows(2) {
            let a = point_key(&pair[0]);
            let b = point_key(&pair[1]);
            adjacency.entry(a.clone()).or_default().insert(b.clone());
            adjacency.entry(b).or_default().insert(a);
        }
    }
    let mut connected = IndexSet::new();
    connected.insert(start.clone());
    let mut stack = vec![start];
    while let Some(key) = stack.pop() {
        if let Some(neighbors) = adjacency.get(&key) {
            for next in neighbors {
                if connected.insert(next.clone()) {
...
pub fn are_node_port_point_pairs_connected_by_routes(routes: &[Value], node: &Value) -> bool {
    if let Some(pairs) = node["portPointsInPairs"]
        .as_array()
        .filter(|pairs| !pairs.is_empty())
    {
        for pair in pairs {
            let start = &pair[0];
            let end = &pair[1];
            let keys = get_connected_point_keys_for_connection(
                routes,
            ...
            if !keys.contains(&point_key(end)) {
                return false;
            }
        }
        return true;
    ...
    for (name, points) in groups {
        for [start, end] in get_connection_port_point_pairs(&points) {
            let keys =
                get_connected_point_keys_for_connection(routes, name, point_key(points[start]));
            if !keys.contains(&point_key(points[end])) {
                return false;
            }
        }
    }
...
pub fn repair_disconnected_same_root_port_points(routes: &[Value], node: &Value) -> Vec<Value> {
    ...
    for (name, points) in groups {
        ...
        let mut connected =
            get_connected_point_keys_for_connection(&repaired, name, point_key(points[0]));
        for point in &points[1..] {
            ...
            let bridge = repaired.iter().find(|route| {
                if route["rootConnectionName"]
                    .as_str()
                    .or(route["connectionName"].as_str())
                    != Some(root)
                    || route["connectionName"].as_str() == Some(name)
                {
                    return false;
                }
                let geometry = route["route"].as_array().expect("Route points required");
                let (Some(start), Some(end)) = (geometry.first(), geometry.last()) else {
                    return false;
                };
                let start = point_key(start);
                let end = point_key(end);
                (connected.contains(&start) && end == key)
                    || (connected.contains(&end) && start == key)
            ...
            route["connectionName"] = Value::from(name);
            route["rootConnectionName"] = Value::from(root);
            for point in route["route"]
                .as_array_mut()
                .expect("Route points required")
            {
                point["connectionName"] = Value::from(name);
                point["rootConnectionName"] = Value::from(root);
            }
            // Cloning the Value copies via and jumper endpoint objects too.
            if route["jumpers"].is_null() {
                route.as_object_mut().unwrap().remove("jumpers");
            }
            repaired.push(route);
            connected =
                get_connected_point_keys_for_connection(&repaired, name, point_key(points[0]));
