...
fn unique_available_z(node: &Value) -> Vec<f64> {
    ...
    {
        z.iter()
            .map(|v| v.as_f64().expect("Layer must be numeric"))
            .collect()
    } else {
        node["portPoints"]
            .as_array()
            ...
            .iter()
            .map(|p| p["z"].as_f64().unwrap_or(0.0))
            .collect()
    };
...
pub fn has_impossible_same_layer_crossing_geometry(node: &Value) -> bool {
    unique_available_z(node).len() == 1
        && get_intra_node_crossings_using_circle(node)["numSameLayerCrossings"]
            .as_u64()
            .unwrap()
            > 0
}

pub fn create_invalid_direct_connection_routes(
    node: &Value,
    thickness: f64,
    diameter: f64,
) -> Vec<Value> {
    let mut groups: IndexMap<&str, Vec<&Value>> = IndexMap::new();
    let z = unique_available_z(node).first().copied();
...
pub fn create_invalid_same_layer_crossing_routes(
    node: &Value,
    thickness: f64,
    diameter: f64,
) -> Vec<Value> {
