use crate::solvers::hyper_parameter_supervisor_solver::{
    Candidate, CandidateFactory, HyperParameterDef, HyperParameterSupervisorSolver,
};
...
pub fn get_combination_defs() -> Vec<Vec<&'static str>> {
    vec![
        ...
        vec!["majorCombinations", "orderings6", "cellSizeFactor"],
        vec!["noVias"],
        vec!["orderings50"],
        vec!["flipTraceAlignmentDirection", "orderings6"],
...
pub fn get_hyper_parameter_defs() -> Vec<HyperParameterDef> {
    vec![
        HyperParameterDef {
            ...
            possible_values: vec![json!({"SINGLE_LAYER_NO_DIFFERENT_ROOT_INTERSECTIONS":true})],
        },
        HyperParameterDef {
        ...
        HyperParameterDef {
            name: "orderings6",
            possible_values: ORDERING_SHUFFLE_SEEDS
                .iter()
                .map(|seed| json!({"SHUFFLE_SEED":seed}))
                .collect(),
        },
        HyperParameterDef {
            name: "cellSizeFactor",
            possible_values: vec![
                json!({"CELL_SIZE_FACTOR":0.5}),
                json!({"CELL_SIZE_FACTOR":1}),
            ],
        },
        HyperParameterDef {
            name: "flipTraceAlignmentDirection",
            possible_values: vec![json!({"FLIP_TRACE_ALIGNMENT_DIRECTION":true})],
        },
        HyperParameterDef {
            name: "noVias",
            possible_values: vec![json!({"CELL_SIZE_FACTOR":2,"VIA_PENALTY_FACTOR_2":10})],
        },
        HyperParameterDef {
            name: "orderings50",
            possible_values: (0..20).map(|i| json!({"SHUFFLE_SEED":100+i})).collect(),
        },
        HyperParameterDef {
            name: "throughObstacle",
            possible_values: vec![json!({"THROUGH_OBSTACLE":true})],
        },
        HyperParameterDef {
            name: "closedFormSingleTrace",
            possible_values: vec![json!({"CLOSED_FORM_SINGLE_TRANSITION":true})],
        },
        HyperParameterDef {
        ...
        HyperParameterDef {
            name: "highDensityA01",
            possible_values: vec![
                json!({"HIGH_DENSITY_A01":true,"SHUFFLE_SEED":ORDERING_SHUFFLE_SEEDS[0]}),
            ],
        },
        HyperParameterDef {
            name: "highDensityA03",
...
impl PortfolioSingleIntraNodeSolver {
    ...
    pub fn get_node_segment_count(&self) -> usize {
        if let Some(pairs) = self.node_with_port_points["portPointsInPairs"].as_array() {
            return pairs.len().max(1);
        }
    ...
    pub fn get_candidate_progress(solver: &dyn Candidate, node_segment_count: usize) -> f64 {
        ...
        progress_or_zero(state.progress).clamp(0.0, 1.0)
    }
    ...
    pub fn refresh_dynamic_iteration_limit(&mut self) {
        ...
        self.max_iterations = js_max(
            self.iterations as f64 + 1.0,
            self.iterations as f64 + remaining,
        );
    ...
    pub fn add_supervised_candidate(
        &mut self,
    ...
    pub fn expand_adaptive_search(
        &mut self,
        factory: &mut dyn CandidateFactory,
    ) -> Result<(), String> {
        if self.adaptive_search_expanded {
            return Ok(());
        }
        self.adaptive_search_expanded = true;
        for seed in &ORDERING_SHUFFLE_SEEDS[1..] {
            self.add_supervised_candidate(
                json!({"HIGH_DENSITY_A01":true,"SHUFFLE_SEED":seed}),
                factory,
            )?;
        }
        self.refresh_dynamic_iteration_limit();
        ...
        let best = self
            ...
            .unwrap_or(&[])
            .iter()
            .fold(0.0, |best, record| {
                js_max(
                    best,
                    Self::get_candidate_progress(record.solver.as_ref(), count),
                )
            });
    ...
    pub fn step_inner(&mut self, factory: &mut dyn CandidateFactory) -> Result<(), String> {
        if self.supervisor.supervised_solvers.is_none() {
            self.initialize_solvers(factory)?;
        }
        if !self.adaptive_search_expanded
            && self
                .supervisor
    ...
    pub fn step(&mut self, factory: &mut dyn CandidateFactory) -> Result<(), String> {
        if self.supervisor.solved || self.supervisor.failed {
            return Ok(());
        }
        self.iterations += 1;
        ...
        if !self.supervisor.solved && self.iterations as f64 > self.max_iterations {
            let limit = math_utils::js_number_to_string(self.max_iterations);
            self.supervisor.error = Some(format!(
                "PortfolioSingleIntraNodeSolver ran out of iterations (MAX_ITERATIONS={limit})"
            ));
            self.supervisor.failed = true;
        }
        Ok(())
    ...
    pub fn compute_g(solver: &dyn Candidate, hyper_parameters: &Value) -> f64 {
        ...
        if hyper_parameters["MULTI_HEAD_POLYLINE_SOLVER"].as_bool() == Some(true) {
            return 1000.0
                + (hyper_parameters["ITERATION_PENALTY"]
                    .as_f64()
                    .unwrap_or(0.0)
                    + iterations)
                    / 10_000.0
                + 10_000.0
                    * (hyper_parameters["SEGMENTS_PER_POLYLINE"]
                        .as_f64()
                        .expect("Polyline segment count required")
                        - 3.0);
        }
        iterations / 10_000.0
    }
