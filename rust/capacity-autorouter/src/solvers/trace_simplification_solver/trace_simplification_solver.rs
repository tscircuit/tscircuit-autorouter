...
use crate::solvers::useless_via_removal_solver::{useless_via_removal_solver::{UselessViaRemovalSolver,UselessViaRemovalSolverInput},can_endpoint_connect_on_layer::TerminalLayers};
use crate::solvers::same_net_via_merger_solver::same_net_via_merger_solver::{SameNetViaMergerSolver,SameNetViaMergerSolverInput};
use crate::solvers::simplified_path_solver::multi_simplified_path_solver::{MultiSimplifiedPathSolver,MultiSimplifiedPathParams};
use crate::solvers::crossing_via_reduction_solver::crossing_via_reduction_solver::{CrossingViaReductionSolver,CrossingViaReductionSolverInput};
    ...
    fn _step(&mut self)->Result<(),String>{
        ...
        if let Some(child)=self.active_sub_solver.clone(){
            ...
            if state.solved {
                ...
                if self.extract_mode!="null" {crate::bindings::trace_simplification::connectivity_read_barrier::check(&self.params.conn_map)?;let routes=child.routes();self.validate_preserved_route_endpoints(&routes)?;self.hd_routes=self.mark_through_obstacle_segments(&routes);self.hd_routes_array_identity=next_identity();}
