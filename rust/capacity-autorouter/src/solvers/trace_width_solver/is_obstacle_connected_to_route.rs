...
pub fn are_ids_connected(
    ...
    let Some(map) = conn_map else {
        return false;
    };
    let Some(a) = map.id_to_net_map.get(left).filter(|s| !s.is_empty()) else {
        return false;
    };
    let Some(b) = map.id_to_net_map.get(right).filter(|s| !s.is_empty()) else {
        return false;
    };
    a == b || b == left
}

pub fn is_obstacle_connected_to_route(
    obstacle: &Value,
    route: &Value,
    conn_map: Option<&RepairConnectivityMap>,
) -> bool {
