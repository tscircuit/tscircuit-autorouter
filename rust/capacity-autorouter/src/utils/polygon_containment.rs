...
use crate::bindings::trace_simplification::math_utils::{
    do_segments_intersect, get_segment_intersection, point_to_segment_distance,
};
...
fn are_points_close(a: Point2, b: Point2) -> bool {
...
pub fn is_point_in_or_on_polygon(point: Point2, polygon: &[Point2]) -> bool {
    ...
    for i in 0..polygon.len() {
        if is_point_on_segment(point, polygon[i], polygon[(i + 1) % polygon.len()]) {
            return true;
        }
    }
...
pub fn does_segment_cross_polygon_boundary(
    start: Point2,
    end: Point2,
    polygon: &[Point2],
    margin: f64,
    ...
    if !is_point_in_or_on_polygon(start, polygon) || !is_point_in_or_on_polygon(end, polygon) {
        return true;
    }
    for i in 0..polygon.len() {
        ...
        if !do_segments_intersect(start, end, edge_start, edge_end) {
            if !start_on_edge
                && !end_on_edge
                && minimum_distance_between_segments(start, end, edge_start, edge_end)
                    < margin - EPSILON
            {
                return true;
            }
        ...
        if let Some(intersection) = get_segment_intersection(start, end, edge_start, edge_end) {
            if (start_on_edge && are_points_close(intersection, start))
                || (end_on_edge && are_points_close(intersection, end))
            {
                continue;
            }
            if are_points_close(intersection, start) || are_points_close(intersection, end) {
                continue;
            }
        }
        return true;
