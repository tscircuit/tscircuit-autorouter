use crate::bindings::connectivity::map_layer_name_to_z::map_layer_name_to_z;
...
pub struct Input {
    pub strings: Vec<circuit_json_to_connectivity_map::WireString>,
    pub prototype_values: Vec<(usize, Option<usize>)>,
    #[serde(deserialize_with = "circuit_json_to_connectivity_map::deserialize_js_number")]
...
fn layers_key(layers: &[usize], map: &ConnectivityMap, count: f64) -> String {
    let mut layers: Vec<String> = layers
        .iter()
        .map(|id| {
            ryu_js::Buffer::new()
                .format(map_layer_name_to_z(&map.strings[*id], count))
                .to_owned()
        })
        .collect();
...
pub fn get_connectivity_map_from_simple_route_json(input: Input) -> ConnectivityMap {
    let mut map = ConnectivityMap::new(
        input
            .strings
            .into_iter()
            .map(circuit_json_to_connectivity_map::WireString::into_units)
            .collect(),
        input
            .prototype_values
            .into_iter()
    ...
    for connection in input.connections {
        for root in connection.roots {
            if !map.add_connections(&[vec![connection.name, root]]) {
                return map;
            }
        }
        if let Some(net) = connection.net
            && !map.add_connections(&[vec![connection.name, net]])
        {
            return map;
        }
        ...
        for point in connection.points {
            let layers = if let Some(layers) = point.layers {
                ...
                ryu_js::Buffer::new()
                    .format(map_layer_name_to_z(
                        &map.strings[point.layer.expect("Point layer required")],
                        input.layer_count,
                    ))
                    .to_owned()
            };
            let coordinate = map.intern(
                format!("{}:{layers}", point_hash(point.x, point.y))
                    .encode_utf16()
                    .collect(),
            );
            if !map.add_connections(&[vec![connection.name, coordinate]]) {
                return map;
            }
            if let Some(port) = point.port
                && !map.add_connections(&[vec![connection.name, port]])
            {
                return map;
            }
            if let Some(id) = point.point_id
                && !map.add_connections(&[vec![connection.name, id]])
            {
                return map;
            }
        }
    ...
    for obstacle in input.obstacles {
        let coordinate = map.intern(
            format!(
                "{}:{}",
                point_hash(obstacle.x, obstacle.y),
                layers_key(&obstacle.layers, &map, input.layer_count),
            )
            .encode_utf16()
            .collect(),
        );
        let group: IndexSet<_> = obstacle
            .id
            .into_iter()
            .chain(obstacle.connected)
            .chain(obstacle.off_board)
            .chain([coordinate])
            .collect();
        if !group.is_empty() && !map.add_connections(&[group.into_iter().collect()]) {
            return map;
        }
    }
