...
pub fn clone_and_shuffle_array<T: Clone>(arr: &[T], seed: f64) -> Vec<T> {
    if seed == 0.0 || arr.is_empty() { return arr.to_vec(); }
    if arr.len() <= 4 {
        let options: &[&[usize]] = match arr.len() {
            ...
            4 => &[
                &[0, 1, 2, 3], &[2, 0, 1, 3], &[1, 3, 2, 0], &[3, 0, 1, 2],
                &[0, 2, 1, 3], &[2, 1, 3, 0], &[3, 0, 2, 1], &[1, 2, 0, 3],
                &[3, 1, 0, 2], &[0, 3, 2, 1], &[2, 3, 0, 1], &[2, 3, 1, 0],
                &[1, 2, 3, 0], &[3, 1, 2, 0], &[0, 1, 3, 2], &[0, 2, 3, 1],
                &[0, 3, 1, 2], &[1, 0, 2, 3], &[1, 0, 3, 2], &[1, 3, 0, 2],
                &[2, 0, 3, 1], &[2, 1, 0, 3], &[3, 2, 0, 1], &[3, 2, 1, 0],
            ],
            _ => unreachable!(),
        };
        let index = seed % options.len() as f64;
        assert!(index >= 0.0 && index.fract() == 0.0, "Invalid preshuffled case");
        return options[index as usize].iter().map(|&i| arr[i].clone()).collect();
    }
