use crate::bindings::trace_simplification::math_utils::min;
...
pub fn calculate_45_degree_paths(a: Point2, b: Point2) -> Vec<Vec<Point2>> {
    ...
    let mid3 = Point2 {
        x: a.x + sign_x * min_dist,
        y: a.y + sign_y * min_dist,
    };
    if (mid3.x - a.x) * sign_x >= 0.0
        && (mid3.x - b.x) * sign_x <= 0.0
        && (mid3.y - a.y) * sign_y >= 0.0
        && (mid3.y - b.y) * sign_y <= 0.0
    {
