use crate::utils::convert_hd_route_to_simplified_route::{
    ConvertOptions, convert_mutable_hd_route_to_simplified_route, is_multilayer,
    obstacle_connected_to_mutable_route, route_point_to_value,
};
...
impl Pipeline7Converter {
    pub fn new(
        ...
        obstacles: &[Value],
        layer_count: usize,
        default_via_hole_diameter: f64,
    ) -> Self {
        let mut net_names = IndexMap::new();
        for c in original {
            net_names
                .entry(c["name"].as_str().expect("Connection name").to_owned())
                .or_insert_with(|| c["__netConnectionName"].clone());
        }
        let prepared = connections
            .iter()
            .map(|connection| {
                let points = connection["pointsToConnect"]
                    .as_array()
                    .expect("Connection points");
                assert_eq!(
                    points.len(),
                    2,
                    "Pipeline7 output connection requires two points"
                );
                let connects = points
                    .iter()
                    .filter_map(|p| {
                ...
                let output = connection
                    .get("__netConnectionName")
                    .filter(|v| !v.is_null())
                    .or_else(|| net_names.get(name).filter(|v| !v.is_null()))
                    .or_else(|| {
        ...
        Self {
            prepared,
            multilayer_obstacles: obstacles
                .iter()
                .filter(|o| is_multilayer(o))
                .cloned()
                .collect(),
    ...
    pub fn convert<'a>(
        ...
        self.convert_typed(routes.iter(), conn)
            .into_iter()
            .map(|converted| {
    ...
    pub fn convert_typed<'a>(
        &mut self,
        routes: impl IntoIterator<Item = &'a MutableRoute>,
        conn: Option<&RepairConnectivityMap>,
    ) -> Vec<ConvertedPipeline7Trace> {
        ...
        for (connection_index, (connection, _, output)) in self.prepared.iter().enumerate() {
            ...
            let Some(routes) = by_name.get(name) else {
                continue;
            };
            for (index, route) in routes.iter().enumerate() {
                let key = (
                    route.connection_name.clone(),
                    route.root_connection_name.clone(),
                );
                let connected = self.connected.entry(key).or_insert_with(|| {
                    self.multilayer_obstacles
                        .iter()
                        .filter(|o| obstacle_connected_to_mutable_route(o, route, conn))
                        .cloned()
                        .collect()
                });
                let converted = convert_mutable_hd_route_to_simplified_route(
                    route,
                    self.layer_count,
                    &ConvertOptions {
                        ...
                        obstacles: &[],
                        connected_multilayer_obstacles: Some(connected),
                        conn_map: conn,
                    },
                );
