use crate::ported::solvers::hyper_high_density_solver::portfolio_single_intra_node_solver as portfolio_solver;
...
enum SharedEngine {
    ...
    Specialized(Rc<RefCell<intra_node_routing::specialized_solver::SpecializedEngine>>),
}
...
pub(crate) fn share_specialized(engine: Rc<RefCell<intra_node_routing::specialized_solver::SpecializedEngine>>) -> u32 {
...
impl PortfolioCandidate {
    fn step_general(&mut self) -> Result<(), String> {
        ...
        if !self.state.solved && self.state.iterations as f64 > self.state.max_iterations {
            ...
            self.state.error = Some(format!("CachedIntraNodeRouteSolver ran out of iterations (MAX_ITERATIONS={})", intra_node_routing::js_number::js_number_to_string(self.state.max_iterations)));
        }
        Ok(())
    }
}
...
impl PortfolioCore {
    pub(crate) fn state(&self) -> intra_node_routing::specialized_base_solver::BaseSolverState {
    ...
    pub(crate) fn step(&mut self) -> Result<(), JsValue> {
        ...
        if !self.solver.supervisor.solved && iterations as f64 > self.solver.max_iterations {
            self.solver.supervisor.failed = true;
            self.solver.supervisor.error = Some(format!("portfolio_solver::PortfolioSingleIntraNodeSolver ran out of iterations (MAX_ITERATIONS={})", intra_node_routing::js_number::js_number_to_string(self.solver.max_iterations)));
        }
        self.complete_terminal()
    }
