...
struct RouteRegistry {
    ...
    points: HashMap<i64,Rc<RefCell<repair::internal_types::RoutePoint>>>,
...
impl RouteRegistry {
    fn new(input: Routes) -> Self {
        Self { arrays:vec![input],array_clients:HashMap::new(),identities:HashMap::new(),identity_clients:HashMap::new(),points:HashMap::new(),point_clients:HashMap::new(),route_sources:HashMap::new(),point_sources:HashMap::new() }
    }

    fn native_identity(&mut self, kind:u8, id:i64) -> u64 {
        if id>=0 { return id as u64; }
        let value=*self.identities.entry((kind,id)).or_insert_with(repair::internal_types::next_identity);
        self.identity_clients.insert((kind,value),id); value
    }

    fn client_identity(&self, kind:u8, id:u64) -> i64 {
        self.identity_clients.get(&(kind,id)).copied().unwrap_or(id as i64)
    }

    fn array(&self,id:i64) -> Result<Routes,String> {
        let index=if id>=0 { id as usize } else { *self.array_clients.get(&id).ok_or("Unknown route array identity")? };
        self.arrays.get(index).cloned().ok_or_else(||"Unknown route array identity".to_owned())
    }

    fn packet(&mut self, routes: &Routes) -> Value {
        let index = if let Some(index) = self.arrays.iter().position(|item| Routes::ptr_eq(item,routes)) { index }
        else { self.arrays.push(routes.clone()); self.arrays.len()-1 };
        let id=self.array_clients.iter().find_map(|(&client,&value)|(value==index).then_some(client)).unwrap_or(index as i64);
        let mut entries=Vec::new();
        for route in routes.iter() {
            let route_id=self.client_identity(0,route.identity);
            let source_id=*self.route_sources.entry(Rc::as_ptr(&route.metadata) as usize).or_insert(route_id);
            let mut point_ids=Vec::new();let mut point_sources=Vec::new();
            for point in &route.route {
                ...
                let source=*self.point_sources.entry(point.borrow().metadata_identity).or_insert(point_id);
                point_ids.push(point_id);point_sources.push(source);
            }
    ...
    fn apply_mutations(&mut self, mutations:&Value) -> Result<(),String> {
        let Some(mutations)=mutations.as_array() else { return Ok(()); };
        if mutations.is_empty() { return Ok(()); }
        let mut updates=HashMap::new();let mut point_arrays=HashMap::new();let mut via_arrays=HashMap::new();
        let mut layouts=Vec::new();
        for packet in mutations {
            let array_id=packet["id"].as_i64().ok_or("Mutation array identity")?;
            let mut layout=Vec::new();
            for entry in packet["routes"].as_array().ok_or("Mutation route entries")? {
                let client=entry["id"].as_i64().ok_or("Mutation route identity")?;
                let identity=self.native_identity(0,client);
                let point_array_identity=self.native_identity(1,entry["pointArrayId"].as_i64().ok_or("Point array identity")?);
                let via_array_identity=self.native_identity(2,entry["viaArrayId"].as_i64().ok_or("Via array identity")?);
                let mut route=MutableRoute::from_owned_value(entry["value"].clone());
                route.identity=identity;route.point_array_identity=point_array_identity;route.via_array_identity=via_array_identity;
                let point_ids=entry["pointIds"].as_array().ok_or("Mutation point identities")?;
                if point_ids.len()!=route.route.len() { return Err("Mutation point count differs".to_owned()); }
                    ...
                    if let Some(existing)=self.points.get(&client) {
                        if existing.borrow().to_value()!=point.borrow().to_value() {
                            *existing.borrow_mut()=point.borrow().clone();
                            self.point_sources.insert(existing.borrow().metadata_identity,client);
                        }
                        *point=existing.clone();
                    } else {
                        self.points.insert(client,point.clone());self.point_clients.insert(Rc::as_ptr(point) as usize,client);
                        self.point_sources.insert(point.borrow().metadata_identity,client);
                    }
                }
                self.route_sources.insert(Rc::as_ptr(&route.metadata) as usize,client);
                point_arrays.insert(point_array_identity,route.route.clone());via_arrays.insert(via_array_identity,route.vias.clone());
                updates.insert(identity,route);layout.push(identity);
            }
            if array_id<0 && !self.array_clients.contains_key(&array_id) {
                self.array_clients.insert(array_id,self.arrays.len());self.arrays.push(Routes::new(Vec::new()));
            }
        ...
        for array in &self.arrays {
            ...
            let routes=array.iter().map(|route| {
                if let Some(update)=updates.get(&route.identity) { changed=true;return update.shallow_clone(); }
                let mut route=route.shallow_clone();
                if let Some(points)=point_arrays.get(&route.point_array_identity) { route.route=points.clone();changed=true; }
                if let Some(vias)=via_arrays.get(&route.via_array_identity) { route.vias=vias.clone();changed=true; }
                route
            }).collect();
            if changed { array.replace(routes); }
        }
        for (id,layout) in layouts {
            self.array(id)?.replace(layout.iter().map(|id|updates.get(id).expect("Mutated route identity").shallow_clone()).collect());
        }
        Ok(())
    }
}
...
impl DrcEvaluator for CallbackEvaluator {
    fn snapshot(&mut self, routes: &Routes, topology: bool, legacy: bool) -> Result<DrcSnapshot,String> {
        ...
        *self.patch.borrow_mut()=output.get("state").cloned();
        ...
        if output["thrown"].as_bool()==Some(true) { return Err("Global DRC callback failed".to_owned()); }
...
impl GlobalDrcForceImproveSolver {
    ...
    pub fn new(params: Ts<GlobalDrcSolverParams>, #[wasm_bindgen(unchecked_param_type = "(input: GlobalDrcCallbackInput) => GlobalDrcCallbackOutput")] callback: js_sys::Function) -> Result<Self,JsValue> {
        ...
        let conn=params.get("connMap").filter(|value|!value.is_null()).cloned();
        let engine=Rc::new(repair::solver_helpers::BroadRepulsionEngine::new(params["srj"].clone(),conn.clone(),crate::repair_math()));
        let mut default_evaluator=None;
        let make_callback=|reference| -> Evaluator { Rc::new(RefCell::new(CallbackEvaluator {
            callback:callback.clone(),reference,registry:registry.clone(),state:state.clone(),output:output.clone(),patch:patch.clone(),pending_error:pending_error.clone(),
        })) };
        let evaluator: Evaluator=if params["useHostEvaluator"].as_bool()==Some(true) { make_callback(false) } else {
            let native=Rc::new(RefCell::new(StandaloneDrcEvaluator::new(params["srj"].clone(),conn,
                params["enableTraceViaOwnerTargeting"].as_bool().unwrap_or(false),
                autorouting_drc::autorouting_drc_engine::DrcMath { hypot:js_sys::Math::hypot,sin:js_sys::Math::sin,cos:js_sys::Math::cos }).map_err(js_error)?));
            default_evaluator=Some(native.clone()); native
        };
        let reference=if params["hasReferenceEvaluator"].as_bool()==Some(true) { Some(make_callback(true)) } else { None };
        let mut solver=force_improve_solver::GlobalDrcForceImproveSolver::new(params,routes,engine,evaluator,reference).map_err(|error|pending_error.borrow_mut().take().unwrap_or_else(||js_error(error)))?;
        if default_evaluator.is_none() || solver.params["hasReferenceEvaluator"].as_bool()==Some(true) {
            solver.callback_state=Some(state); solver.callback_output=Some(output); solver.callback_patch=Some(patch);
        }
        registry.borrow_mut().arrays.push(solver.guarded_input_hd_routes.clone());
        Ok(Self {solver,registry,default_evaluator,pending_error})
    }
    ...
    pub fn step_inner(&mut self, state: Ts<GlobalDrcSolverState>, final_acceptance: bool) -> Result<Ts<GlobalDrcSolverState>,JsValue> {
        ...
        if let Some(conn) = state.get("connectivity") {
            let conn = if conn.is_null() { None } else { Some(conn.clone()) };
            self.solver.engine = Rc::new(repair::solver_helpers::BroadRepulsionEngine::new(self.solver.params["srj"].clone(),conn.clone(),crate::repair_math()));
            if let Some(evaluator) = &self.default_evaluator {
                evaluator.borrow_mut().engine.conn_map = conn.map(serde_json::from_value).transpose().map_err(js_error)?;
            }
        }
