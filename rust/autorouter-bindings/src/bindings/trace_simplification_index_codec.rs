...
use trace_simplification::graph_codec::GraphCodec;
use trace_simplification::data_structures::{
    obstacle_tree::{NativeObstacleTree, ObstacleSpatialHashIndex, ImportedObstacleIndex, RbushNode},
    high_density_route_spatial_index::{HighDensityRouteSpatialIndex, StoredSegment, StoredVia},
};
...
pub(crate) fn decode_obstacle_index(value: &Value, codec: &mut GraphCodec) -> Result<Rc<RefCell<ObstacleSpatialHashIndex>>, String> {
    ...
    if value["kind"] != "native" {
        ...
        return Ok(Rc::new(RefCell::new(ObstacleSpatialHashIndex { identity: value["indexId"].as_u64().unwrap_or_else(trace_simplification::types::next_identity), storage: codec.read_obstacles(&value["storage"])?, native: NativeObstacleTree::new(Vec::new()), imported: Some(imported), host_query: None })));
    }
    ...
    Ok(Rc::new(RefCell::new(ObstacleSpatialHashIndex {
        identity: value["indexId"].as_u64().unwrap_or_else(trace_simplification::types::next_identity), native, imported: None, host_query: None, storage: codec.read_obstacles(&value["storage"])?
    })))
}
...
pub(crate) fn decode_hd_route_index(value: &Value, codec: &mut GraphCodec) -> Result<Rc<RefCell<HighDensityRouteSpatialIndex>>, String> {
    ...
    Ok(Rc::new(RefCell::new(HighDensityRouteSpatialIndex {
        identity: value["indexId"].as_u64().unwrap_or_else(trace_simplification::types::next_identity), segment_buckets, via_buckets,
...
fn decode_rbush(value: &Value, codec: &mut GraphCodec) -> Result<RbushNode,String> {
    ...
    Ok(RbushNode {
        bounds: trace_simplification::types::Bounds {
