use crate::ported::solvers::hyper_high_density_solver::grow_shrink_high_density_intra_node_solver::grow_shrink_high_density_intra_node_solver as growth_solver;
use crate::ported::solvers::high_density_solver::high_density_solver;
...
use crate::ported::solvers::high_density_solver::high_density_solver::{HighDensityNodeSolver};
...
thread_local! {
    static LAST_EXCEPTION: RefCell<Option<JsValue>> = RefCell::new(None);
    ...
    static CACHE_COUNTS: RefCell<(f64, f64)> = RefCell::new((0.0, 0.0));
...
impl GrowthPortfolio for PortfolioChild {
    ...
    fn step(&mut self) -> Result<(), String> { self.engine.borrow_mut().step().map_err(error_text) }
    fn state(&self) -> GrowthPortfolioState {
        let state = self.engine.borrow().state();
        GrowthPortfolioState { iterations: state.iterations, max_iterations: state.max_iterations,
            solved: state.solved, failed: state.failed, progress: state.progress, error: state.error }
    }
    fn set_max_iterations(&mut self, iterations: f64) { self.engine.borrow_mut().set_max_iterations(iterations); }
    fn solved_routes(&self) -> Result<Vec<Value>, String> { Ok(self.engine.borrow().output_routes.clone()) }
...
impl HighDensityNodeSolver for PortfolioChild {
    fn id(&self) -> u32 { self.id }
    fn state(&self) -> BaseSolverState { self.engine.borrow().state() }
    fn step(&mut self, _observe_parent: Option<&mut dyn FnMut(bool) -> Result<(), String>>) -> Result<(), String> { self.engine.borrow_mut().step().map_err(error_text) }
    fn solved_routes(&self) -> Result<Vec<Value>, String> { Ok(self.engine.borrow().output_routes.clone()) }
    fn route_count(&self) -> Result<usize, String> { Ok(self.engine.borrow().output_routes.len()) }
...
impl HighDensityNodeSolver for ExternalChild {
    ...
    fn step(&mut self, _observe_parent: Option<&mut dyn FnMut(bool) -> Result<(), String>>) -> Result<(), String> {
        let state = self.call("step")?;
        self.solver_type = state["solverType"].as_str().ok_or("External child solverType required")?.to_owned();
        self.state = serde_json::from_value(state).map_err(|error| error.to_string())?;
        self.node = self.call("node")?;
        Ok(())
    }
    fn solved_routes(&self) -> Result<Vec<Value>, String> { serde_json::from_value(self.call("routes")?).map_err(|error| error.to_string()) }
    fn node_with_port_points(&self) -> &Value { &self.node }
    fn solver_type_name(&self) -> String { self.solver_type.clone() }
    fn growth_attempts(&self) -> Option<f64> { None }
    fn visualize(&self, _: &dyn Fn(&str, f64) -> String) -> Result<Value, String> { self.call("visualize") }
}
...
impl HighDensityNodeSolver for GeneralChild {
    fn id(&self) -> u32 { self.id }
    fn state(&self) -> BaseSolverState { self.engine.borrow().state.clone() }
    fn step(&mut self, _observe_parent: Option<&mut dyn FnMut(bool) -> Result<(), String>>) -> Result<(), String> {
        let mut core = self.engine.borrow_mut();
        if core.state.solved || core.state.failed { return Ok(()); }
        ...
        let status = {
            ...
            (solver.solved, solver.failed, solver.error.clone(), solver.solved_routes.len() as f64 / solver.total_connections as f64)
        };
        ...
        if !core.state.solved && core.state.iterations as f64 > core.state.max_iterations {
            ...
            core.state.error = Some(format!("IntraNodeRouteSolver ran out of iterations (MAX_ITERATIONS={})", intra_node_routing::js_number::js_number_to_string(core.state.max_iterations)));
        }
        Ok(())
    }
    fn solved_routes(&self) -> Result<Vec<Value>, String> {
        let core = self.engine.borrow();
        core.engine.borrow().routes().iter().map(|route| serde_json::to_value(route).map_err(|error| error.to_string())).collect()
    }
    ...
    fn node_with_port_points(&self) -> &Value { &self.node }
    fn solver_type_name(&self) -> String { self.engine.borrow().solver_type.clone() }
    fn growth_attempts(&self) -> Option<f64> { None }
    fn visualize(&self, _: &dyn Fn(&str, f64) -> String) -> Result<Value, String> {
...
impl GrowthCore {
    fn state(&self) -> BaseSolverState {
        ...
        BaseSolverState { max_iterations: solver.max_iterations, solved: solver.solved, failed: solver.failed,
            iterations: solver.iterations, progress: solver.progress, error: solver.error.clone() }
    }

    fn step(&mut self, inner: Option<(usize, f64)>, mut observe_parent: Option<&mut dyn FnMut(bool) -> Result<(), String>>) -> Result<(), String> {
        ...
        let mut factory = |params: Value| -> Result<Box<dyn GrowthPortfolio>, String> {
            ...
            Ok(Box::new(PortfolioChild { id, engine, node, visualize: visualize.clone() }))
        };
        ...
        let validation: Option<&mut dyn FnMut(&[Value], &Value) -> Result<(bool, Option<Value>), String>> = if custom_validator && self.validator.is_some() { Some(&mut validator) } else { None };
        ...
        } else { self.solver.step(&mut factory, validation) }
    }
...
impl HighDensityNodeSolver for GrowthChild {
    fn id(&self) -> u32 { self.id }
    fn state(&self) -> BaseSolverState { self.engine.borrow().state() }
    fn step(&mut self, observe_parent: Option<&mut dyn FnMut(bool) -> Result<(), String>>) -> Result<(), String> { self.engine.borrow_mut().step(None, observe_parent) }
    fn solved_routes(&self) -> Result<Vec<Value>, String> { Ok(self.engine.borrow().solver.solved_routes.clone()) }
    fn route_count(&self) -> Result<usize, String> { Ok(self.engine.borrow().solver.solved_routes.len()) }
    fn node_with_port_points(&self) -> &Value { &self.node }
...
#[wasm_bindgen]
pub struct GrowShrinkHighDensityIntraNodeSolver { engine: Rc<RefCell<GrowthCore>> }
...
impl GrowShrinkHighDensityIntraNodeSolver {
    ...
    pub fn step_inner(&self, iterations: usize, maximum: f64) -> Result<(), JsValue> {
        self.engine.borrow_mut().step(Some((iterations, maximum)), None).map_err(return_error)
    }

    pub fn solve(&self) -> Result<(), JsValue> {
        let mut core = self.engine.borrow_mut();
        while !core.solver.solved && !core.solver.failed { core.step(None, None).map_err(return_error)?; }
        Ok(())
    }
        ...
        for key in ["activeId", "winnerId", "failedIds"] {
            if let Some(value) = patch.get(key) {
                let ids: Vec<u32> = if key == "failedIds" {
                    serde_json::from_value(value.clone()).map_err(|error| JsValue::from_str(&error.to_string()))?
                } else if value.is_null() { Vec::new() } else { vec![value.as_u64().ok_or_else(|| JsValue::from_str("Child ID required"))? as u32] };
                let mut children: Vec<Box<dyn GrowthPortfolio>> = Vec::new();
                for id in ids {
                    let engine = if let Some(engine) = core.portfolios.get(&id) { engine.clone() } else {
                    ...
                    children.push(Box::new(PortfolioChild { id, engine, node, visualize: core.visualize.clone() }));
                }
                match key { "activeId" => core.solver.active_sub_solver = children.pop(), "winnerId" => core.solver.winning_solver = children.pop(), _ => core.solver.failed_solvers = children }
            }
        }
        let s = &mut core.solver;
        macro_rules! field { ($key:literal,$name:ident) => { if let Some(value) = patch.get($key) { s.$name = serde_json::from_value(value.clone()).map_err(|error| JsValue::from_str(&error.to_string()))?; } }; }
        field!("MAX_ITERATIONS",max_iterations); field!("iterations",iterations); field!("solved",solved); field!("failed",failed);
        field!("error",error); field!("constructorParams",constructor_params); field!("nodeWithPortPoints",node_with_port_points);
        field!("scaleFactor",scale_factor); field!("growthAttempts",growth_attempts); field!("maxGrowthAttempts",max_growth_attempts);
        field!("stats",stats); field!("solvedRoutes",solved_routes);
        if let Some(value) = patch.get("progress") { s.progress = value.as_f64().unwrap_or(f64::NAN); }
        Ok(())
    }
...
impl HighDensitySolver {
    ...
    pub fn new(params: Ts<HighDensityMappedValue>,
        ...
        Ok(Self { children: HashMap::new(), solver, factory, visualize, transparentize, completed })
    }
    ...
    pub fn child_state(&self, id: u32) -> Result<Ts<HighDensityState>, JsValue> {
        let state = match self.children.get(&id) {
            ...
            Some(BoardChildReference::Portfolio(engine, _)) => engine.upgrade().ok_or_else(|| JsValue::from_str("Released Portfolio child"))?.borrow().state(),
    ...
    pub fn set_cache_counts(hits: f64, misses: f64) { set_cache_counts(hits, misses); }

    #[wasm_bindgen(js_name = stepInner)]
    pub fn step_inner(&mut self, iterations: usize, maximum: f64) -> Result<u8, JsValue> {
        let previous_child = self.solver.active_sub_solver.as_ref().map(|child| child.id());
        self.solver.base.iterations = iterations;
        self.solver.base.max_iterations = maximum;
        self.run(false)?;
        let current_child = self.solver.active_sub_solver.as_ref().map(|child| child.id());
        Ok(u8::from(self.solver.base.solved) + 2 * u8::from(self.solver.base.failed)
    ...
    pub fn restore(&mut self, patch: Ts<HighDensityMappedValue>) -> Result<(), JsValue> {
        ...
        if let Some(value) = patch.get("failedIds") {
            let ids: Vec<u32> = serde_json::from_value(value.clone()).map_err(|error| JsValue::from_str(&error.to_string()))?;
            let mut failed = Vec::new();
            for id in ids { failed.push(self.resolve_child(id)?); }
            self.solver.failed_solvers = failed;
        }
        let s = &mut self.solver;
        macro_rules! field { ($key:literal,$name:ident) => { if let Some(value) = patch.get($key) { s.$name = serde_json::from_value(value.clone()).map_err(|error| JsValue::from_str(&error.to_string()))?; } }; }
        macro_rules! base { ($key:literal,$name:ident) => { if let Some(value) = patch.get($key) { s.base.$name = serde_json::from_value(value.clone()).map_err(|error| JsValue::from_str(&error.to_string()))?; } }; }
        base!("MAX_ITERATIONS",max_iterations); base!("iterations",iterations); base!("solved",solved); base!("failed",failed); base!("error",error);
        if let Some(value) = patch.get("progress") { s.base.progress = value.as_f64().unwrap_or(f64::NAN); }
        field!("unsolvedNodePortPoints",unsolved_node_port_points); field!("routes",routes); field!("colorMap",color_map);
        field!("obstacles",obstacles); field!("connMap",conn_map); field!("nodePfById",node_pf_by_id);
        field!("nodeSolveMetadataById",node_solve_metadata_by_id); field!("stats",stats);
        field!("viaDiameter",via_diameter); field!("traceWidth",trace_width); field!("obstacleMargin",obstacle_margin);
        field!("effort",effort); field!("layerCount",layer_count); field!("captureSearchDebug",capture_search_debug);
        field!("useGrowShrinkHighDensityIntraNodeSolver",use_grow_shrink_high_density_intra_node_solver);
        field!("preserveTerminalPcbPortIds",preserve_terminal_pcb_port_ids);
        field!("growShrinkMaxInnerIterationsPerGrowthAttempt",grow_shrink_max_inner_iterations_per_growth_attempt);
        field!("growShrinkFallbackToInvalidGeometryOnFailure",grow_shrink_fallback_to_invalid_geometry_on_failure);
        Ok(())
    }
}
...
impl HighDensitySolver {
    fn resolve_child(&mut self, id: u32) -> Result<Box<dyn HighDensityNodeSolver>, JsValue> {
        if !self.children.contains_key(&id) {
            if let Some((callback, solver_type)) = SHARED_EXTERNAL.with(|registry| registry.borrow_mut().remove(&id)) {
                let mut child = ExternalChild { id, callback, solver_type, node: Value::Null, state: BaseSolverState::default() };
                let state = child.call("state").map_err(return_error)?;
                child.solver_type = state["solverType"].as_str().ok_or_else(|| JsValue::from_str("External child solverType required"))?.to_owned();
                child.state = serde_json::from_value(state).map_err(|error| JsValue::from_str(&error.to_string()))?;
                child.node = child.call("node").map_err(return_error)?;
                self.children.insert(id, BoardChildReference::External(child.clone()));
            ...
            if let Some(engine) = SHARED_GENERAL.with(|registry| registry.borrow_mut().remove(&id)) {
                ...
                self.children.insert(id, BoardChildReference::General(engine.clone(), node.clone()));
                return Ok(Box::new(GeneralChild { id, engine, node, visualize: self.visualize.clone() }));
            }
            if let Some(engine) = SHARED_GROWTH.with(|registry| registry.borrow_mut().remove(&id)) {
                let node = engine.borrow().solver.node_with_port_points.clone();
                self.children.insert(id, BoardChildReference::Growth(Rc::downgrade(&engine), node.clone()));
                return Ok(Box::new(GrowthChild { id, engine, node }));
            }
            let engine = take_shared_portfolio(id).map_err(return_error)?;
            let node = engine.borrow().solver.node_with_port_points.clone();
            self.children.insert(id, BoardChildReference::Portfolio(Rc::downgrade(&engine), node.clone()));
            return Ok(Box::new(PortfolioChild { id, engine, node, visualize: self.visualize.clone() }));
        }
        match self.children.get(&id).unwrap() {
            BoardChildReference::External(child) => {
                let mut child = child.clone();
                child.state = serde_json::from_value(child.call("state").map_err(return_error)?).map_err(|error| JsValue::from_str(&error.to_string()))?;
                child.node = child.call("node").map_err(return_error)?;
                Ok(Box::new(child))
            }
            BoardChildReference::General(engine, node) => Ok(Box::new(GeneralChild { id,
                engine: engine.clone(), node: node.clone(), visualize: self.visualize.clone() })),
            BoardChildReference::Portfolio(engine, node) => Ok(Box::new(PortfolioChild { id,
                engine: engine.upgrade().ok_or_else(|| JsValue::from_str("Portfolio handle was released"))?, node: node.clone(), visualize: self.visualize.clone() })),
            BoardChildReference::Growth(engine, node) => Ok(Box::new(GrowthChild { id,
                engine: engine.upgrade().ok_or_else(|| JsValue::from_str("Growth handle was released"))?, node: node.clone() })),
        }
    }
    ...
    fn run(&mut self, solve: bool) -> Result<(), JsValue> {
        ...
        let mut factory = |_node: Value, settings: &high_density_solver::HighDensitySolver| -> Result<Box<dyn HighDensityNodeSolver>, String> {
            let growth = settings.use_grow_shrink_high_density_intra_node_solver;
            let value = callback.call1(&JsValue::UNDEFINED, &JsValue::from_bool(growth)).map_err(error_text)?;
            let id = value.as_f64().ok_or("HighDensity factory handle required")? as u32;
            if growth {
                let engine = SHARED_GROWTH.with(|registry| registry.borrow_mut().remove(&id)).ok_or("Unknown Growth handle")?;
                let node = engine.borrow().solver.node_with_port_points.clone();
                children.borrow_mut().insert(id, BoardChildReference::Growth(Rc::downgrade(&engine), node.clone()));
                Ok(Box::new(GrowthChild { id, engine, node }))
            } else {
                let engine = take_shared_portfolio(id)?;
                let node = engine.borrow().solver.node_with_port_points.clone();
                children.borrow_mut().insert(id, BoardChildReference::Portfolio(Rc::downgrade(&engine), node.clone()));
                Ok(Box::new(PortfolioChild { id, engine, node, visualize: visualize.clone() }))
            }
        };
        loop {
            if solve && (self.solver.base.solved || self.solver.base.failed) { break; }
            let previous = self.solver.active_sub_solver.as_ref().map(|child| child.id());
            let result = if solve { self.solver.step(&mut factory, &mut cache_stats) } else { self.solver._step(&mut factory, &mut cache_stats) };
            result.map_err(return_error)?;
            if let Some(id) = previous {
                if self.solver.active_sub_solver.as_ref().map(|child| child.id()) != Some(id) {
