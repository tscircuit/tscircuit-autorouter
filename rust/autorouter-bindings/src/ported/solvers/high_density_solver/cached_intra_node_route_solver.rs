...
fn rounded(value: f64) -> H {
    H::Number(intra_node_routing::geometry::js_round(value * 200.0) / 200.0)
}
...
pub fn compute_intra_node_cache_key(
    ...
    compute(snapshot_json, locale).map_err(|error| JsValue::from_str(&error))
}

fn compute(snapshot_json: &str, locale: &js_sys::Function) -> Result<String, String> {
