...
use crate::ported::solvers::hyper_high_density_solver::grow_shrink_high_density_intra_node_solver::grow_shrink_high_density_intra_node_solver::DEFAULT_MAX_GROWTH_ATTEMPTS;
use crate::ported::utils::merge_route_segments::merge_route_segments;
            ...
            for (id, metadata) in &self.node_solve_metadata_by_id {
                ...
                if status == "solved" {
                    ...
                    graphics["rects"].as_array_mut().unwrap().push(json!({"center":node["center"],"layer":"hd_node_markers",
                        "width":intra_node_routing::specialized_utils::math::max(width*0.1,0.12),"height":intra_node_routing::specialized_utils::math::max(height*0.1,0.12),"fill":"red","label":label}));
                }
            }
        }
