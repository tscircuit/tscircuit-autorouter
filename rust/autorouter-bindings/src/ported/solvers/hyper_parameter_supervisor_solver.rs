...
pub trait Candidate {
    ...
    fn is_specialized(&self) -> bool { false }
...
pub fn get_hyper_parameter_combinations(defs: &[&HyperParameterDef]) -> Vec<Value> {
    let Some((current, remaining)) = defs.split_first() else { return vec![json!({})]; };
    ...
    for value in &current.possible_values {
        for sub in &sub_combinations {
            ...
            for (key, value) in value.as_object().expect("Hyperparameter value must be an object") {
...
impl HyperParameterSupervisorSolver {
    pub fn new() -> Self {
        Self { greedy_multiplier: 1.2, min_substeps: 1, supervised_solvers: None,
            winning_solver: None, active_sub_solver: None, solved: false, failed: false,
            error: None, next_id: 0 }
    }

    pub fn add_candidate(&mut self, hyper_parameters: Value, solver: Box<dyn Candidate>, g: f64) -> usize {
        let id = self.next_id;
        self.next_id += 1;
        self.supervised_solvers.as_mut().expect("Initialize candidate list before adding candidates")
            .push(SupervisedSolver { id, hyper_parameters, solver, h: 0.0, g, f: g });
        id
    }

    pub fn initialize_solvers(&mut self, defs: &[HyperParameterDef], combinations: Option<&[Vec<&str>]>, factory: &mut dyn CandidateFactory, compute_g: impl Fn(&dyn Candidate, &Value) -> f64) -> Result<(), String> {
        ...
        for combination in combinations.unwrap_or(&default) {
            let selected: Vec<_> = defs.iter().filter(|def| combination.contains(&def.name)).collect();
    ...
    pub fn compute_g(solver: &dyn Candidate) -> f64 {
        let state = solver.state(); state.iterations as f64 / state.max_iterations
    }

    pub fn compute_h(solver: &dyn Candidate) -> f64 {
        let progress = solver.state().progress;
        1.0 - if progress == 0.0 || progress.is_nan() { 0.0 } else { progress }
    }

    pub fn get_supervised_solver_with_best_fitness(&self) -> Option<usize> {
        let mut best_fitness = f64::INFINITY;
        let mut best = None;
        for (index, record) in self.supervised_solvers.as_deref().unwrap_or(&[]).iter().enumerate() {
            let state = record.solver.state();
            if state.solved { return Some(index); }
            if state.failed { continue; }
            if record.f < best_fitness { best_fitness = record.f; best = Some(index); }
        }
        best
    }
    ...
    pub fn get_failure_message(&mut self) -> String {
        ...
        records.sort_by(|left, right| (right.f - left.f).partial_cmp(&0.0).unwrap_or(std::cmp::Ordering::Equal));
        let errors: Vec<_> = records.iter().take(5).map(|record| record.solver.state().error.clone().unwrap_or_default()).collect();
        format!("All solvers failed in hyper solver. Example failures: {}", errors.join(", "))
    }

    pub fn step_initialized(&mut self, compute_g: impl Fn(&dyn Candidate, &Value) -> f64, compute_h: impl Fn(&dyn Candidate) -> f64) -> Result<(), String> {
        let Some(index) = self.get_supervised_solver_with_best_fitness() else {
            self.failed = true; self.error = Some(self.get_failure_message()); return Ok(());
        };
        ...
        if record.solver.state().solved {
            self.solved = true; self.winning_solver = Some(record.id);
        }
        Ok(())
    }
}
