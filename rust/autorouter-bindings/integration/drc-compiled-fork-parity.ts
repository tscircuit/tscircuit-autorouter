...
const reference=new ReferenceDrcEngine(structuredClone(srj),{connMap:referenceMap})
...
try {
  assert.equal(JSON.stringify(fork.evaluate(traces,true)),JSON.stringify(baseline))
  ...
  assert.equal(JSON.stringify(fork.evaluate(traces,true)),JSON.stringify(baseline),"Fork connectivity remains its constructor snapshot")
  ...
  assert.equal(JSON.stringify(fork.evaluate(traces,true)),JSON.stringify(baseline),"Compiled obstacle snapshot remains immutable")
  fork.setConnectivity({idToNetMap:initialMap.idToNetMap})
  assert.equal(fork.evaluate(traces,true).errors.length,0)
