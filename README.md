...
Set `allowBlindAndBuriedVias: false` to make every via generated by the
autorouter span the complete `top`-to-`bottom` board stack. Omission and `true`
preserve existing layer-to-layer spans for standalone SimpleRouteJson backward
compatibility. Unchanged vias supplied in input traces retain their authored
spans. Generated through vias occupy every crossed copper layer; routing fails
closed when that physical barrel collides with unrelated copper or a generic
keepout. Copper pours are treated as plane regions with via antipads. Dense HDI
fanouts that require blind or buried vias can therefore be unsatisfiable when
this option is `false`; set it to `true` only when the fabrication process
supports blind or buried vias.
