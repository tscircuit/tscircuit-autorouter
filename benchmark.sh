...
Options:
  ...
  --high-density-backend NAME    Select typescript (default) or wasm for A01/A03; build WASM first
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --sample|--sample-numbers)
      ...
      shift 2
      ;;
    --high-density-backend)
      case "${2:-}" in
        typescript|wasm) export HIGH_DENSITY_BACKEND="$2" ;;
        *) echo "Expected --high-density-backend typescript|wasm" >&2; exit 1 ;;
      esac
      shift 2
      ;;
...
if [ "${HIGH_DENSITY_BACKEND:-typescript}" = "wasm" ] && [ ! -f rust/high-density-wasm/pkg/high_density_wasm_bg.wasm ]; then
  echo "Build WASM first: npm run build --prefix rust/high-density-wasm" >&2
  exit 1
fi
